...
{name={edge computing},
	   description={Edge computing\index{edge computing} refers to the placement of
					...
					\gls{data}-generating \glspl{device}. In contrast to \gls{cloudcomputing}, edge
					computing distributes computation across many \glspl{device} and may keep raw
					\gls{data} local, a property often exploited by \glspl{flsystem}.
		\\
...
{name={mobile device},
	description={A mobile \gls{device}\index{mobile device} is a portable computing
				 \gls{device} equipped with local processing, storage, sensors, and wireless
				 communication capabilities \cite{DistributedSystems}. Examples of mobile 
				 \glspl{device} include smartphones, tablets, or wearables \cite{Lane2015,Tsoukas2024}. 
				 In \gls{ml}, mobile \glspl{device} commonly act as \gls{data} sources and 
				 computational infrastructure and may participate in \glspl{edgecomputing} 
				 or \glspl{flsystem}.
		\\
