...
{name={edge computing},
	   description={Edge computing\index{edge computing} refers to the placement of
					...
					In \gls{ml}, edge computing supports low-latency inference and reduced
...
{name={edge device},
	...
	such as \gls{data} preprocessing, local \gls{model} \gls{training}, 
	or \gls{inference} \cite{Hashash2021}. \\ 
	See also: \gls{edgecomputing}, \gls{device}.},
	type=mlsystems,
	first={edge device},
	plural={edge devices},
...
{name={preprocessing},
 description={Preprocessing\index{preprocessing} refers to the set of operations
			  applied to raw \gls{data} before it is fed into a \gls{ml} \gls{algorithm} \cite{hastie01statisticallearning}. 
			  The goal of preprocessing is to transform the \gls{data} into a form
			  that is more suitable for follow-up stages of a \gls{mlpipeline}. 
	...
	\\
	See also: \gls{mlpipeline}, \gls{feature}, \gls{data}.},
 first={preprocessing},
 type=mlsystems,
 text={preprocessing}
}
...
{name={ML pipeline},
 description={The term ML pipeline\index{ML pipeline} refers to a composition (concatenation)
			  of several \glspl{function} within in \gls{mlsystem}. The individual \glspl{function} 
			  include \gls{data} preprocessing, \gls{featlearn}, \gls{model} \gls{training} and \gls{inference}. 
