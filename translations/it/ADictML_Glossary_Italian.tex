...
\newglossaryentry{geometricmedian}{
	...
	description={The geometric median\index{geometric median} of a set of vectors $\vx^{(1)}, \ldots, \vx^{(\samplesize)}$ 
		...
		$$\vz \in \argmin_{\vy \in \mathbb{R}^{\dimlocalmodel}} \sum_{\sampleidx=1}^{\samplesize} \normgeneric{\vy - \vx^{(\sampleidx)}}{}.$$
		Here, $\normgeneric{\cdot}{}$ can be any \gls{norm} defined on the \gls{euclidspace} $\mathbb{R}^{\dimlocalmodel}$.},
