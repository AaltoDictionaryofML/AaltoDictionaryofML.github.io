...
{name={FedAvg},
	...
	lato client, seguite dall'aggregazione dei \glspl{modelparam} aggiornati sul server
	\cite{pmlr-v54-mcmahan17a}. L’aggiornamento locale presso il client $\nodeidx=1,\ldots,\nrnodes$ 
	al tempo $\iteridx$ parte dai \glspl{modelparam} correnti $\weights^{(\iteridx)}$ forniti 
	...
	\end{tikzpicture}
	\end{center}
		\caption{Illustrazione di una singola iterazione di FedAvg, che consiste nella trasmissione dei \glspl{modelparam} da parte del server, 
...
it does not take into account contraints on the optimization variable (i.e., the \glspl{modelparam}). 
Projected\index{projected gradient descent (projected GD)} \gls{gd} 
extends basic \gls{gd} to handle constraints on the optimization variable (i.e., the \glspl{modelparam}). 
...
{name={grafo pesato},
	description={Un \gls{graph}\index{grafo pesato} a cui archi  
		sono assegnati pesi numerici. Tipicamente, questi pesi degli archi 
		sono numeri reali non negativi. Ad esempio, se un grafo rappresenta 
		una rete stradale con nodi che rappresentano intersezione e archi che rappresentano 
		tratti stradali, il peso dell'arco potrebbe rappresentare la capacità (misurata in
		quantità massima di veicoli all'ora) del tratto stradale  \cite{NewmannBook}.  
		\\ 
		Si veda anche : \gls{graph}.},
	first={grafo pesato},
	type=math,
	firstplural={grafi pesati}, 
	plural={grafi pesati}, 
	text={grafi pesati} 
}
...
{name={grafo},
	description={Un grafo\index{graph} $\graph = \pair{\nodes}{\edges}$ 
		è una coppia che consiste di un insieme di nodi $\nodes$ e un insieme di archi $\edges$. 
		Nella sua forma piú generale, un grafo è definito da una \gls{map} che 
		assegna ad ogni $\edgeidx \in \edges$ una coppia di nodi \cite{RockNetworks}. 
		Salvo diversamente specificato, con il termine grafo ci si riferisce ad un grafo non orientato. 
		Un grafo semplice non orientato si ottiene identificando ogni arco $\edgeidx \in \edges$ 
		con un insieme che contiene due nodi distinti  $\{\nodeidx,\nodeidx'\}$. 
		Grafi pesati specificano anche un valore numerico \gls{weights} $\edgeweight_{\edgeidx}$ per ogni 
		arco $\edgeidx \in \edges$.
		\\ 
...
{name={algorithm},
  description={An\index{algorithm} algorithm is a precise, step-by-step specification for 
	...
	transform a given \gls{trainset} into \glspl{modelparam} through a sequence of \gls{gradstep}s. 
...
{name={online algorithm},
	description={An\index{online algorithm} online \gls{algorithm} processes input \gls{data} incrementally, 
		...
		\gls{onlineGD} (online gradient descent), which incrementally updates \glspl{modelparam} as new \gls{datapoint}s 
		arrive.
	},
...
{name={estimation error},
	description={Consider\index{estimation error} \gls{datapoint}s, each with \gls{featurevec} $\featurevec$ and \gls{label} 
		...
		\glspl{modelparam} $\weights$, we can define the estimation error as $\Delta \weights = \widehat{\weights} - \overline{\weights}$ \cite{kay,hastie01statisticallearning}.},
...
{name={federated averaging (FedAvg)},
	description={FedAvg\index{federated averaging (FedAvg)} refers to an iterative \gls{fl} \gls{algorithm} that alternates between separately training \gls{localmodel}s and combining the updated local \glspl{modelparam}. The training of \gls{localmodel}s 
...
{name={FedProx},
	description={FedProx\index{FedProx} refers to an iterative \gls{fl} \gls{algorithm} that alternates between separately training \gls{localmodel}s and combining the updated local \glspl{modelparam}. In contrast to \gls{fedavg}, which uses 
...
\newglossaryentry{objfunc}{name={objective function}, description={An\index{objective function} 
		...
		as the \glspl{modelparam} $\weights$ of a \gls{hypothesis} $\hypothesis^{(\weights)}$, to 
...
\newglossaryentry{cfl}{name={clustered federated learning (CFL)}, description={
		...
		similarity of \glspl{modelparam} across well-connected subsets of the \gls{empgraph}.},
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge\index{ridge regression} 
		\gls{regression} learns the \gls{weights} $\weights$ of a linear \gls{hypothesis} map $\hypothesis^{(\weights)}(\featurevec)= \weights^{T} \featurevec$. The quality of a particular choice for the \glspl{modelparam} $\weights$ is measured by the sum 
...
\newglossaryentry{ai}{name={artificial intelligence (AI)}, description={
		...
		AI systems rarely have access to a labeled \gls{trainset} that allows the average \gls{loss} to be measured for any possible choice of \glspl{modelparam}. 
		Instead, AI systems use observed \gls{reward} signals to obtain a (point-wise) estimate for the 
		\gls{loss} incurred by the current choice of \glspl{modelparam}.},first={AI},text={AI} }
...
\newglossaryentry{gdmethods}{name={gradient-based methods}, 
	description={\Gls{gradient}-based\index{gradient-based methods} 
		...
		of a \gls{differentiable} \gls{objfunc} of the \glspl{modelparam}. These 
		methods construct a sequence of approximations to an optimal choice for 
		\glspl{modelparam} that results in a \gls{minimum} (or \gls{maximum}) value of the \gls{objfunc}. 
		As their name indicates, \gls{gradient}-based methods use the \gls{gradient}s of the \gls{objfunc} 
		evaluated during previous iterations to construct new, (hopefully) improved \glspl{modelparam}. 
...
\newglossaryentry{sgd}{name={subgradient descent}, description={\Gls{subgradient}\index{subgradient descent} 
		...
		as a function of the \glspl{modelparam} $\weights$ that select a \gls{hypothesis} $\hypothesis^{(\weights)} \in \hypospace$.},first={subgradient descent},text={subgradient descent} }
...
\newglossaryentry{stochGD}{name={stochastic gradient descent (SGD)}, description={Stochastic\index{stochastic gradient descent (SGD)} 
		...
		\gls{emprisk} (as a function of the \glspl{modelparam} $\weights$), 
...
Consider \index{online gradient descent (online GD)} an \gls{ml} method that learns \glspl{modelparam} 
...
\newglossaryentry{weights}{name={weights},
	description={Consider\index{weights} a parametrized \gls{hypospace} $\hypospace$. 
		We\index{weights} use the term weights for numeric \glspl{modelparam} that are 
...
\newglossaryentry{paramspace}{name={parameter space},
		...
		\glspl{modelparam} (see Figure \ref{fig_param_space_dict}). Many important \gls{ml} methods 
