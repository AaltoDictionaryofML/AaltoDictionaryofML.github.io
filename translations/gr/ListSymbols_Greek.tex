...
\begin{align}
	&\argmin_{\weights \in \mathcal{C}} f(\weights) &\quad &\parbox{.70\textwidth}{\foreignlanguage{greek}{Το σύνολο των ελαχιστοποιητών 
    		για μια \glsentryuseriii{function} πραγματικής τιμής $f: \mathcal{C} \rightarrow \mathbb{R}$.}
		...
		σε ένα σύνολο $\mathcal{B}$, η οποία αποδίδει σε κάθε είσοδο $a \in \mathcal{A}$ μία καλά ορισμένη} \glsentryuseriii{output} $f(a) \in \mathcal{B}$. 
		\foreignlanguage{greek}{Το σύνολο $\mathcal{A}$ είναι το} \glsentryuseri{domain} \foreignlanguage{greek}{της} \glsentryuserii{function} 
		$f$ \foreignlanguage{greek}{και το σύνολο $\mathcal{B}$ είναι το} \glsentryuseri{co-domain}
...
\newpage
\begin{align} 
	&\linspan{\mathbf{B}}  & \quad &  \parbox{.85\textwidth}{\foreignlanguage{greek}{Το εύρος ενός} \glsentryuserii{matrix} $\mathbf{B} \in \mathbb{R}^{a \times b}$, 
		\foreignlanguage{greek}{που είναι ο} \glsentryuseri{subspace} \foreignlanguage{greek}{όλων των γραμμικών συνδυασμών των στηλών του $\mathbf{B}$, 
		έτσι ώστε $\linspan{\mathbf{B}} = \big\{  \mathbf{B} \va : \va \in \mathbb{R}^{b} \big\} \subseteq \mathbb{R}^{a}$.} 
...
\begin{align}
	&\featurevec \sim \probdist^{(\vz)}  &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Η} \glsentryuseri{rv} $\featurevec$ \foreignlanguage{greek}{κατανέμεται  
		σύμφωνα με την} \glsentryuseriii{probdist} $\probdist^{(\vz)}$ \cite{klenke2020probability}, \cite{BillingsleyProbMeasure}.
	...
	&\expect_{p} \{ f(\datapoint) \}  &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Η} \glsentryuseri{expectation}  
		...
		\foreignlanguage{greek}{είναι $\probdist^{(\datapoint)}$. Αν η} \glsentryuseri{probdist} \foreignlanguage{greek}{είναι προφανής από τα συμφραζόμενα,
...
\newpage
\begin{align}
	&\probdist^{(\truelabel|\featurevec)} &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Μία} \glsentryuseri{condprobdist} \foreignlanguage{greek}{μίας}
		\glsentryuserii{rv} $\truelabel$ \foreignlanguage{greek}{δεδομένης (ή υπό τον όρο) της τιμής μίας άλλης} 
	...
	&\probdist^{(\featurevec;\weights)} &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Μία παραμετροποιημένη} \glsentryuseri{probdist} \foreignlanguage{greek}{μίας} 
		...
		\glsentryuseriii{vector} $\weights$. \foreignlanguage{greek}{Για παράδειγμα, $\probdist^{(\featurevec;\weights)}$ θα μπορούσε να είναι μία} 
...
\begin{align}
	...
	&\simplex{\nrcluster} &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Το} \gls{probabilitysimplex}, \foreignlanguage{greek}{το οποίο αποτελείται 
		από όλα τα} \glsentryuservi{vector} $\vp = \big( p_{1}, \,\ldots, \,p_{\nrcluster} \big)\,^{T} \in \mathbb{R}^{\nrcluster}$ 
	        \foreignlanguage{greek}{με μη αρνητικές καταχωρίσεις που αθροίζουν στο ένα, δηλαδή $p_{\clusteridx} \geq 0$ για $\clusteridx=1, \,\ldots, \,\nrcluster$ και}
		$\sum_{\clusteridx=1}^{\nrcluster} p_{\clusteridx} = 1$.
		\\ \foreignlanguage{greek}{Βλέπε επίσης:} \gls{pmf}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\entropy{x} &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Η} \glsentryuseri{entropy} \foreignlanguage{greek}{μίας} \glsentryuserii{discreteRV} $x$. 
		\\ \foreignlanguage{greek}{Βλέπε επίσης:} \gls{entropy}, \gls{discreteRV}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\samplespace &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Ένας} \glsentryuseri{samplespace} \foreignlanguage{greek}{όλων των πιθανών} 
		\glsentryuserv{outcome} \foreignlanguage{greek}{ενός} \glsentryuserii{randomexperiment}. 
		\\ \foreignlanguage{greek}{Βλέπε επίσης:} \gls{samplespace}, \gls{outcome}, \gls{randomexperiment}, \gls{event}.}  \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\sigmaalgebra &\quad & \parbox{.85\textwidth}{\foreignlanguage{greek}{Μία συλλογή} \glsentryuserv{measurable} \foreignlanguage{greek}{υποσυνόλων ενός} 
		\glsentryuserii{samplespace} $\samplespace$. 
...
\begin{align}
	...
	&\valerror &\quad &\parbox{.80\textwidth}{\foreignlanguage{greek}{Το} \glsentryuseri{valerr} \foreignlanguage{greek}{μίας} \glsentryuserii{hypothesis} $\hypothesis$, 
		\foreignlanguage{greek}{το οποίο είναι η μέση της} \glsentryuseri{loss} \foreignlanguage{greek}{που προκαλείται σε ένα} \glsentryuseriii{valset}.
...
\begin{align}
 	&\graph = \pair{\nodes}{\edges} & \quad & \parbox{.80\textwidth}{\foreignlanguage{greek}{Ένας} \glsentryuseri{undirectedgraph} \foreignlanguage{greek}{του οποίου 
		οι κόμβοι $\nodeidx \in \nodes$ αντιπροσωπεύουν} \glsentryuservi{device} \foreignlanguage{greek}{εντός ενός} \glsentryuserii{empgraph}. 
		\foreignlanguage{greek}{Οι μη κατευθυνόμενες σταθμισμένες ακμές $\edges$ αντιπροσωπεύουν τη συνεκτικότητα μεταξύ} \glsentryuserv{device} 
		...
		\\ \foreignlanguage{greek}{Βλέπε επίσης:} \gls{undirectedgraph}, \gls{device}, \gls{empgraph}, \gls{dataset}, \gls{learningtask}.}\nonumber \\[2mm] \hline \nonumber\\[-5mm]
