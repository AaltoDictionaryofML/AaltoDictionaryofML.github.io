...
\newglossaryentry{modelsel}
{
	...
	description={En\index{selección de modelo} \gls{ml}, la selección de modelo se refiere al 
		...
		con el menor \gls{valerr} \cite[Ch. 6]{MLBasics}.}, 
	first={selección de modelo},text={selección de modelo}  
}


\newglossaryentry{linclass}{name={clasificador lineal}, description={
		Considere\index{clasificador lineal} \gls{datapoint}s caracterizados por \gls{feature}s numéricos $\featurevec \in \mathbb{R}^{\nrfeatures}$ 
	    y una \gls{label} $\truelabel \in \labelspace$ perteneciente a algún \gls{labelspace} finito $\labelspace$. 
		Un clasificador lineal se caracteriza por tener \gls{decisionregion}s que están separadas por hiperplanos en
		$\mathbb{R}^{\featuredim}$ \cite[Ch. 2]{MLBasics}.},
		first={lasificador lineal},text={lasificador lineal} 
		
		}
		
\newglossaryentry{erm}
		{
			name={minimización del riesgo empírico (ERM)},
			description={La minimización del riesgo empírico\index{minimización del riesgo empírico (ERM)} es el problema de optimización 
				que consiste en encontrar una \gls{hypothesis} (de un \gls{model}) que tenga el \gls{minimum} \gls{loss} promedio 
				(o \gls{emprisk}) en un \gls{dataset} dado $\dataset$ (es decir, el \gls{trainset}). 
				Muchos métodos de \gls{ml} se derivan de la \gls{emprisk} mediante elecciones de diseño específicas 
				para el \gls{dataset}, el \gls{model} y el \gls{loss} \cite[Ch. 3]{MLBasics}.},
			first={minimización del riesgo empírico (ERM)},text={ERM} 
		}
