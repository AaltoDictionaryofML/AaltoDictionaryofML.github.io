...
\newglossaryentry{psd}
{
	name={semi-definido positivo (psd)},
	description={
		Una\index{semi-definido positivo (psd)} matriz simétrica (de valores reales) 
		$\mQ = \mQ^{T} \in \mathbb{R}^{\featuredim \times \featuredim}$ 
		se denomina psd si $\featurevec^{T} \mQ \featurevec \geq 0$ para cualquier vector 
		$\featurevec \in \mathbb{R}^{\featuredim}$. 

		La propiedad de ser psd se puede extender de matrices a mapas de \gls{kernel} 
		simétricos (de valores reales) $\kernel: \featurespace \times \featurespace \rightarrow \mathbb{R}$ 
		(con $\kernel(\featurevec,\featurevec') = \kernel(\featurevec',\featurevec)$) de la siguiente manera: 

		Para cualquier conjunto finito de \gls{featurevec}s $\featurevec^{(1)},\dots,\featurevec^{(\samplesize)}$, 
		la matriz resultante $\mQ \in \mathbb{R}^{\samplesize \times \samplesize}$ con entradas 
		$Q_{\sampleidx,\sampleidx'} = \kernelmap{\featurevec^{(\sampleidx)}}{\featurevec^{(\sampleidx')}}$ 
		es psd \cite{LearningKernelsBook}.
	},
	first={semi-definido positivo (psd)},text={psd}  
}
...
\newglossaryentry{feature}
{
	name={característica},
	description={
		Una\index{característica} característica de un \gls{datapoint} es una de sus propiedades que se puede 
		medir o calcular fácilmente sin necesidad de supervisión humana. 

		Por ejemplo, si un \gls{datapoint} es una imagen digital (por ejemplo, almacenada como un archivo \texttt{.jpeg}), 
		podríamos usar las intensidades de rojo-verde-azul de sus píxeles como características. 

		Los sinónimos específicos del dominio para el término "característica" son "covariable", 
		"variable explicativa", "variable independiente", "variable de entrada", "predictor (variable)" o "regresor" \cite{Gujarati2021}, \cite{Dodge2003}, \cite{Everitt2022}. 
	},
	first={característica},text={característica}  
}
