...
\newglossaryentry{hardclustering}{name={agrupamiento rígido}, description={El agrupamiento rígido\index{agrupamiento rígido} 
	se refiere a la tarea de dividir un conjunto dado de \gls{datapoint}s en (pocos) \gls{cluster}s no superpuestos. 
...
\newglossaryentry{softclustering}{name={agrupamiento suave}, description={El agrupamiento suave\index{agrupamiento suave} 
	se refiere a la tarea de dividir un conjunto dado de \gls{datapoint}s en (pocos) \gls{cluster}s superpuestos. 
	...
	consiste en interpretar los \gls{datapoint}s como \gls{iid} \gls{realization}s de un \gls{gmm}. 
...
\newglossaryentry{clustering}{name={agrupamiento}, description={Los métodos de agrupamiento\index{agrupamiento} 
	descomponen un conjunto dado de \gls{datapoint}s en pocos subconjuntos, que se denominan \gls{cluster}s. 
	Cada \gls{cluster} está compuesto por \gls{datapoint}s que son más similares entre sí que con respecto a 
	los \gls{datapoint}s fuera del \gls{cluster}. Los diferentes métodos de agrupamiento utilizan 
	distintas medidas de similitud entre \gls{datapoint}s y diferentes formas de representación de los \gls{cluster}s. 
...
\newglossaryentry{cluster}{name={clúster}, description={Un\index{clúster} clúster es un subconjunto de 
	\gls{datapoint}s que son más similares entre sí que a los \gls{datapoint}s fuera del clúster. 
	La medida cuantitativa de similitud entre los \gls{datapoint}s es una decisión de diseño. 
	Si los \gls{datapoint}s se caracterizan por vectores de \gls{feature} euclidianos 
	$\featurevec \in \mathbb{R}^{\nrfeatures}$, podemos definir la similitud entre dos \gls{datapoint}s 
...
\newglossaryentry{svm}{name={máquina de vectores de soporte (SVM)}, description={La\index{máquina de vectores de soporte (SVM)} 
		...
		la Figura \ref{fig_svm_gls_dict}, su objetivo es separar al máximo los \gls{datapoint}s de 
		...
		\begin{figure}[H]
			...
			\end{center}
		\end{figure}
		La variante básica de la \gls{svm} solo es útil si los \gls{datapoint}s de diferentes categorías pueden ser (aproximadamente)
...
\newglossaryentry{tv}{name={variación total}, description={Vea \gls{gtv}\index{variación total}.},
 ...
 \newglossaryentry{cvxclustering}{name={agrupamiento convexo}, 
	...
	coinciden. Un \gls{cluster} consiste entonces en aquellos \gls{datapoint}s $\sampleidx \in \{1,\ldots,\samplesize\}$ 
...
\newglossaryentry{stochGD}{name={descenso de gradiente estocástico (SGD)}, description={El\index{descenso de gradiente estocástico (SGD)} 
		...
		(por ejemplo, cuando los \gls{datapoint}s están almacenados en bases de datos distribuidas por todo el mundo). Para evaluar el \gls{gradient} del
		...
		sobre todos los \gls{datapoint}s del \gls{trainset}. Una aproximación estocástica se obtiene al reemplazar esta suma 
		$\sum_{\sampleidx=1}^{\samplesize} \nabla_{\weights} \lossfunc{\datapoint^{(\sampleidx)}}{\weights}$ 
		...
		A estos \gls{datapoint}s seleccionados aleatoriamente se les denomina a menudo \gls{batch}. 
...
% Labels
		...
		\gls{datapoint}s del \gls{trainset} (indexados por $\sampleidx=1,\ldots,\samplesize$) 
...
El proceso de aprendizaje utiliza \gls{datapoint}s $\datapoint^{(\timeidx)}$ que llegan en instantes de tiempo consecutivos $\timeidx=1,2,\ldots$. 
Interpretamos los \gls{datapoint}s $\datapoint^{(\timeidx)}$ como copias \gls{iid} de una \gls{rv} $\datapoint$. 
...
Sin embargo, este límite solo puede evaluarse si esperamos un tiempo infinito para recolectar todos los \gls{datapoint}s. 
...
$\weights^{(T+1)}$ entregados por el descenso por gradiente en línea tras observar $T$ \gls{datapoint}s $\datapoint^{(1)},\ldots, \datapoint^{(T)}$ 
...
secuencia finita (o lista) de \gls{datapoint}s $\datapoint^{(1)},\ldots,\datapoint^{(m)}$ que 
...
{name=tamaño de la muestra,
	description={El \index{tamaño de la muestra} número de \gls{datapoint}s 
...
\newglossaryentry{device}{name={dispositivo},description={
				...
				generalmente nos referimos a un ordenador que puede leer \gls{datapoint}s de diferentes 
				fuentes y, a su vez, entrenar un \gls{model} de \gls{ml} utilizando estos \gls{datapoint}s.},
...
\newglossaryentry{llm}{name={modelo de lenguaje de gran escala (LLM)},description={
	...
	restantes como \gls{feature}s de \gls{datapoint}s. Esta construcción requiere 
...
\newglossaryentry{probdist}{name={distribución de probabilidad},
	...
	interpretar los \gls{datapoint}s como \gls{iid} \gls{realization}s de una \gls{rv}. Las propiedades típicas 
	de tales \gls{datapoint}s están gobernadas por la distribución de \gls{probability} de esta \gls{rv}. 
...
\newglossaryentry{datanorm}{name={normalización de datos},
	...
	aplicadas a los \gls{featurevec}s de \gls{datapoint}s para mejorar los aspectos \gls{statasp} o \gls{compasp} 
...
\newglossaryentry{dataaug}{name={aumentación de datos},
	description={Los métodos de aumentación de \gls{data} \index{aumentación de datos} añaden \gls{datapoint}s sintéticos 
	a un conjunto existente de \gls{datapoint}s. Estos \gls{datapoint}s sintéticos se obtienen mediante 
	perturbaciones (por ejemplo, añadir ruido a mediciones físicas) o transformaciones 
	(por ejemplo, rotaciones de imágenes) de los \gls{datapoint}s originales. Estas perturbaciones y 
	transformaciones son tales que los \gls{datapoint}s sintéticos resultantes deben 
	...
	forma eficiente de \gls{regularization}.
		...
		\begin{center}
			\begin{tikzpicture}
			  ...
			  \vspace*{-11mm}
		\end{center}
		\caption{La aumentación de \Gls{data} aprovecha las simetrías intrínsecas de los \gls{datapoint}s en 
...
\gls{datapoint}s individuales, que se caracterizan por \gls{feature}s y \gls{label}s. 
