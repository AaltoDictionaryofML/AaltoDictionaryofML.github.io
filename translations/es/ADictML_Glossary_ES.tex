...
{name={aprendizaje federado horizontal (horizontal FL)},
	description={
		El aprendizaje federado horizontal\index{aprendizaje federado horizontal (horizontal FL)} utiliza \gls{localdataset}es constituidos por diferentes
		\gls{datapoint}, pero emplea las mismas \gls{feature}s para caracterizarlos \cite{HFLChapter2020}.
		...
		Cada región espaciotemporal representa un \gls{datapoint} individual, caracterizado por los mismas \gls{feature}s
...
{name={reducción de dimensionalidad},
	description={
		...
		nuevos \gls{feature}s. Estos métodos pueden utilizarse para visualizar \gls{datapoint}
...
{name={aprendizaje automático (ML)},
	description={
		...
		una \gls{label} a partir de los \gls{feature}s de un \gls{datapoint}. Los métodos de ML logran esto
		...
		\gls{datapoint} (sus \gls{feature}s y \gls{label}),
...
{name={tarea de aprendizaje},description=
	{Consideremos\index{tarea de aprendizaje} un \gls{dataset} $\dataset$ constituido por varios \gls{datapoint}, cada uno 
...
\newglossaryentry{linmodel}{name={modelo lineal},
	description={Consideremos\index{modelo lineal} \gls{datapoint}, cada uno caracterizado por una \gls{featurevec} numérica
...
\newglossaryentry{probmodel}
{
	...
	description={Un \gls{model} probabilístico\index{modelo probabilístico} interpreta los \gls{datapoint}
...
{name={cota superior de confianza (UCB)},
	description={Considera \index{cota superior de confianza (UCB)} una aplicación de \gls{ml} 
...
para \gls{datapoint} caracterizados por \gls{feature}s y \gls{label}s numéricos.
...
{name={agrupamiento en grafos},
	description={El \gls{clustering} en \gls{graph}s\index{agrupamiento en grafos} tiene como objetivo agrupar \gls{datapoint} que están representados como nodos de un \gls{graph} $\graph$.
...
{name={error de estimación},
	description={Consideremos\index{error de estimación} varios \gls{datapoint}, cada uno con un \gls{featurevec} $\featurevec$ y una \gls{label} 
...
{name={precisión (accuracy)},
	description={Consideremos\index{precisión (accuracy)} \gls{datapoint} caracterizados por\gls{feature}s $\featurevec \in \featurespace$ y 
...
{name={espacio de etiquetas},
	description={Consideremos\index{espacio de etiquetas} una aplicación de \gls{ml} que involucra \gls{datapoint} caracterizados por \gls{feature}s 
...
{name={histograma},
	description={Un histograma \index{histograma} considera un \gls{dataset} $\dataset$ que consiste en $\samplesize$ \gls{datapoint}
...
{name={conjunto de datos},
	description={Un\index{conjunto de datos} conjunto de datos se refiere a una colección de \gls{datapoint}. Estos 
...
{name={variable aleatoria (RV)},
 ...
 description={Un\index{conjunto de entrenamiento} conjunto de entrenamiento es un \gls{dataset} $\dataset$ que consiste en algunos \gls{datapoint} usados en \gls{erm} 
...
{name={conjunto de validación},
  description={Un\index{conjunto de validación} conjunto de \gls{datapoint} usado para 
...
\newglossaryentry{testset}
{
	...
	description={Un\index{conjunto de prueba} conjunto de \gls{datapoint} que no ha sido 
...
\newglossaryentry{linclass}{name={clasificador lineal}, description={
	    Consideremos\index{clasificador lineal} \gls{datapoint} caracterizados por \gls{feature}s numéricos $\featurevec \in \mathbb{R}^{\nrfeatures}$ 
...
\newglossaryentry{multilabelclass}{name={clasificación multi-etiqueta}, description={Los problemas y métodos de  
		\gls{classification} multi-etiqueta\index{multi-label classification} utilizan \gls{datapoint}
...
\newglossaryentry{ssl}{name={aprendizaje semi-supervisado (SSL)}, description={El aprendizaje semi-supervisado (SSL)\index{aprendizaje semi-supervisado (SSL)} 
		utiliza \gls{datapoint} no etiquetados para apoyar el aprendizaje de una \gls{hypothesis} 
		a partir de \gls{labeled datapoint}s \cite{SemiSupervisedBook}. Este enfoque es particularmente útil 
		para aplicaciones de \gls{ml} que ofrecen una gran cantidad de \gls{datapoint} no etiquetados, pero solo un número limitado
...
\newglossaryentry{regularizer}{name={regularizador}, description={Un regularizador\index{regularizador} 
		...
		sus errores de \gls{prediction} en \gls{datapoint} fuera del \gls{trainset}. \Gls{ridgeregression} 
...
\newglossaryentry{regularization}{name={regularización}, description={
		...
		\begin{enumerate}[label=\arabic*)]
			...
			copias perturbadas de los \gls{datapoint} originales en $\dataset$. Un ejemplo de dicha perturbación  
...
\newglossaryentry{rerm}{
	...
	description={El \gls{erm} básico aprende una \gls{hypothesis} (o entrena un \gls{model}) $\hypothesis \in \hypospace$ 
		...
		ocurrir al predecir \gls{label}s para \gls{datapoint} fuera del \gls{trainset}. Esta intuición  
...
\newglossaryentry{generalization}{name={generalización}, 
	description={Muchos\index{generalización} de los sistemas actuales de \gls{ml} (y \gls{ai})  
		...
		\gls{datapoint} $\vz^{(1)},\ldots,\vz^{(\samplesize)}$, que sirven como un \gls{trainset} $\trainset$. 
		...
		\gls{probmodel} como la \gls{iidasspt} para la generacion de \gls{data}: los \gls{datapoint} en 
...
\newglossaryentry{clustasspt}{name={suposición de agrupamiento}, description={La suposición de agrupamiento\index{suposición de agrupamiento} 
		...
		Los \Gls{datapoint} en el mismo \gls{cluster}son más similares entre sí que aquellos
		que están fuera del \gls{cluster} \cite{SemiSupervisedBook}. Obtenemos diferentes 
		métodos de \gls{clustering} utilizando diferentes nociones de similitud entre \gls{datapoint}.},first={suposición de agrupamiento},text={suposición de agrupamiento} }
...
\newglossaryentry{featuremap}{name={mapa de atributos}, description={Un mapa de atributos\index{mapa de atributos} se 
 ...
 \newglossaryentry{simgraph}{name={grafo de similitud}, 
 	description={Algunas\index{grafo de similitud} aplicaciones de \gls{ml} generan \gls{datapoint} que 
...
\newglossaryentry{kernel}{name={kernel}, 
	description={Considere\index{kernel} \gls{datapoint} caracterizados por un \gls{featurevec} $\featurevec \in \featurespace$ 
...
\newglossaryentry{kernelmethod}{name={método de kernel}, 
	...
	los \gls{datapoint} presentan una geometría "más conveniente" en el \gls{featurespace}. 
	Por ejemplo, en un problema de \gls{classification} binaria, el uso de \gls{featurevec}s transformados $\vz$ podría 
	permitirnos utilizar \gls{linmodel}s, incluso si los \gls{datapoint} no son linealmente 
...
\end{center}
\caption{
Cinco \gls{datapoint} caracterizados por \gls{featurevec}s $\featurevec^{(\sampleidx)}$ 
...
permiten separar los \gls{datapoint} usando un \gls{linclass}.  \label{fig_linsep_kernel_dict}}
\end{figure}
},first={método de kernel},text={método de kernel} }
	

\newglossaryentry{cm}{name={matriz de confusión}, 
	description={Considere\index{matriz de confusión} \gls{datapoint} caracterizados por \gls{feature}s $\featurevec$ 
		...
		la matriz de confusión es la fracción de \gls{datapoint} con la \gls{label} $\truelabel\!=\! \clusteridx$ y la
...
\newglossaryentry{featuremtx}{name={matriz de atributos}, 
	description={Considere\index{matriz de atributos} un \gls{dataset} $\dataset$ 
		con $\samplesize$ \gls{datapoint} caracterizados por \gls{featurevec}s $\featurevec^{(1)},\ldots,\featurevec^{(\samplesize)} \in \mathbb{R}^{\nrfeatures}$. Es conveniente
...
description={DBSCAN\index{agrupamiento basado en densidad para aplicaciones espaciales con ruido (DBSCAN)} 
	es un \gls{algorithm} de \gls{clustering} para \gls{datapoint} caracterizados por \gls{featurevec}s numéricos. 
	...
	y \gls{gmm}, DBSCAN emplea una noción diferente de similitud entre \gls{datapoint}. 
	DBSCAN considera que dos \gls{datapoint} son similares si están conectados a través de una secuencia (camino) 
	de \gls{datapoint} intermedios cercanos. Por lo tanto, DBSCAN puede considerar que dos \gls{datapoint} 
...
\newglossaryentry{iid}{name={independientes e idénticamente distribuidos (i.i.d.)}, description={Es\index{independientes e idénticamente distribuidos (i.i.d.)} útil interpretar los
		\gls{datapoint} $\datapoint^{(1)},\ldots,\datapoint^{(\samplesize)}$ 
...
\newglossaryentry{outlier}{name={valor atípico (outlier)}, description={Muchos\index{valor atípico (outlier)} métodos de \gls{ml}  
		están motivados por la \gls{iidasspt}, que interpreta los \gls{datapoint} como \gls{realization}s de 
		...
		Sin embargo, en algunas aplicaciones, los \gls{data} están compuestos por una mayoría de \gls{datapoint} regulares
		que cumplen con la \gls{iidasspt} y un pequeño número de \gls{datapoint} que presentan propiedades estadísticas  
        fundamentalmente diferentes en comparación con los \gls{datapoint} regulares. Nos referimos a un \gls{datapoint} que se desvía significativamente de las propiedades estadísticas  
...
\newglossaryentry{eerm}{name={minimización del riesgo empírico explicable (EERM)}, description={La\index{minimización del riesgo empírico explicable (EERM)} 
		...
		para los \gls{datapoint} en un \gls{trainset} \cite{Zhang:2024aa}.},first={minimización del riesgo empírico explicable (EERM)},text={EERM} }
...
\newglossaryentry{highdimregime}{name={régimen de alta dimensión}, description={El\index{régimen de alta dimensión} 
		...
		es mayor que el \gls{samplesize}, es decir, el número de \gls{datapoint} etiquetados en el \gls{trainset}. 
		...
		utilizados para caracterizar los \gls{datapoint} excede el número de \gls{datapoint} en el \gls{trainset}. 
		Otro ejemplo de métodos de \gls{ml} que operan en el régimen de alta dimensión son las \gls{ann}s grandes, que tienen
		muchos más \gls{weights} ajustables (y términos de sesgo) que el número total de \gls{datapoint} en el \gls{trainset}. 
...
\newglossaryentry{maxlikelihood}{name={máxima verosimilitud}, description={
		Considera\index{máxima verosimilitud} \gls{datapoint} $\dataset=\big\{ \datapoint^{(1)}, \ldots, \datapoint^{(\samplesize)} \}$ 
...
\newglossaryentry{em}{name={esperanza-maximización (EM)}, description={
		...
		Considera un \gls{probmodel} $\prob{\datapoint; \weights}$ para los \gls{datapoint} $\dataset$ generados en alguna 
...
\newglossaryentry{ppca}{name={análisis de componentes principales probabilístico (PPCA)}, description={El análisis de componentes principales probabilístico (PPCA)\index{análisis de componentes principales probabilístico (PPCA)} 
		extiende el \gls{pca} básico mediante el uso de un \gls{probmodel} para los \gls{datapoint}. El \gls{probmodel} de PPCA
...
\newglossaryentry{polyreg}{name={regresión polinómica}, description={La\index{regresión polinómica} 
		\gls{regression} polinómica tiene como objetivo aprender un mapa de \gls{hypothesis} polinómico para predecir una \gls{label}
		 numérica en función de los \gls{feature}s numéricos de un \gls{datapoint}. Para \gls{datapoint} caracterizados por un único 
...
\newglossaryentry{ridgeregression}{name={regresión ridge}, description={La\index{regresión ridge} 
	    ...
	    \gls{sqerrloss} es equivalente a reemplazar cada \gls{datapoint} original por la \gls{realization} 
	    de (infinitos) \gls{iid} \gls{rv}s centrados alrededor de estos \gls{datapoint} (vea \gls{regularization}).},first={regresión ridge},text={regresión ridge}}
...
\newglossaryentry{iidasspt}{name={suposición de independencia e idéntica distribución (suposición i.i.d.)}, description={La suposición \gls{iid} 
		\index{suposición de independencia e idéntica distribución (suposición i.i.d.)} interpreta los \gls{datapoint} de un\gls{dataset} como las 
...
\newglossaryentry{modelparams}{name={parámetros del modelo}, 
	description={Los \gls{parameters}\index{parámetros del modelo} de un \gls{model} son cantidades que se utilizan para seleccionar un mapa de \gls{hypothesis} específico dentro de un \gls{model}. 
	Podemos pensar en una lista de \gls{parameters} del \gls{model} como un identificador único para un mapa de \gls{hypothesis}, 
...
\newglossaryentry{ai}{name={inteligencia artificial (IA)}, description={
	La IA\index{inteligencia artificial (IA)} se refiere a sistemas que se comportan de manera racional en el sentido de maximizar una \gls{reward} a largo plazo. 
	El enfoque basado en \gls{ml} para la IA consiste en entrenar un \gls{model} para predecir acciones óptimas. 
	Estas predicciones se calculan a partir de observaciones sobre el estado del entorno. 
	La elección de la \gls{lossfunc} distingue las aplicaciones de IA de las aplicaciones de \gls{ml} más básicas. 
	Los sistemas de IA rara vez tienen acceso a un \gls{trainset} etiquetado que permita medir la \gls{loss} promedio para cualquier posible elección de \gls{modelparams}. 
	En su lugar, los sistemas de IA utilizan señales de \gls{reward} observadas para obtener una estimación puntual de la \gls{loss} incurrida por la elección actual de \gls{modelparams}.},first={AI},text={AI} }

\newglossaryentry{reward}{name={recompensa}, description={Una recompensa se refiere a alguna\index{recompensa} 
	cantidad observada (o medida) que nos permite estimar la \gls{loss} incurrida por la \gls{prediction} 
	(o decisión) de una \gls{hypothesis} $\hypothesis(\featurevec)$. Por ejemplo, en una 
	aplicación de \gls{ml} para vehículos autónomos, $\hypothesis(\featurevec)$ podría representar 
...
\newglossaryentry{hardclustering}{name={agrupamiento rígido}, description={El agrupamiento rígido\index{agrupamiento rígido} 
	se refiere a la tarea de dividir un conjunto dado de \gls{datapoint} en (pocos) \gls{cluster}s no superpuestos. 
	El método de agrupamiento rígido más utilizado es \gls{kmeans}.},first={agrupamiento rígido},text={agrupamiento rígido} }
	
\newglossaryentry{softclustering}{name={agrupamiento suave}, description={El agrupamiento suave\index{agrupamiento suave} 
	se refiere a la tarea de dividir un conjunto dado de \gls{datapoint} en (pocos) \gls{cluster}s superpuestos. 
	Cada \gls{datapoint} se asigna a varios \gls{cluster}s diferentes con diversos grados de pertenencia. 
	Los métodos de agrupamiento suave determinan el \gls{dob} (o asignación suave a un \gls{cluster}) 
	para cada \gls{datapoint} y cada \gls{cluster}. Un enfoque fundamentado para el agrupamiento suave 
	consiste en interpretar los \gls{datapoint} como \gls{iid} \gls{realization}s de un \gls{gmm}. 
	De este modo, se obtiene una elección natural para el \gls{dob} como la \gls{probability} 
	condicional de que un \gls{datapoint} pertenezca a un componente específico de la mezcla.},first={agrupamiento suave},text={agrupamiento suave} }
	
\newglossaryentry{clustering}{name={agrupamiento}, description={Los métodos de agrupamiento\index{agrupamiento} 
	descomponen un conjunto dado de \gls{datapoint} en pocos subconjuntos, que se denominan \gls{cluster}s. 
	Cada \gls{cluster} está compuesto por \gls{datapoint} que son más similares entre sí que con respecto a 
	los \gls{datapoint} fuera del \gls{cluster}. Los diferentes métodos de agrupamiento utilizan 
	distintas medidas de similitud entre \gls{datapoint} y diferentes formas de representación de los \gls{cluster}s. 
	El método de agrupamiento \gls{kmeans} utiliza el vector de \gls{feature} promedio (media del \gls{cluster}) 
	de un \gls{cluster} como su representante. Un método popular de \gls{softclustering} basado en \gls{gmm} 
	representa un \gls{cluster} mediante una \gls{mvndist}.},first={agrupamiento},text={agrupamiento} }
	
\newglossaryentry{cluster}{name={clúster}, description={Un\index{clúster} clúster es un subconjunto de 
	\gls{datapoint} que son más similares entre sí que a los \gls{datapoint} fuera del clúster. 
	La medida cuantitativa de similitud entre los \gls{datapoint}s es una decisión de diseño. 
	Si los \gls{datapoint} se caracterizan por vectores de \gls{feature} euclidianos 
	$\featurevec \in \mathbb{R}^{\nrfeatures}$, podemos definir la similitud entre dos \gls{datapoint} 
	a través de la distancia euclidiana entre sus vectores de \gls{feature}.},first={clúster},text={clúster} }
...
\newglossaryentry{huberloss}{name={pérdida de Huber}, description={La\index{pérdida de Huber} 
		\gls{loss} de Huber unifica la \gls{sqerrloss} y la \gls{abserr}.},first={pérdida de Huber},text={pérdida de Huber} }

\newglossaryentry{svm}{name={máquina de vectores de soporte (SVM)}, description={La\index{máquina de vectores de soporte (SVM)} 
		SVM es un método de \gls{classification} binaria que  
		aprende un mapa de \gls{hypothesis} lineal. Por lo tanto, al igual que la  \gls{linreg} y la \gls{logreg}, 
		también es una instancia de \gls{erm} para el \gls{linmodel}. Sin embargo, la
		SVM utiliza una \gls{lossfunc} diferente a la empleada en esos métodos. Como se ilustra en 
		la Figura \ref{fig_svm_gls_dict}, su objetivo es separar al máximo los \gls{datapoint} de 
		las dos clases diferentes en el \gls{featurespace} (es decir, el principio de margen \gls{maximum}). 
		...
		\begin{figure}[H]
			...
			\end{center}
		\end{figure}
		La variante básica de la \gls{svm} solo es útil si los \gls{datapoint} de diferentes categorías pueden ser (aproximadamente)
		separables linealmente. Para una aplicación de \gls{ml} donde las categorías no son separables linealmente basadas en las \gls{feature}s
	    originales (crudas), es posible aplicar la \gls{svm} a \gls{feature}s transformadas.
		Estas \gls{feature}s transformadas se pueden obtener aplicando un \gls{featuremap} derivado de un \gls{kernel}.
...
\newglossaryentry{eigenvector}{name={vector propio}, description={Un\index{vector propio} 
		...
		tal que $\mathbf{A} \vx = \lambda \vx$ para algún \gls{eigenvalue} $\lambda$.},first={vector propio},text={vector propio} }
...
\newglossaryentry{evd}{name={descomposición en valores propios (EVD)}, 
	...
	\gls{eigenvector}s de la matriz $\mA$. La matriz diagonal 
	${\bm \Lambda} = {\rm diag} \big\{ \eigval{1},\ldots,\eigval{\dimlocalmodel} \big\}$ 
	contiene los \gls{eigenvalue}s $\eigval{\featureidx}$ correspondientes a los \gls{eigenvector}s $\vv^{(\featureidx)}$. 
	Es importante notar que esta descomposición solo existe si la matriz $\mA$ es diagonalizable.},first={descomposición en valores propios (EVD)},text={EVD} }
...
\newglossaryentry{tv}{name={variación total}, description={Vea \gls{gtv}\index{variación total}.},
 ...
 \newglossaryentry{cvxclustering}{name={agrupamiento convexo}, 
 	description={Considere\index{agrupamiento convexo} un \gls{dataset} 
 	$\featurevec^{(1)},\ldots,\featurevec^{(\samplesize)} \in \mathbb{R}^{\nrfeatures}$. 
 	El \gls{clustering} \Gls{convex} aprende vectores $\weights^{(1)},\ldots,\weights^{(\samplesize)}$ minimizando 
	...
	denota la $p$-\gls{norm} (para $p\geq1$).  
	Resulta que muchos de los vectores óptimos $\widehat{\weights}^{(1)},\ldots,\widehat{\weights}^{(\samplesize)}$ 
	coinciden. Un \gls{cluster} consiste entonces en aquellos \gls{datapoint} $\sampleidx \in \{1,\ldots,\samplesize\}$ 
...
\newglossaryentry{gdmethods}{name={métodos de gradiente}, 
	...
	métodos de gradiente son técnicas iterativas para encontrar el \gls{minimum} (o \gls{maximum}) 
	de una \gls{objfunc} \gls{differentiable} respecto a los \gls{modelparams}. 
	Estos métodos construyen una secuencia de aproximaciones hacia una elección óptima de 
	\gls{modelparams} que resulta en un \gls{minimum} (o \gls{maximum}) valor de la \gls{objfunc}. 
	Como su nombre indica, los métodos basados en gradiente utilizan los \gls{gradient}s de la \gls{objfunc} 
	evaluados en iteraciones previas para construir nuevos \gls{modelparams} (esperablemente) mejorados. 
	Un ejemplo importante de un método basado en gradiente es el \gls{gd}.},
...
\newglossaryentry{sgd}{name={descenso por subgradiente}, description={El\index{descenso por subgradiente} 
	descenso por subgradiente es una \gls{generalization} del \gls{gd} que no requiere la diferenciabilidad 
	de la función a minimizar. Esta \gls{generalization} se obtiene al reemplazar el concepto 
	de \gls{gradient} por el de \gls{subgradient}. Similar a los \gls{gradient}s, los \gls{subgradient}s 
	permiten construir aproximaciones locales de una \gls{objfunc}. La \gls{objfunc} 
	podría ser el \gls{emprisk} $\emperror\big( \hypothesis^{(\weights)} \big| \dataset \big)$ visto 
	como una función de los \gls{modelparams} $\weights$ que seleccionan una \gls{hypothesis} $\hypothesis^{(\weights)} \in \hypospace$.}
...
\newglossaryentry{stochGD}{name={descenso de gradiente estocástico (SGD)}, description={El\index{descenso de gradiente estocástico (SGD)} 
		descenso de gradiente estocástico es una variante del \gls{gd} en la que se reemplaza el \gls{gradient} de la \gls{objfunc} 
		por una aproximación estocástica. Una aplicación principal del descenso por gradiente estocástico  es entrenar un
		\gls{model} parametrizado mediante \gls{erm} sobre un \gls{trainset} $\dataset$ que es muy grande o no está fácilmente disponible 
		(por ejemplo, cuando los \gls{datapoint} están almacenados en bases de datos distribuidas por todo el mundo). Para evaluar el \gls{gradient} del
		\gls{emprisk} (como función de los \gls{modelparams} $\weights$), 
		se requiere calcular una suma $\sum_{\sampleidx=1}^{\samplesize} \nabla_{\weights} \lossfunc{\datapoint^{(\sampleidx)}}{\weights}$  
		sobre todos los \gls{datapoint} del \gls{trainset}. Una aproximación estocástica se obtiene al reemplazar esta suma 
		$\sum_{\sampleidx=1}^{\samplesize} \nabla_{\weights} \lossfunc{\datapoint^{(\sampleidx)}}{\weights}$ 
		...
		A estos \gls{datapoint} seleccionados aleatoriamente se les denomina a menudo \gls{batch}. 
		El tamaño del \gls{batch}, denotado $|\batch|$ es un parámetro importante del descenso por gradiente estocástico. \gls{gd}. 
		El descenso por gradiente estocástico con $|\batch|> 1$ se conoce como mini-\gls{batch} SGD \cite{Bottou99}. 		
		\begin{figure}[H]
			\centering
...
% Labels
			\end{tikzpicture}
		\caption{El descenso por gradiente estocástico (\gls{gd}) para \gls{erm} aproxima el \gls{gradient} 
		$\sum_{\sampleidx=1}^{\samplesize} \nabla_{\weights} \lossfunc{\datapoint^{(\sampleidx)}}{\weights}$ 
		reemplazando la suma sobre todos los 
		\gls{datapoint} del \gls{trainset} (indexados por $\sampleidx=1,\ldots,\samplesize$) 
...
Considere \index{descenso por gradiente en línea (online GD)} un método de \gls{ml} que aprende los \gls{modelparams} 
$\weights$ a partir de un \gls{paramspace} $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$. 
El proceso de aprendizaje utiliza \gls{datapoint} $\datapoint^{(\timeidx)}$ que llegan en instantes de tiempo consecutivos $\timeidx=1,2,\ldots$. 
Interpretamos los \gls{datapoint} $\datapoint^{(\timeidx)}$ como copias \gls{iid} de una \gls{rv} $\datapoint$. 
El \gls{risk} $\expect\{ \lossfunc{\datapoint}{\weights} \}$ de una \gls{hypothesis} $\hypothesis^{(\weights)}$ 
...
Podríamos usar este límite como la \gls{objfunc} para aprender los \gls{modelparams} $\weights$. 
Sin embargo, este límite solo puede evaluarse si esperamos un tiempo infinito para recolectar todos los \gls{datapoint}. 
Algunas aplicaciones de \gls{ml} requieren métodos que aprendan en línea: tan pronto como llega un nuevo \gls{datapoint} $\datapoint^{(\timeidx)}$ 
en el tiempo $\timeidx$, actualizamos los \gls{modelparams} actuales $\weights^{(\timeidx)}$. 
Nótese que el nuevo \gls{datapoint} $\datapoint^{(\timeidx)}$ contribuye con el componente $\lossfunc{\datapoint^{(\timeidx)}}{\weights}$ 
al \gls{risk}. Como sugiere el nombre, el descenso por gradiente en línea actualiza $\weights^{(\timeidx)}$ mediante un (proyectado) \gls{gradstep}:
\begin{equation} 
...
\end{equation} 
Nótese que \eqref{equ_def_ogd_dict} es un \gls{gradstep} para el componente actual $\lossfunc{\datapoint^{(\timeidx)}}{\cdot}$ 
del \gls{risk}. La actualización \eqref{equ_def_ogd_dict} ignora todos los componentes anteriores $\lossfunc{\datapoint^{(\timeidx')}}{\cdot}$ 
para $\timeidx' < \timeidx$. Por tanto, podría suceder que, comparado con $\weights^{(\timeidx)}$, los \gls{modelparams} actualizados 
$\weights^{(\timeidx+1)}$ aumenten el \gls{loss} promedio retrospectivo $\sum_{\timeidx'=1}^{\timeidx-1} \lossfunc{\datapoint^{(\timeidx')}}{\cdot}$. 
Sin embargo, para un \gls{learnrate} $\lrate_{\timeidx}$ elegido apropiadamente, se puede demostrar que el descenso por gradiente en línea 
es óptimo en escenarios de interés práctico. Por óptimo, entendemos que los \gls{modelparams} 
$\weights^{(T+1)}$ entregados por el descenso por gradiente en línea tras observar $T$ \gls{datapoint} $\datapoint^{(1)},\ldots, \datapoint^{(T)}$ 
...
\begin{tikzpicture}[x=1.5cm,scale=1.5, every node/.style={font=\footnotesize}]
	...
	\end{tikzpicture}
\end{center} 
\caption{Una instancia de descenso por \gls{gd}  en línea que actualiza los \gls{modelparams} $\weights^{(\timeidx)}$ 
usando el \gls{datapoint} $\datapoint^{(\timeidx)} = \feature^{(\timeidx)}$ que llega en el tiempo $\timeidx$. 
Esta instancia emplea la \gls{sqerrloss} $\lossfunc{\datapoint^{(\timeidx)}}{\weight} = (\feature^{(\timeidx)} - \weight)^{2}$.
}
\end{figure}},
...
\newglossaryentry{pca}{name={análisis de componentes principales (PCA)}, description={El PCA\index{análisis de componentes principales (PCA)} 
		determina una \gls{featuremap} lineal tales que las nuevos \gls{feature}s 
		permiten reconstruir los \gls{feature}s originales con el \gls{minimum} error de reconstrucción \cite{MLBasics}.},first={análisis de componentes principales (PCA)},text={PCA} }
	
\newglossaryentry{loss}{name={pérdida}, description={Los métodos de \gls{ml}\index{pérdida} usan una \gls{lossfunc} $\lossfunc{\datapoint}{\hypothesis}$ 
para medir el error incurrido al aplicar una \gls{hypothesis} específica a un \gls{datapoint} específico. 
Con un pequeño abuso de notación, usamos el término pérdida tanto para la función de \gls{lossfunc} $\loss$ en sí 
como para el valor específico $\lossfunc{\datapoint}{\hypothesis}$, para un \gls{datapoint} $\datapoint$ 
y una \gls{hypothesis} $\hypothesis$.},first={pérdida},text={pérdida} }

\newglossaryentry{lossfunc}{name={función de pérdida}, description={Una\index{función de pérdida} función de \gls{loss} es una aplicación
		...
		Asigna un número real no negativo (es decir, la \gls{loss}) $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$
		a un par que consiste en un \gls{datapoint}, con \gls{feature}s $\featurevec$ y una \gls{label} $\truelabel$, 
		y una \gls{hypothesis} $\hypothesis \in \hypospace$. 
		El valor $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$ cuantifica la discrepancia entre la \gls{label} verdadera $\truelabel$ 
		y la \gls{prediction} $\hypothesis(\featurevec)$. Valores bajos (cercanos a cero) de $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$ 
		indican una discrepancia menor entre la \gls{prediction} $\hypothesis(\featurevec)$ y la \gls{label} $\truelabel$. 
		La Figura \ref{fig_loss_function_gls_dict} muestra una función de \gls{loss} para un \gls{datapoint} dado, 
		con \gls{feature}s $\featurevec$ y \gls{label} $\truelabel$, como función de la \gls{hypothesis} $\hypothesis \in \hypospace$. 
		\begin{figure}[H]
			\begin{center}
...
\newglossaryentry{decisiontree}{name={árbol de decisión}, description={Un\index{árbol de decisión} 
		árbol de decisión es una representación similar a un diagrama de flujo de un mapa de \gls{hypothesis} $\hypothesis$. 
		Más formalmente, un árbol de decisión es un \gls{graph} dirigido que contiene un nodo raíz que lee
		el \gls{featurevec} $\featurevec$ de un\gls{datapoint}. El nodo raíz luego transfiere 
		el \gls{datapoint} a uno de sus nodos hijos basado en alguna prueba elemental sobre los \gls{feature}s $\featurevec$. 
		Si el nodo hijo receptor no es un nodo hoja, es decir, tiene sus propios nodos hijos, 
		representa otra prueba. Según el resultado de la prueba, el \gls{datapoint} se transfiere
		a uno de sus descendientes. Esta prueba y transferencia del \gls{datapoint} continúa
		hasta que el \gls{datapoint}termina en un nodo hoja (que no tiene nodos hijos). 
\begin{figure}[H]
...
Un ejemplo importante de espacio de Hilbert es el \gls{euclidspace} $\mathbb{R}^{\featuredim}$, para alguna dimensión $\featuredim$, 
...
secuencia finita (o lista) de \gls{datapoint} $\datapoint^{(1)},\ldots,\datapoint^{(m)}$ que 
se obtiene o interpreta como la \gls{realization} de $\samplesize$ \gls{iid} \gls{rv}s 
con una \gls{probdist} común $p(\datapoint)$. La longitud $\samplesize$ de 
la secuencia se denomina \gls{samplesize}.},first={muestra},text={muestra}}
	
\newglossaryentry{samplesize}
{name=tamaño de la muestra,
	description={El \index{tamaño de la muestra} número de \gls{datapoint}
	individuales contenidos en un \gls{dataset}.},first={tamaño de la muestra},text={tamaño de la muestra}
}
...
\newglossaryentry{ann}{
	...
	las \gls{feature}s de un \gls{datapoint} en su entrada a una \gls{prediction} 
	para la correspondiente \gls{label} en su salida. La unidad fundamental de una 
	RNA es la neurona artificial, que aplica una \gls{actfun} a sus 
...
{name=bosque aleatorio,
	description={Un \index{bosque aleatorio} bosque aleatorio es un conjunto (o ensamblaje) de diferentes \gls{decisiontree}s. 
	Cada uno de estos \gls{decisiontree}s se obtiene al ajustar una copia perturbada del 
	\gls{dataset} original.},first = {bosque aleatorio}, text={bosque aleatorio}
}

\newglossaryentry{bagging}{name={bagging},description={El \index{bagging} bagging (o agregación por bootstrap) 
es una técnica genérica para mejorar (la robustez de) un método de \gls{ml} dado. La idea es utilizar el \gls{bootstrap} 
para generar copias perturbadas de un \gls{dataset} dado y luego aprender una \gls{hypothesis} separada para 
cada copia. Posteriormente, se predice la \gls{label} de un \gls{datapoint} combinando o agregando las 
\gls{prediction}s individuales de cada \gls{hypothesis} separada. Para mapas de \gls{hypothesis} que entregan valores 
numéricos de \gls{label}, esta agregación podría implementarse calculando el promedio de las \gls{prediction}s individuales.},first={bootstrap aggregation (bagging)},text={bagging}}

\newglossaryentry{gd}{name={descenso por gradiente (GD)},description={El \gls{gradient}\index{descenso por gradiente (GD)} 
desceso por gradiente es un método iterativo para encontrar el \gls{minimum} de una función \gls{differentiable} $f(\weights)$ 
con un argumento vectorial $\weights \in \mathbb{R}^{\featurelen}$. Considera una estimación o 
aproximación actual $\weights^{(\itercntr)}$ para el \gls{minimum} de la función $f(\weights)$. Queremos encontrar un nuevo vector (mejor) $\weights^{(\itercntr+1)}$ 
que tenga un valor objetivo menor $f(\weights^{(\itercntr+1)}) < f\big(\weights^{(\itercntr)}\big)$ que 
la estimación actual $\weights^{(\itercntr)}$. Esto se puede lograr típicamente utilizando un \gls{gradstep}.
		\begin{equation} 
		...
		\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{Un \gls{gradstep} singular  \eqref{equ_def_GD_step_dict} hacia el minimizador $\overline{\weights}$ de $f(\weights)$.}
...
\newglossaryentry{abserr}{name={pérdida por error absoluto},description={
	Considera un \gls{datapoint} con \gls{feature}s $\featurevec \in \featurespace$ y una 
	\gls{label} numérica $\truelabel \in \mathbb{R}$. La \gls{loss}\index{pérdida por error absoluto} 
	por error absoluto incurrida por una \gls{hypothesis} $\hypothesis: \featurespace \rightarrow \mathbb{R}$ 
	se define como $|\truelabel - \hypothesis(\featurevec)|$, es decir, la diferencia absoluta entre 
	la \gls{prediction} $\hypothesis(\featurevec)$ y la \gls{label} verdadera $\truelabel$.},
			first={pérdida por error absoluto},text={pérdida por error absoluto}}

\newglossaryentry{device}{name={dispositivo},description={
				Cualquier\index{dispositivo} sistema físico que pueda usarse para almacenar y procesar \gls{data}. En el contexto de \gls{ml}, 
				generalmente nos referimos a un ordenador que puede leer \gls{datapoint} de diferentes 
				fuentes y, a su vez, entrenar un \gls{model} de \gls{ml} utilizando estos \gls{datapoint}.},
				first={dispositivo},text={dispositivo}}

\newglossaryentry{llm}{name={modelo de lenguaje de gran escala (LLM)},description={
	Los \gls{model}s de lenguaje de gran escala\index{modelo de lenguaje de gran escala (LLM)} son un término genérico para métodos de \gls{ml} 
	que procesan y generan texto similar al humano. Estos métodos suelen 
	usar \gls{deepnet}s con miles de millones (o incluso billones) de \gls{parameters}. 
	Una elección ampliamente utilizada para la arquitectura de red se conoce como 
	Transformers \cite{vaswani2017attention}. El entrenamiento de \gls{model}s de lenguaje de gran escala a menudo 
	se basa en la tarea de predecir algunas palabras que se eliminan intencionadamente 
	de un corpus de texto extenso. Así, podemos construir \gls{labeled datapoint}s 
	simplemente seleccionando algunas palabras de un texto como \gls{label}s y las palabras 
	restantes como \gls{feature}s de \gls{datapoint}. Esta construcción requiere 
	muy poca supervisión humana y permite generar \gls{trainset}s suficientemente 
	grandes para \gls{model}s de lenguaje de gran escala.},
					first={modelo de lenguaje de gran escala (LLM)},text={LLM}}


\newglossaryentry{huberreg}{name={regresión de Huber},description={
	La \gls{regression}\index{regresión de Huber} de Huber se refiere a métodos basados en \gls{erm} 
	que utilizan la \gls{huberloss} como medida del error de \gls{prediction}. 
	Dos casos especiales importantes de la \gls{regression} de Huber son \gls{ladregression} y 
	\gls{linreg}. Ajustar el parámetro de umbral de la \gls{huberloss} permite al usuario
	balancear la robustez del \gls{abserr} frente a los beneficios computacionales de la \gls{smooth} \gls{sqerrloss}.},
...
\newglossaryentry{ladregression}{name={regresión por desviación absoluta mínima},description={
		...
		es una instancia de \gls{erm} que utiliza el \gls{abserr}. Es un caso especial de  
		\gls{huberreg}.},
...
\newglossaryentry{bayesrisk}{name={riesgo de Bayes},description={Considera un \gls{probmodel} con una 
\gls{probdist} conjunta $p(\featurevec,\truelabel)$ para los \gls{feature}s $\featurevec$ 
y la \gls{label} $\truelabel$ de un \gls{datapoint}. El \index{riesgo de Bayes} riesgo de Bayes 
es el \gls{risk} \gls{minimum} posible que puede alcanzarse por cualquier \gls{hypothesis} 
$\hypothesis: \featurespace \rightarrow \labelspace$. Cualquier \gls{hypothesis} que alcance 
el riesgo de Bayes se denomina un \gls{bayesestimator} \cite{LC}.},first={riesgo de Bayes},text={riesgo de Bayes}}
	
\newglossaryentry{bayesestimator}{name={estimador de Bayes},description={Considera\index{estimador de Bayes} 
un \gls{probmodel} con una \gls{probdist} conjunta $p(\featurevec,\truelabel)$ para los \gls{feature}s $\featurevec$ y la \gls{label} 
$\truelabel$ de un \gls{datapoint}. Para una \gls{lossfunc} dada $\lossfunc{\cdot}{\cdot}$, denominamos a una \gls{hypothesis} 
$\hypothesis$ como un estimador de Bayes si su \gls{risk} $\expect\{\lossfunc{\pair{\featurevec}{\truelabel}}{\hypothesis}\}$ es el 
\gls{minimum} \cite{LC}. Nótese que la propiedad de una \gls{hypothesis} de ser un estimador de Bayes depende de 
la \gls{probdist} subyacente y de la elección de la \gls{lossfunc} $\lossfunc{\cdot}{\cdot}$.},
		first={Bayes estimator},text={Bayes estimator}}


\newglossaryentry{weights}{name={pesos},
	description={Considera\index{weights} un \gls{hypospace} parametrizado $\hypospace$. 
	Usamos el término pesos para los \gls{modelparams} numéricos que se 
	utilizan para escalar las \gls{feature}s o sus transformaciones con el fin de calcular $\hypothesis^{(\weights)} \in \hypospace$. 
	Un \gls{linmodel} utiliza los pesos $\weights=\big(\weight_{1},\ldots,\weight_{\nrfeatures}\big)^{T}$ para calcular 
	la combinación lineal $\hypothesis^{(\weights)}(\featurevec)= \weights^{T} \featurevec$. 
	Los pesos también se utilizan en las \gls{ann}s para formar combinaciones lineales de las \gls{feature}s o de las salidas de las neuronas en capas ocultas.},
	first={pesos},text={pesos}}
	
\newglossaryentry{probdist}{name={distribución de probabilidad},
	description={Para\index{distribución de probabilidad} analizar métodos de \gls{ml}, puede ser útil 
	interpretar los \gls{datapoint} como \gls{iid} \gls{realization}s de una \gls{rv}. Las propiedades típicas 
	de tales \gls{datapoint} están gobernadas por la distribución de \gls{probability} de esta \gls{rv}. 
	La distribución de \gls{probability} de una \gls{rv} binaria $\truelabel \in \{0,1\}$ 
	se especifica completamente mediante las probabilidades $\prob{\truelabel = 0}$ y 
	$\prob{\truelabel=1}\!=\!1\!-\!\prob{\truelabel=0}$. La distribución de \gls{probability} 
	de una \gls{rv} con valores reales $\feature \in \mathbb{R}$ puede especificarse 
	mediante una \gls{pdf} $p(\feature)$ tal que $\prob{ \feature \in [a,b] } \approx  p(a) |b-a|$. 
	En el caso más general, una distribución de \gls{probability} se define mediante una medida de \gls{probability} \cite{GrayProbBook,BillingsleyProbMeasure}.},
...
\newglossaryentry{pdf}{name={función de densidad de probabilidad (pdf)},
	description={La\index{función de densidad de probabilidad (pdf)} \gls{probability} densidad de probabilidad $p(\feature)$ 
	de una \gls{rv} con valores reales $\feature \in \mathbb{R}$ es una representación particular de su \gls{probdist}. 
	Si la función de densidad de \gls{probability} existe, se puede usar para calcular la \gls{probability} de que $\feature$ tome un valor 
	...
	La función de densidad de \gls{probability} de una \gls{rv} vectorial $\featurevec \in \mathbb{R}^{\featuredim}$ (si existe) 
	permite calcular la \gls{probability} de que $\featurevec$ pertenezca a una región (medible) $\mathcal{R}$ mediante 
...
\newglossaryentry{parameters}{name={parámetros},
	description={Los\index{parámetros} parámetros de un \gls{ml} \gls{model} son cantidades ajustables (es decir, entrenables o modificables) 
	que nos permiten elegir entre diferentes funciones de \gls{hypothesis}. 
	Por ejemplo, el \gls{linmodel} $\hypospace \defeq \{\hypothesis^{(\weights)}: \hypothesis^{(\weights)}(\feature)= \weight_{1} \feature + \weight_{2}\}$ 
	consiste en todas las funciones de \gls{hypothesis} $\hypothesis^{(\weights)}(\feature)= \weight_{1} \feature + \weight_{2}$ 
	con una elección particular de los parámetros $\weights = \big(\weight_{1},\weight_{2}\big)^{T} \in \mathbb{R}^{2}$. 
	Otro ejemplo de parámetros son los \gls{weights} asignados a las conexiones 
	entre neuronas de una \gls{ann}.},
...
\newglossaryentry{lln}{name={ley de los grandes números},
	...
	convergencia del promedio de un número creciente (grande) de \gls{iid} \gls{rv}s 
	hacia la \gls{mean} de su \gls{probdist} común. Diferentes instancias de la 
...
\newglossaryentry{stopcrit}{name={criterio de parada},
	description={Muchos \index{criterio de parada} métodos de \gls{ml} utilizan \gls{algorithm}s iterativos que construyen una 
	secuencia de \gls{modelparams} (como los \gls{weights} de un mapa lineal o 
	los \gls{weights} de una \gls{ann}). Estos parámetros (idealmente) convergen a una elección óptima 
	para los \gls{modelparams}. En la práctica, dados recursos computacionales 
	...
	la iteración.},first={criterio de parada},text={criterio de parada}}
...
\newglossaryentry{kCV}{name={validación cruzada de $k$ particiones ($k$-fold CV)},
	...
	método para aprender y validar una \gls{hypothesis} utilizando un \gls{dataset} dado. 
	Este método divide el \gls{dataset} equitativamente en $k$ subconjuntos o particiones 
	y luego ejecuta $k$ repeticiones de entrenamiento de \gls{model} (por ejemplo, mediante \gls{erm}) y \gls{validation}. 
	Cada repetición utiliza una partición diferente como \gls{valset} y las $k-1$ particiones 
	restantes como \gls{trainset}. El resultado final es el promedio de los \gls{valerr}s obtenidos 
	desde las $k$ repeticiones.},first={$k$-fold cross-validation ($k$-fold CV)},text={$k$-fold CV}}
...
\newglossaryentry{convex}{name={convexo},
	description={Un\index{convexo} subconjunto $\mathcal{C} \subseteq \mathbb{R}^{\featuredim}$ del
		\gls{euclidspace} $\mathbb{R}^{\featuredim}$ se denomina convexo si contiene
		...
		\end{figure}},first={convexo},text={convexo}}
		...
		description={Una\index{suave} función con valores reales $f: \mathbb{R}^{\dimlocalmodel} \rightarrow \mathbb{R}$ 
			es suave si es \gls{differentiable} y su \gls{gradient} $\nabla f(\weights)$ es continuo en todos $\weights \in \mathbb{R}^{\dimlocalmodel}$  \cite{nesterov04}, \cite{CvxBubeck2015}. Una función suave $f$ se denomina $\beta$-suave si el \gls{gradient} 
			...
			más suave es $f$. Los problemas de optimización con una \gls{objfunc} suave pueden resolverse eficazmente mediante \gls{gdmethods}. 
			De hecho, los \gls{gdmethods} aproximan la \gls{objfunc} localmente alrededor de una elección actual $\weights$ 
			utilizando su \gls{gradient}. Esta aproximación funciona bien si el \gls{gradient} no 
			cambia demasiado rápido. Podemos precisar esta afirmación informal estudiando el efecto de un solo
			\gls{gradstep} con \gls{stepsize} $\lrate=1/\beta$ (vea Fig. \ref{fig_gd_smooth_dict}). 
			\begin{figure}[H] 
				\begin{center} 
...
\newglossaryentry{paramspace}{name={espacio de parámetros},
		...
		un \gls{model} de \gls{ml}  $\hypospace$ es el conjunto de todas las elecciones factibles para los
		\gls{modelparams} (vea la Figura \ref{fig_param_space_dict}). Muchos métodos de \gls{ml} importantes 
		usan un \gls{model} que está parametrizado por vectores del \gls{euclidspace} $\mathbb{R}^{\dimlocalmodel}$. 
		Dos ejemplos ampliamente utilizados de \gls{model}s parametrizados son los \gls{linmodel}s 
		y los \gls{deepnet}s. El espacio de parámetros es entonces a menudo un subconjunto $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$, 
		por ejemplo, todos los vectores $\weights \in \mathbb{R}^{\dimlocalmodel}$ con una \gls{norm} menor a uno.
		\begin{figure}[H]
			\begin{center}
			\begin{tikzpicture}
			...
			\end{tikzpicture}
			\end{center} 
			\caption{El espacio de parámetros $\paramspace$ de un \gls{model} de\gls{ml} $\hypospace$ consiste en
			todas las elecciones factibles para los \gls{modelparams}. Cada elección $\weights$ para los \gls{modelparams} 
			seleccionan un mapa de \gls{hypothesis}  $\hypothesis^{(\weights)} \in \hypospace$.
...
\newglossaryentry{datanorm}{name={normalización de datos},
	description={La normalización de \gls{data} \index{normalización de datos} se refiere a transformaciones 
	aplicadas a los \gls{featurevec}s de \gls{datapoint} para mejorar los aspectos \gls{statasp} o \gls{compasp} 
	del método de \gls{ml}. Por ejemplo, en \gls{linreg} con \gls{gdmethods} que utilizan 
	una \gls{learnrate} fija, la convergencia depende de controlar la \gls{norm} de los \gls{featurevec}s 
	en el \gls{trainset}. Un enfoque común es normalizar los \gls{featurevec}s de modo que su 
	\gls{norm} no exceda de uno \cite[Ch.\ 5]{MLBasics}.},
	first={data normalization},text={data normalization}}

\newglossaryentry{dataaug}{name={aumentación de datos},
	description={Los métodos de aumentación de \gls{data} \index{aumentación de datos} añaden \gls{datapoint} sintéticos 
	a un conjunto existente de \gls{datapoint}. Estos \gls{datapoint} sintéticos se obtienen mediante 
	perturbaciones (por ejemplo, añadir ruido a mediciones físicas) o transformaciones 
	(por ejemplo, rotaciones de imágenes) de los \gls{datapoint} originales. Estas perturbaciones y 
	transformaciones son tales que los \gls{datapoint} sintéticos resultantes deben 
	tener la misma \gls{label}. Como ejemplo, una imagen de un gato rotada sigue siendo 
	una imagen de un gato, aunque sus \gls{featurevec}s (obtenidos al apilar las intensidades de color de los píxeles) 
	sean muy diferentes (ver Figura \ref{fig_symmetry_dataaug_dict}). La aumentación de \gls{data} puede ser una 
	forma eficiente de \gls{regularization}.
		\begin{figure}[H]
		\begin{center}
			\begin{tikzpicture}
			  ...
			  \vspace*{-11mm}
		\end{center}
		\caption{La aumentación de \Gls{data} aprovecha las simetrías intrínsecas de los \gls{datapoint} en 
			algún \gls{featurespace} $\featurespace$. Podemos representar una simetría mediante
		    ...
		    podría representar el efecto de rotar una imagen de un gato $\eta$ grados. Un \gls{datapoint} 
		    con \gls{featurevec} $\featurevec^{(2)} = \mathcal{T}^{(\eta)} \big(\featurevec^{(1)} \big)$ debete
		    tener la misma \gls{label} $\truelabel^{(2)}=\truelabel^{(1)}$ que un \gls{datapoint} 
		    con \gls{featurevec} $\featurevec^{(1)}$.\label{fig_symmetry_dataaug_dict}}
		 \end{figure} },first={aumentación de datos},text={aumentación de datos}}
	
	
\newglossaryentry{localdataset}{name={conjunto de datos local},description={El \index{conjunto de datos local} concepto de un \gls{dataset} local se encuentra 
entre el concepto de un \gls{datapoint} y un \gls{dataset}. Un \gls{dataset} local consiste en varios 
\gls{datapoint} individuales, que se caracterizan por \gls{feature}s y \gls{label}s. 
A diferencia de un único \gls{dataset} utilizado en métodos \gls{ml} básicos, un \gls{dataset} local también 
está relacionado con otros \gls{dataset}s locales mediante diferentes nociones de similitud. Estas similitudes 
pueden surgir de \gls{probmodel}s o de la infraestructura de comunicación y 
están codificadas en las aristas de un \gls{empgraph}.},first={conjunto de datos local},text={conjunto de datos local}}
	
\newglossaryentry{localmodel}{name={modelo local},description={Considera\index{modelo local} una colección 
		de \gls{localdataset}s que están asignados a los nodos de un \gls{empgraph}. Un \gls{model} local $\localmodel{\nodeidx}$ 
		es un \gls{hypospace} asignado a un nodo $\nodeidx \in \nodes$. Diferentes nodos podrían tener asignados
		diferentes \gls{hypospace}s, es decir, en general $\localmodel{\nodeidx} \neq \localmodel{\nodeidx'}$ para diferentes
		nodos $\nodeidx, \nodeidx' \in \nodes$.  },first={modelo local},text={modelo local}}
...
{name={información mutua (MI)},
 ...
 entre dos \gls{rv}s $\featurevec$, $\truelabel$ definidas en el mismo \gls{probspace} 
 ...
 $\truelabel$ puede predecirse bien únicamente a partir de $\featurevec$. Esta \gls{prediction} podría obtenerse mediante una 
	 \gls{hypothesis} aprendida por un método de \gls{ml} basado en \gls{erm}.
	 }, first={MI}, text={MI} 
}
...
\newglossaryentry{zerogradientcondition}{name={condición de gradiente cero},
	...
	una \gls{objfunc} \gls{smooth} y \gls{convex} $f(\weights)$. Una condición necesaria y 
	suficiente para que un vector $\widehat{\weights} \in \mathbb{R}^{\dimlocalmodel}$ 
	resuelva este problema es que el \gls{gradient} $\nabla f \big( \widehat{\weights} \big)$ 
	sea el vector cero, 
...
\newglossaryentry{dataminprinc}{name={principio de minimización de datos},
	description={La regulación europea de protección de \gls{data} \index{principio de minimización de datos} 
