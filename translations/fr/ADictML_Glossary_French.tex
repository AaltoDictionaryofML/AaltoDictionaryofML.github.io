...
{name={apprentissage automatique (ou apprentissage machine)},
		 description={ML\index{apprentissage automatique (ou apprentissage machine)} aims to predict 
	 ...
	 through the minimization of a \gls{lossfunc} \cite{MLBasics,HastieWainwrightBook}. 
...
{name={apprentissage multitâche},description=
	{Multitask learning\index{apprentissage multitâche} aims at leveraging relations between 
	...
	first={apprentissage multitâche},text={apprentissage multitâche}
}

\newglossaryentry{learningtask}
{name={tâche d'apprentissage},description=
	{Consider\index{tâche d'apprentissage} a \gls{dataset} $\dataset$ constituted by several \gls{datapoint}s, each of them 
	 ...
	 (via \gls{multitask learning} methods) is typically preferable over solving them separately \cite{Caruana:1997wk}, \cite{JungGaphLassoSPL}, \cite{CSGraphSelJournal}.}, text={tâche d'apprentissage}, plural={tâches d'apprentissage}
}
...
\newglossaryentry{linmodel}{name={modèle linéaire},
	description={Consider\index{modèle linéaire}  \gls{datapoint}s, each characterized by a numeric \gls{featurevec} 
   ...
   first={modèle linéaire},text={modèle linéaire}}
...
\newglossaryentry{mvndist}{name={loi normale multivariée}, 
	description={La\index{loi normale multivariée} loi normale multivariée $\mvnormal{\vm}{\mC}$ est une 
		famille importante de \glspl{probdist} pour une \gls{rv} continue $\featurevec \in \mathbb{R}^{\nrfeatures}$ \cite{BertsekasProb,GrayProbBook,Lapidoth09}. 
		Cette famille est paramétrée par la \gls{mean} $\vm$ et la \gls{covmtx} $\mC$ de $\featurevec$. 
		Si la \gls{covmtx} est inversible, la \gls{probdist} de $\featurevec$ est donnée par 
		$$p(\featurevec) \propto \exp\bigg(-(1/2) \big( \featurevec - \vm \big)^{T} \mC^{-1} \big( \featurevec - \vm \big) \bigg).$$}, 
	first={loi normale multivariée},text={loi normale multivariée}}
...
\newglossaryentry{probability}{name={probabilité},
	description={We\index{probabilité} assign a probability value, typically chosen in the 
		interval $[0,1]$, to each event that might occur in a random experiment \cite{KallenbergBook,BertsekasProb,BillingsleyProbMeasure,HalmosMeasure}.},first={probabilité},text={probabilité}}
...
\newglossaryentry{gaussrv}{name={variable aléatoire normale/gaussienne (VA normale)},description={
		A \index{variable aléatoire normale/gaussienne (VA normale)} standard Gaussian \gls{rv} is a 
		real-valued \gls{rv} $x$ with \gls{pdf} \cite{papoulis,BertsekasProb,GrayProbBook}
		\begin{equation}
			\nonumber
...
},first={variable aléatoire normale (VA normale)},text={VA normale}}
...
{
	name={voisinage},
	description={The\index{voisinage} neighborhood of a node $\nodeidx \in \nodes$ is 
	the subset of nodes constituted by the \gls{neighbors} of $\nodeidx$.},
	first={voisinage},text={voisinage} 
}
...
{
	name={biais},
	description={Consider\index{biais} an \gls{ml} method using a parametrized \gls{hypospace} $\hypospace$. 
...
{name={risque empirique},
  description={The empirical \gls{risk}\index{risque empirique} $\emprisk{\hypothesis}{\dataset}$ 
  ...
  first={risque empirique},text={risque empirique} 
}
...
{name={graphe},
	description={A graph\index{graphe} $\graph = \pair{\nodes}{\edges}$ is a pair that consists of 
		...
		edge $\edgeidx \in \edges$.},first={graphe},text={graphe} 
}
...
{name={federated learning network (FL network)},
	description={A FL network\index{federated learning network (FL network)} is an 
...
{name={norme},
	description={Une norme\index{norme} est une fonction qui associe à chaque élément (vecteur) 
		d’un espace vectoriel un nombre réel positif ou nul. Cette fonction doit être 
		homogène et définie, et elle doit satisfaire l'inégalité triangulaire \cite{HornMatAnalysis}. },
	...
	%	description={The GM\index{geometric median (GM)} of a set of input vectors $\vx^{(1)}, \ldots, \vx^{(\samplesize)}$ 
		...
		%		\begin{figure}[H]
			...
			%		\caption{\label{opt_cond_GM}
				%			The optimality condition for \eqref{equ_geometric_median} states that the unit vectors from the 
				%			geometric median $\vz$ to the input vectors must sum to zero. This balance breaks if too many 
				%			vectors are perturbed, but remains robust as long as a majority are clean (i.e., not perturbed).
				%	\end{figure}
			%	
			...
			%	(optimality) condition of \eqref{equ_geometric_median}.
			%},
...
{name={fonction d'activation},
	description={Each\index{fonction d'activation} artificial neuron within an \gls{ann} is 
...
first={fonction d'activation},text={fonction d'activation} 
}
...
{name={régression},
	description={Regression\index{régression} problems revolve around the 
...
{name={dérivable},
	description={Une fonction\index{dérivable} à valeurs réelles $f: \mathbb{R}^{\featuredim} \rightarrow \mathbb{R}$ 
...
{name={gradient},
	description={Pour\index{gradient} une fonction à valeurs réelles $f: \mathbb{R}^{\featuredim} \rightarrow \mathbb{R}: \weights \mapsto f(\weights)$, 
		un vecteur $\vg$ tel que $\lim_{\weights \rightarrow \weights'} \frac{f(\weights) - \big(f(\weights')+ \vg^{T} (\weights- \weights') \big) }{\| \weights-\weights'\|}=0$ 
		est appelé le gradient de $f$ en $\weights'$. Si un tel vecteur existe, on le note $\nabla f(\weights')$ ou $\nabla f(\weights)\big|_{\weights'}$ \cite{RudinBookPrinciplesMatheAnalysis}.},
...
{name={hypothèse},
	description={A\index{hypothèse} hypothesis refers to a map (or function) $\hypothesis: \featurespace \rightarrow \labelspace$ from the 
...
{name={dimension effective},
	description={The\index{dimension effective} effective dimension $\effdim{\hypospace}$ of 
	...
	first={dimension effective},text={dimension effective}  
}

\newglossaryentry{labelspace}
{name={espace des étiquettes},
	description={Consider\index{espace des étiquettes} an \gls{ml} application that involves \gls{datapoint}s characterized by \gls{feature}s 
		...
		or $\labelspace = \{ \mbox{``cat image''}, \mbox{``no cat image''} \}$.}, first={espace des étiquettes},text={espace des étiquettes}  
}

\newglossaryentry{prediction}
{name={prédiction},
	description={A\index{prédiction} prediction is an estimate or approximation for some 
...
{name={espace des caractéristiques},
	description={
		The\index{espace des caractéristiques} \gls{feature} space of a given \gls{ml} application or method is 
	...
	first={espace des caractéristiques},text={espace des caractéristiques}  
}
...
{name={semi-définie positive},
	description=
	{A\index{semi-définie positive} (real-valued) symmetric matrix $\mQ = \mQ^{T} \in \mathbb{R}^{\featuredim \times \featuredim}$ 
	...
	first={semi-définie positive},text={sdp}  
}

\newglossaryentry{feature}
{name={caractéristique},
	description={A\index{caractéristique} feature of a \gls{datapoint} is one of its properties that can be 
		...
		}, first={caractéristique},text={caractéristique}  
}

\newglossaryentry{featurevec}
{name={vecteur de caractéristiques},
	description={\Gls{feature} vector refers to a\index{vecteur de caractéristiques} vector $\vx = \big(x_{1},\ldots,x_{\nrfeatures}\big)^{T}$ 
	...
	vectors that belong to an infinite-dimensional vector space (e.g., see \gls{kernelmethod}). 
		}, first={vecteur de caractéristiques},text={vecteur de caractéristiques}  
}


\newglossaryentry{label}
{name={étiquette},
	description={A\index{étiquette} higher-level fact or quantity of interest associated with a \gls{datapoint}. 
...
{name={données},
	 description={Data\index{données} refers to objects that carry information. These 
...
{name={jeu de données},
	description={A\index{jeu de données} dataset refers to a collection of \gls{datapoint}s. These 
 ...
 use, and other contextual information \cite{DatasheetData2021}.},first={jeu de données},text={jeu de données}, plural= {jeux de données}
}
...
{name={variable aléatoire (VA)},
	description={Une variable aléatoire\index{variable aléatoire (VA)} est une fonction qui associe à chaque élément 
		d’un \gls{probspace} $\mathcal{P}$ une valeur dans un espace cible \cite{BillingsleyProbMeasure,GrayProbBook}. 
		L'\gls{probspace} est composé d’événements élémentaires et est muni d’une mesure de \gls{probability} 
		qui associe des probabilités à des sous-ensembles de $\mathcal{P}$. 
		On distingue différents types de VA : 
		\begin{itemize} 
			\item les {VA binaires}, qui associent des événements élémentaires à un ensemble de deux valeurs distinctes, telles que 
			$\{-1,1\}$ ou $\{\text{chat}, \text{pas chat}\}$ ; 
			\item les {VA à valeurs réelles}, qui prennent des valeurs réelles ( dans $\mathbb{R}$) ;  
			\item les {VA vectorielles}, qui associent des événements élémentaires à l’\gls{euclidspace} $\mathbb{R}^{\featuredim}$.  
		\end{itemize} 
		La théorie des \glspl{probability} utilise le concept d’espaces mesurables pour définir rigoureusement 
		et étudier les propriétés de (grandes) collections de VA \cite{BillingsleyProbMeasure}.}, 
	first={variable aléatoire (VA)},text={VA}  }

 
 \newglossaryentry{probspace}{
 	name={espace probabilisé}, 
 	description={A\index{espace probabilisé} \gls{probability} space is a mathematical 
...
{name={réalisation},
	description={Consider\index{réalisation} an \gls{rv} $x$ which maps each element 
	...
	an element $\omega' \in \mathcal{P}$ with $x(\omega') = a'$.}, first={réalisation},text={réalisation}  }

\newglossaryentry{trainset}
{name={ensemble d'entraînement (ou d'apprentissage)},
description={A\index{ensemble d'entraînement (ou d'apprentissage)} training set is a \gls{dataset} $\dataset$ which consists of some \gls{datapoint}s used in \gls{erm} 
	...
	the validation error (e.g., using a different \gls{hypospace} or collecting more \gls{datapoint}s) \cite[Sec. 6.6]{MLBasics}.},first={ensemble d'entraînement (ou d'apprentissage)},text={ensemble d'entraînement}  
}
...
{
	name={lot},
	description={In\index{lot} the context of \gls{stochGD}, a batch refers to a randomly 
...
\newglossaryentry{trainerr}
{
	...
	description={The\index{erreur d'entrainement} average \gls{loss} of a \gls{hypothesis} when 
		...
		which is achieved by a solution of \gls{erm}.},first={erreur d'entrainement},text={erreur d'entrainement}  
}

\newglossaryentry{datapoint}
{name={point de données},
description={A\index{point de données} \gls{data} point is any object that conveys information \cite{coverthomas}. \Gls{data} points might be 
...
{name={erreur de validation},
 description={Consider\index{erreur de validation} a \gls{hypothesis} $\learnthypothesis$ which is 
 	...
 	to as the \gls{validation} error.},first={erreur de validation},text={erreur de validation}  
}
...
{name={ensemble de validation (ou jeu de validation)},
  description={A\index{ensemble de validation (ou jeu de validation)} set of \gls{datapoint}s used to estimate 
  	...
  	using a different \gls{hypospace}).},first={ensemble de validation (ou jeu de validation)},text={ensemble de validation}  
}
...
\newglossaryentry{erm}{name={minimisation du risque empirique (MRE)}, description={\Gls{emprisk} minimization\index{minimisation du risque empirique (MRE)} is the optimization problem of finding 
	...
	first={minimisation du risque empirique (MRE)},text={MRE} }
...
\newglossaryentry{regularization}{name={régularisation}, description={
		A\index{régularisation} key challenge of modern \gls{ml} applications is that they often 
		...
		\end{figure} 
		\newpage
		},first={régularisation},text={régularisation} }
...
\newglossaryentry{stepsize}{name={taille de pas}, description={
		See\index{taille de pas} \gls{learnrate}.}, 
	first={taille de pas},text={taille de pas} }

\newglossaryentry{learnrate}{name={taux d'apprentissage}, description={Consider\index{taux d'apprentissage} 
	...
	first={taux d'apprentissage},text={taux d'apprentissage} }
...
\newglossaryentry{iid}{name={indépendantes et identiquement distribuées (i.i.d.)}, description={It\index{indépendantes et identiquement distribuées (i.i.d.)} can be useful to 
	...
	first={indépendantes et identiquement distribuées (i.i.d.)},text={{i.i.d.}} }
...
\newglossaryentry{decisionregion}{name={région de décision}, description={Consider\index{région de décision} 
		...
		region of the \gls{hypothesis} $\hypothesis$.},first={région de décision},text={région de décision} }
...
\newglossaryentry{covmtx}{name={matrice de covariance}, 
	...
	first={matrice de covariance},text={matrice de covariance} }
...
\newglossaryentry{hypospace}{name={espace des hypothèses}, description={Every\index{espace des hypothèses} 
		...
		\gls{hypothesis} space might be the \gls{linmodel}.},first={espace des hypothèses},text={espace des hypothèses} }
	
\newglossaryentry{model}{name={modèle}, description={In\index{modèle} the context of \gls{ml} methods, 
		the term model typically refers to the \gls{hypospace} employed by an 
		\gls{ml} method \cite{ShalevMLBook,MLBasics}.},first={modèle},text={modèle} }

\newglossaryentry{modelparams}{name={paramètres du modèle}, 
	description={\Gls{model} \gls{parameters}\index{paramètres du modèle} are quantities that 
...
\newglossaryentry{eigenvalue}{name={valeur propre}, description={We\index{valeur propre} refer to a 
		number $\lambda \in \mathbb{R}$ as an eigenvalue of a square matrix $\mathbf{A} \in \mathbb{R}^{\featuredim \times \featuredim}$ 
		if there is a non-zero vector $\vx \in \mathbb{R}^{\featuredim} \setminus \{ \mathbf{0} \}$ such that $\mathbf{A} \vx = \lambda \vx$. },first={valeur propre},text={valeur propre} }
...
\newglossaryentry{gdmethods}{name={méthodes basées sur le gradient}, 
	description={\Gls{gradient}-based\index{méthodes basées sur le gradient} 
		...
		first={méthodes basées sur le gradient},text={méthodes basées sur le gradient} }
...
\newglossaryentry{loss}{name={perte}, description={\gls{ml}\index{perte} methods use a 
		...
		and \gls{hypothesis} $\hypothesis$.},first={perte},text={perte} }

\newglossaryentry{lossfunc}{name={fonction de perte (ou de coût)}, description={A\index{fonction de perte (ou de coût)} \gls{loss} function is a map 
	...
	\end{figure}
 },first={fonction de perte (ou de coût)},text={fonction de perte} }
...
\newglossaryentry{ann}{
	...
	description={An\index{réseau de neurones artificiels (RNA)} ANN 
	...
	first={réseau de neurones artificiels (RNA)},
	text={RNA}
}
...
\newglossaryentry{device}{name={dispositif},description={
				Any\index{dispositif} physical system that can be used to store and process \gls{data}. In the context of \gls{ml}, 
				...
				first={dispositif},text={dispositif}}
...
\newglossaryentry{weights}{name={poids},
	description={Consider\index{poids} a parametrized \gls{hypospace} $\hypospace$. 
		...
		outputs of neurons in hidden layers.},first={poids},text={poids}}
...
\newglossaryentry{probdist}{name={loi de probabilité},
	description={Pour\index{loi de probabilité} analyser les méthodes d'\gls{ml}, il peut être utile 
		...
		Les propriétés typiques de ces \glspl{datapoint} sont alors régies par la loi de \gls{probability} 
		...
		d’une \gls{rv} à valeur réelle $\feature \in \mathbb{R}$ peut être spécifiée 
...
\newglossaryentry{parameters}{name={paramètres},
	description={The\index{paramètres} parameters of an \gls{ml} \gls{model} are tunable 
		...
		between neurons of an \gls{ann}.},first={paramètres},text={paramètres}}
...
\newglossaryentry{localdataset}{name={jeu de données local},description={The\index{jeu de données local} concept of a local \gls{dataset} is 
		...
		are encoded in the edges of an \gls{empgraph}.},first={jeu de données local},text={jeu de données local}}
