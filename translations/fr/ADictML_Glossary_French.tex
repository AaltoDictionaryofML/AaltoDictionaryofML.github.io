...
\newglossaryentry{onlinelearning}
{
	...
	description={Certaines méthodes d’\gls{ml} \index{apprentissage incrémental (ou en ligne)} sont conçues pour traiter les \glspl{datapoint} 
		...
		une séquence chronologique d’observations. À chaque instant $\timeidx$, les méthodes d’apprentissage incrémental mettent à jour (ou affinent) l’\gls{hypothesis} actuelle $\hypothesis^{(\timeidx)}$ (ou les \gls{modelparams} $\weights^{(\timeidx)}$) à partir du nouveau \gls{datapoint} observé $\datapoint^{(\timeidx)}$.
		 \\
...
{name={matrice},
	description={Une matrice\index{matrice} de taille $\samplesize \times \nrfeatures$ est un tableau bidimensionnel de nombres, noté
		...
		\begin{itemize}
			...
			$$
			Un exemple important de système linéaire est la condition d’optimalité pour les \gls{modelparams} en \gls{linreg}.
