...
{name={fonction}, plural={fonctions}, 
	description={Une fonction\index{fonction} entre deux ensembles $\mathcal{U}$ et $\mathcal{V}$ associe à chaque élément $u \in \mathcal{U}$ un unique élément $v \in \mathcal{V}$ \cite{RudinBookPrinciplesMatheAnalysis}. 
...
{name={norme},
	description={Une norme\index{norme} est une \gls{function} qui associe à chaque élément (\gls{vector}) d’un \gls{vectorspace} un réel positif ou nul. Cette fonction doit être homogène, définie positive, et satisfaire l’inégalité triangulaire \cite{HornMatAnalysis}.
		\\
...
{name={apprentissage automatique (ou apprentissage machine)},
	description={L’\index{apprentissage automatique} \gls{ml} vise à prédire une \gls{label} à partir des \glspl{feature} d’un \gls{datapoint}. Les méthodes d’apprentissage automatique réalisent cela en apprenant une \gls{hypothesis} (ou \gls{model}) issue d’un \gls{hypospace} par la minimisation d’une \gls{lossfunc} \cite{MLBasics,HastieWainwrightBook}. Une formulation précise de ce principe est donnée par la \gls{erm}. Les différentes méthodes d’apprentissage automatique sont obtenues par divers choix pour les \glspl{datapoint} (leurs \glspl{feature} et leur \gls{label}), le \gls{model} et la \gls{lossfunc} \cite[Ch. 3]{MLBasics}.
		\\ 
...
{name={matrice inverse},
	description={On définit la \gls{matrix} inverse\index{matrice inverse} $\mA^{-1}$ d'une \gls{matrix} carrée $\mA \in \mathbb{R}^{n \times n}$ de rang maximal, c’est-à-dire dont les colonnes sont linéairement indépendantes. Dans ce cas, on dit que $\mA$ est inversible, et son inverse satisfait:
		\[
		\mA \mA^{-1} = \mA^{-1} \mA = \mI.
		\]
		Une \gls{matrix} carrée est inversible si et seulement si son \gls{det} est non nul. Les \glspl{matrix} inverses sont fondamentales pour la résolution de systèmes d'équations linéaires et dans la solution explicite de la \gls{linreg} \cite{Strang2007}, \cite{Horn91}. 
	    Le concept de \gls{matrix} inverse peut être étendu aux \glspl{matrix} non carrées ou de rang non maximal. On peut définir une « inverse à gauche » $\mB$ telle que $\mB \mA = \mI$, ou une « inverse à droite » $\mC$ telle que $\mA \mC = \mI$. Pour les \glspl{matrix} rectangulaires ou singulières, la \gls{pseudoinverse} de Moore–Penrose, notée $\mA^{+}$, fournit une généralisation unifiée de la \gls{matrix} inverse \cite{GolubVanLoanBook}.	
		\begin{figure}[H]
		\centering
		...
		\end{tikzpicture}
		\caption{Une \gls{matrix} $\mathbf{A}$ représente une transformation linéaire de $\mathbb{R}^{2}$. La \gls{matrix} inverse $\mathbf{A}^{-1}$ représente la transformation inverse. \label{fig_matrix_inverse_dict}} 
	\end{figure}	
...
\newglossaryentry{det}
{
	name={déterminant},
	description={
		Le\index{déterminant} déterminant $\det\,(\mA)$ d'une \gls{matrix} carrée 
...
\newglossaryentry{rv}
{name={variable aléatoire (VA)}, plural={VA},
	description={Une VA\index{variable aléatoire (VA)} est une \gls{function} qui associe les issues d’une expérience aléatoire à un espace de valeurs \cite{BillingsleyProbMeasure}, \cite{GrayProbBook}. 
...
{name={espace échantillon}, sort = {espace echantillon}, 
	description={Un\index{espace échantillon} espace \gls{sample} est l’ensemble de toutes les issues possibles d’une expérience aléatoire \cite{papoulis,BillingsleyProbMeasure,AshProbMeasure,BertsekasProb}. 
	\\	
		...
		Voir la Figure \ref{fig:covariance-examples_dict} pour des exemples visuels.
			\begin{figure}[H]
				\begin{tikzpicture}
					...
					\end{scope}
				\end{tikzpicture}
				\caption{Un \glspl{scatterplot} illustrant des \glspl{realization} issues de trois \glspl{probmodel} différents pour deux \glspl{rv} avec des valeurs de covariance différentes. (a) Négative. (b) Nulle. (c) Positive.}
...
{name={espace probabilisé}, 
	description={Un\index{espace probabilisé} espace probabilisé est une structure mathématique qui permet de raisonner sur une expérience aléatoire, par exemple l’observation d’un phénomène physique. 
...
{name={réalisation}, plural={réalisations},
	description={Considérons\index{réalisation} une \gls{rv} $\featurevec$ qui associe à chaque issue $\omega \in \mathcal{P}$ d’un \gls{probspace} $\mathcal{P}$ un élément $a$ d’un espace mesurable $\mathcal{N}$ \cite{RudinBookPrinciplesMatheAnalysis}, \cite{BillingsleyProbMeasure}, \cite{HalmosMeasure}. 
...
{name={vecteur normal centré réduit}, 
	description={Un\index{vecteur normal centré réduit} \gls{vector} normal centré réduit est un \gls{vector} aléatoire $\vx=\big(x_{1}, \ldots, x_{\nrfeatures}\big)\,^{T}$ 
...
\newglossaryentry{covmtx}{name={matrice de covariance}, 
	description={La\index{matrice de covariance} \gls{matrix} de covariance d’une \gls{rv} $\vx \in \mathbb{R}^{\featuredim}$ 
...
{name={processus stochastique},
	description={
		Un processus stochastique\index{processus stochastique} est une collection de 
...
\newglossaryentry{sbm}
{name={modèle à blocs stochastiques (SBM)},
	description={Le\index{modèle à blocs stochastiques (SBM)} SBM est un \gls{model} génératif probabiliste 
...
{name={algorithme stochastique}, 
	...
	description={Un\index{algorithme stochastique} \gls{algorithm} \gls{stochastic} utilise un mécanisme aléatoire 
...
{name={groupe (ou cluster)}, 
	plural={groupes}, 
	description={Un\index{groupe (ou cluster)} groupe (parfois aussi appelé amas) est un sous-ensemble de \glspl{datapoint} 
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				\begin{axis}[
					...
					xlabel={$x_1$},
					ylabel={$x_2$},
					title={Groupes de points de données},
					xmin=0, xmax=10,
					ymin=0, ymax=10,
					axis lines=left,
...
{name={vecteur de caractéristiques},
	description={Un \index{vecteur de caractéristiques} \gls{vector} de \glspl{feature} est un \gls{vector} 
...
{
	name={lot},
	description={Dans\index{lot} le contexte de la \gls{stochGD}, un lot désigne un sous-ensemble choisi aléatoirement dans l’\gls{trainset} complet. On utilise les \glspl{datapoint} de ce sous-ensemble pour estimer le \gls{gradient} de l’\gls{trainerr} et, par la suite, mettre à jour les \glspl{modelparam}.
		\\
...
\newglossaryentry{regularization}{
	...
	description={
		...
		\begin{enumerate}[label=\arabic*)]
			...
			via des contraintes sur les \glspl{modelparam} (par exemple $w_{1} \in [0.4,0.6]$ pour 
		...
		\begin{figure}[H]
			\begin{center} 
				...
				\end{tikzpicture}
				\caption{Trois approches pour la régularisation: 1) \gls{dataaug}; 2) pénalisation de la perte; et 3) élagage du \gls{model} (via des contraintes sur les \glspl{modelparam}). \label{fig_equiv_dataaug_penal_dict} }
			\end{center}
		\end{figure}
...
\newglossaryentry{gdmethods}{
	...
	description={
		...
		d’une \gls{objfunc} des \glspl{modelparam} \gls{differentiable}. Ces méthodes construisent une suite d’approximations 
		d’un choix optimal des \glspl{modelparam} qui aboutit à une valeur \gls{minimum} (ou \gls{maximum}) de la \gls{objfunc}. 
		Comme leur nom l’indique, les méthodes basées sur le \gls{gradient} utilisent les \glspl{gradient} de la \gls{objfunc} 
		évalués lors des itérations précédentes pour construire de nouveaux \glspl{modelparam} (espérons-le) améliorés. 
		Un exemple important d’une méthode basée sur le \gls{gradient} est la \gls{gd}.
		\\
		Voir aussi: \gls{gradient}, \gls{minimum}, \gls{maximum}, \gls{differentiable}, \gls{objfunc}, \glspl{modelparam}, \gls{gd}.
	},
...
\newglossaryentry{eigenvalue}{
	name={valeur propre},
	description={
		On qualifie\index{valeur propre} de valeur propre d'une \gls{matrix} carrée $\mathbf{A} \in \mathbb{R}^{\featuredim \times \featuredim}$ 
		le nombre $\lambda \in \mathbb{R}$ s’il existe un \gls{vector} non nul $\vx \in \mathbb{R}^{\featuredim} \setminus \{ \mathbf{0} \}$ 
...
{name={semi-définie positive},
	description={
		...
		est dite semi-définie positive\index{semi-définie positive} si $\featurevec\,^{T} \mQ \featurevec \geq 0$ pour tout \gls{vector} $\featurevec \in \mathbb{R}^{\featuredim}$. 
...
\newglossaryentry{weights}{name={poids},
	description={Considérons\index{poids} un \gls{hypospace} paramétré $\hypospace$. 
		On utilise le terme poids pour désigner des \glspl{modelparam} numériques 
		...
		\end{figure}
		Voir aussi: \gls{hypospace}, \glspl{modelparam}, \gls{feature}, \gls{linmodel}, \gls{ann}.},first={poids},text={poids}, plural={poids}}
...
{name={noyau (fonction)}, 
	description={Considérons\index{noyau (fonction)} un ensemble de \glspl{datapoint}, chacun représenté par un \gls{featurevec} 
...
\newglossaryentry{LapMat}{
	...
	description={La\index{matrice laplacienne} structure d’un \gls{graph} $\graph$, avec 
		...
		\glspl{matrix} spéciales associées à $\graph$. L’une de ces \glspl{matrix} est la \gls{matrix} laplacienne de $\graph$: $\mL^{(\graph)} \in \mathbb{R}^{\nrnodes \times \nrnodes}$, définie pour un \gls{graph} $\graph$ non orienté et pondéré \cite{Luxburg2007,Ng2001}. 
		...
		\begin{figure}[H]
			\begin{center}
				...
				\caption{\label{fig_lap_mtx_dict} À gauche: Un \gls{graph} non orienté $\graph$ avec trois sommets $\nodeidx=1,2,3$. 
					À droite: La \gls{matrix} laplacienne $\LapMat{\graph}  \in \mathbb{R}^{3 \times 3}$ de $\graph$.} 
			\end{center}
		\end{figure}
	...
	description={Un sous-ensemble $\mathcal{C} \subseteq \mathbb{R}^{\featuredim}$ de l'\gls{euclidspace} $\mathbb{R}^{\featuredim}$ est dit convexe s’il contient le segment de droite qui relie deux points $\vx, \vy\!\in\!\cluster$ quelconques de cet ensemble. Une \gls{function} $f\!:\!\mathbb{R}^{\dimlocalmodel}\!\rightarrow\!\mathbb{R}$ est convexe si son \gls{epigraph} $\big\{ \big( \weights\,^{T},t \big)\,^{T}\!\in\!\mathbb{R}^{\dimlocalmodel\!+\!1}\!:\!t\!\geq\!f(\weights) \}$ est un ensemble convexe \cite{BoydConvexBook}. On illustre un exemple d’ensemble convexe et de \gls{function} convexe dans la Figure \ref{fig_convex_set_function_dict}.
		\begin{figure}[H]
			\begin{center}
				...
				\vspace*{-8mm}
			\end{center}
			\caption{(a) Un ensemble convexe $\cluster \subseteq \mathbb{R}^{\dimlocalmodel}$. 
				(b) Une \gls{function} convexe $f: \mathbb{R}^{\dimlocalmodel} \rightarrow \mathbb{R}$.\label{fig_convex_set_function_dict}}
		\end{figure}
...
\newglossaryentry{pseudoinverse}{
	name={pseudo-inverse},
	description={La \index{pseudo-inverse}pseudo-inverse de Moore–Penrose $\mA^{+}$ d’une \gls{matrix} $\mA \in \mathbb{R}^{\samplesize \times \nrfeatures}$ généralise la notion de \gls{inverse} \cite{GolubVanLoanBook}. La pseudo-inverse apparaît naturellement dans le cadre de la \gls{ridgeregression} appliquée à un \gls{dataset} avec des \glspl{label} arbitraires $\vy$ et une \gls{featuremtx} $\mX = \mA$ \cite[Ch.\ 3]{hastie01statisticallearning}. Les \glspl{modelparam} appris par la \gls{ridgeregression} sont donnés par
		\[
		...
		\]
		Voir aussi: \gls{inverse}, \gls{ridgeregression}, \gls{dataset}, \gls{label}, \gls{featuremtx}, \glspl{modelparam}.
	},
...
\newglossaryentry{probability}{
	name={probabilité},
	description={On\index{probabilité} associe une valeur de probabilité, typiquement choisie dans l’intervalle $[0,1]$, à chaque \gls{event} pouvant se produire dans une expérience aléatoire \cite{KallenbergBook,BertsekasProb,BillingsleyProbMeasure,HalmosMeasure}.},
	first={probabilité},
	text={probabilité}
}
...
{name={indépendantes et identiquement distribuées (i.i.d.)}, 
	description={
		...
		indépendante et identiquement distribuée (i.i.d.)\index{indépendantes et identiquement distribuées (i.i.d.)} 
...
{name={événement}, sort = {evenement}, 
	description={
		Considérons\index{événement} une \gls{rv} $\featurevec$, définie sur un \gls{probspace} $\mathcal{P}$, 
...
{name={mesurable}, 
	description={
		Considérons\index{mesurable} une expérience aléatoire, comme l’enregistrement de la température de l’air dans une station météo \gls{fmi}. 
		...
		\begin{figure}
			\begin{tikzpicture}
				...
				\draw[->] (0,0) -- (8.5,0) node[right] {température [$^\circ$C]};
				% Marques tous les 10 degrés de -20 à 60
				\foreach \x/\label in {0/-20, 1/-10, 2/0, 3/10, 4/20, 5/30, 6/40, 7/50, 8/60} {
					\draw (\x,0.1) -- (\x,-0.1);
					\node[below] at (\x,-0.1) {\label};
				}
				% Zone mesurable : température < 0°C
				\fill[blue!20] (0,0.3) rectangle (2,0.6);
				\node[above] at (1,0.6) {$\omega < 0^\circ$C};
				% Zone mesurable : 35°C < omega < 55°C
...
{name={descente de gradient stochastique (SGD)}, 
	description={La \gls{gd} stochastique\index{descente de gradient stochastique (SGD)} 
		...
		\gls{emprisk} (en tant que \gls{function} des \glspl{modelparam} $\weights$), 
		...
		\end{figure}
		Voir aussi: \gls{gd}, \gls{gradient}, \gls{objfunc}, \gls{stochastic}, \gls{model}, \gls{erm}, \gls{trainset}, \gls{datapoint}, \gls{emprisk}, \gls{function}, \glspl{modelparam}, \gls{batch}, \gls{parameter}.},
...
{name={écart de généralisation}, sort = {ecart de generalisation}, 
...
\newglossaryentry{ridgeregression}{name={régression Ridge}, description={La \gls{regression} Ridge\index{régression Ridge} apprend les \gls{weights} $\weights$ d’une fonction \gls{hypothesis} linéaire $\hypothesis^{(\weights)}(\featurevec)= \weights\,^{T} \featurevec$. La qualité d’un choix particulier des \glspl{modelparam} $\weights$ est mesurée par la somme 
		...
		\\
		Voir aussi: \gls{regression}, \gls{weights}, \gls{hypothesis}, \gls{map}, \glspl{modelparam}, \gls{sqerrloss}, \gls{labeled datapoint}, \gls{trainset}, \gls{norm}, \gls{regularization}, \gls{parameter}, \gls{datapoint}, \gls{realization}, \gls{iid}, \gls{rv}.},first={régression Ridge},text={régression Ridge}}
...
\newglossaryentry{gd}{name={descente de gradient},description={	     
		La descente de gradient\index{descente de gradient} est une méthode itérative permettant de trouver le \gls{minimum} d’une \gls{function} \gls{differentiable} $f: \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$. 
		Elle génère une suite d’estimations $\weights^{(0)}, \weights^{(1)}, \weights^{(2)}, \ldots$ qui (idéalement) convergent vers un \gls{minimum} de $f$. 
		À chaque itération $\itercntr$, la descente de gradient affine l’estimation actuelle $\weights^{(\itercntr)}$ en effectuant un pas dans la direction de la plus forte pente descendante de l’approximation linéaire locale. 
		Cette direction est donnée par le gradient négatif $\nabla f(\weights^{(\itercntr)})$ de la \gls{function} $f$ à l’estimation courante $\weights^{(\itercntr)}$.
		La règle de mise à jour est alors donnée par :
		\begin{equation} 
			\label{equ_def_GD_step_dict}
			\weights^{(\itercntr\!+\!1)} = \weights^{(\itercntr)} - \lrate \nabla f(\weights^{(\itercntr)}),
		\end{equation} 
		où $\lrate > 0$ est une \gls{stepsize} choisie suffisamment petite. 
		Pour une valeur bien choisie de $\lrate$, la mise à jour réduit généralement la valeur de la fonction, c’est-à-dire $f(\weights^{(\itercntr+1)}) < f(\weights^{(\itercntr)})$. 
		La figure~\ref{fig_basic_GD_step_dict} illustre un \gls{gradstep} de descente de gradient.
		\begin{figure}[H]
			\begin{center}
...
\newglossaryentry{projgd}{
	...
	description={Considérons une méthode basée sur la \gls{erm} qui utilise un \gls{model} paramétré avec un \gls{paramspace} $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$. Même si la \gls{objfunc} de la \gls{erm} est \gls{smooth}, nous ne pouvons pas utiliser une \gls{gd} classique, car elle ne prend pas en compte les contraintes sur la variable d’optimisation (c’est-à-dire les \glspl{modelparam}). La \gls{gd} avec projection étend la \gls{gd} classique pour gérer les contraintes sur la variable d’optimisation. Une seule itération de \gls{gd} avec projection consiste à d’abord effectuer un \gls{gradstep}, puis à projeter le résultat sur l'\gls{paramspace}.
		...
		\end{figure}
	Voir aussi: \gls{erm}, \gls{model}, \gls{paramspace}, \gls{objfunc}, \gls{smooth}, \gls{gd}, \glspl{modelparam}, \gls{gradstep}, \gls{projection}.
	},
...
\newglossaryentry{ai}{name={intelligence artificielle (IA)}, description={
		...
		Les systèmes d’IA ont rarement accès à un \gls{trainset} étiqueté qui permettrait de mesurer la \gls{loss} moyenne pour tout choix possible des \glspl{modelparam}.
		À la place, les systèmes d’IA utilisent des signaux de \gls{reward} observés pour obtenir une estimation (ponctuelle) de la
		\gls{loss} engendrée par le choix actuel des \glspl{modelparam}.
		\\
		Voir aussi: \gls{reward}, \gls{ml}, \gls{model}, \gls{lossfunc}, \gls{trainset}, \gls{loss}, \glspl{modelparam}.
...
\newglossaryentry{decisionboundary}{name={frontière de décision}, description={
		...
		La frontière de décision de $\hypothesis$ est l’ensemble des \glspl{vector} $\featurevec \in \mathbb{R}^{\featuredim}$
		qui se trouvent entre différentes \glspl{decisionregion}. Plus précisément, un
		\gls{vector} $\featurevec$ appartient à la frontière de décision si et seulement si chaque \gls{neighborhood}
		$\{ \featurevec': \| \featurevec - \featurevec' \| \leq \varepsilon \}$, pour tout $\varepsilon > 0$, contient au moins deux \glspl{vector} avec des images différentes par la \gls{function}.
		\\
...
{name={réduction de dimension},
	description={La réduction de dimension\index{réduction de dimension} désigne 
		...
		\begin{itemize} 
			...
			Par exemple, les méthodes de \gls{linreg} doivent inverser une \gls{matrix} dont la taille dépend 
...
\newglossaryentry{featuremtx}{name={matrice de caractéristiques}, 
	description={Considérons\index{matrice de caractéristiques} un \gls{dataset} $\dataset$ 
		...
		\gls{matrix} de \glspl{feature} $\mX \defeq \big(\featurevec^{(1)},\ldots,\featurevec^{(\samplesize)}\big)\,^{T}$ 
...
\newglossaryentry{gradstep}{name={pas de gradient (pas)},description={Étant donnée une \gls{function} \gls{differentiable} à valeurs réelles $f(\cdot): \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$ 
		...
		\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{Le pas classique \eqref{equ_def_gd_basic_dict} transforme un \gls{vector} donné $\weights$ 
				en le \gls{vector} mis à jour $\weights'$. Il définit un opérateur 
...
\newglossaryentry{linearmap}{
	...
	description={
		...
		$f(c\vx) = c f(\vx)$, pour tous les \glspl{vector} $\vx, \vy \in \mathbb{R}^n$ et les scalaires $c \in \mathbb{R}$. 
		En particulier, $f(\mathbf{0}) = \mathbf{0}$. Toute \gls{map} linéaire peut être représentée comme une multiplication matricielle 
		$f(\vx) = \mA \vx$ pour une certaine \gls{matrix} $\mA \in \mathbb{R}^{m \times n}$. 
...
\newglossaryentry{paramspace}{
	...
	description={L'\index{espace des paramètres} espace des \glspl{parameter} $\paramspace$ d’un \gls{model} d'\gls{ml} $\hypospace$ est l’ensemble de tous les choix possibles pour les \glspl{modelparam} (voir Figure \ref{fig_param_space_dict}). 
		De nombreuses méthodes importantes en \gls{ml} utilisent un \gls{model} paramétré par des \glspl{vector} de l’\gls{euclidspace} $\mathbb{R}^{\dimlocalmodel}$. 
		...
		par exemple tous les \glspl{vector} $\weights \in \mathbb{R}^{\dimlocalmodel}$ dont la \gls{norm} est inférieure à un.
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}
				...
				\end{tikzpicture}
			\end{center} 
			\caption{L’espace des \glspl{parameter} $\paramspace$ d’un \gls{model} d'\gls{ml} $\hypospace$ contient tous les choix possibles pour les \glspl{modelparam}. Chaque choix $\weights$ pour les \glspl{modelparam} sélectionne une \gls{hypothesis} $\hypothesis^{(\weights)} \in \hypospace$. 
				\label{fig_param_space_dict}} 
		\end{figure}
		Voir aussi: \gls{parameter}, \gls{model}, \glspl{modelparam}.},
 ...
 \newglossaryentry{projection}{name={projection}, 
	description={Considérons\index{projection} un sous-ensemble $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$ de 
		...
		d’un \gls{vector} $\weights \in \mathbb{R}^{\dimlocalmodel}$ sur $\paramspace$ comme
		\begin{equation} 
			\label{equ_def_proj_generic_dict}
			\projection{\paramspace}{\weights} = \argmin_{\weights' \in \paramspace} \normgeneric{\weights - \weights'}{2}. 
		\end{equation}
		Autrement dit, $\projection{\paramspace}{\weights}$ est le \gls{vector} dans $\paramspace$ qui est le plus proche de $\weights$. 
...
\newglossaryentry{proxop}{
	...
	description={Étant donné\index{opérateur proximal} une \gls{function} \gls{convex} $f(\weights')$, 
		...
		$$
		Comme illustré à la Figure~\ref{fig_proxoperator_opt_dict}, évaluer l’opérateur proximal revient à minimiser une version pénalisée de $f(\weights')$. Le terme de pénalité est la distance euclidienne quadratique pondérée à un \gls{vector} donné $\weights$.
		L’opérateur proximal peut être interprété comme une \gls{generalization} du \gls{gradstep} de gradient, défini pour une fonction \gls{smooth} et \gls{convex} $f(\weights')$. En effet, effectuer un \gls{gradstep} avec une \gls{stepsize} $\lrate$ à partir du \gls{vector} actuel $\weights$ revient à appliquer l’opérateur proximal à la \gls{function} linéarisée $\tilde{f}(\weights') = \left( \nabla f(\weights) \right)\,^{T} (\weights' - \weights)$, avec $\rho = 1/\lrate$.
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}[scale=0.8]
				...
				\end{tikzpicture}
			\end{center}
			\caption{Un \gls{gradstep} généralisé met à jour un \gls{vector} $\weights$ en minimisant une version pénalisée de la \gls{function} $f(\cdot)$. Le terme de pénalité correspond à la distance euclidienne quadratique pondérée entre la variable d’optimisation $\weights'$ et le \gls{vector} donné $\weights$.}
...
{name={apprentissage par renforcement},
	description={
		L’apprentissage par renforcement\index{apprentissage par renforcement} désigne un cadre d’\gls{onlinelearning} dans lequel on ne peut évaluer l’utilité que d’une seule \gls{hypothesis} (c’est-à-dire un choix de \gls{model} et de \glspl{parameter}) à chaque instant $\timeidx$. En particulier, les méthodes d'apprentissage par renforcement appliquent l’\gls{hypothesis} actuelle $\hypothesis^{(\timeidx)}$ au \gls{featurevec} $\featurevec^{(\timeidx)}$ du \gls{datapoint} nouvellement reçu. L’utilité de la \gls{prediction} obtenue $\hypothesis^{(\timeidx)}(\featurevec^{(\timeidx)})$ est quantifiée par un signal de \gls{reward} $\reward^{(\timeidx)}$.
		\begin{figure}
			\begin{center}
					\begin{tikzpicture}[scale=1]
						\draw[->] (-2, 0) -- (6, 0);
					...
					\end{tikzpicture}
			\caption{Trois pas de temps consécutifs $\timeidx,\timeidx+1,\timeidx+2$ avec les \glspl{lossfunc} correspondantes $\loss^{(\timeidx)},
				\loss^{(\timeidx+1)}, \loss^{(\timeidx+2)}$. Au pas de temps $\timeidx$, une méthode d'apprentissage par renforcement peut évaluer la \gls{lossfunc} uniquement pour une \gls{hypothesis} spécifique $\hypothesis^{(\timeidx)}$, ce qui donne le signal de \gls{reward} $\reward^{(\timeidx)}=-\loss^{(\timeidx)}(\hypothesis^{(\timeidx)})$.}
			\end{center}
		\end{figure}
...
\newglossaryentry{onlineGD}{
	...
	description={
		Considérons\index{descente de gradient en ligne (ou incrémentale)} une méthode d'\gls{ml} qui apprend des \glspl{modelparam} 
		...
		Cette limite peut être utilisée comme \gls{objfunc} pour apprendre les \glspl{modelparam} $\weights$. 
		...
		arrive à l’instant $\timeidx$, on met à jour les \glspl{modelparam} actuels $\weights^{(\timeidx)}$. Notons que 
		...
		pour $\timeidx' < \timeidx$. Il peut donc arriver que, comparé à $\weights^{(\timeidx)}$, les \glspl{modelparam} mis à jour 
		...
		optimale dans des contextes pertinents d'un point de vue pratique. Par optimale, on entend que les \glspl{modelparam} 
		...
		\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center} 
			\caption{Un exemple de descente de gradient en ligne qui met à jour les \glspl{modelparam} $\weights^{(\timeidx)}$ 
		...
		\end{figure}
	Voir aussi: \gls{ml}, \glspl{modelparam}, \gls{paramspace}, \gls{datapoint}, \gls{iid}, \gls{rv}, \gls{risk}, \gls{hypothesis}, \gls{objfunc}, \gls{gd}, \gls{gradstep}, \gls{loss}, \gls{learnrate}, \gls{sqerrloss}.
	},
...
{name={produit de Kronecker}, 
	description={Le produit de Kronecker \index{produit de Kronecker} de deux \glspl{matrix} $\mA \in \mathbb{R}^{m \times n}$ 
...
{name={noyau (matrice)},
	description={
		Le noyau\index{noyau (matrice)} d'une \gls{matrix} $\mA \in \mathbb{R}^{\nrfeatures' \times \nrfeatures}$, 
...
{name={matrice},
	description={Une matrice\index{matrice} de taille $\samplesize \times \nrfeatures$ est un tableau bidimensionnel de nombres, noté
		$$
		\mA = \begin{bmatrix}
		...
		\begin{itemize}
			...
			correspond à un \gls{datapoint} et chaque colonne à une \gls{feature} ou une \gls{label} associée à ce point.
		\end{itemize}
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}[x=2cm]
				...
				\end{tikzpicture}
			\end{center}
			\caption{Une matrice $\mA$ définit une \gls{linearmap} entre deux \glspl{vectorspace}.} 
		\end{figure}
		Voir aussi: \gls{linearmap}, \gls{dataset}, \gls{linmodel}. },
	first={matrice},
