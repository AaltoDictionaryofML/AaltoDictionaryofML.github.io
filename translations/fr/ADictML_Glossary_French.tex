...
\newglossaryentry{mvndist}{name ={loi normale multivariée}, 
	description={The\index{loi normale multivariée} multivariate normal distribution 
		$\mvnormal{\vm}{\mC}$ is an important \gls{probmodel} for numeric \glspl{featurevec}. 
		It is a family of \glsps{probdist} for a vector-valued \gls{rv} 
...
\newglossaryentry{gaussrv}{name={variable aléatoire normale/gaussienne (VA normale)},description={
		...
		Gaussian random vectors are a special case of Gaussian processes which are  
		...
		See also: \gls{probdist}, \gls{probdist}, \gls{probspace}.
...
{name={gradient},
	description={For\index{gradient} a real-valued function 
		...
		it is referred to as the gradient of $f$ at $\weights'$. If it exists, the \gls{gradient} is unique and 
...
{name={caractéristique},
	description={A\index{caractéristique} feature of a \gls{datapoint} is one of 
		its properties that can be measured or computed easily without the need 
		for human supervision. For example, if a \gls{datapoint} is a digital 
		image (e.g., stored as a \texttt{.jpeg} file), then we could use the 
		red-green-blue intensities of its pixels as features. Domain-specific 
		synonyms for the term feature are "covariate," "explanatory variable," 
		"independent variable," "input (variable)," "predictor (variable)," or 
		"regressor" \cite{Gujarati2021}, \cite{Dodge2003}, \cite{Everitt2022}. 
	}, 
...
\newglossaryentry{model}{name={modèle}, description={In\index{modèle} the context of \gls{ml}, 
		...
		fields but with different menaing. For example, a \gls{probmodel} refers to a parametrized 
