...
\newglossaryentry{gaussrv}{name={variable aléatoire normale (VA normale)},description={
		A \index{variable aléatoire normale (VA normale)} standard Gaussian \gls{rv} is a 
...
{name={vecteur caractéristique},
	description={\Gls{feature} vector refers to a\index{vecteur caractéristique} vector $\vx = \big(x_{1},\ldots,x_{\nrfeatures}\big)^{T}$ 
	...
	vectors that belong to an infinite-dimensional vector space (e.g., see \gls{kernelmethod}). 
		}, first={vecteur caractéristique},text={vecteur caractéristique}  
}
...
{name={jeu de données},
 ...
 use, and other contextual information \cite{DatasheetData2021}.},first={jeu de données},text={jeu de données}  
}
...
{name={ensemble de validation},
  description={A\index{ensemble de validation} set of \gls{datapoint}s used to estimate 
  	...
  	using a different \gls{hypospace}).},first={ensemble de validation},text={ensemble de validation}  
}

\newglossaryentry{testset}
{name={ensemble de test},
	description={A\index{ensemble de test} set of \gls{datapoint}s that have  
		been used neither to train a \gls{model} (e.g., via \gls{erm}) nor in a \gls{valset} 
		to choose between different \gls{model}s.},first={ensemble de test},text={ensemble de test}  
}
...
{name=forêt d'arbres décisionnels,
	description={A\index{forêt d'arbres décisionnels} random forest is a set (or ensemble) of different \gls{decisiontree}s. 
...
\newglossaryentry{device}{name={appareil},description={
				Any\index{appareil} physical system that can be used to store and process \gls{data}. In the context of \gls{ml}, 
