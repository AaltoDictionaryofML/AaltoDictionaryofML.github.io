...
\newglossaryentry{gaussrv}{
	...
	first={variable aléatoire normale centrée réduite (VA normale centrée réduite)},
...
{name={espace des étiquettes},
	description={
		Considérons une\index{espace des étiquettes} application d'\gls{ml} dans laquelle les \glspl{datapoint} sont caractérisés par 
		des \glspl{feature} et des \glspl{label}. L’espace des \glspl{label} correspond à l’ensemble de toutes les valeurs 
		possibles que peut prendre l'\gls{label} d’un \gls{datapoint}. Les méthodes de \gls{regression}, qui visent 
		à prédire des \glspl{label} numériques, utilisent souvent l’espace des étiquettes $\labelspace = \mathbb{R}$. 
		Les méthodes de \gls{classification} binaire utilisent un espace des étiquettes constitué de deux éléments 
		différents, par exemple $\labelspace = \{-1,1\}$, $\labelspace = \{0,1\}$, ou 
		$\labelspace = \{ \mbox{``image de chat''}, \mbox{``pas une image de chat''} \}$.},
	first={espace des étiquettes},
	text={espace des étiquettes}
}
...
{name={espace des caractéristiques},
	...
	text={espace des caractéristiques}
}
...
\newglossaryentry{dataset}{
	...
	description={
		...
		\begin{figure}[H]
			...
			\end{center}
			\caption{\label{fig_cows_dataset}« Cows in the Swiss Alps » (\textit{Des vaches dans les Alpes Suisses}) par User:Huhu Uet - sous licence 
...
{name={ensemble d'entraînement (ou d'apprentissage)},
	description={Un\index{ensemble d'entraînement (ou d'apprentissage)} ensemble d'entraînement est un \gls{dataset} $\dataset$ composé de certains \glspl{datapoint} utilisés dans le cadre d'une \gls{erm} 
		...
		pour améliorer l'erreur de validation (par exemple, en utilisant un autre \gls{hypospace} ou en collectant plus de \glspl{datapoint}) \cite[Sec. 6.6]{MLBasics}.},first={ensemble d'entraînement (ou d'apprentissage)},text={ensemble d'entraînement}  
}
