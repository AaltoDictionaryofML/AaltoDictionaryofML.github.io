...
\begin{align} 
	...
	&\mathbf{A} \otimes \mathbf{B} &\quad & \parbox{.75\textwidth}{Le produit de Kronecker des matrices $\mathbf{A}$ et $\mathbf{B}$ \cite{Golub1980}. \\ Voir aussi: \gls{kroneckerproduct}} \nonumber
\end{align}
...
\begin{align}
	&\prob{\featurevec;\weights} & \quad & \parbox{.75\textwidth}{
		...
		La \gls{probdist} dépend d'un vecteur de \glspl{parameter} $\weights$. Par exemple, $\prob{\featurevec;\weights}$ pourrait être une 
		\gls{mvndist} avec un vecteur de \glspl{parameter} $\weights$ donné par les composantes du vecteur de \gls{mean} $\expect \{ \featurevec \}$ 
		et la \gls{covmtx} $\expect \bigg \{ \big( \featurevec - \expect \{ \featurevec \}\big) \big( \featurevec - \expect \{ \featurevec \}\big)^{T}  \bigg\}$. \\
		Voir aussi : \gls{probdist}, \gls{rv}, \gls{parameter}, \gls{mvndist}, \gls{mean}, \gls{covmtx}.} \nonumber \\[2mm] \hline \nonumber\\[-5mm]
...
\begin{align}
	...
	[2mm] Voir aussi : \gls{hypothesis}, \gls{modelparams}, \gls{map}
	} \nonumber \\[2mm] \hline \nonumber\\[-5mm]
...
\begin{align}
	...
	&\indsubgraph{\graph}{\cluster} & \quad & \parbox{.75\textwidth}{
		Le sous-\gls{graph} induit de $\graph$ utilisant les nœuds de $\cluster \subseteq \nodes$. \\
...
\begin{align}
	...
	&\maxnodedegree^{(\graph)} & \quad & \parbox{.75\textwidth}{
		Le \gls{nodedegree} pondéré maximal (parmi les \glspl{nodedegree} pondérés des nœuds) d’un \gls{graph} $\graph$. \\
		[2mm] Voir aussi : \gls{maximum}, \gls{nodedegree}, \gls{graph}.
	} \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\localdataset{\nodeidx} & \quad & \parbox{.75\textwidth}{
