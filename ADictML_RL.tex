...
{name={policy evaluation (reinforcement learning)},
 description={Policy evaluation refers to computing the \gls{statevaluefunction} $\statevaluefunc{\policy}$ 
		                   of a given \gls{policy} $\policy$ in a \gls{mdp}. One widely used method is based 
		                   on the characterization of $\statevaluefunc{\policy}$ as a \gls{fixedpoint} of 
		                   the \gls{bellmanoperator} $\fixedpointop^{(\policy)}$.} , 
