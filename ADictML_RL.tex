...
{name={Markov decision process (MDP)},
	description={An MDP\index{Markov decision process (MDP)} is a mathematical 
		...
		decision-making problems where an agent interacts with an \gls{environment} to 
...
{name={agent},
 description={An agent\index{agent} denotes any system that implements some form of 
              \gls{onlinelearning} \cite{SuttonEd2,RusselNorvig}. During each time 
			  step $\timeidx$, an agent receives a \gls{featurevec} $\featurevec^{(\timeidx)}$ 
			  which provide (typically incomplete) information about the underlying \gls{state} 
			  ...
			  See also: \gls{reinforcementlearning}, \gls{reward}, \gls{hypothesis}, \gls{action}.},
		first={agent},
		type={reinflearning}, 
		plural={agents}, 
		text = {agent},
...
{name={environment (reinforcement learning)},
 description={An environment\index{environment} denotes the external system with which an \gls{agent} 
			  ...
			  agent with a \gls{featurevec} $\featurevec^{(\timeidx)}$ and, in response to the 
			 \gls{action} $\arm^{(\timeidx)}$ by the \gls{agent}, a \gls{reward} $\reward^{(\timeidx)}$ 
			  \cite{SuttonEd2}.\\ 
			  See also: \gls{agent}, \gls{reward}, \gls{featurevec}.},
 first={environment},
 plural={environments},
 text={environment},
