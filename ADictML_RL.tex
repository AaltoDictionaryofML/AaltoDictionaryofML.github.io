...
{name={Markov decision process (MDP)},
	description={An MDP\index{Markov decision process (MDP)} is a mathematical 
		...
		decision-making problems where an \gls{agent} interacts with an \gls{environment} to 
...
{name={agent},
	description={An agent\index{agent} denotes any system that implements some form of 
		...
		It then receives a \gls{reward} $\reward^{(\timeidx)}$ that quantifies the usefulness of this \gls{action} 
		and, in turn, guides the adaptation of its \gls{hypothesis} (or \gls{modelparam}). 
		\\ 
...
{name={environment (reinforcement learning)},
	description={An environment\index{environment} denotes the external system with which an \gls{agent} 
		interacts over time. During each time step $\timeidx$, the environment provides the 
		\gls{agent} with a \gls{featurevec} $\featurevec^{(\timeidx)}$ and, in response to the 
		\gls{action} $\arm^{(\timeidx)}$ by the \gls{agent}, a \gls{reward} $\reward^{(\timeidx)}$ \cite{SuttonEd2}.
		\\ 
		See also: \gls{agent}, \gls{featurevec}, \gls{reward}.},
	first={environment},
	plural={environments},
	text={environment},
	firstplural={environments},
	type=reinflearning
}
