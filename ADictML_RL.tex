...
{name={Markov decision process (MDP)},
	description={An MDP\index{Markov decision process (MDP)} is a mathematical 
		...
		achieve a goal\cite{SuttonEd2}, \cite{Bertsekas2012}.
			%%%reference Bertsekas2012 missing from the .bib file
		\\
...
{name={Bellman operator},
	description={The Bellman \gls{operator}\index{Bellman operator} $\fixedpointop$ 
		associated with a \gls{mdp} is defined on the space of all 
		...
		of an optimal \gls{policy}. Beside the Bellman \gls{operator} 
		...
		solving \glspl{mdp}, such as \gls{valueiteration} and \gls{policy} \gls{iteration}\cite{SuttonEd2}.
			%%%reference Bellman1957 missing from the .bib file
		\\
		See also: \gls{mdp}, \gls{valuefunction}, \gls{policy}, \gls{valueiteration}. }, 
