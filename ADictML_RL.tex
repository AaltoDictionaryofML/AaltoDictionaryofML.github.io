...
{name={Bellman operator},
	description={The Bellman operator \index{Bellman operator} $\fixedpointop$ 
	             associated with an \gls{mdp} maps a \gls{valuefunction} $\valuefunc: \statespace \rightarrow \mathbb{R}$ 
				...
				\[
		       	\valuefunc'(\state) =\max_{\arm \in \actionspace}
					\Bigl(
						\reward(\state,\arm)
					+ \gamma \, \expect\{ \valuefunc(\state') \mid \state,\arm \}
				\Bigr),
					\]
				where $\gamma \in (0,1)$ is a discount factor and $\state'$ is the 
				next state generated according to the transition \gls{function}. 
				$\state' \sim \prob{\state' \mid \state,\arm}$.
				The \gls{statevaluefunction} $\valuefunc^\star$ 
				of the optimal \gls{policy} $\policy^\star$ 
				is a \gls{fixedpoint} of the Bellman operator, 
				$\valuefunc^\star = \mathcal{T} \valuefunc^\star$.},
