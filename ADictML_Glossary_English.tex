...
{name={dimensionality reduction},
	description={Dimensionality reduction\index{dimensionality reduction} refers 
		...
		a (typically large) set of raw \glspl{feature} $\feature_{1},\ldots,\feature_{\nrfeatures}$ 
		...
		\begin{itemize} 
			\item {\bf Statistical Benefit.} It typcially reduces the risk of \gls{overfitting} as 
			reducing the number of \glspl{feature} often reduces the \gls{effdim} of a \gls{model}. 
			\item {\bf Computational Benefit.} Using fewer \glspl{feature} means less computation 
			...
			naturally represented by a large nubmer of greayscale values (one of for each pixel).
		\end{itemize} 
		 \begin{figure}[ht]
		 \centering
		...
		% % Example points with mini-images (can replace adjustbox with \includegraphics)
			...
			learned \glspl{feature}s $(z_1, z_2)$ and visualized in a \gls{scatterplot}.}
...
{name={machine learning (ML)},
	 ...
	 obtained from different design choices for \gls{datapoint}s (i.e., their \gls{feature}s and \gls{label}), 
...
{name={explainability},description=
		{We\index{explainability} define the (subjective) explainability of an \gls{ml} method 
			...
			\gls{model} via the conditional (or differential) entropy of its \gls{prediction}s, given the user \gls{prediction}s \cite{JunXML2020}, \cite{Chen2018}. 
		},
...
{name={squared error loss},
 description={The squared error\index{squared error loss} \gls{loss} 
 	measures the \gls{prediction} error of a \gls{hypothesis} $\hypothesis$ 
 	when predicting a numeric \gls{label} $\truelabel \in \mathbb{R}$ from 
 	the \gls{feature}s $\featurevec$ of a \gls{datapoint}. It is defined as 
\begin{equation} 
	\nonumber
...
{name={projected gradient descent (projected GD)},
 description={Consider an \gls{erm}-based method that uses a parametrized \gls{model} 
  with \gls{paramspace} $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$. Even if 
...
{name=differential privacy (DP),
  description={Consider\index{differential privacy (DP)} some \gls{ml} method $\algomap$ 
  	that reads in a \gls{dataset} (e.g., the \gls{trainset} used for \gls{erm}) and delivers some 
  	output $\algomap(\dataset)$. The output could be either the learned \gls{modelparams} 
  	or the \gls{prediction}s for specific \gls{datapoint}s. DP is a precise measure of \gls{privleakage} 
  	incurred by revealing the output. Roughly speaking, an \gls{ml} method is differentially 
  	private if the \gls{probdist} of the output $\algomap(\dataset)$ does not change too 
  	much if the \gls{sensattr} of one \gls{datapoint} in the \gls{trainset} is changed. 
  	Note that DP builds on a \gls{probmodel} for an \gls{ml} method, i.e., we interpret its 
  	output $\algomap(\dataset)$ as the \gls{realization} of an \gls{rv}. The randomness in 
  	the output can be ensured by intentionally adding the \gls{realization} of an auxiliary \gls{rv} 
  	(i.e., adding noise) to the output of the \gls{ml} method.}, 
	first = {DP}, 
	text={DP} 
}
...
{name={distributed algorithm},
	description={A\index{distributed algorithm} distributed \gls{algorithm} is an \gls{algorithm} designed for 
		...
		and the messages received from other \gls{device}s. One important application of distributed 
		\gls{algorithm}s is in \gls{fl} where a network of \gls{device}s collaboratively trains a personal \gls{model} 
		for each \gls{device}. 
		},
...
{name={feature},
	description={A\index{feature} feature of a \gls{datapoint} is one of 
...
\newglossaryentry{objfunc}{
	...
	description={An\index{objective function} objective function is a map that assigns a numeric 
		...
		optimize (see Fig.\ \ref{fig_obj_func}).  In \gls{ml} context, the optimization variable could 
		...
		\\
		See also: \gls{loss}, \gls{lossfunc}, \gls{risk}, \gls{emprisk}.
		\begin{figure}[H]
			\begin{center}
			\begin{tikzpicture}[scale=1.0]
				% Axes
		...
		as the \gls{modelparams} of a \gls{ml} \gls{model}, to a value that measures the usefulness 
...
\newglossaryentry{rerm}{
	...
	description={Basic \gls{erm} learns a \gls{hypothesis} (or trains a \gls{model}) $\hypothesis \in \hypospace$ 
		...
		This leads to regularized \gls{erm}\index{regularized empirical risk minimization (RERM)} such that
		\begin{equation}
			\label{equ_def_rerm}
...
\newglossaryentry{generalization}{name={generalization}, 
	description={Many\index{generalization} current \gls{ml} (and \gls{ai}) systems 
		...
		\gls{probmodel} such as the \gls{iidasspt} for \gls{data} generation, based on which the \gls{datapoint}s in 
...
\newglossaryentry{dbscan}{name={density-based spatial clustering of applications with noise (DBSCAN)}, 
	description={DBSCAN\index{density-based spatial clustering of applications with noise (DBSCAN)} refers to a \gls{clustering} \gls{algorithm} for \gls{datapoint}s that are characterized by numeric \gls{featurevec}s. 
		...
		via a sequence (i.e., path) of close-by intermediate \gls{datapoint}s. Thus, DBSCAN might consider 
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge\index{ridge regression} 
	    ...
	    the average \gls{sqerrloss} is equivalent to replacing original \gls{datapoint}s by the \gls{realization}s 
...
\newglossaryentry{expectation}{name={expectation}, 
	description={
...
\newglossaryentry{model}{name={model}, description={In\index{model} the context of \gls{ml}, 
...
description={\Gls{gradient}\index{gradient descent (GD)} 
descent is an iterative method for finding the \gls{minimum} of a \gls{differentiable} 
