...
{type=systems, 
	name={algorithm},
	description={An\index{algorithm} is a precise, step-by-step procedure specifying how to 
	produce an output from a given input within a finite number of computational steps \cite{Cormen:2022aa}. 
...
%    $${\rm input},s_1​,s_2​,\ldots,s_T​,{\rm output}$$
	...
	Each execution, in turn, is a sequence ${\rm input}, s_{1},\ldots,s_{T},{\rm output}$ 
	that must respect the constraints inherent to a computer that implements the algorithm.  
	...
	Crucially, an algorithm encompasses more than just a mapping from input to output; the intermediate 
	computational steps $s_1,\ldots,s_T$ are essential for analyzing algorithmic correctness, efficiency, 
	and resource usage.
	%Note that an algorithm is more than a specification of the mapping from ${\rm input}$ to ${\rm output}$, 
	%Indeed, an algorithm also includes the intermediate computational steps $s_{1},\ldots,s_{T}$. 
	},
