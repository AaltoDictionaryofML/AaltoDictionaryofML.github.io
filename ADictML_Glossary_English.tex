...
{name={stability},
	description={
		Consider\index{stability} some \gls{ml} method $\algomap$ that reads in a \gls{dataset} (e.g., the \gls{trainset} 
		used for \gls{erm}) and delivers some output $\algomap(\dataset)$ (e.g., by solving \gls{erm}). 
		The output could be either the learned \gls{modelparams} or the \gls{prediction}s for specific 
		\gls{datapoint}s. A key property of such a method is its stability towards small changes in the 
		\gls{trainset} $\dataset$. Different quantitative measures for stability of a \gls{ml} method $\algomap(\dataset)$ 
		have been proposed which allow to bound the \gls{risk} (or \gls{generalization} error) 
		incurred by $\algomap(\dataset)$ \cite[Ch. 13]{ShalevMLBook}.}, 
