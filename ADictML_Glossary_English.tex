...
{name={online algorithm},
	description={An\index{online algorithm} online \gls{algorithm} processes input \gls{data} incrementally, 
		...
		executions. However, the execution sequence for an online \gls{algorithm} has a distinct structure such that
		$${\rm in}_{1},s_1,{\rm out}_{1},{\rm in}_{2},s_2,{\rm out}_{2},\ldots,{\rm in}_{T},s_T,{\rm out}_{T}.$$ 
		Each execution begins from an initial state (i.e., \(\text{in}_{1}\)) and proceeds through alternating computational steps, 
...
\newglossaryentry{cm}{name={confusion matrix}, 
	description={Consider\index{confusion matrix} \gls{datapoint}s, which are characterized by \gls{feature}s $\featurevec$ 
...
\newglossaryentry{model}{name={model}, description={In\index{model} the context of \gls{ml} methods, 
		...
		\gls{ml} method \cite{MLBasics}, \cite{ShalevMLBook}.},first={model},text={model} }
