...
\newglossaryentry{generalization}{name={generalization}, 
	description={Many\index{generalization} current \gls{ml} (and \gls{ai}) systems 
		are based on \gls{erm}: At their core, they train a \gls{model} (learn a \gls{hypothesis} 
...
\newglossaryentry{highdimregime}{name={high-dimensional regime}, description={The\index{high-dimensional regime} 
		...
		Another example of \gls{ml} methods that operate in the high-dimensional regime are large \gls{ann}s, having 
...
\newglossaryentry{datanorm}{name={data normalization},
		description={Data normalization\index{data normalization} refers to transformations 
			applied to the \gls{featurevec}s of \gls{datapoint}s. The goal of data normalization 
			is to adjust \gls{featurevec}s $\featurevec'$ to possess properties that enhance either the 
			\gls{statasp} or \gls{compasp} of the overall \gls{ml} method. For instance, in \gls{linreg} 
			with \gls{gdmethods} using a fixed \gls{learnrate}, ensuring convergence requires 
			controlling the norm of the \gls{featurevec}s in the \gls{trainset}. This can be achieved 
			by normalizing \gls{featurevec}s such that $\normgeneric{\featurevec'}{2} \leq 1$.},
