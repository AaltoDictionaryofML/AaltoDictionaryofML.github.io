...
{name={feature learning},
	description={Consider an \gls{ml} application with \gls{datapoint}s characterized by 
		...
		of potential maps $\featuremapvec$, and a quantitative measure of the usefulness of 
...
{name={vertical federated learning (vertical FL)},description=
	...
	first={vertical federated learning (vertical FL)},text={vertical \gls{fl}}
} 
...
\newglossaryentry{linmodel}{name={linear model},
	description={Consider\index{linear model}  \gls{datapoint}s, each characterized by a numeric \gls{featurevec} 
...
\newglossaryentry{gradstep}{name={gradient step},description={Given a \gls{differentiable} 
		...
		\begin{figure}[htbp]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{The basic \gls{gradient} step \eqref{equ_def_gd_basic} maps a given vector $\weights$ 
		...
		in a \gls{neighbourhood} whose size is determined by the \gls{stepsize} $\lrate$ - a linear approximation 
		...
		as we used for the \gls{stepsize} in \eqref{equ_def_gd_basic_dict}. The larger we choose $\lrate$ in 
...
\newglossaryentry{connected}{name ={connected graph}, description={An\index{connected graph} 
		...
		first={connected},text={connected}}
...
\newglossaryentry{zerooneloss}{name={$0/1$ \gls{loss}},
	...
	equal to $1$ if the \gls{prediction} is wrong, $\lossfunczo{\pair{\featurevec}{\truelabel}}{\hypothesis}=1$ 
	when $\predictedlabel\neq\truelabel$.},
	sort=zerooneloss, 
...
\newglossaryentry{gdpr}{name={general data protection regulation (GDPR)},description={
			...
			\begin{itemize}
				...
				\item \Gls{data} subject rights: Including the rights to access, rectify, and delete 
				personal \gls{data}, as well as to object to automated decision-making and profiling.
...
\newglossaryentry{nn}
{
	...
	description={Nearest neighbour\index{nearest neighbour} methods learn a \gls{hypothesis} 
		...
		is solely determined by the nearest \gls{neighbours} within a given \gls{dataset}. Different 
		methods use different metrics for determining the nearest \gls{neighbours}. If \gls{datapoint}s 
...
{name={upper confidence bound},
	description={Consider\index{upper confidence bound} an \gls{ml} 
		application which requires to predict, for each new time instant,
		 ...
		 is a multi-armed bandit $\ldots$. can be modelled as a \gls{realization} of an \gls{rv} 
...
{name={optimism in face of uncertainty},
	description={\gls{ml}\index{optimism in face of uncertainty} methods learn \gls{modelparams} $\weights$ 
...
\end{tikzpicture}
\caption{\gls{ml} methods learn \gls{modelparams} $\weights$ by using some estimate $f(\weights)$ for 
...
{name={federated learning network (FL network)},
	description={A federated network\index{federated learning network (FL network)} is an undirected weighted \gls{graph} whose 
		...
		Each node in a federated network represents some \gls{device}, capable to collect a \gls{localdataset} 
...
{name={risk},
	description={Consider\index{risk} a \gls{hypothesis} $\hypothesis$ used to predict the \gls{label} 
		...
		of an \gls{rv}. The \gls{iidasspt} allows to define the risk of a \gls{hypothesis} 
...
{name={baseline},
    ...
    we can precisely determine the \gls{minimum} achievable \gls{risk} among any \gls{hypothesis} 
  ...
  % h(x) label for the first cross
		...
		can achieve \gls{minimum} \gls{risk} (under \gls{sqerrloss}) by using the \gls{bayesestimator} $\mu_{\truelabel|\featurevec}$ 
...
{name={graph clustering},
	description={\Gls{graph} \gls{clustering}\index{graph clustering} aims at 
		...
		can quantity the extend of these similarities by an \gls{edgeweight} \cite{Luxburg2007,FlowSpecClustering2021}. }, 
...
%					\draw[dashed, gray] (0, 1) -- (1, 1);
    		...
    		\begin{minipage}{0.4\textwidth}
				...
				\caption{\label{fig_lap_mtx_specclustering_dict} {\bf Top.} Left: An undirected \gls{graph} 
					$\graph$ with four nodes $\nodeidx=1,2,3,4$, each representing a \gls{datapoint}. Right: \gls{LapMat} 
...
{name={mean squared estimation error},
	description={Consider\index{mean squared estimation error} an \gls{ml} method that 
...
{name={feature vector},
	...
	For some \gls{ml} methods, it can be more convenient to work with \gls{feature} 
...
{name={dataset},
	description={A\index{dataset} dataset consists of one or more \gls{datapoint}s and is 
		...
		used to describe datasets and the \gls{ml} methods that operate on them \cite{silberschatz2019database}, \cite{abiteboul1995foundations}, \cite{hoberman2009data}, \cite{ramakrishnan2002database}. 
	...
	\end{tabular}
	\caption{Temperature observations at \gls{fmi} station Kustavi Isokari.}
...
{name={labeled datapoint},
 description={A\index{labeled datapoint} \gls{datapoint} whose \gls{label} is known or has been determined 
 	by some means which might require human labour.},
...
{name={random variable (RV)},
 ...
 \newglossaryentry{probspace}{
 	...
 	description={A\index{probability space} \gls{probability} space is a mathematical 
 		model of a physical process (a random experiment) with an uncertain outcome. 
...
{name={realization},
	...
	(outcome or elementary event) $\omega \in \mathcal{P}$ of a \gls{probspace} $\mathcal{P}$ 
...
description={A\index{training set} \gls{dataset} $\dataset$, constituted by some \gls{datapoint}s used in \gls{erm} 
	...
	\gls{valerr} of $\learnthypothesis$ allows to diagnose the \gls{ml} method and informs how to improve 
...
\newglossaryentry{erm}{name={empirical risk minimization (ERM)}, description={\Gls{emprisk} minimization\index{empirical risk minimization (ERM)} is the optimization problem of finding 
		a \gls{hypothesis} (out of a \gls{model}) with \gls{minimum} average \gls{loss} (or \gls{emprisk}) on a given \gls{dataset} 
		$\dataset$ (the \gls{trainset}). Many \gls{ml} methods are obtained from 
...
\newglossaryentry{ssl}{name={semi-supervised learning}, description={Semi-supervised\index{semi-supervised learning} 
		learning methods use unlabeled datapoints to support the learning of a \gls{hypothesis} 
...
\newglossaryentry{regularization}{name={regularization}, description={
        ...
        The choice of which route to use for regularization can be based on the 
...
\newglossaryentry{generalization}{name={generalization}, 
	description={Many\index{generalization} current \gls{ml} (and \gls{ai}) systems 
		...
		Using  a \gls{probmodel} allows to explore the outside of the \gls{trainset} by 
		drawing additional \gls{iid} \gls{realization}s from $p(\vz)$. Moreover, using the \gls{iidasspt} 
		allows to define the \gls{risk} of a trained \gls{model} $\learnthypothesis \in \hypospace$ as 
...
% Label for p(z)
		                   	...
		                   	\caption{Two \gls{datapoint}s $\datapoint^{(1)},\datapoint^{(2)}$ that are used as a \gls{trainset} 
		                   		...
		                   		outside $\trainset$ either by an \gls{iidasspt} with underlying \gls{probdist} $p(\datapoint)$ 
...
\newglossaryentry{gtv}{name={generalized total variation (GTV)}, description={A\index{generalized total variation (GTV)} 
...
\newglossaryentry{datapoisoning}{name={data poisoning}, description={\Gls{data}\index{data poisoning} 
		...
		steer the training of an \gls{ml} model \cite{Liu2021,PoisonGAN}. The protection against 
...
\newglossaryentry{featuremap}{name={feature map}, description={A\index{feature map} map 
		...
		simpler (or more linear) in the new \gls{featurespace}, allowing to use \gls{linmodel}s 
  ...
  \newglossaryentry{lasso}{name={least absolute shrinkage and selection operator (Lasso)}, 
	description={The Lasso\index{least absolute shrinkage and selection operator (Lasso)} is an 
		instance of \gls{srm} to learn the \gls{weights} $\weights$ of a linear map 
...
allow to separate the \gls{datapoint}s using a \gls{linclass}.  \label{fig_linsep_kernel_dict}}
\end{figure}
...
\newglossaryentry{dbscan}{name={density-based spatial clustering of applications with noise (DBSCAN)}, 
	description={DBSCAN\index{density-based spatial clustering of applications with noise (DBSCAN)} refers to a \gls{clustering} algorithm for \gls{datapoint}s that are characterized by numeric \gls{featurevec}s. 
...
\newglossaryentry{cfl}{name={clustered federated learning (CFL)}, description={
		...
		same \gls{cluster} have similar statistical properties. CFL aggregates \gls{localdataset}s 
...
\newglossaryentry{decisionboundary}{name={decision boundary}, description={Consider\index{decision boundary} a 
		...
		if each \gls{neighbourhood} $\{ \featurevec': \| \featurevec - \featurevec' \| \leq \varepsilon \}$, 
...
\newglossaryentry{eerm}{name={explainable empirical risk minimization (EERM)}, description={An\index{explainable empirical risk minimization (EERM)} 
...
\newglossaryentry{kmeans}{name={$k$-means}, description={The\index{$k$-means} $k$-\gls{mean}s algorithm 
...
\newglossaryentry{xml}{name={explainable machine learning (explainable ML)}, description={Explainable\index{explainable machine learning (explainable ML)} 
		\gls{ml} methods aim at complementing each \gls{prediction} with an \gls{explanation} for 
...
\newglossaryentry{highdimregime}{name={high-dimensional regime}, description={The\index{high-dimensional regime} 
		...
		far more tunable \gls{weights} (and bias terms) than the number of \gls{datapoint}s in the \gls{trainset}. 
...
\newglossaryentry{gmm}{name={Gaussian mixture model (GMM)}, description={A GMM\index{Gaussian mixture model (GMM)} 
		is a particular type of a \gls{probmodel} for a numeric vector $\featurevec$ (e.g., 
		...
		\gls{mean} vector $\clustermean^{(\clusteridx)}$, and the \gls{covmtx} $\clustercov^{(\clusteridx)}$ for each $\clusteridx=1,\ldots,\nrcluster$. 
...
\newglossaryentry{maxlikelihood}{name={maximum likelihood}, description={
		...
		that are interpreted as the \gls{realization} of \gls{iid} \gls{rv}s with a common \gls{probdist} $\prob{\datapoint; \weights}$ which 
		...
		of observing the \gls{dataset}. Thus, the \gls{maximum} likelihood estimator is a 
...
\newglossaryentry{em}{name={expectation-maximization (EM)}, description={
		...
		we repeat the computation of the conditional \gls{expectation} (the E step) and the update 
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge\index{ridge regression} 
		...
		\gls{labeled datapoint}s (the \gls{trainset}). The second component is the scaled squared 
...
\newglossaryentry{expectation}{name={expectation}, description={
		...
		which we interpret as the \gls{realization} of an \gls{rv} with \gls{probdist} $p(\featurevec)$. 
...
\newglossaryentry{logreg}{name={logistic regression}, description={Logistic\index{logistic regression} \gls{regression} learns a 
		...
		on some \gls{labeled datapoint}s (the \gls{trainset}).},
...
for the logistic \gls{loss} applies only if for the \gls{labelspace} $\labelspace = \{ -1,1\}$ and using 
...
\newglossaryentry{iidasspt}{name={independent and identically distributed assumption (i.i.d.\ assumption)}, description={The \gls{iid} 
		...
		\gls{realization} of \gls{iid} \gls{rv}s.},first={independent and identically distributed assumption (i.i.d.\ assumption)},text={i.i.d.\ assumption} }
...
\newglossaryentry{hypospace}{name={hypothesis space}, description={Every\index{hypothesis space} 
		...
		is an (approximately) linear relation between \gls{feature}s and \gls{label}, a useful choice for the 
...
\newglossaryentry{modelparams}{name={model parameters}, 
	description={\Gls{model} parameters\index{model parameters} are quantities that 
	are used to select a specific \gls{hypothesis} map from a \gls{model}. 
	We can think of \gls{model} parameters as a unique identifier for a \gls{hypothesis} 
...
\newglossaryentry{ai}{name={artificial intelligence (AI)}, description={
		...
		predicing optimal actions. These predictions are computed from observations about the state of the 
		environment. The choice of \gls{lossfunc} sets AI applications apart from more basic \gls{ml} applications. 
		AI systems rarely have access to a labelled \gls{trainset} that allows the average \gls{loss} to be measured for any possible choice of \gls{modelparams}. 
...
\newglossaryentry{reward}{name={reward}, description={Some\index{reward} observed 
		(or measured) quantity that allows to estimate the \gls{loss} incurred by the \gls{prediction} 
		...
		the current steering direction of vehicle. We could construct a reward from the 
...
\newglossaryentry{softclustering}{name={soft clustering}, description={Soft \gls{clustering}\index{soft clustering} 
		...
		Each \gls{datapoint} is assigned to several different \gls{cluster}s with varying \gls{dob}. Soft \gls{clustering} 
...
\newglossaryentry{svm}{name={support vector machine (SVM)}, description={The\index{support vector machine (SVM)} 
		...
		SVM uses a different \gls{lossfunc} than those methods. As illustrated in 
		Figure \ref{fig_svm_gls_dict}, it aims to maximally separate \gls{datapoint}s from 
		the two different classes in the \gls{featurespace} (\gls{maximum} margin principle). 
		...
		\begin{figure}[htbp]
			\begin{center}
				...
				\caption{The \gls{svm} learns a \gls{hypothesis} (or \gls{classifier}) $\hypothesis^{(\weights)}$ with 
					\gls{minimum} average soft-margin \gls{hingeloss}. Minimizing this \gls{loss} is equivalent 
...
\newglossaryentry{sgd}{name={subgradient descent}, description={\Gls{subgradient}\index{subgradient descent} 
		...
		allow to construct local approximations of an \gls{objfunc}. The \gls{objfunc} 
...
to the \gls{risk}. As it name suggests, online \gls{gd} updates $\weights^{(\timeidx)}$ via a (projected) \gls{gradstep}
\begin{equation} 
...
\begin{tikzpicture}[x=1.5cm,scale=1.5, every node/.style={font=\footnotesize}]
	...
	\end{tikzpicture}
\end{center} 
\caption{An instance of online \gls{gd} that updates the \gls{modelparams} $\weight^{(\timeidx)}$ 
...
\newglossaryentry{pca}{name={principal component analysis (PCA)}, description={PCA\index{principal component analysis (PCA)} 
		...
		allow to reconstruct the original \gls{feature}s with the \gls{minimum} reconstruction error \cite{MLBasics}.},first={principal component analysis (PCA)},text={PCA} }
...
\newglossaryentry{lossfunc}{name={loss function}, description={A\index{loss function} \gls{loss} function is a map 
		...
		It assigns a pair consisting of a \gls{datapoint}, with \gls{feature}s $\featurevec$ 
		and \gls{label} $\truelabel$, and a \gls{hypothesis} $\hypothesis \in \hypospace$, a 
		non-negative real number (the \gls{loss}) $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$. The 
		...
		\begin{figure}[htbp]
			...
			\caption{Some \gls{loss} function $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$ for a fixed \gls{datapoint}, with 
				\gls{featurevec} $\featurevec$ and \gls{label} $\truelabel$, and varying \gls{hypothesis} $\hypothesis$. 
				\gls{ml} methods try to find (and learn) a \gls{hypothesis} that incurs minimal \gls{loss}.}
...
\newglossaryentry{API} 
{name={application programming interface (API)},
	description={An\index{application programming interface (API)} API defines a precise specification of the functionalities, services, 
		...
		the system \cite{RestfulBook2013}. For example, an \gls{ml} service 
		...
		\end{quote}
		This API allows users to send a \gls{featurevec} and receive a predicted 
		\gls{label} along with a confidence score.
	},
	first={application programming interface (API)},
	text={API}
}
...
\newglossaryentry{sample}{name={sample},description={A\index{sample} 
		...
		is obtained or interpreted as the \gls{realization}s of $\samplesize$ \gls{iid} \gls{rv}s 
...
\newglossaryentry{gd}{name={gradient descent (GD)},description={\Gls{gradient}\index{gradient descent (GD)} 
		...
		approximation $\weights^{(\itercntr)}$ for \gls{minimum}. We would like to find a new (better) vector $\weights^{(\itercntr+1)}$ 
...
\newglossaryentry{huberreg}{name={Huber regression},description={
			...
			against the computational benefits of the smooth \gls{sqerrloss}.},
...
\newglossaryentry{pdf}{name={probability density function (pdf)},
	description={The\index{probability density function (pdf)} \gls{probability} density function $p(\feature)$ 
        ...
        allows to compute the \gls{probability} of $\featurevec$ belonging to a (measurable) region $\mathcal{R}$ via 
...
\newglossaryentry{stopcrit}{name={stopping criterion},
	description={Many\index{stopping criterion} \gls{ml} methods use iterative algorithms that construct a 
		sequence of \gls{modelparams} (such as the \gls{weights} of a linear map or 
		the \gls{weights} of an \gls{ann}) that (hopefully) converge to an optimal choice 
...
\newglossaryentry{paramspace}{name={parameter space},
		...
		e.g., all vectors $\weights \in \mathbb{R}^{\dimlocalmodel}$ with \gls{norm} smaller than one.
		\begin{figure} 
			\begin{center}
			\begin{tikzpicture}
...
\newglossaryentry{localdataset}{name={local dataset},description={The\index{local dataset} concept of a local \gls{dataset} is 
		...
		related to other local \gls{dataset}s via different notions of similarities. These similarities 
...
{name={mutual information (MI)},
 description={The\index{mutual information (MI)} MI $\mutualinformation{\featurevec}{\truelabel}$ 
	...
	$\truelabel$ can be well predicted solely from $\featurevec$. This \gls{prediction} could be obtained by a 
		\gls{hypothesis} learnt by an \gls{erm}-based \gls{ml} method. 
	 }, first={MI}, text={MI} 
}
