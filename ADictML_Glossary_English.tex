...
\newglossaryentry{gradstep}{name={gradient step},description={Given a \gls{differentiable} 
		...
		\begin{figure}[htbp]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{The basic \gls{gradient} step \eqref{equ_def_GD_step_dict} maps a given vector $\weights$ 
...
\newglossaryentry{trustAI}{
	name={trustworthy artificial intelligence (trustworthy AI)},description=
...
\newglossaryentry{probmodel}
{
	...
	description={A probabilistic \gls{model}\index{probabilistic model} interprets \gls{datapoint}s 
		...
		involves \gls{parameters} which have to be manually chosen or learnt via statistical inference 
...
{name={optimism in the face of uncertainty},
	...
	with sufficiently high probability. As optimists, we choose the \gls{modelparams} 
...
{name={distributed algorithm},
	description={A \index{distributed algorithm} distributed algorithm is an \gls{algorithm} designed for 
		...
		which is implemented on a single device, a distributed \gls{algorithm} is 
		executed concurrently on multiple devices with computational capabilities. 
		Similar to a classical \gls{algorithm}, a distributed \gls{algorithm} can be modelled as a 
		...
		and messages received from other devices. One important application of distributed 
		algorithms is in \gls{fl} where a network of devices collaboratively train a personal \gls{model} 
		for each device. 
		},
...
{name={algorithm},
  description={An\index{algorithm} algorithm is a precise, step-by-step specification for 
	...
	transform a given \gls{trainset} into \gls{modelparams} though a sequence of \gls{gradstep}s. 
    This informal characterization can be formalized rigorously via different mathematical models \cite{Sipser2013}. 
    One very simple model of an algorithm is a collection of possible executions. Each execution is a sequence:
    $${\rm input},s_1,s_2,\ldots,s_T,{\rm output}$$ 
    that respects the constraints inherent to the computer that executes the algorithm.
...
{name={online algorithm},
	description={An\index{online algorithm} online \gls{algorithm} processes input data incrementally, 
		receiving data items sequentially and making decisions or producing outputs (or decisions) immediately 
		...
		outputs (or decisions), and inputs. Specifically, at step \(\iteridx\), the algorithm performs a computational step 
		\(s_{\iteridx}\), generates an output \(\text{out}_{\iteridx}\), and then subsequently 
		receives the next input \(\text{in}_{\iteridx+1}\). A notable example of an online algorithm in \gls{ml} is 
		\gls{onlineGD} (online gradient descent), which incrementally updates \gls{modelparams} as new data points 
		arrive.
	},
...
{name={federated averaging (FedAvg)},
	description={FedAvg\index{federated averaging (FedAvg)} refers to an iterative \gls{fl} algorithm that alternates between separately training \gls{localmodel}s and combining the updated local \gls{modelparams}. The training of \gls{localmodel}s 
...
{name={FedProx},
	description={FedProx\index{FedProx} refers to an iterative \gls{fl} algorithm that alternates between separately training \gls{localmodel}s and combining the updated local \gls{modelparams}. In contrast to \gls{fedavg}, which uses 
...
{name={dataset},
	description={A\index{dataset} dataset refers to a collection of \gls{datapoint}s. These 
		...
		and \gls{model} \gls{validation}. Note that our notion of a \gls{dataset} is very flexible as 
		...
		As a case in point, Fig.\ \ref{fig_cows_dataset} depicts a \gls{dataset} that consists of cows as 
		\gls{datapoint}s. 
		\begin{figure}[htbp]
				\begin{center}
	  ...
	  \end{figure}
       Quite often, a \gls{ml} engineer does not have direct access to a dataset. Indeed, accessing the 
       dataset in Fig.\ \ref{fig:cowsintheswissalps} would require to visit the cow herd in the Alps. Instead, 
       we need to use an approximation (or representation) of the \gls{dataset} which is more convenient 
       ...
       One of the most widely adopted data model is the relational model, which organizes data as a table (or relation) \cite{codd1970relational,silberschatz2019database}.
		...
		\begin{table}[h]
			...
			\begin{tabular}{lcccc}
				...
				\hline
			\end{tabular}
			\caption{A relation (or table) that represents the dataset in Fig.\ \ref{fig:cowsintheswissalps}.}
			\label{tab:cowdata}
		\end{table}
...
\newglossaryentry{parameters}{name={parameters},
	description={The\index{parameters} parameters of an \gls{ml} \gls{model} are tunable 
		(learnable or adjustable) quantities that allow us to choose between different \gls{hypothesis} maps. 
