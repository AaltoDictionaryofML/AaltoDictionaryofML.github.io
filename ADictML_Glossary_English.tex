...
{name={machine learning (ML)},
		 description={Machine learning\index{machine learning (ML)} (ML) aims to predict 
...
{name={autoencoder},
	description={An autoencoder\index{autoencoder} is an \gls{ml} method that jointly learns an encoder map 
...
\newglossaryentry{mvndist}{name ={multivariate normal distribution}, description={The\index{multivariate normal distribution} 
		...
		This family is parametrized by the mean $\vm$ and the \gls{covmtx} $\mC$ of $\featurevec$. 
...
\newglossaryentry{underfitting}{name={underfitting},description={Consider\index{underfitting} 
		an \gls{ml} method that uses \gls{erm} to learn a \gls{hypothesis} with the minimum \gls{emprisk} 
...
\newglossaryentry{gdpr}{name={general data protection regulation (GDPR)},description={
			The\index{general data protection regulation (GDPR) } general data protection regulation (GDPR) 
			...
			The GDPR has significant implications for how data is collected, stored, and used in \gls{ml}  
			...
			\begin{itemize}
				...
				\item Transparency and \gls{explainability}: \gls{ml} systems should enable their users to 
...
\newglossaryentry{trustAI}{name={trustworthy AI},description=
...
{name=differential privacy (DP),
  description={
  	Consider\index{differential privacy} some \gls{ml} method $\algomap$ that reads in a \gls{dataset} (e.g., the \gls{trainset} 
...
{name={flow-based clustering},
	description={Flow-basted clustering\index{flow-based clustering} groups the nodes 
		...
		vectors. These \gls{feature} vectors are built from network flows between 
...
{name={GTV minimization},
	description={GTV minimization\index{GTV minimization} is an instance of \gls{rerm} 
...
{name={networked federated learning},
	description={Networked\index{networked federated learning} federated learning refers 
		to methods that learn personalized models in a distributed fashion. These methods learn from \gls{localdataset}s 
...
{name={label space},
	description={Consider\index{label space} an \gls{ml} application that involves \gls{datapoint}s characterized by features 
...
{name={feature},
	description={A\index{feature} feature of a \gls{datapoint} is one of its properties that can be 
		...
		\emph{independent variable}, \emph{input (variable)}, \emph{predictor (variable)} or \emph{regressor} \cite{Gujarati2021}, \cite{Dodge2003}, \cite{Everitt2022}. 
...
{name={label},
	description={A\index{label} higher-level fact or quantity of interest associated with a \gls{datapoint}. 
		...
		include \emph{response variable}, \emph{output variable}, and \emph{target} \cite{Gujarati2021}, \cite{Dodge2003}, \cite{Everitt2022}.
 },
...
{name={random variable (RV)},
 description={A random variable is a function that maps from 
 	a \gls{probspace} $\mathcal{P}$ to a value space \cite{BillingsleyProbMeasure,GrayProbBook}. 
 	The \gls{probspace} consists of elementary events and is equipped with a probability 
 	measure that assigns probabilities to subsets of $\mathcal{P}$. 
 	Different types of random variables include:  
 	\begin{itemize} 
 	\item {\bf Binary random variables}, which map elementary events to a set of two distinct values, such as 
 	$\{-1,1\}$ or $\{\text{cat}, \text{no cat}\}$.  
 	\item {\bf Real-valued random variables}, which take values in the real numbers $\mathbb{R}$.  
 	\item {\bf Vector-valued random variables}, which map elementary events to the \gls{euclidspace} $\mathbb{R}^{\featuredim}$.  
 	\end{itemize} 
 	Probability theory uses the concept of measurable spaces to rigorously define 
 	and study the properties of (large) collections of random variables \cite{,BillingsleyProbMeasure}.}, first={RV},text={RV}  }
 
 \newglossaryentry{probspace}{
 	name={probability space}, 
 	description={A\index{probability space} probability space is a mathematical 
 		model of a physical process (a \emph{random experiment}) with an uncertain outcome. 
 	   Formally, a probability space $\mathcal{P}$ is a triplet $(\Omega, \mathcal{F}, P)$, where:
 		\begin{itemize} 
 		\item  $\Omega$ is a \emph{sample space}, containing all possible elementary outcomes of a random experiment.
 		\item  $\mathcal{F}$ is a \emph{sigma-algebra}, a collection of subsets of $\Omega$ (called events) that satisfies 
 		certain closure properties under set operations.
 		\item $P$ is a \emph{probability measure}, a function that assigns a probability $P(\mathcal{A}) \in [0,1]$ 
 		...
 		\end{itemize}
 		Probability spaces provide the foundation for defining \gls{rv}s and to reason about 
...
{name={data point},
description={A\index{data point}data point is any object that conveys information \cite{coverthomas}. Data points might be 
...
\newglossaryentry{ssl}{name={semi-supervised learning}, description={Semi-supervised\index{semi-supervised learning} 
		...
		for \gls{ml} applications that offer a large amount of unlabeled \gls{datapoint}s, but only a limited 
...
\newglossaryentry{datapoisoning}{name={data poisoning}, description={Data\index{data poisoning} 
		...
		data poisoning is particularly important in distributed \gls{ml} applications where \gls{dataset}s are de-centralized.},first={data poisoning},text={data poisoning} }
...
\newglossaryentry{featuremap}{name={feature map}, description={A\index{feature map} map 
		...
		followed by a \gls{linmodel} in the form of the output layer. Another reason for learning a \gls{featuremap} 
		 ...
		 ensures interpretability \cite{Ribeiro2016}. The special case of a \gls{feature} map delivering 
...
%	 				% 		% Left part - Graph
 		 		...
 		 		\begin{minipage}{0.45\textwidth}
	 			 	 \begin{equation} 
	 				 		 ...
	 				 		 \end{equation} 
	 			 \end{minipage}
	 	 \caption{\label{fig_lap_mtx_dict} Left: Some undirected graph $\graph$ with three nodes $\nodeidx=1,2,3$. 
...
\newglossaryentry{kernelmethod}{name={kernel method}, 
	description={A\index{kernel method} kernel method is an \gls{ml} method that uses a 
...
\newglossaryentry{dbscan}{name={density-based spatial clustering of applications with noise}, 
	description={Density-based spatial clustering of applications with noise\index{DBSCAN} refers to a clustering algorithm for \gls{datapoint}s that are characterized by numeric feature vectors. 
...
\newglossaryentry{fl}{name={federated learning (FL)}, description={Federated\index{federated learning} 
		learning is an umbrella term for \gls{ml} methods that train models in a collaborative 
...
\newglossaryentry{cfl}{name={clustered federated learning (CFL)}, description={
		Clustered\index{clustered federated learning} \gls{fl} (CFL) assumes that \gls{localdataset}s 
...
\newglossaryentry{outlier}{name={outlier}, description={Many\index{outlier} \gls{ml} methods 
		...
		that conform with an \gls{iidasspt} as well as a small number of data points that have fundamentally different 
...
\newglossaryentry{xml}{name={explainable ML}, description={Explainable\index{explainable ML} 
...
\newglossaryentry{gmm}{name={Gaussian mixture model}, description={A Gaussian\index{Gaussian mixture model} 
		...
		mean vector $\clustermean^{(\clusteridx)}$, and the \gls{covmtx} $\clustercov^{(\clusteridx)}$ for each $\clusteridx=1,\ldots,\nrcluster$. 
...
\newglossaryentry{em}{name={expectation-maximization}, description={
		...
		ML application. The \gls{maxlikelihood} estimator for the \gls{modelparams} $\weights$ is obtained by maximizing 
		$\prob{\dataset; \weights}$. However, the resulting oprimtization problem might be computationally 
		challenging. Expectation-maximization approximates the \gls{maxlikelihood} estimator by introducing a latent 
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge\index{ridge regression} 
		regression learns the \gls{weights} $\weights$ of a linear \gls{hypothesis} map $\hypothesis^{(\weights)}(\featurevec)= \weights^{T} \featurevec$. The quality of a particular choice for the \gls{modelparams} $\weights$ is measured by the sum 
...
\newglossaryentry{iidasspt}{name={i.i.d.\ assumption}, description={The \gls{iid} 
		assumption\index{i.i.d.} interprets \gls{datapoint}s of a \gls{dataset} as the 
		\gls{realization} of \gls{iid} \gls{rv}s.},first={i.i.d.\ assumption},text={i.i.d.\ assumption} }

\newglossaryentry{hypospace}{name={hypothesis space}, description={Every\index{hypothesis space} 
		practical \gls{ml} method uses a hypothesis space (or \gls{model}) $\hypospace$. The hypothesis space 
...
\newglossaryentry{model}{name={model}, description={In\index{model} the context of \gls{ml} methods, 
		the term \emph{model} typically refers to the \gls{hypospace} employed by an 
...
\newglossaryentry{ai}{name={artificial intelligence}, description={
		...
		maximizing a long-term \gls{reward}. The ML-based approach to AI is to train a \gls{model} for  
...
\newglossaryentry{reward}{name={reward}, description={Some\index{reward} observed 
		...
		ML application to self-driving vehicles, $\hypothesis(\featurevec)$ could represent 
...
\newglossaryentry{svm}{name={support vector machine}, description={The\index{support vector machine} 
		support vector machine (SVM) is a binary \gls{classification} method that 
		learns a linear \gls{hypothesis} map. Thus, like \gls{linreg} and \gls{logreg}, 
		it is also an instance of \gls{erm} for the \gls{linmodel}. However, the support 
		vector machine uses a different \gls{lossfunc} than those methods. As illustrated in 
		Figure \ref{fig_svm_gls_dict}, it aims to maximally separate \gls{datapoint}s from 
		the two different classes in the \gls{feature} space (\emph{maximum margin principle}). 
...
Consider \index{online GD} an \gls{ml} method that learns \gls{modelparams} 
...
Some \gls{ml} applications require methods that learn \emph{online}: as soon as a new \gls{datapoint} $\datapoint^{(\timeidx)}$ 
...
\newglossaryentry{lossfunc}{name={loss function}, description={A\index{loss function} loss function is a map 
		...
		Figure \ref{fig_loss_function_gls_dict} depicts a \gls{lossfunc} for a given \gls{datapoint}, 
		...
		\begin{figure}[htbp]
			...
			\caption{Some \gls{lossfunc} $\lossfunc{(\featurevec,\truelabel)}{\hypothesis}$ for a fixed \gls{datapoint}, with 
				...
				ML methods try to find (and learn) a \gls{hypothesis} that incurs minimal \gls{loss}.}
...
\newglossaryentry{API} 
{name={application programming interface (API)},
	description={An\index{application programming interface (API)} application programming 
...
\newglossaryentry{ann}{
	...
	description={An\index{artificial neural network} artificial neural network (ANN) 
...
\newglossaryentry{gd}{name={gradient descent (GD)},description={Gradient\index{gradient descent} 
		...
		a single \gls{gd} step \eqref{equ_def_GD_step_dict}.
		\begin{figure}[htbp]
			\begin{center}
				\begin{tikzpicture}[scale=0.8]
					\draw[loosely dotted] (-4,0) grid (4,4);
				...
				\end{tikzpicture}
			\end{center}
			\caption{A single gradient step \eqref{equ_def_GD_step_dict} towards the minimizer $\overline{\weights}$ of $f(\weights)$.}
...
\newglossaryentry{abserr}{name={absolute error loss},description={
			...
			\gls{label} $\truelabel \in \mathbb{R}$. The absolute error loss \index{absolute error loss} 
...
\newglossaryentry{llm}{name={Large Language Model},description={
	Large Language Models (LLMs)\index{large language model (LLM)} is an umbrella term for \gls{ml} methods 
...
\newglossaryentry{huberreg}{name={Huber regression},description={
			Huber regression\index{Huber regression} refers to \gls{erm}-based methods 
...
\newglossaryentry{pdf}{name={probability density function (pdf)},
	description={The\index{probability density function} probability density function (pdf) $p(\feature)$ 
        ...
        allows to compute the probability of $\featurevec$ belonging to a (measurable) region $\mathcal{R}$ via 
...
\newglossaryentry{datanorm}{name={data normalization},
	description={Data normalization\index{data normalization} refers to transformations 
		...
		a fixed \gls{learnrate}, convergence depends on controlling the norm of \gls{featurevec}s 
...
\newglossaryentry{dataaug}{name={data augmentation},
	description={Data augmentation\index{data augmentation} methods add synthetic \gls{datapoint}s 
		...
		are very different (see Figure \ref{fig_symmetry_dataaug_dict}). Data augmentation can be an 
...
\newglossaryentry{localdataset}{name={local dataset},description={The\index{local dataset} concept of a local dataset is 
		...
		In contrast to a single \gls{dataset} used in basic \gls{ml} methods, a local dataset is also 
