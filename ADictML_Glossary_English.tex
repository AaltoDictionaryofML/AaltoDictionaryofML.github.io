...
{name={horizontal federated learning (HFL)},
	description={HFL\index{horizontal federated learning (HFL)} uses 
		\gls{localdataset}s constituted by different \gls{datapoint}s but 
		uses the same \gls{feature}s to characterize them \cite{HFLChapter2020}.
		...
		\gls{feature}s of different spatio-temporal regions. Each spatio-temporal 
		region represents an individual \gls{datapoint}, each characterized by the 
		same \gls{feature}s (e.g., daily temperature or air pressure).
		\\ 
		See also: \gls{fl}, \gls{vfl}, \gls{cfl} },
	first={horizontal FL},text={HFL}
} 
...
{name={mean},
 description={The \index{mean} mean of a \gls{rv} $\featurevec$, taking 
...
%		% Optional dotted line from w to bad
	...
	are the majority \cite[Thm. 2.2.]{Lopuhaae1991}.},
...
{name={bagging},
 description={Bagging\index{bagging} (or bootstrap aggregation) 
