...
{name={provider (AI Act)},
	description={The\index{provider (AI Act)} EU AI Act defines a provider 
		as a natural or legal person, public
		authority, agency (e.g., the European Medicines Agency), or other body
		that develops an \gls{aisystem} or a
		general-purpose AI \gls{model} and places it on the market or puts it
		into service under the provider's own name or trademark, whether for payment
		or free of charge (Article~3(3), \cite{EUAIAct2024}).
		The provider bears the primary regulatory obligations under the AI Act,
		including conformity assessment, technical documentation, and monitoring.
		A provider is distinct from a deployer, who integrates an already-placed
		\gls{aisystem} into their own products or processes.
		For example, a company that trains a \gls{model} for credit scoring
		and offers it via an \gls{apiendpoint} acts as a provider, whereas
		a bank that queries that \gls{apiendpoint} to support its own loan
		decisions acts as a deployer.
		\\
		See also: \gls{aisystem}, \gls{model}, \gls{apiendpoint}, \gls{mlaas}.},
	first={provider},
	plural={providers},
...
{name={interpretability},
	description={An \gls{ml} method is interpretable\index{interpretability} for a 
		...
		In the context of Fig.\ \ref{fig_aug_simulatability_dict}, interpretability of an \gls{ml} 
	 	...
	 	on an arbitrary \gls{testset}. This contrasts with \gls{explainability}, 
		where the user is provided with \glspl{explanation} to better understand the \glspl{prediction}
		of $\learnthypothesis$ on a specific \gls{testset} $\dataset'$. These \glspl{explanation} 
		may take the form of saliency \glspl{map} or reference examples from the \gls{trainset}.
	 	\\ 
...
{name={counterfactual explanation},
	description={A counterfactual \gls{explanation}\index{counterfactual explanation} 
		is a type of \gls{explanation} that enhances the \gls{transparency} 
		of a trained \gls{ml} \gls{model}. It describes how the \glspl{feature} 
		of a given \gls{datapoint} would need to change in order to obtain 
		a different \gls{prediction}. Consider a trained \gls{model} 
		$\learnthypothesis:\featurespace \rightarrow\labelspace$ and a 
		\gls{featurevec} $\featurevec$ with \gls{prediction} $\learnthypothesis(\featurevec)=\predictedlabel$. 
		A counterfactual \gls{explanation} specifies an alternative 
		\gls{featurevec} $\featurevec'$ such that
		$$\learnthypothesis(\featurevec')  \neq \predictedlabel.$$
		\\
		See also: \gls{explanation}, \gls{explainability}, \gls{classification}.},
...
{name={explainability},
	description={We\index{explainability} define the (subjective) explainability of an \gls{ml} method 
		...
		\gls{model} via the conditional (differential) \gls{entropy} of its \glspl{prediction}, given the 
...
{name={FAIR principles},
	description={The FAIR principles\index{FAIR principles} are guidelines for 
		scientific \gls{data} management. The aim is to make research 
		...
		supports the traceability and reproducible inspection of an \gls{mlsystem} \cite{Samuel2020}. 
		\\
		See also: \gls{data}, \gls{trustAI}.},
...
{name={SHapley Additive exPlanations (SHAP)},
	description={SHAP\index{SHapley Additive exPlanations (SHAP)} is a post hoc method for 
		explaining the \gls{prediction} $\learnthypothesis(\featurevec)$ 
		of a trained \gls{model} $\learnthypothesis \in \hypospace$ 
		at a given \gls{featurevec} $\featurevec=\big( \feature_1,\,\dots,\,\feature_d\big)^{T}$.  
		SHAP values are computed after \gls{model} \gls{training} and can 
		be used to analyze the relative importance of different \glspl{feature} 
		for the \gls{prediction} $\learnthypothesis(\featurevec)$ \cite{Molnar2019}.
		\\
		See also: \gls{prediction}, \gls{training}. },
...
{name={automated decision-making},
	description={Automated decision-making\index{automated decision-making} 
		refers to \gls{ml} applications that use \glspl{prediction} 
...
{name={general data protection regulation (GDPR)},
	description={The\index{general data protection regulation (GDPR)} GDPR
		...
		\begin{itemize}
			...
			rectify, and delete their \gls{personaldata}, as well as to object to 
			\gls{automateddecisionmaking} and \gls{profiling}.
...
{name={deep fake},
	description={Deep fakes are synthetic\index{deep fake} media generated or 
		...
		a real person, object, or \gls{event}. Deep fakes are typically produced using 
             	generative methods, trained to imitate visual, audio, or audiovisual 
            	characteristics of real \gls{data}. From a system perspective, deep fakes are 
...
{name={transparency},
	description={Transparency\index{transparency} is a fundamental requirement for 
		...
		\gls{ai}-based lending decisions should be accompanied by \glspl{explanation} of 
		contributing factors, such as income level or credit history. These \glspl{explanation} 
