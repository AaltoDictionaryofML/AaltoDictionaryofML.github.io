...
\newpage
\begin{align}
    	&\argmin_{\weights \in \mathcal{C}} f(\weights) &\quad &\parbox{.70\textwidth}{The set of minimizers 
		for a real-valued \gls{function} $f: \mathcal{C} \rightarrow \mathbb{R}$. 
...
\begin{align}
	&\indicatorfunc{\mathcal{A}}(x) & \quad & \parbox{.70\textwidth}{The indicator \gls{function} of a set $\mathcal{A}$ 
...
\begin{align}
	...
	&\probdist^{(\featurevec,\truelabel)} &\quad & \parbox{.85\textwidth}{A (joint) \gls{probdist} of an \gls{rv} 
		...
		\gls{label}.} \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\probdist^{(\truelabel|\featurevec)} &\quad & \parbox{.85\textwidth}{A conditional \gls{probdist} 
...
\begin{align} 
	...
	&\simplex{\nrcluster} &\quad & \parbox{.85\textwidth}{The 
	   probability simplex which consists of all \glspl{vector} $\vp = \big( p_{1}, \,\ldots, \,p_{\nrcluster} \big)\,^{T} \in \mathbb{R}^{\nrcluster}$ 
...
\begin{align} 
		...
		\\ See also: \gls{event}.}   \nonumber \\[2mm] \hline \nonumber\\[-5mm] 
	&\sigmaalgebra &\quad & \parbox{.85\textwidth}{A collection of \gls{measurable} 
	            subsets of a \gls{samplespace} $\samplespace$. 
	...
	&\mathcal{P} &\quad & \parbox{.85\textwidth}{A \gls{probspace} that consists of a \gls{samplespace} $\samplespace$, a 
		\gls{sigmaalgebra} $\sigmaalgebra$ of \gls{measurable} subsets of 
		    $\samplespace$, and a \gls{probdist} $\prob{ \cdot}$.
...
\begin{align}
 	&\graph = \pair{\nodes}{\edges} & \quad & \parbox{.80\textwidth}{An \gls{undirectedgraph} whose nodes $\nodeidx \in \nodes$ represent 
		...
		\\ See also: \gls{undirectedgraph}, \gls{device}, \gls{empgraph}, \gls{dataset}, \gls{learningtask}. }\nonumber \\[2mm] \hline \nonumber\\[-5mm]
...
\begin{align} 
	...
	&\localparams{\nodeidx} & \quad & \parbox{.70\textwidth}{The local \glspl{modelparam} of \gls{device} $\nodeidx$ within an \gls{empgraph}.
		\\ See also: \gls{modelparam}, \gls{device}, \gls{empgraph}. } \nonumber \\[2mm] \hline \nonumber\\[-5mm]
