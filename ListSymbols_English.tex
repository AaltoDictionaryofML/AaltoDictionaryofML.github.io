...
\begin{align}
    ...
    &\sphere{\nrnodes} &\quad &\parbox{.70\textwidth}{The set of unit-\gls{norm} vectors in $\mathbb{R}^{\nrnodes+1}$.
	 ...
	 &\operatorname{epi}(f)  & \quad & \parbox{.70\textwidth}{The \gls{epigraph} of a real-valued function 
	 ...
	 &  \frac{\partial f(\weight_{1},\ldots,\weight_{\nrfeatures})}{\partial \weight_{\featureidx}} & \quad & \parbox{.70\textwidth}{The partial derivative (if it exists) of 
	 		a real-valued function $f: \mathbb{R}^{\featuredim}\rightarrow \mathbb{R}$ with respect to\ $\weight_{\featureidx}$\cite[Ch. 9]{RudinBookPrinciplesMatheAnalysis}.
			} \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	 &\nabla f(\weights) & \quad & \parbox{.70\textwidth}{The \gls{gradient} of a \gls{differentiable} real-valued function 
...
\begin{align} 
	...
	&\determinant{\mC} &\quad & \parbox{.85\textwidth}{The determinant of the matrix $\mC$. } \nonumber \\[2mm] \hline \nonumber\\[-5mm]
...
\begin{align} 
	...
	&\mathcal{N}(\clustermean, \mathbf{C}) &\quad & \parbox{.85\textwidth}{The \gls{mvndist} of a vector-valued 
...
\begin{align}
	...
	&\hypothesis(\cdot)  &\quad &\parbox{.90\textwidth}{A \gls{hypothesis} map that reads in \glspl{feature} $\featurevec$ of a \gls{datapoint} 
...
\begin{align}                          
	...
	&\regparam &\quad &\parbox{.85\textwidth}{A \gls{regularization} parameter that controls 
...
\begin{align} 
	&\weights  &\quad & \parbox{.85\textwidth}{A parameter vector $\weights = \big(\weight_{1},\ldots,\weight_{\featuredim}\big)^{T}$ 
		of a \gls{model}, e.g., the \gls{weights} of a \gls{linmodel} or in an \gls{ann}.
		\\ See also: \gls{model}, \gls{weights}, \gls{linmodel}, \gls{ann}. }     \nonumber \\[2mm] \hline \nonumber\\[-5mm]
	&\hypothesis^{(\weights)}(\cdot)  &\quad &\parbox{.85\textwidth}{A \gls{hypothesis} map that involves tunable \gls{modelparams} 
...
\begin{align} 
	&\maxnodedegree^{(\graph)} & \quad & \parbox{.85\textwidth}{The \gls{maximum} weighted node degree of a \gls{graph} $\graph$.
