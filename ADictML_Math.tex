...
{name={optimization problem}, 
	description={An\index{optimization problem} \gls{optimization} problem is a mathematical 
		   ...
		   The goal in \gls{optimization} is to find those values $\weights \in \paramspace$ 
...
{name={gradient step}, 
	description={Given a \gls{differentiable} 
		...
		the new \gls{vector} (see Fig. \ref{fig_basic_GD_step_single_dict}) as follows:
		\begin{equation}
		...
		More formally, the \gls{gradient} step amounts to applying the \gls{operator} 
		 ...
		 a \gls{firmlynonexpansiveop} \cite[Corollary 18.16]{BausckeCombette}. Moreover, for 
		 a \gls{strcvx} and \gls{smooth} \gls{function} $f(\cdot)$ and an appropriate choice of \gls{learnrate}, 
		 one can verify that $\mathcal{T}^{(f,\lrate)}$ is a \gls{contractop} \cite[Th. 2.1.14]{nesterov04}. 
