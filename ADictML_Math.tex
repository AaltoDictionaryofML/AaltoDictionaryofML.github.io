...
{name={proximal algorithm},
	description={A\index{proximal algorithm} proximal algorithm refers to an \gls{optmethod} 
	             that uses \glspl{proxop} as the main computational primitive \cite{ProximalMethods}. 
				 Proximal algorithms generalize \glspl{gdmethod} by replacing 
				 ...
				 and \gls{convex} \gls{function} conincides with the \gls{proxop} 
				 of a local linear approximation. 
				 Proximal algorithms can 
	...
	description={The\index{Laplacian matrix} structure of a \gls{graph} $\graph$, with 
		...
		\begin{tikzpicture}
 		...
 		\begin{minipage}{0.45\textwidth}
				...
				\end{minipage}
	 	\end{minipage}
	 	\caption{\label{fig_lap_mtx_dict} (a) Some \gls{undirectedgraph} $\graph$ with three nodes $\nodeidx=1,\,2,\,3$. 
...
{name={Lagrangian},
	description={Consider a \gls{constrainedoptprob} of the following form:
		...
		\begin{figure}[H]
			...
			% Labels
			\end{tikzpicture}
		\caption{An \gls{optproblem} with an \gls{objfunc} $f(\weights)$ and 
...
{name={dual problem},
	description={Consider a \gls{constrainedoptprob}, which we 
		...
		\begin{figure}[H]
     		  	...
     		  	\end{tikzpicture}
     		\caption{The dual problem of a \gls{constrainedoptprob} is to find a \gls{supportinghyperplane} with 
...
{name={gradient step}, 
	description={Given a \gls{differentiable} 
		...
		More formally, the \gls{gradient} step amounts to applying the \gls{operator} 
		 ...
		 for a sufficently small \gls{learnrate} $\lrate > 0$, one can verify 
