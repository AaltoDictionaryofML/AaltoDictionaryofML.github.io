...
{name={conjugate prior},
	description={A prior distribution $p(\theta)$ is \textbf{conjugate} to a likelihood $p(x|\theta)$ if the resulting posterior distribution $p(\theta|x)$ resides in the same functional family as the prior.
    Exponential Family Likelihood: For a likelihood function expressed in the natural form of the exponential family:
	This relationship is defined by the fundamental Bayesian update:
	\begin{equation}
   		p(\theta | x) \propto p(x | \theta) \, p(\theta)
	\end{equation}
	In this general form, conjugacy ensures that the algebraic structure of the \textit{Prior} is preserved after being multiplied by the \textit{Likelihood}. This transforms the difficult task of Bayesian inference into a simple update of the distribution's parameters
    For example: The Exponential Family Likelihood: For a likelihood function expressed in the natural form of the exponential family:
    \begin{equation}
        p(x|\theta) = h(x) \exp \left( \theta^\top T(x) - \psi(\theta) \right)
    \end{equation}
    where $\theta$ is the natural parameter, $T(x)$ is the sufficient statistic, and $\psi(\theta)$ is the log-partition function.
    Conjugate Prior: Given hyperparameters $\alpha$ and $\gamma$, the conjugate prior is defined as:
    \begin{equation}
        p(\theta|\alpha, \gamma) \propto \exp \left( \theta^\top \alpha - \gamma \psi(\theta) \right)
    \end{equation}
    Posterior Update The resulting posterior distribution is:
    \begin{equation}
        \begin{aligned}
            p(\theta|x, \alpha, \gamma) &\propto p(x|\theta)p(\theta|\alpha, \gamma) \\
            &\propto \exp \left( \theta^\top [T(x) + \alpha] - [\gamma + 1] \psi(\theta) \right) \\
            &= p(\theta | T(x) + \alpha, 1 + \gamma)
        \end{aligned}
    \end{equation}
    This demonstrates that the posterior maintains the same form as the prior, with the updated hyperparameters $\alpha' = \alpha + T(x)$ and $\gamma' = \gamma + 1$. 
	This makes Bayesian updating a simple matter of changing parameters rather than solving complex integrals. \cite{barber2012bayesian} },
