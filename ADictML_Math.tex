	...
	description={Consider a constrained \gls{optproblem} of the form
				 \begin{align} 
				  ...
				  \begin{figure}[H]
					...
					\fill ($( -2.6,2.55)!0.8!(4.8,0.85)$) circle (1.6pt) node[above right] {$\pair{f_{0}(\weights)}{f_{1}(\weights)}$};
					% Labels
					\end{tikzpicture}
					\caption{An \gls{optproblem} with \gls{objfunc} $f_{0}(\weights)$ and 
					         a single inequality constraint $f_{1}(\weights) \leq 0$  can be represented 
							 by a set $\mathcal{G} \defeq \big\{ \big(f_{1}(\weights),\,f_{0}(\weights)\big) : \weights \in \mathbb{R}^{\nrfeatures} \big\}$.
							 The value of the Lagrangian $L(\weights, \lambda)$ is the vertical intercept of a line with slope $-\lambda$ 
							 that passes through the point $\pair{f_{1}(\weights)}{f_{0}(\weights)} \in \mathcal{G}$ \cite{BoydConvexBook} 
				...
				Here, the Lagrangian $L(\weights, \lambda)$ is given by the location of the intersection between the vertical 
		        axis and a straight line with slope $-\lambda$ that passes through a point $\pair{f_{1}(\weights)}{f_{0}(\weights)}$. 
		\\
