...
{name={group},
	description={A\index{group} group $(G, \diamond)$ is an algebraic structure \cite[Chap.~1]{artin2016geometric} consisting of a set $G$ and a                 binary operation $\diamond$ which assigns to any two elements $a, b \in G$ another element $a \diamond b \in \mathcal{G}$. For $G$                 to be a group, it must satisfy associativity (i.e., $(a \diamond b) \diamond c = a \diamond (b \diamond c)$ for all $a,b,c \in                     G$), the existence of an identity element $e \in G$ with $a \diamond e = e \diamond a = a$ for all $a \in G$, and the existence of                 an inverse $a^{-1} \in G$ for every element $a \in G$ that satisfies $a^{-1} a = a a^{-1} = e$. A group is called abelian or                       commutative, if $a \diamond b = b \diamond a$ for all $a, b \in G$. Basic examples are the additive group $(\mathbb{R}, +)$  and                   the multiplicative group $(\mathbb{R}\setminus \{0\}, \cdot)$ and similarly, $(\mathbb{C}, +)$ and $(\mathbb{C}\setminus \{0\},                    \cdot)$, respectively, with addition and multiplication on the real (complex) numbers in the usual sense. More complicated group                  structures exist in \glspl{matrix} \cite{artin2016geometric}, such as rotation \glspl{matrix} useful in \gls{ml} for \gls{dataaug}.                 \\
...
{name={field},
	description={A\index{field} field $(\mathbb{F}, +, \cdot)$ is an algebraic structure \cite[Chap.~1]{artin2016geometric} generalizing the real          and complex numbers. 
            Formally, a fields consists of a set $\mathbb{F}$ equipped with two binary operations, an addition $\oplus$ and a multiplication $\odot$, 
            where $(\mathbb{F}, \oplus)$ is an abelian \gls{group} with an identity element $0$, and $(\mathbb{F} \setminus \{0\}, \odot)$ is an abelian \gls{group} with an identity element $1$. Further, it satisfies distributivity, i.e.,  $a \odot (b \oplus c) = (a \odot b) \oplus (a \odot c)$ for all $a, b, c \in \mathbb{F}$. The most common examples are $(\mathbb{R}, +, \cdot)$ and $(\mathbb{C}, +, \cdot)$, with addition and multiplication in the usual sense. Fields provide the set of scalars required to define a \gls{vectorspace}. \\
