...
{name={spectral decomposition},
	description={An \gls{evd} of a \gls{normalmatrix} is called a 
	             spectral decomposition\index{spectral decomposition}. 
...
{name={pseudocontractive operator},
	description={A\index{pseudocontractive operator} \gls{operator}
				...
				\noindent
				\textbf{Example.}
				Let $\hilbertspace=\mathbb{R}$ and define
				\[
				\fixedpointop(\weight)=\weight-\phi(\weight),
				\]
				where $\phi:\mathbb{R}\to\mathbb{R}$ is any nondecreasing function.
				...
				operator that is neither contractive nor non-expansive.
				\medskip
				\noindent
				\textbf{Alternative meaning \cite{Feyzmahdavian2023AsynchJMLR,ParallelDistrBook}.}
				In the literature on asynchronous and parallel optimization,
				an operator $\fixedpointop$ with a \gls{fixedpoint} $\weights^\star$ is called
				pseudocontractive if there exists $\contractfac\in(0,1)$ such that \cite{Feyzmahdavian2023AsynchJMLR,ParallelDistrBook}
				\begin{equation}
				...
				implies linear convergence of the corresponding \gls{fixedpointiter}
				$\weights^{(\iteridx+1)}=\fixedpointop\weights^{(\iteridx)}$ \cite[Proposition 1.2]{ParallelDistrBook}.
				\medskip
				\noindent
				\textbf{Example.}
				Consider the \gls{operator} $\fixedpointop:\mathbb{R}\to\mathbb{R}$ defined by
				\[
				\fixedpointop(\weight)=
				\begin{cases}
...
{name={spectrum},
 description={The spectrum\index{spectrum} of a \gls{linearoperator} $\fixedpointop$ 
			  ...
			  object for an which an associated \gls{linearoperator} can be defined. 
			  For example, the spectrum of a square \gls{matrix} $\mA \in \mathbb{R}^{\dimlocalmodel \times \dimlocalmodel}$ 
  			  ...
  			  As another example, we can define the spectrum of a \gls{stochproc} via a linear 
			  \gls{operator} constructed from its \gls{covariance} \gls{function} \cite{Rasmussen2006Gaussian}.},
 first={spectrum},
 type=math,
 plural={spectra},
 firstplural={spectra},
...
{name={Jacobian matrix},
 description={The Jacobian \gls{matrix}\index{Jacobian matrix} of a vector-valued
	\gls{function} 
	...
	\cite{RudinBookPrinciplesMatheAnalysis,HolderEichholz}. More explicitly, 
	\[
		D_{f}(\featurevec)
		\defeq
		\begin{bmatrix}
	...
	around a given argument vector $\featurevec$. Note that the $i$-th row of the Jacobian \gls{matrix} 
	coincides with the transpose of the \gls{gradient} of one component function 
...
{name={linear operator},
 description={A \index{linear operator} linear \gls{operator} is an 
			  ...
			  are \glspl{vectorspace} and which satisfies 
			\cite{DunfordSchwartz1988}
			\begin{equation}
				\nonumber
				\fixedpointop(\alpha \weights + \beta \weights')
				= \alpha \fixedpointop(\weights) + \beta \fixedpointop(\weights')
			\end{equation}
			for all $\weights,\weights'$ in the \gls{domain} and all scalars 
			$\alpha,\beta$.
							\\
...
{name={covariance function},
 description={A \gls{covariance} \gls{function}\index{covariance function} characterizes the
		...
		\quad \timeidx,\timeidx'\in\indexset. \nonumber
	\end{align}},
