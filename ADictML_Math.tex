...
{name={Hilbert space},
	description={A\index{Hilbert space} Hilbert space $\hilbertspace$ is a complete 
		...
		\caption{For two unit-\gls{norm} \glspl{vector} $\vu, \vv \in \sphere{\nrfeatures} \subseteq \mathbb{R}^{\nrfeatures}$, 
			the \gls{innerproduct} $\innerprod{\vu}{\vv}$ is the expansion coefficient 
			for the \gls{projection} of $\vv$ onto the \gls{subspace} $\{ c \vu: c \in \reals \}$ 
			spanned by $\vu$. The absolute value $|\innerprod{\vu}{\vv}|$ measures the \gls{norm} 
			of this \gls{projection}.\label{fig_hilbertspace_dict}}
		\end{figure}
...
{name={estimator},
	   description={\Glspl{mlsystem} are\index{estimator} often analyzed by using a 
					...
					of the true value of the \glspl{modelparam} $\weights$ \cite{LC,kay}.},
	first={estimator},
	type=math,
	text={estimator},
	plural={estimators},
...
{name={unbiased estimator},
	   description={An \gls{estimator}\index{unbiased estimator} for the \glspl{parameter} of a \gls{probmodel} 
	    			is unbiased if its \gls{expectation} equals the true value of the \glspl{parameter} \cite{LC,kay}.},
...
{name={reproducing kernel Hilbert space (RKHS)},
  description={A\index{reproducing kernel Hilbert space (RKHS)} RKHS $\hilbertspace$ is a  
               special type of \gls{hilbertspace} that consists of 
 			   \glspl{function} defined on a set $\featurespace$ \cite{aronszajn1950}. 
 			   The defining property of a RKHS is that point evaluations are continuous 
 			   ...
 			   \[
 				...
 				\gls{regression} \cite{LearningKernelsBook,Murphy2012}. Another application of 
				RKHSs if for the analysis of minimum \gls{variance} estimation \cite{RKHSExpFamIT2012}.}, 
...
{name={posterior (prediction)}, 
	description={The\index{posterior} study and design of \gls{ml} methods is often based on a 
				...
				of $\truelabel$ given (or conditioned on) $\featurevec$ \cite{BishopBook,LC,papoulis}.
		\\
...
{name={prior distribution},
	description={In the probabilistic\index{prior distribution} analysis of an \gls{mlsystem}, one typically distinguishes 
	             between observed \glspl{datapoint} and unknown \gls{parameter} of a \gls{probmodel}. 
				 A prior distribution is a postulated \gls{probdist} over the possible choices for 
				 these \glspl{modelparam} before (prior to) observing any \gls{data} \cite{BishopBook}. 
				 Given a conditional \gls{probdist} of the \gls{data}, given the \gls{modelparam}, 
				 we use update the prior to obtain the \gls{posteriordist} of the \gls{modelparam}.},
...
{name={posterior distribution},
	description={In the probabilistic\index{prior distribution} analysis of an \gls{mlsystem}, 
				 one typically distinguishes between observed \glspl{datapoint} and unknown 
				 \gls{parameter} of a \gls{probmodel}. The posterior distribution\index{posterior distribution} 
	             is the conditional \gls{probdist} of the unknown \gls{modelparam}, 
				 given (or conditioned on) the observed \glspl{datapoint} \cite{BishopBook}.\\ 
				 See also: \gls{priordist}, \gls{condprobdist}, \gls{probdist}.},
...
\newglossaryentry{conjugateprior}
 {name={conjugate prior},
	...
	likelihood model) $\probdist^{(\featurevec\mid\weights)}$ if the resulting 
 	\gls{posteriordist} $\probdist^{(\weights \mid\featurevec)}$ belongs to 
 	the same parametric family as $\probdist^{(\weights)}$ \cite{barber2012bayesian,BishopBook}. 
 	One important example for a conjugate prior and likelihood model is 
 	...
 	likelihood model $\probdist^{(\featurevec\mid\weights)}$ that is a \gls{mvndist} 
 	with mean $\weights$ and covariance $\covmtxgeneric$, the resulting posterior 
 	$\probdist^{(\weights \mid\featurevec)}$ is also a \gls{mvndist} \cite[Appendix B]{BishopBook}\\
 	See also: \gls{probdist}, \gls{condprobdist}, \gls{mvndist}.},
...
{name={exponential family},
 description={An \index{exponential family}exponential family is a particular \gls{probmodel} 
			  ...
			  a vector of sufficient statistics $\mathbf{T}(\featurevec)$. 
			  ...
			  \[ p(\featurevec ;  \weights) = h(\featurevec)\,\exp\!\big(\weights^{T} \mT(\featurevec) - A(\weights)\big),
  			  \]
  			  where $\weights$ denotes the natural parameter and $A(\weights)$ is the log-partition 
       		  function ensuring normalization $\int_{\featurevec} p(\featurevec ;  \weights) d\featurevec = 1$.
  			  Exponential family distributions arise as maximum-entropy solutions under constraints 
			  on the expected values of the sufficient statistics. Thus, we can view them 
			  as the least informative \gls{probdist} for given \gls{expectation} of the sufficient 
