...
{name={alternating direction method of multipliers (ADMM)},
 description={The \index{alternating direction method of multipliers (ADMM)} 
			  ...
			  + g(\featurevec'), \mbox{ subject to } \mathbf{A} \featurevec - \mathbf{B}\featurevec' = \vc.
			  \]
			 }, 
	...
	description={Consider a constrained \gls{optproblem} of the form
				 \begin{align} 
				  ...
				  Here, $f_0: \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$ is the 
					 \gls{objfunc}, $f_i: \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$, 
					$i=1,\,\ldots,\,k$ are the inequality constraint \glspl{function}, 
					and $g_j: \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$, 
					$j=1,\,\ldots,\,l$ are the equality constraint \glspl{function}.		
				  The Lagrangian\index{Lagrangian} of the above \gls{optproblem} of %standard
				  the form is defined as \cite{BoydConvexBook}
				 \begin{equation}
					\nonumber
				...
				\glspl{optproblem} and the design of \glspl{optmethod}.  	
		\\
...
{name={Karush--Kuhn--Tucker (KKT) conditions},
 description={Consider a constrained \gls{optproblem} with \gls{lagrangian}
			  ...
			  + \sum_{j=1}^{l} \nu_j g_j(\vx),
			  \]
			  with \gls{differentiable} \glspl{function} $f_0, f_1, \ldots, f_k, g_1, \ldots, g_l$.	
			  ...
			  && \text{(stationarity)} \nonumber\\
			  f_i(\vx) &\le 0, \quad i=1,\ldots,k
			  && \text{(primal feasibility)} \nonumber\\
			  g_j(\vx) &= 0, \quad j=1,\ldots,l
			  && \text{(primal feasibility)} \nonumber\\
			  \lambda_i &\ge 0, \quad i=1,\ldots,k
			  && \text{(dual feasibility)} \nonumber\\
			  \lambda_i f_i(\vx) &= 0, \quad i=1,\ldots,k
			  && \text{(complementary slackness).} \nonumber
			  \end{align}
			  If the primal problem results in a zero \gls{dualitygap}, 
			  any solution $(\vx^\star,{\bm \lambda}^\star,{\bm \nu}^\star)$ 
			  must satisfy the KKT conditions. Conversely, if the primal problem 
			  is \gls{convex} and some regularity conditions hold, any solution 
			  $(\vx^\star,{\bm \lambda}^\star,{\bm \nu}^\star)$ 
			  that satisfies the KKT conditions is also optimal \cite{BoydConvexBook}.
			  The KKT conditions can be written as a \gls{fixedpointeq} 
			  and therfore be solved via \glspl{fixedpointiter}.},
...
{name={dual problem},
 description={Consider a constrained \gls{optproblem}, which we 
              ...
              \]
     		  The \gls{lagrangian} associated with this problem can be used to 
     		  construct a lower bound on the optimal value of the primal problem. 
      		  This\index{dual problem} lower bound is captured by the \gls{function}
     		 \[q({\bm \lambda},{\bm \nu})=\inf_{\vx} L(\vx,{\bm \lambda},{\bm \nu}),
      			 \]
     		  where $L(\vx,{\bm \lambda},{\bm \nu})$ denotes the \gls{lagrangian}. 
  	...
  	description={The\index{singular value decomposition (SVD)} SVD  
		...
		\begin{figure}[htbp]
		 ...
		 \begin{tikzpicture}[>=latex, line cap=round, line join=round, scale=1]
			...
			\begin{scope}[shift={(\xR,0)},rotate=30]
			\draw[thick,->] (0,0) -- (\sone*\len,0) node[midway,below] {$\lambda_1 \vv^{(1)}$};
			\draw[thick,->] (0,0) -- (0,\stwo*\len) node[midway,left] {$\lambda_2 \vv^{(2)}$};
			\end{scope}
		 	\end{tikzpicture}
		 \end{figure}
