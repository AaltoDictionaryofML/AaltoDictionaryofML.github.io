...
{name={likelihood function},
	description={Let $\{p(x \mid w ) : w \in \mathcal{W}\}$ be a probabilistic model, and let 
	$D = \{x_1, \dots, x_N\}$ denote an observed \gls{dataset}.
	The \textbf{likelihood function} is the mapping
	\[
	L : \mathcal{W} \to \mathbb{R}_{\ge 0}, 
	\qquad 
	L(w) := p(D \mid w),
	\]
	that is, the joint probability of the observed data, 
	viewed as a function of the parameter vector $w$ with the \gls{data} held fixed.
	If the observations are assumed \gls{iid}, then
	\[
	L(w) = \prod_{i=1}^{N} p(x_i \mid w).
	\]
	The \textbf{log-likelihood} is defined as
	\[
	\ell(w) := \log L(w),
	\]
	and is commonly maximized in \gls{maxlikelihood} estimation in \gls{ml} contexts.
	The likelihood function is not a \gls{probdist} over 
	$w$; its a \gls{function} on the parameter space 
	that measures how well different values of w explain the observed \gls{data} $D$.
	The importance of the likelihood function in \gls{ml} extends to Bayes' theorem,
	\[
	p(w \mid D) = \frac{p(D \mid w) \, p(w)}{p(D)},
	\]
	the term $p(D \mid w)$ is likelihood function, which 
	updates the \gls{priordist} $p(w)$ about the parameters into the \gls{posteriordist} $p(w \mid D)$ \cite{barber2012bayesian} \cite{BishopBook}.
		\\ 	
		See also: \gls{probdist}, \gls{priordist}, \gls{posteriordist}, \gls{data}, \gls{maxlikelihood} },
