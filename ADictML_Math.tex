...
{name={rank-deficient},
	description={A \gls{matrix} $\mA \in \mathbb{R}^{\samplesize \times \nrfeatures}$ 
		...
		\end{figure} 
		A \gls{rank}-deficient \gls{matrix} has linearly dependent columns,
		meaning that some columns can be expressed as linear combinations of others.
		As a consequence, the linear system $\mA \vw = \vy$ either has no solution
		...
		and the \gls{normalequations} have infinitely many solutions:
...
{name={duality gap},
	description={Consider a \gls{constrainedoptprob} with \gls{lagrangian}
		...
		\\
		See also: \gls{optproblem}, \gls{dualproblem}. },
	first={duality gap},
	text={duality gap},
	type=math
}
...
{name={determinant},
	description={The\index{determinant} determinant $\det\,(\mA)$ of a square \gls{matrix} 
		...
		$\det\,(\mA) = \prod_{\featureidx=1}^{\nrfeatures} \eigval{\featureidx}$ \cite{HornMatAnalysis}.
 		For the special cases $\nrfeatures=2$ (i.e., in two-dimensional space $\mathbb{R}^{2}$) and
		$\nrfeatures=3$ (i.e., in three-dimensional space $\mathbb{R}^{3}$),
