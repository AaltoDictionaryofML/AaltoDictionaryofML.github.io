...
{name={positive definite (pd)},
	description={A\index{positive definite (pd)} (real-valued)
		\gls{symmetricmatrix} $\mQ = \mQ\,^{T} \in \mathbb{R}^{\featuredim \times \featuredim}$
		is referred to as positive definite (pd) if $\featurevec\,^{T} \mQ \featurevec > 0$
		...
		\\
		See also: \gls{psd}, \gls{symmetricmatrix}, \gls{invertible}.},
...
{name = {invertible (matrix)}, 
 description={A square \gls{matrix} $\mA \in \mathbb{R}^{n \times n}$ is 
			  ...
			  identity \gls{matrix} \cite{Strang2007,StrangLinAlg2016,Axler2025}. 
...
{name={inverse matrix},
	description={An inverse \gls{matrix}\index{inverse matrix} $\mA^{-1}$ is defined for a 
 		...
 		columns are \gls{linearlyindep}. In this case, $\mA$ is said to be invertible, 
...
{name={covariance function},
	description={A \gls{covariance} \gls{function}\index{covariance function} characterizes the
		...
		finite second moments, the \gls{covariance} \gls{function} is 
...
sort={posteriordistribution},
	description={In the probabilistic\index{posterior distribution} analysis of a \gls{mlsystem}, 
