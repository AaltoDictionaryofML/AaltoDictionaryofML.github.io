...
{name={proximal operator},
	description={Given\index{proximal operator} a \gls{convex} and \gls{continuous}
		...
		\bigg[ f(\weights')\!+\!\frac{1}{2\lrate} \normgeneric{\weights- \weights'}{2}^{2}\bigg] \quad \mbox{with } \lrate > 0. $$ 
		The factor $\lrate$ controls the relative importance of the \gls{function} $f$ 
		and the \gls{penaltyterm} in the optimization problem defining the proximal 
		...
		scaled squared \gls{eucliddist} to a given \gls{vector} $\weights$ (which is the input to 
		the proximal \gls{operator}). The proximal \gls{operator} can be interpreted as a \gls{generalization} 
		of the \gls{gradstep}, which is defined for a \gls{smooth} \gls{convex} \gls{function} $f(\weights')$. 
		Indeed, taking a \gls{gradstep} with \gls{stepsize} $\lrate$ at the current \gls{vector} $\weights$ 
		is the same as applying the (scaled, by $\lrate$) proximal \gls{operator} of the \gls{function} $\tilde{f}(\weights')= \big( \nabla f(\weights)\big)\,^{T} (\weights'-\weights)$. 
			\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{The proximal \gls{operator} updates a \gls{vector} $\weights$ by minimizing 
			         a penalized version of the \gls{function} $f$. The \gls{penaltyterm} 
					 is the scaled squared \gls{eucliddist} between the \gls{optimization} 
...
{name={likelihood function},
 description={The\index{likelihood function} likelihood function of a \gls{probmodel} 
              with \glspl{parameter} $\weights$ is obtained by evaluating the 
			  \gls{pdf} (or \gls{pmf}) for the observed \gls{dataset} \cite{CasellaBergerStatInf,LC}. 
			  ...
			  The common \gls{pdf} is parametrized by the \glspl{modelparam} $\weights$. 
			  ...
			  The likelihood function can be used to construct a \gls{lossfunc} 
			  for \gls{erm}, which results in \gls{maxlikelihood}. \\
	See also: \gls{probmodel}, \gls{maxlikelihood}, \gls{lossfunc}.},
...
{name={non-expansive operator},
 	description={An\index{non-expansive operator} \gls{operator} 
		...
		not increase distances. In other words,
 		\begin{equation} 
 			\nonumber
...
{name={Euclidean distance}, 
	description={The\index{Euclidean distance} Euclidean distance between two 
...
{name={metric},
	description={A\index{metric} metric is a quantitative \gls{measure} used to compare objects. 
		...
		\\ 
		See also: \gls{acc}, \gls{precision}, \gls{validation}, \gls{loss}.},
	first={metric}, 
	type=math,
	plural={metrics},
	text={metric}
...
{name={strongly convex},
	description={A \gls{function}\index{strongly convex} $f:\mathbb{R}^d \rightarrow \mathbb{R}\cup\{+\infty\}$ is 
				 $\mu$-strongly \gls{convex} (with parameter $\mu>0$) if \cite{Bauschke:2017}
				\[
					...
					linear convergence rates for \glspl{gdmethod}.
					\\ 
...
{name={strictly convex},
	description={A\index{strictly convex} real-valued \gls{function} $f: \mathcal{U} \to \mathbb{R}$ 
		...
		\[
			...
			< \lambda f(\vx)+(1-\lambda)f(\vy).
		\]
            Furthermore, if $f$ is \gls{differentiable}, this condition is 
            equivalent to
		\[
			f(\vy) > f(\vx) + \nabla f(\vx)^{T}(\vy-\vx)
		\]
		for any $\vx\neq\vy$, which implies that $f(\cdot)$ admits a unique minimizer on     any \gls{convex} subset of its \gls{domain}. Unlike \gls{strcvx}                    \glspl{function}, strictly \gls{convex} \glspl{function} do not require a uniform 
