...
{name={indicator function}, 
	description={Consider some process\index{indicator function} that can result in different possible 
		...
		The indicator \gls{function} $\indicatorfunc{\cluster}$ of an event 
		$\cluster \subseteq \samplespace$ is a \gls{function} defined as \cite{BertsekasProb} 
		\[
			\indicatorfunc{\cluster}(\outcome) = \begin{cases}
			1, & \text{if } \outcome \in \cluster \\
			0, & \text{if } \outcome \notin \cluster.
			\end{cases}
		\]
		The notion of an indicator \gls{function} is not limited to \glspl{outcome} of 
		a \gls{randomexperiment}. Utlimately it is just a principled way to 
		represent a set $\cluster$ by a \glspl{function} $\indicatorfunc{\cluster}$ \cite{BoydConvexBook}.
		\\
...
{name={projection}, 
       	description={Consider\index{projection} a subset 
...
{name={unitary matrix},
	description={A square \gls{matrix} $\mU \in \mathbb{C}^{\nrfeatures \times \nrfeatures}$ 
		...
		(or \gls{hermitian} \gls{transpose}) $\mU^{H}$ is also its inverse, i.e., if 
 		\[
 			\mU^{H} \mU = \mU \mU^{H} = \mI.
 		\]
...
{name={inner product},
	description={Consider a \gls{vectorspace} $\featurespace$ over the 
		...
		for all $\featurevec, \featurevec' \in \featurespace$.
 		\\
 		See also: \gls{norm}, \gls{vector}, \gls{metricspace}.},
 	first={inner product},
 	type=math,
 	text={inner product}
}
...
{name={boundary},
	description={Consider\index{boundary} a subset $\mathcal{C} \subseteq \mathbb{R}^n$. 
		...
		set of all points $\vx \in \mathbb{R}^n$, such that every open ball 
                 $\openball{\vx}{\varepsilon} = \{\vy \in \mathbb{R}^n: \norm{\vy - \vx} < \varepsilon\}$ 
		...
		\begin{figure}[H]
			...
			\node (ballLabel) at (2.8, 2.2) {$\openball{\vx}{\varepsilon}$};
			...
			\end{tikzpicture}
		\caption{Boundary of set $\mathcal{C}$ and an open ball $\openball{\vx}{\varepsilon}$ around 
			$\vx \in \partial \mathcal{C}$.}
		\end{figure}
		Note that the boundary $\partial \mathcal{C}$ 
		might contain elements that do not belong to $\cluster$ itself. 
...
{name={strictly convex},
	description={A\index{strictly convex} real-valued \gls{function} $f(\featurevec)$ 
		...
		$\vx,\vy$ in its \gls{domain} and any $\lambda\in(0,1)$, it satisfies \cite[Definition 3.1.1]{BoydConvexBook}
		\[
