...
{name={positive semi-definite (psd)},
	description={A\index{positive semi-definite (psd)} (real-valued) 
		...
		A psd \gls{matrix} $\mQ$ admits a \gls{spectraldecomp} with nonnegative 
		...
		\\
		The psd property induces a partial order on \glspl{symmetricmatrix}:
		We write $\mA \succeq \mB$ (for symmetric $\mA, \mB \in \mathbb{R}^{\featuredim \times \featuredim}$)
		if the difference $\mA - \mB$ is psd \cite[Sec. 2.4]{BoydConvexBook}. In particular, $\mA \succeq \mathbf{0}$ means
		that $\mA$ is psd.
			\\
		See also: \gls{symmetricmatrix}, \gls{kernel}, \gls{pd}.},
...
{name={normal matrix},
	description={A normal \gls{matrix} is a square \gls{matrix} $\mA \in \mathbb{C}^{\nrfeatures \times \nrfeatures}$ 
		...
		Normal \glspl{matrix}\index{normal matrix} admit an orthonormal \gls{basis} of \glspl{eigenvector} and are 
...
{name={spectral decomposition},
	description={An \gls{evd} of a \gls{normalmatrix} is called a spectral decomposition\index{spectral decomposition}. 
		It has the special property that the \glspl{eigenvector} of the \gls{normalmatrix} are orthonormal. 
		This also works the other way, i.e., any \gls{matrix} that has orthonormal \glspl{eigenvector} is a \gls{normalmatrix}.
		\\
		See also: \gls{evd}, \gls{normalmatrix}, \gls{eigenvector}. },
...
{name={linearly independent},
	description={A subset $\{\va^{(1)}, \,\ldots, \,\va^{(\nrfeatures)}\} \in \mathcal{V}$ 
		...
		\quad \text{implies} \quad \alpha_{1} = \alpha_{2} = \ldots = \alpha_{\nrfeatures} = 0.$$ 
			\\ 
		See also: \gls{vectorspace}, \gls{vector}, \gls{dimension}, \gls{basis}, \gls{linearlydep}.}, 
...
{name={basis},
	description={A basis\index{basis} of a finite-dimensional \gls{vectorspace} $\mathcal{V}$ 
		is a set of \gls{linearlyindep} \glspl{vector} $\mathcal{B} = \{\vb^{(1)}, \,\ldots, \,\vb^{(\nrfeatures)}\} 
                 \subseteq \mathcal{V}$ such that any \gls{vector} $\vv \in \mathcal{V}$ 
		can be expressed as a linear combination of the basis \glspl{vector} $\vb^{(1)}, \,\ldots, \,\vb^{(\nrfeatures)}$, i.e.,	
		$$ \vv = \sum_{\featureidx=1}^{\nrfeatures} \alpha_{\featureidx} \vb^{(\featureidx)}
			\quad \text{for some } \alpha_{1}, \,\ldots, \,\alpha_{\nrfeatures}. $$
                 The  scalars $\alpha_{1}, \,\ldots, \,\alpha_{\nrfeatures} $ can be regarded 
		as the coordinates of $\vv$ with respect to the basis $\mathcal{B}$. 
		Any basis of $\mathcal{V}$ has the same number $\nrfeatures$ of elements, 
		which is the \gls{dimension} of $\mathcal{V}$.
			\\
...
{name={wide matrix},
	description={A\index{wide matrix} \gls{matrix} 
		...
		i.e., when $\nrfeatures > \samplesize$ (see Fig.\ \ref{fig_widematrix_dict}).
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				\def\matHeight{2}
			...
			\end{tikzpicture}
		\caption{Illustration of a wide \gls{matrix}. \label{fig_widematrix_dict}}
		\end{figure}
		See also: \gls{matrix}, \gls{tallmatrix}, \gls{johnsonlindenstrausslemma}, \gls{randomprojection}. },
	text={wide matrix}, 
...
{name={tall matrix},
	description={A\index{tall matrix} \gls{matrix} $\featuremtx \in \mathbb{R}^{\samplesize \times \nrfeatures}$ 
		...
		when $\samplesize > \nrfeatures$ (see Fig.\ \ref{fig_tallmatrix_dict}).
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}
				\def\matHeight{5}
				...
				\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick] 
        				...
        				node[midway, below=8pt] {$\nrfeatures$};
			\end{tikzpicture}
		\caption{Illustration of a tall \gls{matrix}. \label{fig_tallmatrix_dict}}
		\end{figure}
		See also: \gls{matrix}, \gls{widematrix}. },
	text={tall matrix}, 
...
{name={random experiment},
	description={A random experiment\index{random experiment} is a physical (or abstract) process 
	 	...
	 	uncertain, the long-run behavior of the \glspl{outcome} of repeated experiments 
		...
		\begin{center}
			...
			% Dotted rounded box enclosing experiment and part of repeat arrow
	 		...
	 		\end{tikzpicture}
	     	\end{center}
		\caption{Random experiment producing an \gls{outcome} $\outcome \in \samplespace$ from a set 
	 	...
	 	\begin{itemize} 
			...
			\item \Gls{stochGD} uses a random experiment at each \gls{iteration} to select a subset of 
			the \gls{trainset}. 
...
{name={moment generating function (MGF)}, 
	description={Consider the\index{moment generating function (MGF)} MGF $\mgf{x}{t}$ 
		...
		As its name indicates, the MGF allows us to compute the moments 
	 	...
	 	\gls{derivative} of $\mgf{x}{t}$ for $t=0$, i.e., $\expect\{ x^{k} \} = \mgfder{x}{k}{0}$. 
	 	...
	 	\begin{figure}[H]
			...
			\end{axis}
			\end{tikzpicture}
		\caption{First few powers of an \gls{rv} $x$. The MGF 
...
{name={rank-deficient},
	description={A \gls{matrix} $\mA \in \mathbb{R}^{\samplesize \times \nrfeatures}$ 
         	is \gls{rank}-deficient\index{rank-deficient} if it is not 
		\gls{fullrank}, i.e., when $\rank{\mA} < \min\{\samplesize,\nrfeatures\}$.
 		\begin{figure}[H]
			\begin{center}
		...
		\end{figure} 
		A \gls{rank}-deficient \gls{matrix} has \gls{linearlydep} columns,
		meaning that some columns can be expressed as linear combinations of others.
		As a consequence, the linear system $\mA \vw = \vy$ has either no solution
		or infinitely many solutions, since different choices of $\vw$ can yield
		the same product $\mA \vw$.
		In \gls{linreg}, the \glspl{modelparam} are found by solving the
		\gls{normalequations} $\featuremtx^{\top}\featuremtx \weights = \featuremtx^{\top} \vy$.
		If the \gls{featuremtx} $\featuremtx$ is \gls{rank}-deficient
		(e.g., because two \glspl{feature} are perfectly correlated),
		then $\featuremtx^{\top}\featuremtx$ is also \gls{rank}-deficient
		and the \gls{normalequations} have infinitely many solutions, i.e.,
		multiple \glspl{modelparam} $\weights$ fit the \gls{trainset} equally well.
		\\
...
{name={full-rank},
 	description={A \gls{matrix} $\mA \in \mathbb{R}^{\samplesize \times \nrfeatures}$ 
		...
		\end{figure} 
  		A square \gls{matrix} is full-\gls{rank} if and only if it is \gls{invertible}. 
		\\ 
...
{name={rank},
	description={The rank\index{rank} of a \gls{matrix} $\mA \in \mathbb{R}^{\samplesize \times \nrfeatures}$, 
 		denoted by $\rank{\mA}$, is the \gls{maximum} number of \gls{linearlyindep} columns 
...
{name={invertible (matrix)}, 
	description={A square \gls{matrix} $\mA \in \mathbb{R}^{n \times n}$ is 
		called invertible\index{invertible} if there exists a 
		\gls{matrix} $\mB \in \mathbb{R}^{n \times n}$ such that 
		$\mA \mB = \mB \mA = \mI$, where $\mI$ is the $n \times n$ 
		identity \gls{matrix} \cite{Strang2007}, \cite{Axler2025}, \cite{StrangLinAlg2016}. 
		In this case, $\mB$ is called the \gls{inverse} of $\mA$ and is denoted by $\mA^{-1}$.
		\\
		See also: \gls{matrix}, \gls{inverse}. },
	first={invertible},
	type=math,
	text={invertible}
}
...
{name={inverse matrix},
	description={An inverse \gls{matrix}\index{inverse matrix} $\mA^{-1}$ is defined for a 
 		...
 		columns are \gls{linearlyindep}. In this case, $\mA$ is said to be \gls{invertible}, 
 		and its inverse satisfies
 		\[
 		\mA \mA^{-1} = \mA^{-1} \mA = \mI.
 		\]  	
     		A square \gls{matrix} is \gls{invertible} if and only if its \gls{det} is nonzero. Inverse \glspl{matrix} are 
 		 ...
 		 \begin{figure}[H]
 			...
 			\end{tikzpicture}
 		\caption{\Gls{matrix} $\mathbf{A}$ representing a linear transformation of $\mathbb{R}^{2}$; inverse \gls{matrix} $\mathbf{A}^{-1}$ 
 			representing the inverse transformation. \label{fig_matrix_inverse_dict}} 
 		\end{figure}
...
{name={matrix},
	description={A matrix\index{matrix} of size $\samplesize \times \nrfeatures$ is a 2-D array of numbers, 
		...
		\end{tikzpicture}
		\end{center}
		\caption{Matrix $\mA$ defining a \gls{linearmap} between two \glspl{vectorspace}. \label{fig_matrix_dict}} 
		\end{figure}
...
{name={hyperplane},
	description={A hyperplane\index{hyperplane} is a $(\nrfeatures-1)$-dimensional affine 
		...
		hyperplane is a set of the following form:
 		\[ 
		...
		\\
		See also: \gls{subspace}, \gls{vectorspace}, \gls{euclidspace}, \gls{decisionboundary}, \gls{innerproduct}. },
	first={hyperplane},
	type=math,
	plural={hyperplanes}, 
...
{name={subspace},
	description={A subset of a \gls{vectorspace} $\mathcal{V}$ is a subspace\index{subspace} of $\mathcal{V}$ if it is also a 
		...
		Fig.\ \ref{fig:subspace} illustrates this for the 
		\gls{vectorspace} $\mathbb{R}^2$ (over the \gls{field} $\mathbb{R}$).
		\begin{figure}[H] 
			\centering
			...
			\end{tikzpicture}
		\caption{Any $\featurevec \in \mathbb{R}^2$ defines a subspace $\mathcal{U}_\featurevec \subset \mathbb{R}^2$ 
			by $\mathcal{U}_\featurevec = \{ \lambda \cdot \featurevec \; : \; \lambda \in \mathbb{R}\} \subset \mathbb{R}^2$. 
			For $\featurevec \neq \mathbf{0}$, this corresponds to straight lines through the origin (illustrated by $\mathcal{U}_1$ 
			and $\mathcal{U}_2$). For the zero \gls{vector} $\featurevec =\mathbf{0}$, this leads to the trivial subspace $\{\mathbf{0}\}$ 
			just containing the zero \gls{vector}. \label{fig:subspace}}
		\end{figure}
		See also: \gls{vectorspace}, \gls{dimension}, \gls{basis}.},
	type=math, 
	plural={subspaces},
	first={subspace},
	text={subspace}
...
{name={multivariate normal distribution}, 
	description={The\index{multivariate normal distribution} multivariate \gls{normaldist}, 
		...
		\gls{covmtx} $\covmtxgeneric$ is \gls{invertible}, the corresponding \gls{probdist} is 
		...
		\[p(\featurevec) = 
 		...
 		\]
		Note that this \gls{pdf} is only defined when $\covmtxgeneric$ is \gls{invertible}.
  		...
  		\[
   		...
   		The family of multivariate \glspl{normaldist} is exceptional among \glspl{probmodel} for numerical 
...
{name={standard normal random vector}, 
	description={A\index{standard normal random vector} standard normal random \gls{vector} 
		...
		\\ 
		See also: \gls{vector}, \gls{rv}, \gls{iid}, \gls{gaussrv}, \gls{mvndist}.}, 
...
{name={continuous}, 
	description={A \gls{function}\index{continuous} $f: \mathbb{R}^{\nrfeatures} \to \mathbb{R}$ is 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{\Gls{function} $f(\feature) = 0.3(\feature-2)^3 + 2.5$, continuous 
		         at every $\feature'$.}
		\end{figure}
...
{name={co-domain}, 
	description={The co-\gls{domain}\index{co-domain} of a \gls{function} 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{\Gls{domain} and co-\gls{domain} of a \gls{function} $f: \mathcal{U} \rightarrow \mathcal{V}$. }
		\end{figure}
	...
	description={A weighted \gls{graph} is a \gls{graph}\index{weighted graph} whose edges 
		...
		\begin{figure}[H] 
			...
			\begin{tikzpicture}[scale=1.5,
				...
				\draw (v2) -- node[midway, right] {$\edgeweight_{\nodeidx_2,\nodeidx_4}$} (v4);
			\end{tikzpicture}
		\caption{Weighted \gls{graph} with four nodes 
			$\nodes = \{\nodeidx_1, \nodeidx_2, \nodeidx_3, \nodeidx_4\}$ 
	...
	description={A graph\index{graph} $\graph = \pair{\nodes}{\edges}$ 
		...
		\begin{figure}[H] 
			...
			\begin{tikzpicture}[scale=1,
				...
				\draw (v2) -- (v4);
			\end{tikzpicture}
		\caption{Simple \gls{undirectedgraph} with four nodes 
			$\nodes = \{\nodeidx_1, \nodeidx_2, \nodeidx_3, \nodeidx_4\}$ 
			and four edges $\edges = \{\{\nodeidx_1,\nodeidx_2\},
			\{\nodeidx_2,\nodeidx_3\}, \{\nodeidx_3,\nodeidx_1\}, 
			\{\nodeidx_2,\nodeidx_4\}\}$.}
		...
		referred to as \gls{edgeweight}, to each edge $\edgeidx \in \edges$.
					\\ 
		See also: \gls{directedgraph}, \gls{undirectedgraph}, \gls{weightedgraph}, \gls{edgeweight}.},
 	first={graph},
 	text={graph}, 
 	firstplural={graphs}, 
	plural={graphs}, 
...
{name={Markov chain},
	description={A Markov chain\index{Markov chain} is a \gls{stochproc} $\{X_\timeidx \}_{\timeidx \in \mathbb{N}}$ 
		...
		\gls{rv} $X_\timeidx$ might represent (the generation of) a \gls{state} of a physical system 
		...
		For all $\timeidx \in \mathbb{N}$,
 		...
 		\end{equation}
 		In other words, the \gls{condprobdist} of the next \gls{state} $X_{\timeidx+1}$ 
		depends on the past $X_{\timeidx},\,X_{\timeidx-1},\,\dots,\,X_{1}$ 
		only through the current \gls{state} $X_{\timeidx}$. The concept of a 
		...
		\\ 
		See also: \gls{stochproc}, \gls{condprobdist}, \gls{mdp}.},
	first={Markov chain},
	type=math,
    	text={Markov chain}, 
...
{name={expectation–maximization (EM)}, 
	description={\index{expectation–maximization (EM)}
		...
		with a \gls{pmf} $\pmf{\feature'}{\cdot;\weights}$. Here, 
		...
		used approach for estimating these \glspl{modelparam} is via the solutions of the following 
		\gls{maxlikelihood} problem:
		\begin{equation}
		...
		yields the following, much easier \gls{maxlikelihood} problem:
		\begin{equation}
		...
		\gls{feature} $\feature$ is available. Thus, we cannot solve \eqref{equ_def_complete_EM_dict} 
		...
		in the form of the \gls{posteriordist} $\pmf{\truelabel'|\feature'}{\cdot;\widehat{\weights}}$
		using the current choice $\widehat{\weights}$ for the \glspl{modelparam}; and 
		2) an M-step in which a surrogate \gls{objfunc} derived from this \gls{posteriordist} is minimized.
		The completion of these two steps constitutes one full \gls{iteration} of 
		the EM method. 
		In more detail, the E-step produces the following \gls{function}:
		\[
...
{name={contractive operator},
	description={An\index{contractive operator} \gls{operator} 
		$\fixedpointop: \mathcal{V} \rightarrow  \mathcal{V}$ on a \gls{normedspace} 
		$(\mathcal{V}, \normgeneric{\,\cdot \,}{})$
		...
		\begin{equation} 
			...
			\normgeneric{ \fixedpointop \weights\!-\!\fixedpointop \weights'}{}  \leq 
			 \contractfac	\normgeneric{\weights\!-\!\weights'}{} \quad \mbox{holds for any } \weights,\weights' \in \mathcal{V}.
		\end{equation} 
		The notion of a contractive \gls{operator} generalizes naturally from \glspl{normedspace} 
		...
		\begin{figure}[H]
    			...
    			\end{tikzpicture}
    		\caption{Contractive \gls{operator} $\fixedpointop:\featurespace\to\featurespace$ 
      			with a unique \gls{fixedpoint} $\featurevec^\star$ satisfying $\fixedpointop \featurevec^\star=\featurevec^\star$.
      			For any two points $\featurevec,\featurevec'$ in the same space, the \gls{distance} between their images
		...
		\\ 
		See also: \gls{operator}, \gls{metricspace}, \gls{banachfixedpoint}, \gls{bellmanoperator}. },
...
{name={pseudocontractive operator},
	description={An\index{pseudocontractive operator} \gls{operator}
		$\fixedpointop:\hilbertspace\rightarrow\hilbertspace$ on a \gls{hilbertspace}
		$\hilbertspace$ is called pseudocontractive if \cite{BausckeCombette}
		\begin{equation}
			\nonumber
			...
			+
			\normgeneric{(\identityop-\fixedpointop)\weights
			-(\identityop-\fixedpointop)\weights'}{}^{2}
		\end{equation}
		holds for all $\weights,\weights'\in\hilbertspace$, where
		$\identityop$ denotes the identity \gls{operator}.
		This condition is equivalent to the monotonicity of the \gls{operator}
		$\identityop-\fixedpointop$ \cite[Example 20.8]{BausckeCombette}.
		Pseudocontractive \glspl{operator} include \glspl{nonexpansiveop} and \glspl{contractop} as 
		special cases. 
		\medskip
		\noindent \\
		Example: Let $\hilbertspace=\mathbb{R}$ and define
		\[
		\fixedpointop(\weight)=\weight-\phi(\weight),
		\]
		where $\phi:\mathbb{R}\to\mathbb{R}$ is any nondecreasing \gls{function}.
		Then, $\identityop-\fixedpointop=\phi$ is monotone, and hence
		$\fixedpointop$ is pseudocontractive \cite{BausckeCombette}.
		For instance, choosing $\phi(\weight)=\max\{0,\weight\}$ yields a pseudocontractive
		\gls{operator} that is neither contractive nor non-expansive.
		\medskip
		\noindent \\
		Alternative meaning \cite{Feyzmahdavian2023AsynchJMLR}, \cite{ParallelDistrBook}:
		In the literature on asynchronous and parallel \gls{optimization},
		an \gls{operator} $\fixedpointop$ with a \gls{fixedpoint} $\weights^\star$ is called
		pseudocontractive if there exists $\contractfac\in(0,1)$ such that \cite{Feyzmahdavian2023AsynchJMLR}, \cite{ParallelDistrBook}
		\begin{equation}
			\label{equ_pseudocontractive_Bertsekas_dict}
			\normgeneric{\fixedpointop\weights-\weights^\star}{}
			\le
			\contractfac\,\normgeneric{\weights-\weights^\star}{}
			\quad\text{for all }\weights\in\hilbertspace.
		\end{equation}
		This notion expresses contraction with respect to the \gls{fixedpoint} and
		implies linear \gls{convergence} of the corresponding \gls{fixedpointiter}
		$\weights^{(\iteridx+1)}=\fixedpointop\weights^{(\iteridx)}$ \cite[Proposition 1.2]{ParallelDistrBook}.
		\medskip
		\noindent \\
		Example: Consider the \gls{operator} $\fixedpointop:\mathbb{R}\to\mathbb{R}$ defined by
		\[
			\fixedpointop(\weight)=
			\begin{cases}
			\frac12 \weight, & \weight\neq 1,\\
			0.9, & \weight=1.
			\end{cases}
		\]
		The unique \gls{fixedpoint} is $\weight^\star=0$, and
		$\lvert\fixedpointop(\weight)-\weight^\star\rvert\le
		0.9\,\lvert \weight-\weight^\star\rvert$ holds for all $\weight \in\mathbb{R}$,
		so $\fixedpointop$ is pseudocontractive in the sense of \eqref{equ_pseudocontractive_Bertsekas_dict}.
		However, $\fixedpointop$ fails to be a \gls{nonexpansiveop}, since arbitrarily small
		perturbations around $\weight=1$ can produce large changes in $\fixedpointop(\weight)$.
		\\
		See also: \gls{operator}, \gls{hilbertspace}, \gls{fixedpoint}.},
	first={pseudocontractive operator},
	text={pseudocontractive operator},
	type=math,
	firstplural={pseudocontractive operators},
	plural={pseudocontractive operators}
}
...
{name={proximal algorithm},
	description={A\index{proximal algorithm} proximal \gls{algorithm} refers to an \gls{optmethod} 
		...
		\\
		See also: \gls{operator}, \gls{hilbertspace}, \gls{fixedpoint}.},
	first={proximal algorithm},
	text={proximal algorithm},
	type=math,
	firstplural={proximal algorithms},
	plural={proximal algorithms}
}

\newglossaryentry{proxop}
{name={proximal operator},
	description={Given\index{proximal operator} a \gls{convex} and \gls{continuous}
		\gls{function} $f(\weights')$, we define its proximal \gls{operator} as \cite{ProximalMethods}, \cite{Bauschke:2017}  
		$$\proximityop{f}{\weights}\defeq \argmin_{\weights' \in \mathbb{R}^{\dimlocalmodel}} \bigg[ f(\weights')\!+\!(1/2) \normgeneric{\weights- \weights'}{2}^{2}\bigg].$$ 
		It is often necessary to evaluate the proximal \gls{operator} for a scaled version $\lrate f$, 
		with some factor $\lrate > 0$, 
		$$\proximityop{\lrate f}{\weights} \defeq \argmin_{\weights' \in \mathbb{R}^{\dimlocalmodel}} 
		\bigg[ f(\weights')\!+\!\frac{1}{2\lrate} \normgeneric{\weights- \weights'}{2}^{2}\bigg].$$ 
		The factor $\lrate$ controls the relative importance of the \gls{function} $f$ 
		and the \gls{penaltyterm} in the \gls{optproblem} defining the proximal 
		\gls{operator}. From an \gls{algorithm} design perspective, the role of the 
		scaling factor $\lrate$ is similar to the \gls{learnrate} in \glspl{gdmethod}. 
		As illustrated in Fig.\ \ref{fig_proxoperator_opt_dict}, evaluating the proximal \gls{operator} 
		amounts to minimizing a penalized variant of $f$. The \gls{penaltyterm} is the 
		...
		is the same as applying the (scaled, by $\lrate$) proximal \gls{operator} of the \gls{function} 
		$\tilde{f}(\weights')= \big( \nabla f(\weights)\big)\,^{T} (\weights'-\weights)$. 
			...
			\begin{center}
				\begin{tikzpicture}[scale=0.8]
					...
					\draw[red, thick, domain=1:3] plot (\x, {2*(\x - 2)*(\x - 2)}) node[below right] {$\frac{1}{2\lrate}\normgeneric{\weights-\weights'}{2}^{2}$};
					% Axes
				...
				\end{tikzpicture}
			\end{center}
			\caption{Proximal \gls{operator} updating a \gls{vector} $\weights$ by minimizing a penalized version 
				of the \gls{function} $f$. The \gls{penaltyterm} is the scaled squared \gls{eucliddist} between the \gls{optimization} 
...
{name={connected}, 
	description={An\index{connected} 
		...
		\begin{figure}[H]
			...
			\end{scope}
			\end{tikzpicture}
		\caption{(a) Disconnected \gls{graph}. (b) Connected \gls{graph}. \label{fig_connected_dict}}
		\end{figure} 
...
{name={Gaussian process (GP)},
	description={A\index{Gaussian process (GP)} GP is a collection of \glspl{rv} 
		...
		$\mu(\featurevec) = \expect\{ f(\featurevec)\}$; and 2) a \gls{covariancefunction}
  	...
  	first={Gaussian process (GP)},
	plural={GPs},
	firstplural={Gaussian processes (GPs)},
  	type=math, 
  	text={GP}
}
...
{name={Gaussian random variable (Gaussian RV)}, 
	description={A\index{Gaussian random variable (Gaussian RV)} standard \gls{gaussian} \gls{rv} is a 
		real-valued \gls{rv} $\feature$ with a \gls{pdf} \cite{BertsekasProb}, \cite{GrayProbBook}, \cite{papoulis}
		\begin{equation}
			\nonumber
		...
		\gls{stochproc} over an arbitrary index set $\mathcal{I}$ such that any restriction 
...
{name={central limit theorem (CLT)},
	description={Consider a \gls{sequence} of \gls{iid} \glspl{rv} \( \feature^{(\sampleidx)} \), for \( \sampleidx = 1, \,2, \,\ldots \), 
		...
		and rescaling. Iteratively applying \( \mathcal{T} \) leads to \gls{convergence} of \( \phi^{(\samplesize)}(t) \) toward the \gls{fixedpoint}
		\[
...
{name={indicator function}, 
	description={Consider some process\index{indicator function} that can result in different possible 
		...
		The indicator \gls{function} $\indicatorfunc{\mathcal{A}}$ of an \gls{event} 
		...
		For a \gls{probspace} $(\samplespace, \sigmaalgebra, \prob{\cdot})$ and $\mathcal{A} \in \sigmaalgebra$, 
		the indicator \gls{function} $\indicatorfunc{\mathcal{A}}$ is an \gls{rv} and it holds that 
		$\expect\{\indicatorfunc{\mathcal{A}}\} = \prob{\mathcal{A}}$.
		\\
		See also: \gls{outcome}, \gls{randomexperiment}, \gls{function}. }, 
...
{name={probabilistic model}, 
	description={A probabilistic \gls{model}\index{probabilistic model} for the 
		...
		that are either chosen manually or learned via statistical \gls{inference} 
...
{name={Laplacian matrix},
	description={The\index{Laplacian matrix} structure of a \gls{graph} $\graph$, with 
		...
		\begin{tikzpicture}
 		...
 		\begin{minipage}{0.45\textwidth}
				...
				\end{minipage}
	 	\end{minipage}
	 	\caption{\label{fig_lap_mtx_dict} (a) \Gls{undirectedgraph} $\graph$ with three nodes $\nodeidx=1,\,2,\,3$. 
	 		 (b) Laplacian \gls{matrix} $\LapMat{\graph}  \in \mathbb{R}^{3 \times 3}$ of $\graph$.} 
	 	\end{center}
	 	\end{figure}
	...
	firstplural={Laplacian matrices},
	text={Laplacian matrix}
}

\newglossaryentry{spectrum}
{name={spectrum},
	description={The spectrum\index{spectrum} of a \gls{linearoperator} $\fixedpointop$ 
  		is the set of values $\lambda$ for which the \gls{operator}
  		$\fixedpointop - \lambda \identityop$ fails to be invertible \cite{Muller2003}. 
  		The notion of a spectrum can then be naturally extended to any kind of 
		object for which an associated \gls{linearoperator} can be defined. 
		For example, the spectrum of a square \gls{matrix} $\mA \in \mathbb{R}^{\dimlocalmodel \times \dimlocalmodel}$ 
  		is obtained via the spectrum of the associated \gls{linearoperator} \cite{Horn91}.
  		As another example, we can define the spectrum of a \gls{stochproc} via a  
		\gls{linearoperator} constructed from its \gls{covariancefunction} \cite{Rasmussen2006Gaussian}.
		\\
		See also: \gls{linearoperator}, \gls{matrix}, \gls{stochproc}. },
	first={spectrum},
	type=math,
	plural={spectra},
	firstplural={spectra},
...
{name={Kullback–Leibler divergence (KL divergence)}, 
 	description={The\index{Kullback–Leibler divergence (KL divergence)} KL divergence 
		is a quantitative \gls{measure} of how different a \gls{probdist} $\probdist^{(1)}$ is from another 
		\gls{probdist} $\probdist^{(2)}$, which is defined on the same \gls{probspace} \cite{coverthomas}. 
		The KL divergence $\kld{\probdist^{(1)}}{\probdist^{(2)}}$ 
		is well defined only if $\probdist^{(1)}$ is absolutely \gls{continuous} with respect to $\probdist^{(2)}$.\\
		Special case: If $\probdist^{(1)}$ 
		and $\probdist^{(2)}$ are described by the \glspl{pmf} $\pmf{x}{\cdot}$ and $\pmf{y}{\cdot}$ 
		on a \gls{samplespace} $\samplespace$, then 
		\[
			\kld{\pmf{x}{\cdot}}{\pmf{y}{\cdot}} \defeq \sum_{\omega \in \samplespace} 
			\pmf{x}{\omega} \log \frac{\pmf{x}{\omega}}{\pmf{y}{\omega}}.
		\]
		In this discrete setting, the KL divergence coincides with the 
		\gls{bregmandivergence} generated by the negative \gls{entropy} \gls{function} \cite[Sec. 4.3]{CvxBubeck2015}
		\[
			\phi(p) \defeq \sum_{\omega \in \samplespace} 
			\pmf{x}{\omega} \log \pmf{x}{\omega}.
		\]
		See also: \gls{probdist}, \gls{bregmandivergence}.},
 	first={Kullback–Leibler divergence (KL divergence)},
	type=math, 
	plural={KL divergences},
	firstplural={Kullback–Leibler divergences (KL divergences)},
	text={KL divergence} 
}
...
{name={mean},
	description={The\index{mean} mean of an \gls{rv} $\featurevec$, which takes 
 		...
 		\gls{expectation} $\expect\{\featurevec\}$. It is defined as the \gls{LebesgueIntegral}
 		of $\featurevec$ with respect to the underlying \gls{probdist} $\prob{\cdot}$ (e.g., 
		...
		a \gls{dataset} to construct a \gls{discreteRV} $\widetilde{\vx}^{(\dataset)}=\vx^{(I)}$ on 
		...
		For an \gls{rv} with a finite second-order moment, i.e., 
		$\expect\{ \normgeneric{\featurevec}{2}^{2} \}$ is well defined and finite, 
	...
	description={A\index{median} median $\med\,(x)$ of a real-valued \gls{rv} $x$ 
		...
		(see Fig.\ \ref{fig_median1_dict}) \cite{LC}. 
 		\begin{figure}[H]
			...
			\end{tikzpicture} 
			\end{center}
		\caption{Median of a real-valued \gls{rv} defined as any number $M$ 
		...
		at random, i.e., $\prob{I = \sampleidx}=1/\samplesize$ for 
 		all $\sampleidx=1, \,\ldots, \,\samplesize$. If the \gls{rv} $x$ is \gls{integrable}, any 
		median of $x$ solves the following \gls{optproblem}:
 		$$\min_{x' \in \mathbb{R}} \expect{|x - x'|}.$$ 
		For the above \gls{rv} $\tilde{x}$ (constructed from a \gls{dataset} $\dataset$), 
		this \gls{optproblem} is \gls{erm} on $\dataset$ using \gls{abserr}. 
 		...
 		the median of a \gls{dataset} $\dataset$ with more than one \gls{datapoint} does not 
		...
		\begin{tikzpicture}[scale=0.7, y=0.5cm, x=0.5cm]
			...
			\end{scope}
		\end{tikzpicture}
		\caption{Median \gls{robustness} to \gls{outlier} contamination. (a) Original \gls{dataset} $\dataset$. (b) Noisy 
...
{name={orthogonality condition},
	description={Consider\index{orthogonality condition} a linear \gls{subspace} 
		$\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$ and a \gls{vector} 
		$\weights \in \mathbb{R}^{\dimlocalmodel}$. A \gls{vector} $\weights^\star \in \paramspace$ 
		is the \gls{projection} $\projection{\paramspace}{\weights}$ if and only if
		\begin{equation}
			\nonumber
			\label{eq:orthogonality_condition_dict}
			\vv^{T} \big( \weights - \weights^\star\big) = 0 
			\quad \text{for all } \vv \in \paramspace .
		\end{equation}
		In other words, the \gls{projection} error $\weights-\weights^\star$ is orthogonal to the
		\gls{subspace} $\paramspace$. The notion of orthogonality can be extended to general 
            	\glspl{hilbertspace}, i.e., \glspl{vectorspace} with \gls{innerproduct}.
		\\
		See also: \gls{vector}, \gls{projection}, \gls{euclidspace}.},
	first={orthogonality condition},
	plural={orthogonality conditions},
	firstplural={orthogonality conditions},
	type=math,
...
{name={constrained optimization problem}, 
	description={A\index{constrained optimization problem} constrained \gls{optproblem} is an 
		\gls{optproblem} that involves constraints on the \gls{optimization} variable. 
		For example, consider the \gls{optproblem}
		\[
			\min_{\weights \in \paramspace} f(\weights),
		\]
		where the \gls{objfunc} $f: \mathbb{R}^{\dimlocalmodel} \to \mathbb{R}$ is to be minimized 
		over a constraint set $\paramspace \subseteq \mathbb{R}^{\dimlocalmodel}$. Here, the constraint 
		set $\paramspace$ restricts the feasible values of the \gls{optimization} variable $\weights$. 
		If $\paramspace = \mathbb{R}^{\dimlocalmodel}$, the above \gls{optproblem} reduces to an 
		unconstrained \gls{optproblem}. 
					\\ 
		See also: \gls{optproblem}.}, 
	first={constrained optimization problem}, 
	plural={constrained optimization problems},
	firstplural={constrained optimization problems},
	type=math,
	text={constrained optimization problem} 
}
...
{name={projected gradient descent (projected GD)},
	description={Consider an \gls{erm}-based method that uses a parameterized \gls{model} with  
		...
		it does not take into account constraints on the \gls{optimization} variable (i.e., the \glspl{modelparam}). 
		...
		\vspace*{-5mm}
		\end{center}
		\caption{Projected \gls{gd} augmenting a basic \gls{gradstep} with a \gls{projection} back 
...
{name={Jacobian matrix},
	description={The Jacobian \gls{matrix}\index{Jacobian matrix} of a \gls{vector}-valued \gls{function} 
		$$\vf:\mathbb{R}^{\nrfeatures}\rightarrow\mathbb{R}^{\nrfeatures'}: 
		\featurevec \mapsto \vf(\featurevec) = \big(f_{1}(\featurevec),\,\ldots,\,f_{\nrfeatures'}(\featurevec) \big)^{T}$$
		is the \gls{matrix} constituted by all first-order \glspl{partialderivative} of $\vf$
		\cite{RudinBookPrinciplesMatheAnalysis}, \cite{HolderEichholz}. More explicitly, 
		\[
			D_{f}(\featurevec)
			\defeq
			\begin{bmatrix}
			...
			\end{bmatrix}.
		\]
		The Jacobian \gls{matrix} represents the best linear approximation of $\vf$
		around a given argument \gls{vector} $\featurevec$. Note that the $i$th row of the Jacobian \gls{matrix} 
		coincides with the \gls{transpose} of the \gls{gradient} of one component \gls{function} 
		$f_i$ evaluated at $\featurevec$.
		\\
		See also: \gls{matrix}, \gls{function}, \gls{partialderivative}, \gls{gradient}.},
	first={Jacobian matrix},
	type=math,
	plural={Jacobian matrices},
	firstplural={Jacobian matrices},
...
sort={linearoperator},
	description={A \index{linear operator} linear \gls{operator} is an 
		\gls{operator} $\fixedpointop$ whose \gls{domain} and \gls{co-domain} 
		are \glspl{vectorspace} and which satisfies \cite{DunfordSchwartz1988}
		\begin{equation}
			\nonumber
			\fixedpointop(\alpha \weights + \beta \weights')
			= \alpha \fixedpointop(\weights) + \beta \fixedpointop(\weights')
		\end{equation}
		for all $\weights,\weights'$ in the \gls{domain} and all scalars $\alpha,\beta$.
							\\
		See also: \gls{operator}, \gls{vectorspace}, \gls{matrix}.},
	first={linear operator},
	type=math,
	plural={linear operators},
	firstplural={linear operators},
...
{name={conditional probability distribution}, 
	description={Consider\index{conditional probability distribution} 
    		...
    		the \gls{rv} $\truelabel$ \cite[Sec. 34]{BillingsleyProbMeasure}, \cite{KallenbergBook}.
		\\
...
{name={vector},
	description={A\index{vector} vector is an element of a \gls{vectorspace}. 
		...
		is the (finite) \gls{dimension} of the space. A vector $\vx \in \mathbb{R}^{\nrfeatures}$ 
		can be represented as a list or 1-D array of real numbers, i.e., 
		...
		\caption{Two equivalent views of a vector $\vx= \big( 2, -1, 3, 0, -2, 1 \big)^{T} \in \mathbb{R}^{6}$.
			(a) As a numeric array. (b) As a \gls{map} $\featureidx \mapsto x_\featureidx$, where
			$\featureidx \in \{1, \,\dots, \,6\}$.}
			\label{fig:vector-function-dual_dict}
		\end{figure}
		See also: \gls{vectorspace}, \gls{euclidspace}, \gls{basis}, \gls{matrix}, \gls{linearmap}.},
	first={vector},
	firstplural={vectors},
	type=math,
	plural={vectors},
...
{name={vector space},
	description={A\index{vector space} \gls{vector} space $\mathcal{V}$ (also called linear space) 
		...
		number \gls{field} (such as the real numbers $\mathbb{R}$ or the complex numbers $\mathbb{C}$). The defining 
		...
		\centering
			...
			\end{tikzpicture}
		\caption{\Gls{vector} space $\mathcal{V}$ is a collection of \glspl{vector} such that 
		...
		\\
		See also: \gls{vector}, \gls{euclidspace}, \gls{basis}, \gls{hilbertspace}, \gls{linmodel}, \gls{linearmap}.},
	first={vector space},
...
{name={stochastic process},
	description={A \gls{stochastic} process\index{stochastic process} is a collection of 
		...
		$\indexset$ \cite{GrayProbBook}, \cite{papoulis}, \cite{Brockwell91}. The index set 
		$\indexset$ typically represents time or space, allowing us to represent 
		...
		Here, the index set $\indexset$ consists of node pairs that index \glspl{rv} whose values 
...
{name={characteristic function},
	description={The characteristic \gls{function}\index{characteristic function} 
		of a real-valued \gls{rv} $x$ is the following \gls{function} \cite[Sec. 26]{BillingsleyProbMeasure}:
		$$ \phi_{x}(t) \defeq \expect { \exp\,(j t x) } \quad \mbox{with } j = \sqrt{-1}.$$
...
{name={entropy},
	description={Entropy\index{entropy} quantifies the \gls{uncertainty} or 
		...
		\Gls{diffentropy} generalizes the concept of entropy from \glspl{discreteRV} to 
	...
	description={A function\index{function} between two sets $\mathcal{U}$ and $\mathcal{V}$ assigns  
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}[>=stealth, node distance=1.2cm and 2.5cm]
				...
				\draw[->] (C) -- (2);
			\end{tikzpicture}
		\caption{Function \( f \colon \mathcal{U} \to \mathcal{V} \) mapping each element 
	...
	description={A set is called countable\index{countable} if its 
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}[>=stealth, node distance=1.0cm, thick]
  			...
  			\draw[->] (a3) -- (n4);
			\end{tikzpicture}
		\caption{\Gls{injective} \gls{function} mapping the elements of a finite set 
...
{name={probability mass function (pmf)}, 
	description={The pmf\index{probability mass function (pmf)} 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{\Gls{discreteRV} $\feature$ taking values in the set $\featurespace = \{\star,\otimes\}$
			with a pmf $\pmf{\feature}{\cdot}$. Three \glspl{dataset} 
			...
			\gls{iid} \glspl{rv} sharing a common pmf $\pmf{\feature}{\cdot}$. 
			\label{fig_pmf_dict}}
		\end{figure}
		A pmf always satisfies $\sum_{\feature' \in \featurespace} \pmf{\feature}{\feature'} = 1$. 
		We can view a pmf as representing a collection of (sufficiently large)  
 	...
 	description={A\index{probability space} \gls{probability} space is a mathematical 
		...
		$(\samplespace, \eventspace, \prob{\cdot})$ where
 		\begin{itemize} 
			...
			for any \gls{countable} \gls{sequence} of pairwise disjoint \glspl{event} $\mathcal{A}_1, \,\mathcal{A}_2, \,\ldots$ in $\eventspace$.
 		\end{itemize}
...
{name={conditional expectation}, 
	description={Consider a numeric \gls{rv} $\featurevec \in \mathbb{R}^{\nrfeatures}$ defined on a 
                 ...
                 $\featurevec$ given (or conditioned on) $\sigmaalgebra'$, denoted by
                 $\expect\{ \featurevec \mid \sigmaalgebra'\}$, is a numeric \gls{rv} 
		that \cite[Sec. 34]{BillingsleyProbMeasure}, \cite{durrett2010probability}:
...
{name={independent and identically distributed (i.i.d.)}, 
	description={A collection of \glspl{rv}\linebreak $\datapoint^{(1)}, \,\ldots, \,\datapoint^{(\samplesize)}$ 
		...
		\cite[Th. 2.19]{klenke2020probability}.
				\\
...
{name={likelihood function},
	description={The\index{likelihood function} likelihood \gls{function} of a \gls{probmodel} 
		with \glspl{parameter} $\weights$ is obtained by evaluating the 
		\gls{pdf} (or \gls{pmf}) for the observed \gls{dataset} \cite{LC}, \cite{CasellaBergerStatInf}. 
		More precisely, consider a \gls{dataset} 
		$\dataset' = \{\datapoint^{(1)}, \,\ldots, \,\datapoint^{(\samplesize)}\}$ 
		that is assumed to be generated via \gls{iid} 
		\glspl{rv} with a common \gls{pdf} $\pdf{\datapoint;\weights}{\cdot}$. 
		The common \gls{pdf} is parameterized by the \glspl{modelparam} $\weights$. 
		The likelihood \gls{function} is then defined by 
		$L^{(\dataset')}(\weights) = \prod_{\sampleidx=1}^{\samplesize} \pdf{\datapoint;\weights}{\datapoint^{(\sampleidx)}}$.
		The likelihood \gls{function} can be used to construct a \gls{lossfunc} 
		for \gls{erm}, which results in \gls{maxlikelihood}. 
		\\
		See also: \gls{probmodel}, \gls{lossfunc}, \gls{maxlikelihood}.},
	first={likelihood function},
	plural={likelihood functions},
	type=math,
	text={likelihood function},
	firstplural={likelihood functions}
}
...
{name={maximum likelihood}, 
	description={Consider\index{maximum likelihood} \glspl{datapoint} 
		...
		with a common \gls{probdist} $\probdist^{(\dataset;\weights)}$, which depends 
		...
		the \gls{pdf} (or \gls{pmf}) 
		$\pdf{\dataset;\weights}{\dataset'} = \prod_{\sampleidx=1}^{\samplesize} 
		\pdf{\datapoint;\weights}{\datapoint^{(\sampleidx)}}$ 
		of the observed \gls{dataset} $\dataset'$. Thus, the \gls{maximum} 
		likelihood \gls{estimator} is a solution to the \gls{optproblem} 
...
{name={measurable}, 
	description={Consider\index{measurable} a \gls{randomexperiment}, such as recording 
		...
		(see Fig.\ \ref{fig_measurable_dict}).
		\begin{figure}[H]
		\begin{center}
		\begin{tikzpicture}
			...
			\vspace*{10mm}
			\end{center}
		\caption{\Gls{samplespace} constituted by all possible temperature values $\outcome$ 
...
{name={majorize-minimize (MM)}, 
	description={Consider an \gls{optproblem} $\min_{\weights \in \paramspace} f(\weights)$ with 
		...
		of \glspl{modelparam} as follows \cite{BishopBook}, \cite{Lange2016MM}, \cite{Hunter01022004}
		(see also Fig.\ \ref{fig:majmin_dict}): 
		During the $\iteridx$th \gls{iteration}, the \gls{objfunc} $f(\cdot)$ 
		...
		The new \glspl{modelparam}  $\weights^{(\iteridx+1)}$ are then obtained by 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Construction of \glspl{modelparam} based on the iterative MM method.}
...
{name={Markov's inequality},
	description={Consider a real-valued nonnegative \gls{rv} $x$ for which 
		the \gls{expectation} $\expect\{ x\}$ exists. \index{Markov's inequality} 
	 	...
	 	\begin{equation}
            		\nonumber
			\prob{x \geq a} \leq \frac{\expect \{ x\}}{a} \quad \mbox{holds for any } a > 0. 
            		%\label{eq:markovsinequality_dict}
    		\end{equation}
    		This inequality can be verified by noting that $\prob{x \geq a}$ is the 
		\gls{expectation} $\expect\{g(x)\}$ with the following \gls{indicatorfunc}:
	 	...
	 	$$ g(x') \leq x'/a \quad \mbox{for all } x' \in \mathbb{R}.$$ 
	 	...
	 	of the \gls{LebesgueIntegral} \cite[p. 50]{folland1999real}. 
			...
			\end{tikzpicture}
            	\caption{\Gls{expectation} $\expect\{x\}$ and \gls{probability} $\prob{x \geq a}$ 
			of a nonnegative \gls{rv} $x$ with a \gls{pdf} $\pdf{x}{\cdot}$,           
                		%a \gls{probdist} $\probdist^{(x)}$ 
			obtained via \glspl{LebesgueIntegral} of 
...
{name={Chebyshev's inequality},
	description={Consider a real-valued \gls{rv} $x$ for which 
		the second moment $\expect\{ x^{2} \}$ exists (and is finite). 
 		...
 		\begin{equation}
           		\nonumber
			\prob{\big|x - \mu \big| \geq \eta } \leq \frac{\sigma^2}{\eta^2} \quad \mbox{for any } \eta > 0.
			...
			\begin{tikzpicture}
	  		...
	  		ylabel style={rotate=270,anchor=south,at={(axis description cs:0,1.02)}},
   			...
   			%   \node[anchor=west] at (axis description cs:0.72,0.82) {$\prob{|x-\mu|\ge \eta}$};
    			\end{axis}
  			\end{tikzpicture}
  		\caption{Chebyshev's inequality provides an upper bound on 
              		the tail \gls{probability} $\prob{|x-\mu|\ge \eta}$ (i.e., the shaded area) of
...
{name={Hoeffding's inequality},
	description={\index{Hoeffding's inequality} Hoeffding's inequality \cite{Hoeffding1963} is a fundamental \gls{concentrationinequ} 
		...
		but it is more restrictive by assuming bounded \glspl{rv}.
    		In the context of \gls{ml}, Hoeffding's inequality can be used for the analysis of \gls{erm} or \gls{mab} methods.
		 \\
    		See also: \gls{concentrationinequ}, \gls{probability}, \gls{rv}, \gls{chebyshevsinequality}, \gls{expectation}, \gls{probspace}, 
		\gls{markovsinequality}.},
	...
	description={An RGG\index{random geometric graph (RGG)} is a \gls{probmodel} for \glspl{graph} 
		...
		for some threshold $r>0$, as illustrated in Fig.\ \ref{fig:rgg}.
   		...
   		\caption{Illustration of an RGG with $ \featurespace = [-5,5]\times[-5,5] \subset \mathbb{R}^2$ and radius $r=2.5$ 
			...
			within radius $r=2.5$) are connected, while node 3 (corresponding to $(3.0, 3.5)^T$) has no other node within that \gls{distance}.}
    			\label{fig:rgg}
    		\end{figure}
...
{name={Banach's fixed-point theorem},
	description={Banach's fixed-point theorem\index{Banach's fixed-point 
		...
		\[ \metric{\weights^{(\iteridx)}}{\weights^\star} \leq \contractfac^{\iteridx}
		 \cdot \metric{\weights^{(0)}}{\weights^\star} \mbox{ \quad for }\iteridx=1,\,2,\,\dots. \]   		
		\begin{figure}[H]
    			\centering
    			\begin{tikzpicture}[>=Latex, font=\small]
   			...
   			% \draw[distline] (fx1) -- (fx2) node[midway, above=6pt, sloped] {$\metric{\weights}{\weights'}$};
    			...
    			\end{tikzpicture}
   	 	\caption{\Gls{contractop} $\fixedpointop:\paramspace\to\paramspace$  
	  		with a unique \gls{fixedpoint} $\weights^\star$ satisfying 
...
{name={optimization problem}, 
	description={An\index{optimization problem} \gls{optimization} problem is a mathematical 
		   structure consisting of an \gls{objfunc} $f: \mathcal{U} \rightarrow \mathcal{V}$ 
		   defined over an \gls{optimization} variable $\weights \in \mathcal{U}$, together with a 
		   ...
		   Here, ``$\leq$'' denotes a general partial order relation, which may differ 
		   from the intrinsic ordering of real numbers \cite[Sec. 2.4]{BoydConvexBook}. 
		   The goal in \gls{optimization} is to find those values $\widehat{\weights} \in \paramspace$ 
		   for which the objective $f(\weights)$ is extremal, i.e., either minimal or maximal 
...
{name={optimization method},
	description={An\index{optimization method} \gls{optimization} method is 
		...
		\gls{optimization} method to \gls{erm}, we obtain a concrete learning 
		\gls{algorithm} \cite{BoydConvexBook}, \cite{nesterov04}, 
		\cite{BertsekasNonLinProgr}.
		 \\
...
{name={convex optimization},
 	description={\Gls{convex} \gls{optimization}\index{convex optimization} studies the 
		...
		consists of a \gls{convex} \gls{objfunc} $f: \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}$  
    		and a \gls{convex} constraint set $\mathcal{C}$ for the \gls{optimization} variable $\weights$.  
		...
		\begin{align} 
			...
			\mbox{s.t.} \quad & g_{\sampleidx}(\weights) \leq 0, \qquad \sampleidx=1,\,\ldots,\,k. \label{equ_def_convx_opt_constr_dict}
		\end{align} 
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[>=stealth, scale=1.0]
  			...
  			\end{tikzpicture}
  		\caption{\Gls{convex} \gls{optproblem} \eqref{equ_def_convx_opt_constr_dict} represented 
		...
		following \gls{epigraph} form \cite[Sec. 5.3]{BoydConvexBook}:
...
{name={Newton's method},
	description={Newton's method\index{Newton's method} is an iterative \gls{optmethod} for finding 
		...
		In particular, starting from an initial estimate $\estparams{0}$, Newton's method iteratively updates the estimate as follows:
		\[
		\estparams{\iteridx+1}= \estparams{\iteridx}- \big( \nabla^2 f\big(\estparams{\iteridx}\big) \big)^{-1} \nabla f\big( \estparams{\iteridx} \big) \quad \mbox{for } \iteridx=0,\,1,\,\ldots.
		\]
		Here, $\nabla f\big(\estparams{\iteridx} \big)$ is the \gls{gradient}, and $\nabla^2 f(\weights^{(\iteridx)})$ is 
		the \gls{hessian} of the \gls{objfunc} $f$. Since using a \gls{quadfunc} as a local approximation is more accurate 
...
{name={Hilbert space},
	description={A\index{Hilbert space} Hilbert space $\hilbertspace$ is a complete 
		...
		induces a \gls{norm} $\normgeneric{\cdot}{\hilbertspace}$ via 
		...
		\\
   		See also: \gls{innerproduct}, \gls{vectorspace}.},
	type=math, 
	first={Hilbert space}, 
	text={Hilbert space},
...
{name={estimator},
	description={\Glspl{mlsystem} are\index{estimator} often analyzed using a 
		\gls{probmodel} $\probdist^{(\featurevec;\weights)}$ for the 
		generation of \gls{data} $\featurevec$ \cite{MLBasics}. The true 
		value of the \glspl{modelparam} $\weights$ is typically unknown and needs 
		to be estimated. An estimator is a \gls{measurable} \gls{function} 
		$\hypothesis(\featurevec)$ that reads in the \gls{data} $\featurevec$ 
		and delivers an estimate (or approximation) $\widehat{\weights} = \hypothesis(\featurevec)$ 
		of the true value of the \glspl{modelparam} $\weights$ \cite{LC},  \cite{kay}.
		\\
		See also: \gls{modelparam}, \gls{measurable}, \gls{function}. },
	first={estimator},
	type=math,
	text={estimator},
	plural={estimators},
	firstplural={estimators}
}

\newglossaryentry{unbiasedest} 
{name={unbiased estimator},
	description={An \gls{estimator}\index{unbiased estimator} for the \glspl{parameter} of a \gls{probmodel} 
	    	is unbiased if its \gls{expectation} equals the true value of the \glspl{parameter} \cite{LC}, \cite{kay}.
		\\
		See also: \gls{estimator}, \gls{parameter}, \gls{expectation}. },
	first={unbiased estimator},
	type=math,
	text={unbiased estimator},
	plural={unbiased estimators},
	firstplural={unbiased estimators}
}

\newglossaryentry{rkhs}
{name={reproducing kernel Hilbert space (RKHS)},
	description={An\index{reproducing kernel Hilbert space (RKHS)} RKHS $\hilbertspace$ is a  
		special type of \gls{hilbertspace} that consists of 
 		\glspl{function} defined on a set $\featurespace$ \cite{aronszajn1950}. 
 		The defining property of an RKHS is that point evaluations are continuous 
 		linear functionals. Each RKHS is associated with a \gls{kernel} 
  		$$\kernel :\featurespace \times\featurespace\to\mathbb{R},$$ 
 		satisfying the reproducing property
 		\[
 			f(\featurevec)=\langle f, \kernel(\featurevec,\cdot)\rangle_{\hilbertspace}
 			\quad \text{for all } f\in\hilbertspace,\; \featurevec\in\featurespace .
 		\]
 		RKHSs arise naturally in the analysis of \glspl{kernelmethod}, 
 		including \gls{kernel}-\gls{ridgeregression}, \gls{svm}, and \gls{GaussProc} 
 		\gls{regression} \cite{Murphy2012}, \cite{LearningKernelsBook}. Another application of 
		RKHSs is for the analysis of \gls{minimum} \gls{variance} estimation \cite{RKHSExpFamIT2012}.
		\\
		See also: \gls{hilbertspace}, \gls{kernel}. }, 
 	first={reproducing kernel Hilbert space (RKHS)},
 	type=math,
 	text={RKHS}, 
 	plural={RKHSs},
 	firstplural={reproducing kernel Hilbert spaces (RKHSs)}
}
...
sort={Cauchysequence},
	description={A\index{Cauchy sequence} Cauchy \gls{sequence} is a \gls{sequence}
		...
		In other words \cite[Definition 3.8]{RudinBookPrinciplesMatheAnalysis}, 
		\[
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Cauchy \gls{sequence} $\big(\feature^{(\sampleidx)}\big)_{\sampleidx\in\mathbb{N}}$ 
...
{name={non-expansive operator},
 	description={An\index{non-expansive operator} \gls{operator} 
		$\fixedpointop: \mathcal{V} \rightarrow \mathcal{V}$ defined on a 
		\gls{normedspace} $(\mathcal{V}, \normgeneric{\,\cdot \,}{})$ is called non-expansive if it does 
		not increase \glspl{distance}. In other words,
 		\begin{equation} 
 			\nonumber
 			\normgeneric{ \fixedpointop \weights - \fixedpointop \weights'}{} 
			\leq 	\normgeneric{\weights - \weights'}{} \quad \mbox{for any } \weights, \weights' \in \mathcal{V}. 
 		\end{equation} 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Result of applying a \gls{contractop} $\fixedpointop$, a \gls{nonexpansiveop} 
		         $\fixedpointop'$, and a \gls{firmlynonexpansiveop} $\fixedpointop''$, all 
			sharing the \gls{fixedpoint} $\weights^\star$. \label{fig_nonexpansiveop_dict}}
       		\end{figure}
...
{name={firmly non-expansive operator},
 	description={An\index{firmly non-expansive operator} \gls{operator} 
		...
		\gls{hilbertspace} $\hilbertspace$ is called firmly non-expansive if it satisfies
 		\begin{equation} 
 			\nonumber
 			\normgeneric{ \fixedpointop \weights - \fixedpointop \weights'}{\hilbertspace}^2  
			\leq 	\innerprod{ \weights - \weights'}{ \fixedpointop \weights - \fixedpointop \weights'} 
			\quad \mbox{for any } \weights, \weights' \in \hilbertspace. 
 		\end{equation}
		By the \gls{cauchyschwarzinequ}, any firmly \gls{nonexpansiveop} is necessarily 
		also a \gls{nonexpansiveop}. 
		Every \gls{fixedpointiter} that uses a firmly \gls{nonexpansiveop} converges 
		to a \gls{fixedpoint} of the \gls{operator} \cite{BausckeCombette}.
		\\ 
 		See also: \gls{fixedpointiter}, \gls{contractop}.},
 	type=math, 
...
{name={fixed-point iteration},
	description={A\index{fixed-point iteration} fixed-point \gls{iteration} is an iterative method 
		...
		In case of \glspl{operator} $\fixedpointop: \mathbb{R}^\featuredim \to \mathbb{R}^\featuredim$,
		it constructs a \gls{sequence} $\weights^{(0)}, \,\weights^{(1)}, \,\ldots$ in $\mathbb{R}^\featuredim$ by 
		repeatedly applying an \gls{operator} $\fixedpointop$, i.e.,
		\begin{equation} 
		 	\label{equ_def_fixed_point_dict} 
		 	\weights^{(\iteridx+1)} = \fixedpointop \weights^{(\iteridx)} \quad \mbox{for } \iteridx=0, \,1, \,\ldots.
		\end{equation} 
		...
		\gls{convex} \gls{function} $f(\weights)$, the \glspl{fixedpoint} of the \gls{operator} $\fixedpointop: \weights \mapsto \weights - \nabla f(\weights)$ 
		...
		$\fixedpointop$ whose \glspl{fixedpoint} are $\widehat{\weights}$. Clearly, we should use an 
		\gls{operator} $\fixedpointop$ in \eqref{equ_def_fixed_point_dict} that reduces the 
		distance (with respect to the \gls{euclidnorm} or another \gls{norm}) to a solution such that
		\begin{equation} 
			\nonumber
		...
		The \gls{operator} $\fixedpointop$ is a \gls{contractop} (with respect to the \gls{euclidnorm}) if, 
		for some $\contractfac \in [0,1)$,
		\begin{equation} 
			\nonumber
			\normgeneric{ \fixedpointop \weights\!-\!\fixedpointop \weights'}{2}  \leq  \contractfac	\normgeneric{\weights\!-\!\weights'}{2} \quad \mbox{holds for any } \weights,\weights'.
		\end{equation}
		...
		defined on the one-dimensional (1-D) space $\mathbb{R}$. Another example of a \gls{firmlynonexpansiveop} 
		...
		\begin{figure}[H]
			\begin{center} 
				...
				\end{tikzpicture}
			\end{center} 
		\caption{Examples of a \gls{nonexpansiveop} $\fixedpointop^{(1)}$, a \gls{firmlynonexpansiveop} 
			$\fixedpointop^{(2)}$, and a \gls{contractop} $\fixedpointop^{(3)}$. \label{fig_examples_nonexp_dict}}
		\end{figure} 
		See also: \gls{contractop}, \gls{proxop}, \gls{bellmanoperator}, \gls{banachfixedpoint}, \gls{policyevaluation}, 
		\gls{valueiteration}.},
...
{name={graph of a function},
	description={Given\index{graph of a function} a \gls{function} $f\!:\!\featurespace \to \labelspace$
		...
		that is widely used in analysis, topology, and \gls{optimization}
		\cite{MunkresTopology2000}, \cite{RockafellarBook}.
		\\
		See also: \gls{function}, \gls{epigraph}. }, 
	first={graph},
	text={graph},
	type=math, 
	firstplural={graphs},
...
{name={fixed-point equation},
	description={A fixed-point equation\index{fixed-point equation} is an equation of the following form:
		\[
		...
		$\fixedpointop$. Many \glspl{optproblem}, including instances of \gls{erm}, 
		can be cast in this form. For example, minimizing a \gls{smooth} \gls{convex} 
		\gls{function} $f$ is equivalent to solving the following fixed-point equation:
		$$\weights = \fixedpointop \weights \quad \mbox{with }  
		\fixedpointop: \weights \mapsto \weights - \eta \nabla f(\weights).$$
		Here, $\lrate>0$ can be chosen freely. The above fixed-point 
		...
		optimality conditions (i.e., the \gls{kktconditions}) of \glspl{optproblem} with constraints as a 
		fixed-point equation \cite{BoydConvexBook}, \cite{pock_chambolle_2016}.
		\\
		See also: \gls{fixedpoint}, \gls{optproblem}. }, 
	type=math, 
...
{name={diagonalizable},
	description={A square \gls{matrix} $\mA \in \mathbb{C}^{\nrfeatures \times \nrfeatures}$ 
		is called diagonalizable\index{diagonalizable} if it is similar  
 		...
 		Formally, $\mA$ is diagonalizable if there exists an \gls{invertible} \gls{matrix} 
		$\mP \in \mathbb{C}^{\nrfeatures \times \nrfeatures}$ such that 
  		\[
  			\mA = \mP \mD \mP^{-1},
  		\]
...
{name={Schur decomposition},
	description={Every square \gls{matrix} $\mA \in \mathbb{C}^{\nrfeatures \times \nrfeatures}$ 
		admits a Schur decomposition\index{Schur decomposition} as follows \cite[Th. 7.1.3]{GolubVanLoanBook}:
		\[
		\mA = \mU \mT \mU^{H}. 
		\]
		...
		we extend $\vx^{(1)}$ to an orthonormal \gls{basis}
...
{name={inner product},
	description={Consider a \gls{vectorspace} $\featurespace$ over the 
		\gls{field} $\mathbb{F}$, where $\mathbb{F}$ is either the \gls{field} of 
		real numbers $\mathbb{R}$ or the \gls{field} of complex numbers $\mathbb{C}$. 
		...
		for all $\featurevec, \featurevec' \in \featurespace$.
 		\\
 		See also: \gls{field}, \gls{norm}, \gls{vector}, \gls{hilbertspace}, \gls{orthogonalitycondition}, \gls{metricspace}.},
 	first={inner product},
 	type=math,
 	text={inner product}
}
...
{name={trace},
 	description={The\index{trace} trace $\tr{\mA}$ of a square \gls{matrix} 
		...
		Formally, it is the following \gls{linearmap}:
		$$\tr{\cdot}: \mathbb{R}^{\nrfeatures \times \nrfeatures} 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
			\end{center}
		\caption{Trace $\tr{\mA}$ of a $3 \times 3$ \gls{matrix} $\mA \in \mathbb{R}^{3\times 3}$ as 
...
{name={sequence},
	description={A sequence\index{sequence} is an ordered collection 
		...
		See also: \gls{function}, \gls{dataset}, \gls{convergence}, \gls{cauchysequence}. }, 
	first={sequence},
	text={sequence},
	type=math, 
...
{name={convergence},
	description={Consider a \gls{sequence}\index{convergence} $\big( a_{\sampleidx} \big)_{\sampleidx \in \mathbb{N}}$ 
		...
		\begin{figure}[H] 
			...
			\end{tikzpicture}
		\caption{Real-valued \gls{sequence} $\big( a_{\sampleidx} \big)_{\sampleidx \in \mathbb{N}}$ 
		...
		with the \gls{metric} $\metric{a_{\sampleidx}}{a^\star}$. 
...
{name={Johnson--Lindenstrauss lemma (JL lemma)},
	description={The\index{Johnson--Lindenstrauss lemma (JL lemma)} JL lemma describes conditions for 
		...
		\[
	  	\nrfeatures' \ge \frac{4 \ln(\samplesize)}{\varepsilon^2/2 - \varepsilon^3/3} \quad \mbox{for some } 0 < \varepsilon < 1,
		\]  
		...
		\begin{figure}[H]
			...
			\end{scope}
			\end{tikzpicture}
		\caption{JL lemma offering precise conditions that guarantee the existence of 
...
{name={alternating direction method of multipliers (ADMM)},
	description={The\index{alternating direction method of multipliers (ADMM)} 
		ADMM is an iterative \gls{optmethod} for solving a structured \gls{optproblem}. 
		In particular, the ADMM can be used to solve an \gls{optproblem} of the following form:
		\begin{align}
		...
		\\
		See also: \gls{optmethod}, \gls{methodofmultipliers}. }, 
...
{name={method of multipliers (MoM)},
	description={The\index{method of multipliers (MoM)} MoM is an 
		iterative \gls{optmethod} for solving a \gls{constrainedoptprob} 
		of the following form \cite{DistrOptStatistLearningADMM}:
		\begin{align}
			\min_{\vx \in \mathbb{R}^{\nrfeatures}} \; f(\vx) \nonumber \\ 
		...
		\gls{vector}. The MoM is based on the \gls{augmentedlagrangian}
		\[L_\rho(\vx,\vy) = f(\vx) + \vy^\top \mathbf{A}\vx
		+ \frac{\rho}{2}\normgeneric{\mathbf{A}\vx - \vb}{2}^{2},
		\]
		where $\vy$ denotes the \gls{vector} of Lagrange multipliers and $\rho>0$
		is a penalty \gls{parameter}. The MoM constructs a \gls{sequence} of estimates 
		$\pair{\vx^{(1)}}{\vy^{(1)}},\,\ldots$ that converge to a solution of 
		the \gls{optproblem}. In particular, during each \gls{iteration} $\iteridx$, 
		...
		\end{align*}
		The MoM can be written as a \gls{fixedpointiter} of the following form:
		\[
		...
		\end{align}
		See also: \gls{optproblem}, \gls{augmentedlagrangian}, \gls{fixedpointiter}. }, 
...
{name={augmented Lagrangian},
	description={The augmented \gls{lagrangian}\index{augmented Lagrangian} is a modification 
		...
		\gls{penaltyterm} for constraint violations. It is used in the \gls{methodofmultipliers} 
		for iteratively solving \glspl{constrainedoptprob}.
		\\
		See also: \gls{methodofmultipliers}.},
	first={augmented Lagrangian},
	text={augmented Lagrangian},
	firstplural={augmented Lagrangians},
...
{name={Lagrangian},
	description={Consider a \gls{constrainedoptprob} of the following form:
		\begin{align} 
			\min_{\weights \in \mathbb{R}^{\nrfeatures}} & f(\weights)   \nonumber \\ 
			\mbox{s.t.} \quad & g_{i}(\weights) \leq 0, \qquad i=1,\,\ldots,\,k \nonumber \\
			& h_j(\weights) = 0, \qquad j=1, \,\ldots,\,l. \nonumber
		\end{align}
		...
		constraints, respectively. The Lagrangian is a useful tool for the analysis of  
		\glspl{constrainedoptprob} and the design of \glspl{optmethod}. For example, the Lagrangian allows us
		to define a \gls{dualproblem}, which provides a lower bound for the optimal value of a  
		\gls{constrainedoptprob}. This lower bound, in turn, can be used to construct a \gls{stopcrit} for 
		iterative \glspl{optmethod} \cite{BoydConvexBook}. Fig.\ \ref{fig_Langrian_dict} provides a 
		geometric interpretation of the Lagrangian for a \gls{constrainedoptprob} with a single 
		inequality constraint ($k=1$) and no equality constraints ($l=0$) (see \cite[Sec. 5.3]{BoydConvexBook}). 
		Here, the Lagrangian $L(\weights, \lambda)$ is the vertical intercept of a straight line with slope $-\lambda$ 
		that passes through a point $\pair{g_{1}(\weights)}{f(\weights)}$. 
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[>=stealth, scale=1.0]
			...
			% Labels
			\end{tikzpicture}
		\caption{\Gls{optproblem} with an \gls{objfunc} $f(\weights)$ and a single inequality constraint $g_{1}(\weights) \leq 0$, represented 
...
{name={Karush--Kuhn--Tucker conditions (KKT conditions)},
	description={Consider a constrained \gls{optproblem} with \gls{lagrangian}
		...
		The \index{Karush--Kuhn--Tucker conditions (KKT conditions)} KKT conditions are  
		...
		\begin{align}
			 \nabla_{\vx} L(\vx, {\bm \lambda}, {\bm \nu}) &= \mathbf{0}
			  && \text{(stationarity)} \nonumber\\
			  g_i(\vx) &\le 0, \qquad i=1,\,\ldots,\,k
			  && \text{(primal feasibility)} \nonumber\\
			  h_j(\vx) &= 0, \qquad j=1,\,\ldots,\,l
			  && \text{(primal feasibility)} \nonumber\\
			  \lambda_i &\ge 0, \qquad i=1,\,\ldots,\,k
			  && \text{(dual feasibility)} \nonumber\\
			  \lambda_i g_i(\vx) &= 0, \qquad i=1,\,\ldots,\,k
		...
		that satisfies the KKT conditions is also optimal for the primal and \glspl{dualproblem} \cite{BoydConvexBook}.
		The KKT conditions can be written as a \gls{fixedpointeq} 
		and can, in turn, be used to construct \glspl{fixedpointiter} for solving 
		the primal and \glspl{dualproblem} \cite{BoydConvexBook}.
		\\
		See also: \gls{optproblem}, \gls{dualitygap}, \gls{convex}, \gls{dualproblem}, \gls{fixedpointiter}. },
	first={Karush--Kuhn--Tucker conditions (KKT conditions)},
	text={KKT conditions},
	type=math
}
...
{name={duality gap},
	description={Consider a \gls{constrainedoptprob} with \gls{lagrangian}
		\[
			L(\vx, {\bm \lambda}, {\bm \nu})
			= f_0(\vx)
		...
		$d^\star$ the optimal value of the associated \gls{dualproblem}. The\index{duality gap} 
		...
		\\
		See also: \gls{dualproblem}, \gls{optproblem}. },
	first={duality gap},
	text={duality gap},
	type=math
}
...
{name={dual problem},
sort={dualproblem},
	description={Consider a \gls{constrainedoptprob}, which we 
 		refer to as primal problem in what follows, of the following form:
    		\begin{align} 
			\min_{\vx \in \mathbb{R}^{\nrfeatures}} & f(\vx) \nonumber \\ 
     			\mbox{s.t.} \quad & g_i(\vx)\le 0, \qquad i=1,\,\ldots,\,k \nonumber \\ 
			& h_j(\vx)=0, \qquad j=1,\,\ldots,\,l, \nonumber
        		\end{align}
		and its associated \gls{lagrangedualfunc} $q({\bm \lambda},{\bm \nu})$. 
		For any ${\bm \lambda} \ge \mathbf{0}$, ${\bm \nu} \in \mathbb{R}^{l}$, 
		and any $\vx$ that satisfies the constraints of the primal problem \cite[Ch. 5]{BoydConvexBook},
		$$q({\bm \lambda},{\bm \nu}) \leq f(\vx).$$
		Making this lower bound maximally tight amounts to the following \gls{optproblem}:
              	\[
              	\max_{{\bm \lambda}\geq\mathbf{0},{\bm \nu}}\; q({\bm \lambda},{\bm \nu}). 
              	\]
              	This \gls{optproblem} is referred to as the dual problem\index{dual problem} associated with 
		...
		\cite [Sec. 5.3]{BoydConvexBook}. The dual problem amounts to finding a \gls{supportinghyperplane} 
		for the set $\mathcal{A}$ with the largest vertical intercept.
		\begin{figure}[H]
     		  	\centering
     		  	\begin{tikzpicture}[>=stealth, scale=1.0]
     		  	...
     		  	\end{tikzpicture}
     		\caption{Dual problem of a \gls{constrainedoptprob}: finding a \gls{supportinghyperplane} with 
     		  	the largest vertical intercept \cite [Sec. 5.3]{BoydConvexBook}.	\label{fig_dual_problem_dict}}
		\end{figure}
		See also: \gls{optproblem}, \gls{lagrangian}. },
	first={dual problem},
	type=math,
	text={dual problem}
}
...
{name={Euclidean space}, 
	description={The\index{Euclidean space} 
		...
		space is equipped with a geometric structure defined by the \gls{innerproduct} 
	...
	type=math, 
	plural={Euclidean spaces}, 
	firstplural={Euclidean spaces},
...
{name={Euclidean norm}, 
	description={The\index{Euclidean norm} 
		Euclidean \gls{norm} (or $2$-\gls{norm}, or $\ell_2$-\gls{norm}) $\normgeneric{\weights}{2}$ of a \gls{vector} 
		...
		The Euclidean \gls{norm} is distinct among all \glspl{norm} on $\mathbb{R}^{\nrfeatures}$, in the sense 
		that it is induced by the \gls{innerproduct} $\vw^{T} \vv$ \cite{RudinBook}, \cite{BoydConvexBook}, \cite{HalmosFiniteDimVecSpace}. 
		In other words, $\normgeneric{\weights}{2}= \sqrt{\weights^{T} \weights}$.	
		\\
		See also: \gls{norm}, \gls{vector}, \gls{metric}, \gls{euclidspace}, \gls{hilbertspace}. },
	first={Euclidean norm},
	type=math,
	plural={Euclidean norms}, 
	firstplural={Euclidean norms},
	text={Euclidean norm} 
}

\newglossaryentry{eucliddist}
{name={Euclidean distance}, 
	description={The\index{Euclidean distance} Euclidean \gls{distance} between two 
		\glspl{vector} $\vw,\vw' \in \mathbb{R}^{\nrfeatures}$ is the \gls{euclidnorm} 
		$\normgeneric{\vw-\vw'}{2}$ of the difference $\vw-\vw'$.	
		\\
		See also: \gls{distance}, \gls{vector}, \gls{euclidspace}. },
	first={Euclidean distance},
	type=math,
	plural={Euclidean distances},
	firstplural={Euclidean distances},
...
{name={boundary},
	description={Consider\index{boundary} a subset $\mathcal{C} \subseteq \mathbb{R}^n$. 
                 The boundary of $\mathcal{C}$, denoted by $\bd{\mathcal{C}}$, is the 
		set of all points $\vx \in \mathbb{R}^n$ such that every closed ball 
                 $\closedball{\vx}{\varepsilon} = \{\vy \in \mathbb{R}^n: \norm{\vy - \vx} \leq \varepsilon\}$ 
                 of radius $\varepsilon > 0$ centered at $\vx$ intersects both $\mathcal{C}$ 
		...
		\begin{figure}[H]
			...
			\node (ballLabel) at (2.8, 2.2) {$\closedball{\vx}{\varepsilon}$};
			\draw[->, thin] (ballLabel) -- ($(xPoint)+(45:0.6cm)$);
			\node[anchor=east] (legendText) at (7.2, 0.0) {boundary $\bd{\mathcal{C}}$};
			\draw[thick, dashed] ($(legendText.west)+(-0.6,0)$) -- ($(legendText.west)+(-0.1,0)$);
			\end{tikzpicture}
		\caption{Boundary of the set $\mathcal{C}$ and a closed ball $\closedball{\vx}{\varepsilon}$ around 
			$\vx\!\in\!\bd{\mathcal{C}}$.}
		\end{figure}
		Note that the boundary $\bd{\mathcal{C}}$ 
		might contain elements that do not belong to $\mathcal{C}$ itself.
		The notion of a boundary can be generalized to arbitrary \glspl{metricspace}, including 
		an \gls{undirectedgraph} with a \gls{metric} given by the length of the shortest path.
		\\
		See also: \gls{metricspace}, \gls{neighborhood}. },
...
{name={neighborhood},
 	description={Consider some \gls{metricspace} $\featurespace$ with a \gls{metric} 
		$d: \featurespace \times \featurespace \rightarrow \mathbb{R}_{+}$.	
 		...
 		is the set of other points having a sufficiently small \gls{distance} to $\featurevec$. 
		...
		If $\featurespace$ is an \gls{undirectedgraph}, which is a special case of 
 		a \gls{metricspace}, the neighborhood of a node $\nodeidx \in \nodes$ 
 		is the set of its \glspl{neighbor}.
 				\\ 
 		See also: \gls{metric}, \gls{distance}, \gls{neighbor}.},
 	first={neighborhood},
	type=math,
...
{name={metric},
	description={A\index{metric} metric is a quantitative \gls{measure} used to compare objects. 
	     	In mathematics, a metric measures the \gls{distance} between two points in a space and must 
		follow specific rules, i.e., the \gls{distance} is always nonnegative, zero only if the 
		...
		\gls{ml}, the term metric refers to a quantitative \gls{measure} of how well a \gls{model} performs (somewhat 
		similar to a \gls{lossfunc}). Examples include \gls{acc}, \gls{precision}, and the average 
		...
		\\ 
		See also: \gls{distance}, \gls{acc}, \gls{precision}, \gls{validation}, \gls{loss}.},
	first={metric}, 
	type=math,
	plural={metrics},
	text={metric}
...
{name={Lagrange dual function},
	description={Consider\index{Lagrange dual function} a \gls{constrainedoptprob}
		...
		denotes the \gls{lagrangian} of the original \gls{optproblem}. 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Lagrange dual \gls{function} $q(\lambda)$, for a given $\lambda\geq0$, represented as the vertical intercept 
			...
			\cite [Sec. 5.3]{BoydConvexBook}.	\label{fig_dual_function_dict}}
		\end{figure}
		See also: \gls{optproblem}, \gls{function}, \gls{lagrangian}, \gls{supportinghyperplane}. },
...
{name={supporting hyperplane},
	description={Let\index{supporting hyperplane} $\mathcal{C} \subseteq \mathbb{R}^d$ be a non-empty set.
		A \gls{hyperplane}
		\[  
			\{ \vx \in \mathbb{R}^d : \langle \va, \vx \rangle = b \},
			\qquad \va \in \mathbb{R}^d \setminus \{ \mathbf{0} \}, \qquad \ b \in \mathbb{R}
		\]
		...
		\end{align*}
		and there exists at least one point $\vx^\star$ in the \gls{boundary} of $\mathcal{C}$ such that
		$\va^{T} \vx = b$ \cite{BoydConvexBook}. The \gls{vector} $\va$ is then a \gls{normalvector} 
		to the supporting \gls{hyperplane}, pointing toward the \gls{halfspace} that does not 
		contain the set $\mathcal{C}$ (see \cite[Sec. 2.5.2]{BoydConvexBook}). 
		\\
		See also: \gls{hyperplane}, \gls{boundary}. }, 
...
{name={state space},
	description={The \gls{state} space\index{state space} of a system is constituted  
		by all possible \glspl{state} of the system at any point in time. 
		\\
...
{name={state},
	description={A state\index{state} is a mathematical representation of the minimal 
		information needed to characterize a system at a given time 
		such that, together with the system dynamics, it suffices
		to predict the system’s future behavior \cite{SuttonEd2}, \cite{BertsekasDynOptII}. 	
		\\
		See also: \gls{featurevec}, \gls{hypothesis}, \gls{action}.},
	first={state},
	text={state}, 
...
{name={gradient},
	description={For\index{gradient} a real-valued \gls{function} 
		...
		For a \gls{differentiable} \gls{function}, the gradient is the \gls{vector} whose entries are the 
		\glspl{partialderivative} of $f$ with respect to each input component, such that 
		...
		\\
		See also: \gls{function}, \gls{vector}, \gls{differentiable}, \gls{partialderivative}, \gls{gdmethod}, \gls{gd}, 
...
{name={Lipschitz continuity},
	description={A\index{Lipschitz continuity} \gls{function} between two \glspl{metricspace} is 
		Lipschitz \gls{continuous} if there exists a constant $L > 0$ such that 
		the \gls{distance} between the \gls{function} values at any two points is at 
		most $L$ times the \gls{distance} between those points \cite{BasicRealAnalysis}. 
		\\
		See also: \gls{function}, \gls{vector}.},
	first={Lipschitz continuity},
	type=math,
	plural={Lipschitz continuities},
	firstplural={Lipschitz continuities},
	text={Lipschitz continuity} 
}

\newglossaryentry{strcvx}
{name={strongly convex},
	description={A \gls{function}\index{strongly convex} $f:\mathbb{R}^d \rightarrow \mathbb{R}\cup\{+\infty\}$ is 
		$\mu$-strongly \gls{convex} (with \gls{parameter} $\mu>0$) if \cite{Bauschke:2017}
		\[
			f(\lambda \vx+(1-\lambda)\vy)
			\le
			\lambda f(\vx)+(1-\lambda)f(\vy)
			-
			\frac{\mu}{2}\lambda(1-\lambda)\normgeneric{\vx-\vy}{2}^{2}
		\]
		for all $\vx,\vy\in\mathbb{R}^d$ and $\lambda\in[0,1]$.
		Equivalently, $f$ is $\mu$-strongly \gls{convex} if, for every 
		$\vg\in\partial f(\vx)$, \cite[Sec. 3.4]{CvxBubeck2015}
		\[
			f(\vy) \ge f(\vx) + \vg^\top (\vy-\vx)
			+ \frac{\mu}{2}\normgeneric{\vy-\vx}{2}^{2}
			\quad \mbox{for all } \vx,\vy \in \mathbb{R}^{d}.
		\]
		If $f$ is \gls{differentiable}, this reduces to \cite[Sec. B.1.1]{CvxAlgBertsekas}
		\[
			f(\vy) \ge f(\vx) + \nabla f(\vx)^\top (\vy-\vx)
			+ \frac{\mu}{2}\normgeneric{\vy-\vx}{2}^{2},
		\]
		and if $f$ is twice \gls{differentiable},
		\[
			\nabla^2 f(\vx) \succeq \mu \mathbf{I} 
			\quad \mbox{for all } \vx \in \mathbb{R}^{d}.
		\]
		Strong convexity implies uniqueness of the minimizer of $f(\vx)$ and enables 
		linear \gls{convergence} rates for \glspl{gdmethod}.
					\\ 
		See also: \gls{function}, \gls{convex}, \gls{differentiable}, \gls{strictlyconvex}.},
...
{name={strictly convex},
	description={A\index{strictly convex} real-valued \gls{function} $f: \mathcal{U} \to \mathbb{R}$ 
		with \gls{convex} \gls{domain} $\mathcal{U} \subset  \mathbb{R}^d$ is strictly \gls{convex} if, 
		for any two distinct $\vx,\vy \in \mathcal{U}$ and any $\lambda\in(0,1)$, 
		it satisfies \cite[Definition 3.1.1]{BoydConvexBook}
		\[
			f(\lambda\vx+(1-\lambda)\vy)
			< \lambda f(\vx)+(1-\lambda)f(\vy).
		\]
		Furthermore, if $f$ is \gls{differentiable}, this condition is equivalent to
		\[
			f(\vy) > f(\vx) + \nabla f(\vx)^{T}(\vy-\vx)
		\]
		for any $\vx\neq\vy$, which implies that $f(\cdot)$ admits a unique minimizer on 
		any \gls{convex} subset of its \gls{domain}. Unlike \gls{strcvx} \glspl{function}, strictly \gls{convex} 
		\glspl{function} do not require a uniform quadratic lower bound.
					\\ 
	...
	description={A directed cycle\index{directed cycle} in a \gls{directedgraph} $\graph=\pair{\nodes}{\edges}$ 
		...
		such that $(\nodeidx_1, \,\nodeidx_2), (\nodeidx_2, \,\nodeidx_3), \,\ldots, (\nodeidx_{k-1}, \,\nodeidx_k), 
	 	...
	 	\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Directed cycle consisting of three nodes \gls{connected} in a closed loop.}
		\end{figure}
...
{name={directed acyclic graph (DAG)},
	description={A\index{directed acyclic graph (DAG)} DAG is a \gls{directedgraph} 
    		that contains no \glspl{directedcycle}. Formally, a DAG $\graph = (\nodes, \edges)$ satisfies 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{(a) DAG defined on three nodes $\nodes=\{1,\,2,\,3\}$. (b) \Gls{directedgraph} 
			on the same nodes that is not a DAG, since it contains a \gls{directedcycle}.}
		\end{figure}
...
{name={directed graph},
	description={A directed \gls{graph}\index{directed graph} contains edges that 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Edges of a directed \gls{graph} with orientation (or direction), 
...
{name={simple function},
	description={A\index{simple function} simple \gls{function} 
 		...
 		where $\indicatorfunc{\cluster}$ denotes the \gls{indicatorfunc} of a 
...
{name={concentration inequality}, 
	description={Concentration inequality refers to an upper bound on the \gls{probability}\index{concentration inequality} that 
		an \gls{rv} deviates more than a prescribed amount from its \gls{expectation} \cite{Wain2019}. 
		\\
		See also: \gls{probability}, \gls{rv}, \gls{expectation}, \gls{markovsinequality}, \gls{chebyshevsinequality}, 
...
{name={covariance function},
	description={A \gls{covariance} \gls{function}\index{covariance function} characterizes the
		second-order dependence structure of a \gls{stochproc}.
		For a real-valued \gls{stochproc} $\{\feature(\timeidx)\}_{\timeidx\in\indexset}$ with
		finite second-order moments, the \gls{covariance} \gls{function} is 
		defined as \cite{Rasmussen2006Gaussian}
		\begin{align}
				\nonumber
		...
		\end{align}
		See also: \gls{covariance}, \gls{function}, \gls{stochproc}. },
	first={covariance function},
	type=math,
	plural={covariance functions},
	firstplural={covariance functions},
...
{name={sample variance}, 
	description={Consider a \gls{dataset} $\dataset$ consisting of \glspl{datapoint} 
		characterized by scalar values $\truelabel^{(1)}, \,\ldots, \,\truelabel^{(\samplesize)} 
		\in \mathbb{R}$. The\index{sample variance} \gls{sample} \gls{variance} of $\dataset$ 
		is defined as the \gls{variance} with respect to the \gls{empiricaldistribution} 
		$\probdist^{(\dataset)}$ induced by $\dataset$. It is given explicitly by 
		$$\frac{1}{\samplesize} \sum_{\sampleidx=1}^{\samplesize} (\truelabel^{(\sampleidx)}\!-\!\widehat{\truelabel})^2,$$ 
		where $\widehat{\truelabel}$ is the \gls{samplemean} of $\dataset$. The 
		\gls{sample} \gls{variance} is a special case of the \gls{samplecovmtx} 
		when each \gls{datapoint} is characterized by a single numerical value.
		\\
		See also: \gls{samplecovmtx}.},
	first={sample variance},
	plural={sample variances},
	firstplural={sample variances},
	type=math, 
...
{name={sample covariance matrix}, 
	description={Consider a \gls{dataset} $\dataset$ consisting of \glspl{datapoint} 
		...
		The\index{sample covariance matrix} \gls{sample} \gls{covmtx} 
...
{name={eigenvalue}, 
	description={We\index{eigenvalue} refer to a number $\eigvalgen \in \mathbb{R}$ 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{\Gls{eigenvector} corresponding to the eigenvalue $\eigvalgen$.
...
{name={eigenvalue decomposition (EVD)}, 
	description={An\index{eigenvalue decomposition (EVD)} EVD
		...
		\glspl{eigenvector} of the \gls{matrix} $\mA$. The diagonal \gls{matrix} 
		...
		\Glspl{matrix} that allow for an EVD are referred to as \gls{diagonalizable}.
				\\
...
{name={singular value decomposition (SVD)}, 
  	description={The\index{singular value decomposition (SVD)} SVD  
		...
		and $\mU = \big( \vu^{(1)},\,\ldots,\,\vu^{(\dimlocalmodel)} \big) \in \mathbb{R}^{\dimlocalmodel \times \dimlocalmodel}$ 
		\cite{GolubVanLoanBook} (see Fig.\ \ref{fig_svd_dict}). 
		\begin{figure}[H]
		 	\centering	
			...
			\end{scope}
		 	\end{tikzpicture}
		\caption{Orthonormal \glspl{matrix} $\mV$ and $\mU$. \label{fig_svd_dict} }
		\end{figure}
		...
		are nonnegative and referred to as \glspl{singularvalue}.
		\\
		See also: \gls{matrix}. },
...
{name={marginal distribution}, 
	description={A\index{marginal distribution} marginal distribution 
		refers to the \gls{probdist} of a subset of the 
		\glspl{rv} belonging to a \gls{stochproc} \cite[p. 266]{BillingsleyProbMeasure}, \cite{papoulis}. 
		For example, consider two \glspl{rv} $(\feature,\truelabel)$ with a joint 
		\gls{probdist} $\probdist^{(\feature,\truelabel)}$. This 
		joint \gls{probdist} fully determines the marginal distributions 
		of $\feature$ and $\truelabel$. The process of computing the marginal 
		distribution of a subset of \glspl{rv} is referred to as \gls{marginalization} \cite{GraphModExpFamVarInfWainJor}.
		\\
		See also: \gls{probdist}, \gls{rv}, \gls{stochproc}, \gls{marginalization}.},
	first={marginal distribution},
	plural={marginal distributions},
	firstplural={marginal distributions},
	type=math,
...
{name={marginalization}, 
	description={The term marginalization refers to the\index{marginalization} process of computing the \gls{probdist} 
		of a subset of \glspl{rv} from the joint \gls{probdist} of a \gls{stochproc} 
		\cite{GraphModExpFamVarInfWainJor}.
		\\
		See also: \gls{probdist}, \gls{rv}, \gls{marginaldist}.},
	first={marginalization},
	plural={marginalizations},
	firstplural={marginalizations},
	type=math,
...
{name={probability density function (pdf)},
	description={The\index{probability density function (pdf)} pdf 
		...
		via a \gls{LebesgueIntegral} as follows \cite[Ch. 3]{BertsekasProb}:
...
{name={convex},
	description={A\index{convex} subset $\mathcal{C} \subseteq \mathbb{R}^{\featuredim}$ of the 
		...
		$$ \alpha \vx + (1-\alpha) \vy \in \mathcal{C} \quad \mbox{for any } \alpha \in [0,1]. $$
		...
		\begin{center}
			...
			\vspace*{-8mm}
		\end{center}
		\caption{(a) Convex set $\cluster \subseteq \mathbb{R}^{\dimlocalmodel}$. 
			(b) Convex \gls{function} $f: \mathbb{R}^{\dimlocalmodel} \rightarrow \mathbb{R}$.\label{fig_convex_set_function_dict}}
		\end{figure}
...
{name={smooth},
	description={A\index{smooth} real-valued \gls{function} $f: \mathbb{R}^{\dimlocalmodel} \rightarrow \mathbb{R}$ 
		...
		$$\normgeneric{\nabla f(\weights) - \nabla f(\weights')}{} \leq \beta \normgeneric{\weights - \weights'}{} \quad \mbox{for any } \weights,\weights' \in \mathbb{R}^{\dimlocalmodel}.$$ 
		The constant $\beta$ quantifies the smoothness of the \gls{function} $f$, i.e., the smaller the $\beta$, 
...
{name={metric space},
	description={A \gls{metric} space\index{metric space} is a set  
		...
		satisfies the above requirements \cite{BasicRealAnalysis}. 
		\begin{figure}[H]
		\centering
		...
		\caption{Examples of \gls{metric} spaces. (a) \gls{euclidspace} $\mathbb{R}^2$ with 
			...
			the shortest-path \gls{distance} as a \gls{metric}.\label{fig:metric_space_examples_dict}}
	    	\end{figure}
...
{name={gradient step}, 
	description={Given a \gls{differentiable} 
		...
		$\nabla f(\weights)$ to obtain the new \gls{vector} (see Fig.\ \ref{fig_basic_GD_step_single_dict}) as follows:
		\begin{equation}
		...
		\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
		\caption{Basic \gls{gradient} step \eqref{equ_def_gd_basic_dict} mapping a given \gls{vector} $\weights$ 
			to the updated \gls{vector} $\weights'$.}
			\label{fig_basic_GD_step_single_dict}
		\end{figure}
		More formally, the \gls{gradient} step amounts to applying the \gls{operator} 
		$$\mathcal{T}^{(f,\lrate)}(\cdot): \mathbb{R}^{\nrfeatures} \rightarrow \mathbb{R}^{\nrfeatures}:
		 	 \weights \mapsto \weights - \lrate \nabla f(\weights).$$ 
		For a \gls{smooth} and \gls{convex} \gls{objfunc} $f$, any of 
		its minimzers $\widehat{\weights}$ is a \gls{fixedpoint} of 
		$\mathcal{T}^{(f,\lrate)}$, i.e., 
		$$ \mathcal{T}^{(f,\lrate)} \widehat{\weights} = \widehat{\weights} \quad \mbox{for any } 
		\lrate >0.$$ What is more, 
		for a sufficiently small \gls{learnrate} $\lrate > 0$, one can verify 
		that in this case $\mathcal{T}^{(f,\lrate)}$ becomes a \gls{firmlynonexpansiveop} 
		\cite[Corollary 18.16]{BausckeCombette}. Moreover, for 
		a \gls{strcvx} and \gls{smooth} \gls{function} $f(\cdot)$ and an 
		appropriate choice of \gls{learnrate}, one can verify that 
		$\mathcal{T}^{(f,\lrate)}$ is a \gls{contractop} \cite[Th. 2.1.14]{nesterov04}.
		...
		this \gls{operator} is known as the \gls{proxop} of $f(\cdot)$ \cite{ProximalMethods}. 
					\\ 
		See also: \gls{differentiable}, \gls{gradient}, \gls{stepsize}, \gls{proxop}, \gls{sgd}, \gls{fixedpointeq}.},
	first={gradient step},
	type=math,
...
{name={mirror descent},
 	description={Mirror descent\index{mirror descent} is an iterative \gls{optmethod}  
		obtained by generalizing the \gls{gradstep}. The 
 		\gls{gradstep} for minimizing a \gls{differentiable} \gls{objfunc} $f:\mathbb{R}^\nrfeatures \to \mathbb{R}$ 
 		can be written as
		\[
		\weights^{(\itercntr+1)} = \argmin_{\weights \in \mathbb{R}^\nrfeatures}
 		\left \langle \nabla f(\weights^{(\itercntr)}),\weights \right \rangle
		...
		Mirror descent replaces the squared \gls{euclidnorm} with a 
 		...
 		\]
 		See also: \gls{gradstep}, \gls{bregmandivergence}, \gls{innerproduct}, \gls{zerogradientcondition}, \gls{proxop}.},
...
{name={Bregman divergence},
	description={The Bregman divergence\index{Bregman divergence} induced by a 
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}
		\caption{Contour lines $\{\weights \in \paramspace \, : \, \phi(\weights) = c\}$ for different values of 
			$c \in \mathbb{R}$ of a \gls{convex} \gls{function} $\phi(\cdot)$ defined on a
		...
		\\
		See also: \gls{gradstep}, \gls{gradient}, \gls{innerproduct}, \gls{proxop}.},
	...
	description={In the context of \gls{ml}, a\index{sample} sample is a finite \gls{sequence} 
		...
		as a set of \glspl{datapoint}, in which case duplicates are not allowed \cite{Everitt2010}, \cite{OxfordStatisticsDictionary}.
 		...
 		\begin{center}
 			...
 			\end{tikzpicture}
 		\end{center}
 		\caption{Sample viewed as a finite \gls{sequence}. 
...
sort={posteriorprediction},
	description={The\index{posterior (prediction)} study and design of \gls{ml} methods is often based on a 
		...
		determined by the \gls{condprobdist} $\probdist^{(\truelabel \mid \featurevec)}$
		of $\truelabel$ given (or conditioned on) $\featurevec$ \cite{BishopBook}, \cite{papoulis}, \cite{LC}.
		\\
		See also: \gls{probmodel}, \gls{condprobdist}, \gls{posteriordist}. }, 
 	first={posterior}, 
 	type=math, 
 	text={posterior} 
}
...
{name={determinant},
	description={The\index{determinant} determinant $\det\,(\mA)$ of a square \gls{matrix} 
		...
		are altered by this transformation (see Fig.\ \ref{fig_det_dict}) \cite{GolubVanLoanBook}, \cite{Strang2007}. 
 		...
 		and $\det\,(\mA) = 0$ collapses volume entirely, indicating that $\mA$ is non-\gls{invertible}. 
		...
		$\det\,(\mA) = \prod_{\featureidx=1}^{\nrfeatures} \eigval{\featureidx}$ \cite{HornMatAnalysis}.
    		For the special cases $\nrfeatures=2$ (i.e., in two-dimensional (2-D) space $\mathbb{R}^{2}$) and 
		$\nrfeatures=3$ (i.e., in three-dimensional (3-D) space $\mathbb{R}^{3}$), 
...
{name={Hessian},
 	description={Consider\index{Hessian} a \gls{function} 
		...
		\vspace*{-5mm}
		\end{center}
		\caption{\Gls{function} $f(\featurevec)$, sufficiently \gls{smooth} at a point $\featurevec'$, locally approximated by 
	...
	description={A denoising\index{denoising autoencoder} \gls{autoencoder} extends the basic 
		...
		\begin{figure}[H]
			...
			\begin{minipage}{0.45\textwidth}
				...
				{\selectfont (b)}
			\end{minipage}
  		\caption{Denoising \gls{autoencoder} reading in (a) a 
			corrupted (i.e., noisy) representation of a \gls{datapoint} 
			and computing a \gls{prediction} for (b) the clean representation.}
...
{name={Granger causality},
	description={See\index{Granger causality} \gls{causality}.
		\\
		See also: \gls{scm}, \gls{stochproc}.},
	first={Granger causality},
	type=math,
...
{name={causality},
	description={In the context of \gls{ml}, causality\index{causality} refers to 
		cause–effect relationships between quantities, such as the 
		\glspl{feature} and \glspl{label} of \glspl{datapoint} 
		\cite{Pearl:2010aa}, \cite{Thurman1988}. Different notions of causality 
		have been developed in the literature. One widely used notion 
		is based on a \gls{scm}, which represents causal mechanisms via 
		functional relationships \cite{Pearl:2010aa}. 
		For time-series \gls{data}, Granger causality defines a predictive 
		notion of causality \cite{Deistler2022}, \cite{Granger1969}. A 
		\gls{stochproc} $X_{1},\,X_{2},\,\ldots$ is said to cause another 
		\gls{stochproc} $Y_{1},\,Y_{2},\,\ldots$ if using past values 
		$X_{\timeidx'}$, $\timeidx' < \timeidx$, improves the \gls{prediction} 
		of $Y_{\timeidx}$ beyond using the past of $Y$ alone. 
		\\ 
		See also: \gls{scm}, \gls{stochproc}.},
	first={causality},
	type=math,
...
\newglossaryentry{scm}
{name={structural causal model (SCM)},
	description={An SCM\index{structural causal model (SCM)} 
		is a mathematical \gls{model} of cause–effect relationships underlying 
		a \gls{data} generation process \cite{Pearl:2010aa}. 
		\\
		See also: \gls{causality}.},
	plural={SCMs},
	firstplural={structural causal models (SCMs)},
	first={structural causal model (SCM)},
	type=math,
	text={SCM}
}
...
{name={average node degree},
	...
	first={average node degree},
	type=math, 
	text={average node degree} 
}

\newglossaryentry{fixedpointcharact}
{name={fixed-point characterization},
	description={The solution of \glspl{optproblem} arising in \gls{ml} 
		can often be characterized by a \gls{fixedpointeq}\index{fixed-point characterization}. 
		As a case in point, consider an \gls{optproblem} with a \gls{smooth} 
		and \gls{convex} \gls{objfunc} $f(\weights)$. Each minimizer $\widehat{\weights}$ 
		of $f(\weights)$ satisfies the \gls{fixedpointeq} 
		$\widehat{\weights} = \fixedpointop^{(\lrate)} \widehat{\weights}$ with 
		$\fixedpointop^{(\lrate)}: \weights \mapsto \weights-\lrate \nabla f(\widehat{\weights})$ \cite{BoydConvexBook}. 
		Here, $\lrate > 0$ is an arbitrary positive constant and 
		$\nabla f(\weights)$ denotes the \gls{gradient} of $f(\weights)$.	
		Once we have found a \gls{fixedpointeq} with an \gls{operator} 
		$\fixedpointop$ that is a \gls{contractop}, we can use a \gls{fixedpointiter} 
		to compute a solution of the underlying \gls{optproblem}. In the 
		above example, the resulting \gls{fixedpointiter} coincides with \gls{gd} for 
		minimizing $f(\weights)$ \cite{BoydConvexBook}.
	               \\ 
		See also: \gls{fixedpointeq}, \gls{fixedpointiter}.},
...
{name={node degree},
	description={The degree\index{node degree} $\nodedegree{\nodeidx}$ of a 
		...
		$\nodedegree{\nodeidx} \defeq \big|\neighbourhood{\nodeidx}\big|$.
		For a weighted \gls{undirectedgraph} $\graph=(\nodes,\edges,\mA)$, 
		we can alternatively define the (weighted) node degree as the sum 
		of the weights of all edges \gls{connected} to node $\nodeidx$, i.e.,
		$\nodedegree{\nodeidx} = \sum_{\nodeidx' \in \nodes} \edgeweight_{\nodeidx,\nodeidx'}$ \cite{OPSAHL2010245}.	
					\\ 
		See also: \gls{undirectedgraph}, \gls{neighbor}, \gls{averagenodedegree}.},
	first={node degree},
	type=math, 
	text={node degree} 
}
...
{name={empirical distribution},
	description={Consider a \gls{dataset} 
		...
		\end{tikzpicture}
    		\end{center}
		\caption{\Gls{dataset} $\dataset$ consisting of $\samplesize=10$ \glspl{datapoint}, each 
...
{name={norm},
	description={A norm\index{norm} is a \gls{function} that maps each (\gls{vector}) element 
		of a \gls{vectorspace} to a nonnegative real number. Formally, a norm 
		$\normgeneric{\, \cdot \,}{}$ on a \gls{vectorspace} $\mathcal{X}$ over a \gls{field} $\mathbb{F}$ is a 
		\gls{function} $\normgeneric{\, \cdot \,}{}: \mathcal{X} \to \mathbb{R}_{+}$ 
		that satisfies the following conditions \cite{HornMatAnalysis} for all $\featurevec, \featurevec' \in \mathcal{X}$ 
		and $\alpha \in \mathbb{F}$:
		\begin{itemize}
			\item Definiteness: $\normgeneric{\featurevec}{} = 0 \iff \featurevec = \mathbf{0}$;
			\item Homogeneity: $\normgeneric{\alpha \featurevec}{} = |\alpha| \cdot \normgeneric{\featurevec}{}$;
			\item Triangle inequality: $\normgeneric{\featurevec + \featurevec'}{} \leq \normgeneric{\featurevec}{} + \normgeneric{\featurevec'}{}$.
		\end{itemize}
		Intuitively, norms measure the length or size of a \gls{vector}.
		Norms can also be used to measure distances between \glspl{vector}, as they define a \gls{metric} by 
		$\metric{\featurevec}{\featurevec'} = \norm generic{\featurevec - \featurevec'}{}$,
		making $(\mathcal{X}, \metric{\cdot}{\cdot})$ a \gls{metricspace}.
		A prominent family of norms used in \gls{ml} is the $\ell_p$-norms for $p \geq 1$, 
		defined for a \gls{vector} $\featurevec \in \mathbb{R}^\featuredim$ as
		$\normgeneric{\featurevec}{p} = \left( \displaystyle \sum_{i=1}^\featuredim |\feature_i|^p \right)^{1/p}$.
		Important instances include the $\ell_1$-norm, the $\ell_2$-norm (or \gls{euclidnorm}), 
		and the $\ell_\infty$-norm $\normgeneric{\featurevec}{\infty} = \max_i |\feature_i|$, which are illustrated
		by the geometry of their respective unit spheres in Fig.\ \ref{fig:norms_unit_balls_dict}.
		In \gls{ml}, norms are fundamental, for instance for defining a \gls{lossfunc} or a \gls{regularizer}. 
		\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=1.5]
			% Axes
			\draw[->] (-1.2,0) -- (1.2,0) node[right] {$x_1$};
			\draw[->] (0,-1.2) -- (0,1.2) node[above] {$x_2$};
			% L-infinity (Square)
			\draw[blue, thick] (-1,-1) rectangle (1,1);
			\node[blue] at (0.8, 1.1) {\tiny $\ell_\infty$};
			% L2 (Circle)
			\draw[red, thick] (0,0) circle (1);
			\node[red] at (0.7, 0.8) {\tiny $\ell_2$};      
			% L1 (Diamond)
			\draw[green!60!black, thick] (1,0) -- (0,1) -- (-1,0) -- (0,-1) -- cycle;
			\node[green!60!black] at (0.4, 0.4) {\tiny $\ell_1$};
		\end{tikzpicture}
		\caption{Unit spheres $\sphere{1}$ with respect to different $\ell_p$-norms for $p=1,\,2,\,\infty$.}
			\label{fig:norms_unit_balls_dict}
		\end{figure}
		See also: \gls{euclidspace}, \gls{hilbertspace},  \gls{innerproduct}, \gls{linreg}, \gls{lasso}.},
	first={norm},
	type=math,
	text={norm},
        plural={norms}
...
{name={group},
	description={A\index{group} group $(G, \diamond)$ is an algebraic structure \cite[Ch.~1]{artin2016geometric} 
		consisting of a set $G$ and a binary operation $\diamond$ that assigns to any two elements $a, b \in G$ 
		another element $a \diamond b \in \mathcal{G}$. For $G$ to be a group, it must satisfy associativity 
		(i.e., $(a \diamond b) \diamond c = a \diamond (b \diamond c)$ for all $a,b,c \in G$), 
		the existence of an identity element $e \in G$ with $a \diamond e = e \diamond a = a$ for all $a \in G$, 
		and the existence of an inverse $a^{-1} \in G$ for every element $a \in G$ that satisfies $a^{-1} a = a a^{-1} = e$. 
		A group is called abelian or commutative if $a \diamond b = b \diamond a$ for all $a, b \in G$. 
		Basic examples are the additive group $(\mathbb{R}, +)$ and the multiplicative group 
		$(\mathbb{R}\setminus \{0\}, \cdot)$; similarly, $(\mathbb{C}, +)$ and $(\mathbb{C}\setminus \{0\}, \cdot)$ 
		are defined using addition and multiplication on the real (complex) numbers in the usual sense. 
		More complicated group structures exist in \glspl{matrix} \cite{artin2016geometric}, such as rotation \glspl{matrix} 
		useful in \gls{ml} for \gls{dataaug}.
		\\
		See also: \gls{vectorspace}, \gls{subspace}, \gls{inverse}, \gls{field}.},
	first={group},
	plural={groups},
	type=math,
	text={group}
...
{name={field},
	description={A\index{field} field $(\mathbb{F}, +, \cdot)$ is an algebraic structure generalizing the real and complex numbers. 
		Formally, a field consists of a set $\mathbb{F}$ equipped with two binary operations, an addition $\oplus$ 
		and a multiplication $\odot$, where $(\mathbb{F}, \oplus)$ is an abelian \gls{group} with an identity element $0$ 
		and $(\mathbb{F} \setminus \{0\}, \odot)$ is an abelian \gls{group} with an identity element $1$. 
		Furthermore, it satisfies distributivity, i.e.,  $a \odot (b \oplus c) = (a \odot b) \oplus (a \odot c)$ for all $a, b, c \in \mathbb{F}$. 
		The most common examples are $(\mathbb{R}, +, \cdot)$ and $(\mathbb{C}, +, \cdot)$, with addition and multiplication 
		in the usual sense. Fields provide the set of scalars required to define a \gls{vectorspace}. 
		\\
		See also: \gls{group}, \gls{vectorspace}, \gls{subspace}, \gls{euclidspace}, \gls{innerproduct}.},
	first={field},
	plural={fields},
	type=math,
	text={field}
...
{name={linearly dependent},
	description={A subset $\{\va^{(1)}, \,\ldots, \,\va^{(\nrfeatures)}\} \in \mathcal{V}$ 
		of a \gls{vectorspace} $\mathcal{V}$ over a \gls{field} $\mathbb{F}$ is linearly 
		dependent\index{linearly dependent} if it is not \gls{linearlyindep}. 
		This occurs if there exists a set of scalars $\alpha_1, \,\ldots, \,\alpha_{\nrfeatures} \in \mathbb{F}$, 
		not all equal to zero, such that 
		$$\sum_{\featureidx=1}^{\nrfeatures} \alpha_{\featureidx} \va^{(\featureidx)} = \mathbf{0}.$$ 
		Intuitively, this means that at least one \gls{vector} in the set can be expressed as a 
		(nontrivial) linear combination of the others. 
		\\ 
		See also: \gls{vectorspace}, \gls{linearlyindep}, \gls{vector}.},
...
sort={CauchySchwarz},
	description={For any \glspl{vector} $\featurevec, \featurevec'$ in an \gls{innerproduct} space (i.e., a \gls{hilbertspace}) 
		over the \gls{field} of real or complex numbers, the Cauchy-Schwarz inequality\index{Cauchy-Schwarz inequality} 
		provides an upper bound on the (squared) absolute value 
		(on $\mathbb{R}$ or $\mathbb{C}$) of their \gls{innerproduct}. More precisely, it states that
		\[
			|\innerprod{\featurevec}{\featurevec'}|^2 \leq \innerprod{\featurevec}{\featurevec} \innerprod{\featurevec'}{\featurevec'}.
		\]
		  Using the \gls{norm} defined by the \gls{innerproduct}, this can be expressed equivalently as
		$|\innerprod{\featurevec}{\featurevec'}| \leq \norm{\featurevec} \norm{\featurevec'}$.
		Equality holds if and only if $\{\featurevec, \featurevec'\}$ is \gls{linearlydep}. 
		\\
		See also: \gls{linearlyindep}, \gls{firmlynonexpansiveop}, \gls{euclidspace}, \gls{orthogonalitycondition}.},
...
{name={normed space},
	description={A normed space\index{normed space} is a \gls{vectorspace} $\mathcal{V}$ equipped with a \gls{norm} 
		...
		\\
		See also: \gls{vectorspace}, \gls{norm}, \gls{metricspace}, \gls{euclidspace}, \gls{hilbertspace}.},
	text={normed space},
	type=math,
	first={normed space},
	firstplural={normed spaces},
	plural={normed spaces}
}

\newglossaryentry{priordist}
{name={prior distribution},
	description={In the probabilistic\index{prior distribution} analysis of an \gls{mlsystem}, one typically distinguishes 
		between observed \glspl{datapoint} and unknown \glspl{parameter} of a \gls{probmodel}. 
		A prior distribution is a postulated \gls{probdist} over the possible choices of 
		these \glspl{modelparam} before (i.e., prior to) observing any \gls{data} \cite{BishopBook}. 
		Given a \gls{condprobdist} of the \gls{data} given the \glspl{modelparam}, 
		we update the prior to obtain the \gls{posteriordist} of the \glspl{modelparam}.
		\\
		See also: \gls{probdist}, \gls{modelparam}, \gls{posteriordist}. },
	first={prior distribution},
	plural={prior distributions},
	firstplural={prior distributions},
	type=math,
...
sort={posteriordistribution},
	description={In the probabilistic\index{posterior distribution} analysis of an \gls{mlsystem}, 
		one typically distinguishes between observed \glspl{datapoint} and unknown 
		\glspl{parameter} of a \gls{probmodel}. The \gls{posterior} distribution\index{posterior distribution} 
		is the \gls{condprobdist} of the unknown \glspl{modelparam}, 
		given (or conditioned on) the observed \glspl{datapoint} \cite{BishopBook}.
		\\ 
		See also: \gls{condprobdist}, \gls{priordist}, \gls{probdist}.},
	first={posterior distribution},
	plural={posterior distributions},
	firstplural={posterior distributions},
	type=math,
...
\newglossaryentry{conjugateprior}
 {name={conjugate prior},
 	description={A \gls{priordist}\index{conjugate prior} $\probdist^{(\weights)}$ over \glspl{modelparam} 
 		$\weights$ is called conjugate to a \gls{condprobdist} (referred to as the 
		likelihood \gls{model}) $\probdist^{(\featurevec\mid\weights)}$ if the resulting 
 		\gls{posteriordist} $\probdist^{(\weights \mid\featurevec)}$ belongs to 
 		the same parametric family as $\probdist^{(\weights)}$ \cite{barber2012bayesian}, \cite{BishopBook}. 
 		One important example of a conjugate prior and likelihood \gls{model} is 
 		obtained from the \gls{mvndist}. In particular, consider the prior $\probdist^{(\weights)}$ 
 		is a \gls{mvndist} $\mvnormal{\meanvecgeneric}{\covmtxgeneric}$ with some 
 		\gls{mean} $\meanvecgeneric$ and \gls{covmtx} $\covmtxgeneric$. For a 
 		likelihood \gls{model} $\probdist^{(\featurevec\mid\weights)}$ that is a \gls{mvndist} 
 		with \gls{mean} $\weights$ and \gls{covariance} $\covmtxgeneric$, the resulting \gls{posterior} 
 		$\probdist^{(\weights \mid\featurevec)}$ is also a \gls{mvndist} \cite[Appendix B]{BishopBook}.
		\\
 		See also: \gls{condprobdist}, \gls{mvndist}, \gls{probdist}.},
 	first={conjugate prior},
 	plural={conjugate priors},
 	firstplural={conjugate priors},
 	type=math,
...
{name={exponential family},
	description={An\index{exponential family} exponential family is a particular \gls{probmodel} 
		for a given \gls{samplespace} $\featurespace$ of possible \glspl{outcome}. It 
		is defined by the specification of a base \gls{measure} $h(\featurevec)$ and 
		a \gls{vector} of sufficient statistics $\mathbf{T}(\featurevec)$. 
		A distribution belongs to the exponential family if its \gls{pdf} (or \gls{pmf})
		can be written as 
		\[ p(\featurevec ;  \weights) = h(\featurevec)\,\exp\!\big(\weights^{T} \mT(\featurevec) - A(\weights)\big),
  		\]
  		where $\weights$ denotes the natural \gls{parameter} and $A(\weights)$ is the log-partition 
       		\gls{function} ensuring normalization $\int_{\featurevec} p(\featurevec ;  \weights) d\featurevec = 1$.
  		Exponential family distributions arise as \gls{maximum}-\gls{entropy} solutions under constraints 
		on the expected values of the sufficient statistics. Thus, we can view them 
		as the least informative \glspl{probdist} for given \glspl{expectation} of the sufficient 
		statistics \cite[Ch. 12]{coverthomas}. 
		\\ 
		See also: \gls{probdist}, \gls{pdf}, \gls{pmf}.},
