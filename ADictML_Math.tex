...
{name={strongly convex},
	description={A\index{strongly convex} continuously \gls{differentiable}  
		\gls{function} $f: \mathcal{U} \to \mathbb{R}$ with \gls{domain} $\mathcal{U} \subset  \mathbb{R}^d$ 
		is strongly \gls{convex} with coefficient $\sigma$ if 
		$f(\vy) \geq f(\vx) + \nabla f(\vx)\,^{T} (\vy - \vx) + (\sigma/2) \normgeneric{\vy - \vx}{2}^{2}$ 
		 for all $\vx, \vy \in \mathcal{U}$ \cite{nesterov04}, \cite[Sec. B.1.1]{CvxAlgBertsekas}.
					\\ 
...
{name={strictly convex},
	description={A\index{strictly convex} real-valued \gls{function} $f: \mathcal{U} \to \mathbb{R}$ 
		with \gls{domain} $\mathcal{U} \subset  \mathbb{R}^d$ is strictly \gls{convex} if, for any two distinct 
		$\vx,\vy \in \mathcal{U}$ and any $\lambda\in(0,1)$, it satisfies \cite[Definition 3.1.1]{BoydConvexBook}
		\[
