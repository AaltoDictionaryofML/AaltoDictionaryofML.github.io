...
{name={method of multipliers},
 description={The \index{method of multipliers} method of multipliers is an 
              iterative \gls{optmethod} for solving a 
			  constrained \gls{optproblem} of the form \cite{DistrOptStatistLearningADMM}
			  \[
			  \min_{\featurevec \in \mathbb{R}^{\nrfeatures}} \; f(\featurevec)
			  \mbox{ subject to } \mathbf{A} \featurevec  = \mathbf{0}.
			  \]
			  ??? DISCUSS FIXED-POINT FORMULATION based on AUGMENTED LAGRANGIAN ???
			 }, 
 first={method of multipliers},
 type=math,
 text={method of multipliers}
}	
...
{name={Lagrangian},
	description={The Lagrangian\index{Lagrangian} of an \gls{optproblem} of %standard
		...
		\end{align}
		is \cite{BoydConvexBook}
		\begin{equation}
			\nonumber
			L(\weights, \lambda, \nu) \defeq f_0(\weights) + \sum_{i=1}^{k} \lambda_i f_i(\weights) + \sum_{j=1}^{l} \nu_j g_j(\weights).
		\end{equation}
		\\
		See also: \gls{optproblem}, \gls{convexopt}, \gls{dual}.},
	first={Lagrangian},
	text={Lagrangian},
	plural={Lagrangians},
	type=math
...
{name={Lagrange dual problem},
	description={The \gls{lagrangian}\index{Lagrange dual problem} of an \gls{optproblem} of %standard
		the form 
		\begin{align}
			\min_{\weights \in \mathbb{R}^{\nrfeatures}} & f_0(\weights)   \nonumber \\ 
			\mbox{s.t.} \quad & f_{i}(\weights) \leq 0, \quad i=1,\,\ldots,\,k \nonumber \\
		...
		See also: \gls{lagrangian}, \gls{optproblem}, \gls{convexopt}.},
