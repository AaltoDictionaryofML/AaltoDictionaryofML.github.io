...
{name={spectral decomposition},
	description={A \gls{evd} of a \gls{normalmatrix} is called a spectral decomposition \index{spectral decomposition}. It has the special property that the \glspl{eigenvector} of the \gls{normalmatrix} are orthonormal. This also works the other way; any \gls{matrix} that has orthonormal \glspl{eigenvector} is a \gls{normalmatrix}.
...
\newglossaryentry{subspace}{name={subspace},description={A subset of a \gls{vectorspace} $\mathcal{V}$ is a subspace\index{subspace} of $\mathcal{V}$ if it is also a \gls{vectorspace} with respect to the same operations as $\mathcal{V}$. Figure \ref{fig:subspace} illustrates this for the 
\gls{vectorspace} $\mathbb{R}^2$ (over the \gls{field} $\mathbb{R}$).
	...
	\centering
        ...
        \end{tikzpicture}
        \caption{Any $\featurevec \in \mathbb{R}^2$ defines a subspace of $\mathcal{U}_\featurevec \subset \mathbb{R}^2$ by $\mathcal{U}_\featurevec = \{ \lambda \cdot \featurevec \; : \; \lambda \in \mathbb{R}\} \subset \mathbb{R}^2$. For $\featurevec \neq \mathbf{0}$, this corresponds to straight lines through the origin (illustrated by $\mathcal{U}_1$ and $\mathcal{U}_2$), for the zero \gls{vector} $\featurevec =\mathbf{0}$ this leads to the trivial subspace $\{\mathbf{0}\}$ just containing the zero \gls{vector}. \label{fig:subspace}}
    \end{figure}
