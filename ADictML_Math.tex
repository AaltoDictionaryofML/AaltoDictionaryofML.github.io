...
{name={linearly independent},
	description={A subset $\{\va^{(1)}, \,\ldots, \,\va^{(\nrfeatures)}\} \in \mathcal{V}$ 
		...
		\quad \text{ implies } \quad \alpha_{1} = \alpha_{2} = \ldots = \alpha_{\nrfeatures} = 0.$$ 
			\\ 
		See also: \gls{linearlydep}, \gls{vectorspace}, \gls{vector}, \gls{dimension}, \gls{basis}.}, 
...
{name={standard normal random vector}, 
	description={A\index{standard normal random vector} standard normal random \gls{vector} 
		is a \gls{rv} $\featurevec=\big(\feature_{1}, \,\ldots, \,\feature_{\nrfeatures}\big)\,^{T}$ 
	...
	description={An\index{contractive operator} \gls{operator} 
		...
		\begin{equation} 
			...
			\normgeneric{ \fixedpointop \weights\!-\!\fixedpointop \weights'}{}  \leq 
			 \contractfac	\normgeneric{\weights\!-\!\weights'}{} \mbox{ holds for any } \weights,\weights' \in \mathcal{V}.
		\end{equation} 
...
{name={Gaussian random variable (Gaussian RV)}, 
	description={A\index{Gaussian random variable (Gaussian RV)} standard \gls{gaussian} \gls{rv} is a 
		real-valued \gls{rv} $\feature$ with \gls{pdf} \cite{BertsekasProb}, \cite{GrayProbBook}, \cite{papoulis}
		\begin{equation}
			\nonumber
...
{name={indicator function}, 
	description={Consider some process\index{indicator function} that can result in different possible 
		...
		the indicator \gls{function} $\indicatorfunc{\mathcal{A}}$ is a \gls{rv} and it holds that 
...
{name={Kullback–Leibler divergence (KL divergence)}, 
 	description={The\index{Kullback–Leibler divergence (KL divergence)} KL divergence 
	             is a quantitative \gls{measure} 
				 for how different a \gls{probdist} $\probdist^{(1)}$ is from another 
				 ...
				 is well-defined only if $\probdist^{(1)}$ is absolutely 
				 continuous with respect to $\probdist^{(2)}$.
				 \textbf{Special case.} If $\probdist^{(1)}$ 
				 and $\probdist^{(2)}$ are described by \glspl{pmf} $\pmf{x}{\cdot}$ and $\pmf{y}{\cdot}$ 
				 ...
				 \gls{bregmandivergence} generated by the negative entropy function \cite[Sec. 4.3]{CvxBubeck2015}
...
{name={probability simplex},
	description={The\index{probability simplex} \gls{probability} simplex
		...
		represents a \gls{pmf} of a \gls{rv} $y \in \{1,\,\ldots,\,\nrcluster\}$.
		\\
...
{name={entropy},
	description={Entropy\index{entropy} quantifies the \gls{uncertainty} or 
		unpredictability associated with a \gls{rv} \cite{coverthomas}. 
...
{name={Markov's inequality},
	description={Consider a real-valued nonnegative \gls{rv} $x$ for which 
		...
		\gls{expectation} $\expect\{g(x)\}$ with the \gls{indicatorfunc} 
...
{name={Hoeffding's inequality},
	description={\index{Hoeffding's inequality} Hoeffding's inequality \cite{Hoeffding1963} is a fundamental \gls{concentrationinequ} 
		...
		Then, Hoeffding's inequality \cite[Th. 2.2.6]{vershynin2018high} states that
    		...
    		In the context of \gls{ml}, Hoeffding's inequality can be used 
			for the analysis of \gls{erm} or \gls{mab} methods. 
		 \\
 	...
 	description={An\index{non-expansive operator} \gls{operator} 
		...
		not increase distances. In other words,  
 		\begin{equation} 
			...
			\leq 	\normgeneric{\weights - \weights'}{} \mbox{ for any } \weights, \weights' \in \mathcal{V}. 
 		\end{equation} 
...
{name={firmly non-expansive operator},
 	description={An\index{firmly non-expansive operator} \gls{operator} 
		...
		By the \gls{cauchyschwarzinequ}, any firmly \gls{nonexpansiveop} is necessarily 
            also a \gls{nonexpansiveop} (with respect to the $\ell_2$-\gls{norm}, or \gls{euclidnorm}). 
		Every \gls{fixedpointiter} that uses a firmly \gls{nonexpansiveop} converges 
		to a \gls{fixedpoint} of the \gls{operator} \cite[Corollary 5.16]{Bauschke:2017}.
		\\ 
 		See also: \gls{fixedpointiter}, \gls{contractop}.},
 	type=math, 
...
{name={fixed-point iteration},
	description={A\index{fixed-point iteration} fixed-point \gls{iteration} is an iterative method 
		...
		The \gls{operator} $\fixedpointop$ is a \gls{contractop} (with respect to the \gls{euclidnorm}) if, for some $\contractfac \in [0,1)$,
		\begin{equation} 
			\nonumber
		...
		\end{figure} 
		See also: \gls{contractop}, \gls{banachfixedpoint}, \gls{proxop}, \gls{bellmanoperator}, \gls{policyevaluation}, \gls{valueiteration}.},
...
{name={alternating direction method of multipliers (ADMM)},
	description={The\index{alternating direction method of multipliers (ADMM)} 
		...
		In particular, the ADMM can be used to solve an \gls{optproblem} of the form 
		\begin{align}
...
{name={Lagrangian},
	description={Consider a constrained \gls{optproblem} of the form
		...
		\glspl{optproblem} and the design of \glspl{optmethod}. For example, the Lagrangian allows us
		to define a \gls{dualproblem}, which provides a lower bound for the optimal value of a constrained 
		\gls{optproblem}. This lower bound, in turn, can be used to construct a \gls{stopcrit} for 
...
{name={dual problem},
	description={Consider a constrained \gls{optproblem}, which we 
 		refer to as primal problem in what follows, of the form	
    		\begin{align} 
			\min_{\vx \in \mathbb{R}^{\nrfeatures}} & f(\vx) \nonumber \\ 
     			\mbox{s.t.} \quad & g_i(\vx)\le 0, \quad i=1,\,\ldots,\,k \nonumber \\ 
			& h_j(\vx)=0, \quad j=1,\,\ldots,\,l. \nonumber
        		\end{align}
		...
		$$q({\bm \lambda},{\bm \nu}) \leq f(\vx).$$
		Making this lower bound maximally tight amounts to the \gls{optproblem}
              	\[
...
{name={supporting hyperplane},
	description={Let\index{supporting hyperplane} $\mathcal{C} \subseteq \mathbb{R}^d$ be a non-empty set.
		...
		\[  
			...
			\quad \va \in \mathbb{R}^d \setminus \{ \mathbf{0} \}, \quad \ b \in \mathbb{R}
		\]
...
{name={gradient},
	description={For\index{gradient} a real-valued \gls{function} 
		...
		$\nabla f(\weights) = [\frac{\partial f}{\partial \feature_1}, \,\dots, \,\frac{\partial f}{\partial \feature_{\featuredim}}]^T$. 
...
{name={strongly convex},
	description={A\index{strongly convex} continuously \gls{differentiable}  
		\gls{function} $f: \mathcal{U} \to \mathbb{R}$ with a \gls{domain} $\mathcal{U} \subset  \mathbb{R}^d$ 
...
{name={smooth},
	description={A\index{smooth} real-valued \gls{function} $f: \mathbb{R}^{\dimlocalmodel} \rightarrow \mathbb{R}$ 
		...
		$$\normgeneric{\nabla f(\weights) - \nabla f(\weights')}{} \leq \beta \normgeneric{\weights - \weights'}{} \mbox{, \quad for any } \weights,\weights' \in \mathbb{R}^{\dimlocalmodel}.$$ 
 ...
 {name={mirror descent},
 	description={Mirror descent\index{mirror descent} is an iterative \gls{optmethod}  
		...
		Mirror descent replaces the squared \gls{euclidnorm} with a 
 		...
 		\]
 		See also: \gls{gradstep}, \gls{innerproduct}, \gls{zerogradientcondition}, \gls{bregmandivergence}, \gls{proxop}.},
...
{name={Bregman divergence},
	description={The Bregman divergence\index{Bregman divergence} induced by a 
		...
		\\
		See also: \gls{gradient}, \gls{gradstep}, \gls{innerproduct}, \gls{proxop}.},
...
{name={average node degree},
	description={The average node degree\index{average node degree} $\avgnodedegree$ of a weighted 
...
{name={fixed-point characterization},
	description={The solution of \glspl{optproblem} arising in \gls{ml} 
				 ...
				 Here, $\lrate > 0$ is an aribitrary positive constant and 
				 ...
				 to compute a solution of the underlying \gls{optproblem}. For the 
...
{name={node degree},
	description={The degree\index{node degree} $\nodedegree{\nodeidx}$ of a 
				 ...
				 of the weights of all edges connected to node $\nodeidx$, i.e.,
...
{name={norm},
	description={A norm\index{norm} is a \gls{function} that maps each (\gls{vector}) element 
        of a \gls{vectorspace} to a nonnegative real number. Formally, a \gls{norm} 
        ...
        Norms can also be used to measure distances between \glspl{vector}, as they define a \gls{metric} by $\metric{\featurevec}{\featurevec'} = \normgeneric{\featurevec - \featurevec'}{}$,
        making $(\mathcal{X}, \metric{\cdot}{\cdot})$ a \gls{metricspace}.
	A prominent family of norms used in \gls{ml} are the $\ell_p$-norms for $p \geq 1$, 
        ...
        by the geometry of their respective unit spheres in Figure \ref{fig:norms_unit_balls_dict}.
...
{name={linearly dependent},
	description={A subset $\{\va^{(1)}, \,\ldots, \,\va^{(\nrfeatures)}\} \in \mathcal{V}$ 
		of a \gls{vectorspace} $\mathcal{V}$ over a \gls{field} $\mathbb{F}$ is linearly dependent
            \index{linearly dependent} if it is not \gls{linearlyindep}. 
            This occurs if there exists a set of scalars $\alpha_1, \ldots, \alpha_{\nrfeatures} \in \mathbb{F}$, 
		not all equal to zero, such that 
		$$\sum_{\featureidx=1}^{\nrfeatures} \alpha_{\featureidx} \va^{(\featureidx)} = \mathbf{0}.$$ 
		Intuitively, this means at least one vector in the set can be expressed as a 
		(non-trivial) linear combination of the others. \\ 
		See also: \gls{linearlyindep}, \gls{vectorspace}, \gls{vector}.},
...
{name={Cauchy-Schwarz inequality},
	description={For any \glspl{vector} $\featurevec, \featurevec'$ in an \gls{innerproduct} space (i.e., a \gls{hilbertspace}) over the \gls{field} of real or complex numbers, 
        the Cauchy-Schwarz inequality\index{Cauchy-Schwarz inequality} provides an upper bound on the (squared) absolute value 
...
{name={normed space},
	description={A normed space\index{normed space} is a \gls{vectorspace} $\mathcal{V}$ equipped with a \gls{norm}                $\normgeneric{\cdot}{}$. Formally, it is denoted as the pair $(\mathcal{V}, \normgeneric{\cdot}{})$. 
		...
		In \gls{ml}, normed spaces are a common framework for \glspl{featurespace} and \gls{paramspace}, most typically as     the \glspl{euclidspace} $\mathbb{R}^\featuredim$ with the \gls{euclidnorm}.  \\
