...
\begin{align}
	...
	\biasterm^2 \quad\quad &\parbox{.75\textwidth}{
		...
		that are modeled as the \gls{realization} of \gls{rv}s. Since the \gls{data} is a \gls{realization} 
	...
	\varianceterm \quad\quad &\parbox{.75\textwidth}{The \gls{variance} of the learned 
		...
		The method is trained on \gls{datapoint}s that are modeled as the \gls{realization} 
...
\begin{align}
	...
	\weights  \quad\quad & \parbox{.75\textwidth}{A parameter vector $\weights = \big(\weight_{1},\ldots,\weight_{\featuredim}\big)^{T}$ 
		of a model, e.g., the \gls{weights} of a \gls{linmodel} or in an \gls{ann}.}     \nonumber \\[2mm] \hline \nonumber\\[-5mm]
