...
{name={dimensionality reduction},
	description={Dimensionality reduction\index{dimensionality reduction} refers 
		...
		$h: \mathbb{R}^{\featuredim} \rightarrow \mathbb{R}^{\featuredim'}$ 
...
{name={feature learning},
	description={Consider an \gls{ml} application with \glspl{datapoint} characterized by 
		raw \glspl{feature} $\vx \in \mathcal{X}$. \Gls{feature} learning\index{feature learning} 
		refers to the task of learning a map 
		$${\bf \Phi}: \mathcal{X} \rightarrow \mathcal{X}': \vx \mapsto \vx',$$ 
		that reads in raw \glspl{feature} $\vx \in \mathcal{X}$ of a \gls{datapoint} and delivers new 
		\glspl{feature} $\vx' \in \mathcal{X}'$ from a new \gls{featurespace} $\mathcal{X}'$. 
		...
		\!\rightarrow\! \mathbb{R}^{d'}\!:\!\vx'\!:=\!\mathbf{F} \vx \mbox{ with some } \mathbf{F} \!\in\! \mathbb{R}^{d' \times d} \big\}.$$ \Gls{pca} measures the usefulness of a specific map ${\bf \Phi}(\vx)= \mathbf{F} \vx$ 
	by the \gls{minimum} linear reconstruction error incurred on a \gls{dataset} such that 
$$ \min_{\mathbf{G} \in \mathbb{R}^{d \times d'}} \sum_{r=1}^{m} \normgeneric{\mathbf{G} \mathbf{F} \vx^{(r)} - \vx^{(r)}}{2}^{2}.$$ 
			\\ 
...
{name={vertical federated learning (VFL)},
	description={
		...
		\[
		\mathcal{D}^{(\mathrm{global})} := \left\{ \left(\vx^{(1)}, y^{(1)}\right), \ldots, \left(\vx^{(m)}, y^{(m)}\right) \right\}.
		\]
		We denote by $\vx^{(r)} = \big( x^{(r)}_{1}, \ldots, x^{(r)}_{\featuredim'} \big)^{T}$, for $r=1,\ldots,m$, 
		...
		\[
		\vx^{(i,r)} = \big( x^{(r)}_{\featureidx_{1}}, \ldots, x^{(r)}_{\featureidx_{\featuredim}} \big)^{T}.
		\]
		Some of the \glspl{device} might also have access to the \glspl{label} $y^{(r)}$, for $r=1,\ldots,m$, 
...
{name={learning task}, plural={learning tasks}, description=
	{Consider\index{learning task} a \gls{dataset} $\mathcal{D}$ constituted by several \glspl{datapoint}, each of them 
	 characterized by \glspl{feature} $\vx$. For example, the \gls{dataset} $\mathcal{D}$ 
	 might be constituted by the images of a particular database. Sometimes it might be useful 
	 to represent a \gls{dataset} $\mathcal{D}$, along with the choice of \glspl{feature}, by a \gls{probdist} $p(\vx)$. 
...
{name={local interpretable model-agnostic explanations (LIME)},description={
		...
		a trained \gls{model} (or learned \gls{hypothesis}) $\widehat{h} \in \mathcal{H}$, 
		which maps the \gls{featurevec} of a \gls{datapoint} to the \gls{prediction} $\widehat{y}= \widehat{h}$. 
		LIME is a technique for explaining 
		the behavior of $\widehat{h}$, locally around a \gls{datapoint} with \gls{featurevec} $\vx^{(0)}$ \cite{Ribeiro2016}. 
		The \gls{explanation} is given in the form of a local approximation $g \in \mathcal{H}'$ of $\widehat{h}$ (see Fig. \ref{fig_lime}). 
		...
		\gls{featurevec} $\vx$ close to $\vx^{(0)}$ and the (pseudo-)\gls{label} $\widehat{h}(\vx)$. 
		...
		\begin{tikzpicture}
			\begin{axis}[
  			...
  			\addplot[blue, thick, domain=0:6] {2 + sin(deg(x))} node[pos=0.85, above right,yshift=3pt] {$\widehat{h}(\vx)$};
			 % Feature value x0
  			\addplot[dashed, gray] coordinates {(3,0) (3,6)};
			% Piecewise constant local approximation g(x)
  			\addplot[red, thick, domain=2.5:3.5] {2 + sin(deg(3))} node[pos=0.9, above] {$g(\vx)$};
			% Optional: mark the point of approximation
  			\addplot[mark=*] coordinates {(3, {2 + sin(deg(3))})};
			\node at (axis cs:3,-0.3) {$\vx^{(0)}$};
			\end{axis}
		  \end{tikzpicture}
		\end{center}
		\caption{To explain a trained \gls{model} $\widehat{h} \in \mathcal{H}$, around a 
		given \gls{featurevec} $\vx^{(0)}$, we can use a local approximation $g \in \mathcal{H}'$. }
		\label{fig_lime}
		\end{figure}
...
\newglossaryentry{linmodel}{name={linear model}, plural={linear models},
	description={Consider\index{linear model} \glspl{datapoint}, each characterized by a numeric \gls{featurevec} 
		$\vx \in \mathbb{R}^{d}$. A linear \gls{model} is 
		a \gls{hypospace} which consists of all linear maps such that 
	\begin{equation} 
		\label{equ_def_lin_model_hypspace_dict}
		\hypospace^{(\featuredim)} := \left\{ h(\vx)= \vw^{T} \vx: \vw \in \mathbb{R}^{\featuredim} \right\}. 
	\end{equation} 
	...
	\gls{prediction} $h(\vx)$. The design choice of $\featuredim$ is guided by \gls{compasp} 
...
\newglossaryentry{proxop}{name={proximal operator},description={Given\index{proximal operator} a \gls{convex} 
		...
		$${\rm\bf prox}_{f(\cdot),\rho}(\vw):= \argmin_{\vw' \in \mathbb{R}^{d}} \bigg[ f(\vw')\!+\!(\rho/2) \left\Vert  {\vw- \vw'} \right\Vert_{2}^{2}\bigg] \mbox{ with } \rho > 0. $$ 
...
\newglossaryentry{mvndist}{name ={multivariate normal distribution}, 
	description={The\index{multivariate normal distribution} multivariate normal distribution 
		...
		$\vx \in \mathbb{R}^{\featuredim}$ \cite{BertsekasProb}, \cite{GrayProbBook}, \cite{Lapidoth09}. 
		Each family member (i.e., a specific \gls{probdist}) is specified by its \gls{mean} ${\bf m}$ and  
		\gls{covmtx} ${\bf C}$. If the \gls{covmtx} is invertible, the \gls{probdist} of $\vx$ can 
		be written as 
		$$p(\vx) \propto \exp\bigg(-(1/2) \big( \vx - {\bf m} \big)^{T} {\bf C}^{-1} \big( \vx - {\bf m} \big) \bigg).$$
					\\ 
...
\newglossaryentry{zerooneloss}{name={$\bf 0/1$ loss},
	description={The $0/1$ \gls{loss}\index{$0/1$ loss} $\lossfunczo{\left( \vx,y \right)}{h}$ 
		measures the quality of a \gls{classifier} $h(\vx)$ that delivers a 
		\gls{prediction} $\hat{\truelabel}$ (e.g., via thresholding \eqref{equ_def_threshold_bin_classifier_dict}) 
		for the \gls{label} $y$ of a \gls{datapoint} with \glspl{feature} $\vx$. It is equal to $0$ if 
		the \gls{prediction} is correct, i.e., 
	$\lossfunczo{\left( \vx,y \right)}{h}=0$ when $\hat{\truelabel}=y$. It is 
	equal to $1$ if the \gls{prediction} is wrong, i.e., $\lossfunczo{\left( \vx,y \right)}{h}=1$ 
	when $\hat{\truelabel}\neqy$.
				\\ 
...
\newglossaryentry{gaussrv}{name={Gaussian random variable (Gaussian RV)}, plural={Gaussian RVs}, description={
		...
		\gls{mean} $\mu$ and \gls{variance} $\sigma^2$ via $x' := \sigma (x+\mu)$. The \gls{probdist} of a 
		Gaussian \gls{rv} is referred to as normal distribution, denoted $\mathcal{N}(\mu,\sigma)$.  \\ 
		A Gaussian random vector $\vx \in \mathbb{R}^{d}$ with 
		\gls{covmtx} $\mathbf{C}$ and \gls{mean} ${\bm \mu}$ can be constructed via 
		$\vx := \mathbf{A} \big( {\bf z} + {\bm \mu} \big)$. Here, ${\bf A}$ 
...
{name={Gaussian process (GP)},
  description={A \index{Gaussian Process (GP)}GP is a collection of \glspl{rv} 
  	$\{f(\vx)\}_{\vx \in \mathcal{X}}$ indexed by input values $\vx$ 
  	from some input space $\mathcal{X}$, such that for any finite subset 
  	$\vx^{(1)}, \ldots, \vx^{(m)} \in \mathcal{X}$, 
  	the corresponding \glspl{rv} $f(\vx^{(1)}, \ldots, \vx^{(m)}$ have a joint 
  	multivariate Gaussian distribution:
  	\[
  	\left( f(\vx^{(1)}, \ldots, \vx^{(m)} \right) \sim \mathcal{N}(\boldsymbol{\mu}, \mathbf{K}).
  	\]
  	For a fixed input space $\mathcal{X}$, a GP is fully specified (or parametrized) by 
  	\begin{itemize}
  		\item a \gls{mean} function $\mu(\vx) = \expect\{ f(\vx)\}$
  		\item and a covariance function $K\big(\vx,\vx'\big)= \expect\{ \big(f(\vx)-\mu(\vx)\big) \big(f(\vx')-\mu(\vx')\big) \big\}$.
  	\end{itemize}
  	\text{Example:} We can interpret the temperature distribution across Finland (at a specific 
  	point in time) as the \gls{realization} of a GP $f(\vx)$, where each input $\vx = (\text{lat}, \text{lon})$ 
  	denotes a geographic location. Temperature observations from \gls{fmi} weather stations provide 
  	samples of $f(\vx)$ at specific locations (see Fig.\ \ref{fig_gp_FMI}). A GP allows us to 
...
description={The squared 
		...
		\gls{hypothesis} $h$ when predicting a numeric \gls{label} $y \in \mathbb{R}$ 
		from the \glspl{feature} $\vx$ of a \gls{datapoint}. It is 
	defined as 
\begin{equation} 
	\nonumber
%	\label{equ_squared_loss_gls}
	L\left((\vx,y),h \right) := \big(y - \underbrace{h(\vx)}_{=\hat{\truelabel}} \big)^{2}. 
\end{equation} 
			\\ 
...
{name=differential privacy (DP),
  description={Consider\index{differential privacy (DP)} some \gls{ml} method $\mathcal{A}$ 
  	...
  	used for \gls{erm}) and delivers some output $\mathcal{A}(\mathcal{D})$. The output 
  	...
  	of the output $\mathcal{A}(\mathcal{D})$ does not change too much if the \gls{sensattr} 
  	of one \gls{datapoint} in the \gls{trainset} is changed. Note that DP 
  	builds on a \gls{probmodel} for an \gls{ml} method, i.e., we interpret its output $\mathcal{A}(\mathcal{D})$ 
...
{name={stability},
	description={
		...
		\gls{dataset} $\mathcal{D}$ (e.g., a \gls{trainset}) to an output $\mathcal{A}(\mathcal{D})$. The output 
		$\mathcal{A}(\mathcal{D})$ can be the learned \gls{modelparams} or the \gls{prediction} delivered 
		by the trained \gls{model} for a specific \gls{datapoint}. Intuitively, $\mathcal{A}$ is 
		stable if small changes in the input \gls{dataset} $\mathcal{D}$ lead to small changes in the 
		...
		\gls{ml} method $\mathcal{A}$ should return the same (or very similar) output $\mathcal{A}(\mathcal{D})$ 
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				\end{axis}
			\end{tikzpicture}
			\caption{Three \glspl{dataset} $\mathcal{D}^{(*)}$, $\mathcal{D}^{(\square)}$, and $\mathcal{D}^{(\triangle)}$, 
...
{name={privacy protection},
	 ...
	 in a \gls{dataset} $\mathcal{D}$ and delivers some output $\mathcal{A}(\mathcal{D})$. The output 
	 could be the learned \gls{modelparams} $\widehat{\vw}$ or the \gls{prediction} 
	 $\hat{\hypothesis}(\vx)$ obtained for a specific \gls{datapoint} with \glspl{feature} 
	 $\vx$. Many important \gls{ml} applications involve \glspl{datapoint} 
		representing humans. Each \gls{datapoint} is characterized by \glspl{feature} $\vx$, 
		potentially a \gls{label} $y$, and a \gls{sensattr} $s$ (e.g., a recent medical diagnosis). 
		Roughly speaking, privacy protection means that it should be impossible to infer, from the output $\mathcal{A}(\mathcal{D})$, 
		any of the \glspl{sensattr} of \glspl{datapoint} in $\mathcal{D}$. Mathematically, privacy protection requires non-invertibility 
...
\newglossaryentry{privleakage}
{
	...
	\gls{dataset} $\mathcal{D}$ and delivers some output, such as the \glspl{prediction} 
	...
	a \gls{datapoint} (which might be a human) of $\mathcal{D}$. Based on a \gls{probmodel} 
...
\newglossaryentry{mean}
{
	...
	description={The \index{mean} mean of an \gls{rv} $\vx$, taking 
 values in an \gls{euclidspace} $\mathbb{R}^{d}$, is its 
 \gls{expectation} $\expect\{\vx\}$. It is defined as the Lebesgue 
 integral of $\vx$ with respect to the underlying \gls{probdist} $P$ (e.g., see \cite{BillingsleyProbMeasure} or \cite{RudinBookPrinciplesMatheAnalysis}), i.e.,
\[
\expect\{\vx\} = \int_{\mathbb{R}^{d}} {\bf x} \, \mathrm{d}P({\bf x}).
\] 
...
{
	name={variance},
	description={The\index{variance} variance of a real-valued \gls{rv} $x$ is defined as the \gls{expectation} 
		$\expect\big\{ \big( x - \expect\{x \} \big)^{2} \big\}$ of the squared difference between $x$ 
		and its \gls{expectation} $\expect\{x \}$. We extend this definition to vector-valued \glspl{rv} $\vx$ 
		as $\expect\big\{ \big\| \vx - \expect\{\vx \} \big\|_{2}^{2} \big\}$.
					\\ 
...
\newglossaryentry{nn}
{
	...
	description={NN\index{nearest neighbor (NN)} methods learn a \gls{hypothesis} 
		$h: \mathcal{X} \rightarrow \mathcal{Y}$ whose function value $h(\vx)$ 
...
\newglossaryentry{bias}
{
	...
	description={Consider\index{bias} an \gls{ml} method using a parametrized \gls{hypospace} $\mathcal{H}$. 
		It learns the \gls{modelparams} $\vw \in \mathbb{R}^{d}$ using the \gls{dataset} $$ \mathcal{D}=\big\{ \pair{\vx^{(r)}}{y^{(r)}} \big\}_{r=1}^{m}.$$ 
		To analyze the properties of the \gls{ml} method, we typically interpret the \glspl{datapoint} as \glspl{realization} 
		of \gls{iid} \glspl{rv}, $$ y^{(r)} = h^{(\overline{\vw})}\big( \vx^{(r)} \big) + \bm{\varepsilon}^{(r)}, r=1,\ldots,m.$$ 
		We can then interpret the \gls{ml} method as an estimator $\widehat{\vw}$ 
		computed from $\mathcal{D}$ (e.g., by solving \gls{erm}). The (squared) bias incurred by the estimate $\widehat{\vw}$ 
		is then defined as $B^{2} := \big\| \expect \{ \widehat{\vw}  \}- \overline{\vw}\big\|_{2}^{2}$.
					\\ 
...
{name={classification},
 description={Classification\index{classification} is the task of determining a 
 	discrete-valued label $y$ for a given \gls{datapoint}, based solely on its 
 	features $\vx$. The label $y$ belongs to a finite set, such as 
...
{
	name={classifier},
	description={A classifier\index{classifier} is a \gls{hypothesis} (i.e., a map) $h(\vx)$ 
		used to predict a \gls{label} taking values from a finite \gls{labelspace}. We might use the 
		function value $h(\vx)$ itself as a \gls{prediction} $\hat{\truelabel}$ for 
		the \gls{label}. However, it is customary to use a map $h(\cdot)$ that delivers 
		...
		we might use a real-valued \gls{hypothesis} map $h(\vx) \in \mathbb{R}$ 
		as a classifier. A \gls{prediction} $\hat{\truelabel}$ can then be obtained via thresholding,  
		 \begin{equation} 
		 	\label{equ_def_threshold_bin_classifier_dict}
		 	\hat{\truelabel} =1   \mbox{ for } h(\vx)\!\geq\!0 \mbox{ and } 	\hat{\truelabel} =-1  \mbox{ otherwise.}
	 		\end{equation}
...
{name={empirical risk},
  description={The empirical \gls{risk}\index{empirical risk} $\emprisk{h}{\mathcal{D}}$ 
  	of a \gls{hypothesis} on a \gls{dataset} $\mathcal{D}$ is the average \gls{loss} incurred 
  	by $h$ when applied to the \glspl{datapoint} in $\mathcal{D}$.
				\\ 
...
{name={node degree},
	description={The degree\index{node degree} $d^{(i)}$ of a node $i \in \mathcal{V}$ 
		in an undirected \gls{graph} is the number of its \gls{neighbors}, i.e., $d^{(i)} := \big|\mathcal{N}^{(i)}\big|$.
					\\ 
...
{name={optimism in the face of uncertainty},
	...
	$\big[ l^{(\vw)},  u^{(\vw)} \big]$ for each choice $\vw$ for the \gls{modelparams}.
		One simple construction is $l^{(\vw)} := f(\vw) - \sigma/2$, $u^{(\vw)} := f(\vw)+ \sigma/2$, 
	...
	according to the most favorable - yet still plausible - value $\tilde{f}(\vw) :=  l^{(\vw)}$ 
...
{name={federated learning network (FL network)},
	description={An \gls{fl} network\index{federated learning network (FL network)} is an 
		...
		and, in turn, train a \gls{localmodel}. \Gls{fl} methods learn a local \gls{hypothesis} $\hypothesis^{(i)}$, for 
...
{name={risk},
	description={Consider\index{risk} a \gls{hypothesis} $h$ used to predict the \gls{label} 
		$y$ of a \gls{datapoint} based on its \glspl{feature} $\vx$. We measure 
		the quality of a particular \gls{prediction} using a \gls{lossfunc} $L\left((\vx,y),h \right)$. 
		If we interpret \glspl{datapoint} as the \glspl{realization} of \gls{iid} \glspl{rv}, 
		also the $L\left((\vx,y),h \right)$ becomes the \gls{realization} 
		of an \gls{rv}. The \gls{iidasspt} allows us to define the risk of a \gls{hypothesis} 
		as the expected \gls{loss} $\expect \big\{L\left((\vx,y),h \right) \big\}$. 
		Note that the risk of $h$ depends on both the specific choice for the \gls{lossfunc} and the 
...
%		quantitative measure of the reliability of a \gls{classification} in the form of the value $|h(\vx)|$. 
...
{name={transparency},
	description={Transparency\index{transparency} is a fundamental requirement for 
		...
		provides a quantitative measure of \gls{classification} reliability through the value $|h(\vx)|$. 
...
{name={baseline},
    ...
    approach to constructing a baseline is via a \gls{probmodel}. In many cases, given a \gls{probmodel} $p(\vx,y)$,  
    ...
    of the \gls{bayesestimator} for the \gls{label} $y$ of a \gls{datapoint}, given
    its \glspl{feature} $\vx$. Note that, for a given choice of \gls{lossfunc}, the 
    \gls{bayesestimator} (if it exists) is completely determined by the \gls{probdist} $p(\vx,y)$ \cite[Ch. 4]{LC}. 
    However, computing the \gls{bayesestimator} and \gls{bayesrisk} presents two 
    main challenges:
    \begin{enumerate}[label=\arabic*)]
    	\item The \gls{probdist} $p(\vx,y)$ is unknown and 
    needs to be estimated.
    	\item Even if $p(\vx,y)$ is known, 
    it can be computationally too expensive to compute the \gls{bayesrisk} exactly \cite{cooper1990computational}. 
   \end{enumerate}
A widely used \gls{probmodel} is the \gls{mvndist} $\left( \vx,y \right) \sim \mathcal{N}({\bm \mu},{\bm \Sigma})$ 
...
\gls{mean} $\mu_{y|\vx}$ of the \gls{label} $y$, given the 
\glspl{feature} $\vx$ \cite{LC}, \cite{GrayProbBook}. The corresponding \gls{bayesrisk} 
is given by the posterior \gls{variance} 
$\sigma^{2}_{y|\vx}$ (see Fig. \ref{fig_post_baseline_dict}).
	\begin{figure}[H]
		\begin{center}
		\begin{tikzpicture}
			% Axes
			...
			\node[anchor=south] at ([yshift=-5pt] \gaussiancenter,2.5) {\small $\mu_{y|\vx}$};
			% Double arrow indicating the variance
			\draw[<->,thick] (\gaussiancenter-1,1) -- (\gaussiancenter+1,1.0);
			\node[anchor=west] at ([yshift=2pt] \gaussiancenter,1.2) {\small $\sigma_{y|\vx}$};
  ...
  % h(x) label for the first cross
  			\node[anchor=north] at (0.5,-0.2) {\small $\hat{\hypothesis}(\vx)$};
		  \end{tikzpicture}
		\end{center}
		\caption{If the \glspl{feature} and the \gls{label} of a \gls{datapoint} are drawn from a \gls{mvndist}, we 
		can achieve the \gls{minimum} \gls{risk} (under \gls{sqerrloss}) by using the \gls{bayesestimator} $\mu_{y|\vx}$ 
		to predict the \gls{label} $y$ of a \gls{datapoint} with \glspl{feature} $\vx$. The corresponding 
		\gls{minimum} \gls{risk} is given by the posterior \gls{variance} $\sigma^{2}_{y|\vx}$. We can use 
...
{name={spectral clustering},
	description={Spectral \gls{clustering}\index{spectral clustering} is a particular instance of 
		...
		to construct \glspl{featurevec} $\vx^{(i)} \in \mathbb{R}^{\featuredim}$ 
...
%					\draw[dashed, gray] (0, 1) -- (1, 1);
    		...
    		\begin{minipage}{0.4\textwidth}
				...
				\caption{\label{fig_lap_mtx_specclustering_dict} {\bf Top.} Left: An undirected \gls{graph} 
					...
					$\vx^{(i)} = \big( v^{(1)}_{i},v^{(2)}_{i} \big)^{T}$. 
...
{name={estimation error},
	description={Consider\index{estimation error} \glspl{datapoint}, each with \gls{featurevec} $\vx$ and \gls{label} 
		$y$. In some applications, we can model the relation between the \gls{featurevec} and the \gls{label}
		of a \gls{datapoint} as $y = \bar{h}(\vx) + \varepsilon$. Here, we 
		use some true underlying \gls{hypothesis} $\bar{h}$ and a noise term $\varepsilon$ 
		which summarizes any modeling or labeling errors. The estimation error incurred by an \gls{ml} 
		method that learns a \gls{hypothesis} $\widehat{h}$, e.g., using \gls{erm}, is defined as 
		$\widehat{h}(\vx) - \bar{h}(\vx)$, for some \gls{featurevec}. 
...
{name={mean squared estimation error (MSEE)},
	description={Consider\index{mean squared estimation error (MSEE)} an \gls{ml} method that 
		learns \gls{modelparams} $\widehat{\vw}$ based on some \gls{dataset} $\mathcal{D}$. 
		If we interpret the \glspl{datapoint} in $\mathcal{D}$ as \gls{iid} \glspl{realization} of an \gls{rv} $\vz$, 
		we define the \gls{esterr} $\Delta \vw := \widehat{w} - \overline{\vw}$. 
...
\newglossaryentry{acc}
{name={accuracy},
	description={Consider\index{accuracy} \glspl{datapoint} characterized by \glspl{feature} $\vx \in \mathcal{X}$ and 
		a categorical label $y$ which takes on values from a finite \gls{labelspace} $\mathcal{Y}$. The 
		accuracy of a \gls{hypothesis} $h: \mathcal{X} \rightarrow \mathcal{Y}$, when applied 
		to the \glspl{datapoint} in a \gls{dataset} $\mathcal{D} = \big\{ \big(\vx^{(1)}, y^{(1)} \big), \ldots, \big(\vx^{(m)},y^{(m)}\big) \big\}$, 
		is then defined as $1 - (1/m)\sum_{r=1}^{m} \lossfunczo{\big(\vx^{(r)},y^{(r)}\big)}{h}$ using the \gls{zerooneloss} $L^{(0/1)}\left(\cdot,\cdot \right)$.
					\\ 
...
{name={expert},
	description={\gls{ml}\index{expert} aims to learn a \gls{hypothesis} $h$ that accurately predicts the \gls{label} 
		...
		An alternative approach to obtaining a \gls{baseline} is to use the \gls{hypothesis} $h'$ learned 
		by an existing \gls{ml} method. We refer to this \gls{hypothesis} $h'$ as an expert \cite{PredictionLearningGames}. \Gls{regret} minimization methods learn a \gls{hypothesis}
...
{name={regret},
	description={The regret\index{regret} of a \gls{hypothesis} $h$ relative to 
		another \gls{hypothesis} $h'$, which serves as a \gls{baseline}, 
		is the difference between the \gls{loss} incurred by $h$ and the \gls{loss} 
		incurred by $h'$ \cite{PredictionLearningGames}. 
		The \gls{baseline} \gls{hypothesis} $h'$ is also referred to as an \gls{expert}.
					\\ 
...
{name={hypothesis},
	description={A\index{hypothesis} hypothesis refers to a map (or function) $h: \mathcal{X} \rightarrow \mathcal{Y}$ from the 
		\gls{featurespace} $\mathcal{X}$ to the \gls{labelspace} $\mathcal{Y}$. 
		Given a \gls{datapoint} with \glspl{feature} $\vx$, we use a hypothesis map $h$
		to estimate (or approximate) the \gls{label} $y$ using the \gls{prediction}  
		$\hat{y} = h(\vx)$. \Gls{ml} is all about learning (or finding) a 
		hypothesis map $h$ such that $y \approx h(\vx)$ 
		for any \gls{datapoint} (having \glspl{feature} $\vx$ and \gls{label} $y$).
					\\ 
...
{name={prediction}, plural={predictions},
	description={A\index{prediction} prediction is an estimate or approximation for some 
		quantity of interest. \Gls{ml} revolves around learning or finding a \gls{hypothesis} map $h$ 
		that reads in the \glspl{feature} $\vx$ of a \gls{datapoint} and delivers a prediction 
		$\widehat{y} := h(\vx)$ for its \gls{label} $y$.
					\\ 
...
{name={histogram},
	description={Consider\index{histogram} a \gls{dataset} $\mathcal{D}$ that consists of $m$ \glspl{datapoint} 
		...
		the corresponding fraction of \glspl{datapoint} in $\mathcal{D}$ that fall into this 
...
{name={bootstrap},
	description={For\index{bootstrap} the analysis of \gls{ml} methods, it is often useful to interpret 
		a given set of \glspl{datapoint} $\mathcal{D} = \big\{ \vz^{(1)},\ldots,\vz^{(m)}\big\}$ 
		...
		\gls{histogram} of $\mathcal{D}$ as an estimator for the underlying \gls{probdist} $p(\vz)$. 
				\\
...
{name={positive semi-definite (psd)},
	...
	{A\index{positive semi-definite (psd)} (real-valued) symmetric matrix $\mathbf{Q} = \mathbf{Q}^{T} \in \mathbb{R}^{d \times d}$ 
	 is referred to as psd if $\vx^{T} \mathbf{Q} \vx \geq 0$ for every vector $\vx \in \mathbb{R}^{d}$. 
	 ...
	 (with $K(\vx,\vx') = K(\vx',\vx)$)
	 as follows: For any finite set of \glspl{featurevec} $\vx^{(1)},\dots,\vx^{(m)}$, 
	 the resulting matrix $\mathbf{Q} \in \mathbb{R}^{m \times m}$ with 
	entries $Q_{r,r'} = \kernelmap{\vx^{(r)}}{\vx^{(r')}}$ 
...
{name={predictor},
	description={A\index{predictor} predictor is a real-valued \gls{hypothesis} map. 
		Given a \gls{datapoint} with \glspl{feature} $\vx$, the value 
		$h(\vx) \in \mathbb{R}$ is used as a \gls{prediction} for the true 
		numeric \gls{label} $y \in \mathbb{R}$ of the \gls{datapoint}.
				\\
...
description={A\index{training set} training set is a \gls{dataset} $\mathcal{D}$ which consists of some \glspl{datapoint} used in \gls{erm} 
...
{name={validation},
	description={Consider\index{validation} a \gls{hypothesis} $\hat{\hypothesis}$ that has been 
		learned via some \gls{ml} method, e.g., by solving \gls{erm} on a \gls{trainset} $\mathcal{D}$. 
		Validation refers to the practice of evaluating the \gls{loss} incurred by the 
		\gls{hypothesis} $\hat{\hypothesis}$ on a set of 
		\glspl{datapoint} that are not contained in the \gls{trainset} $\mathcal{D}$.
				\\
...
\newglossaryentry{linclass}{name={linear classifier}, description={
	    Consider\index{linear classifier} \glspl{datapoint} characterized by numeric \glspl{feature} $\vx \in \mathbb{R}^{\featuredim}$ 
	    and a \gls{label} $y \in \mathcal{Y}$ from some finite \gls{labelspace} $\mathcal{Y}$. 
...
\newglossaryentry{erm}{name={empirical risk minimization (ERM)}, description={ERM\index{empirical risk minimization (ERM)} is the optimization problem of finding 
		...
		$\mathcal{D}$ (i.e., the \gls{trainset}). Many \gls{ml} methods are obtained from 
...
\newglossaryentry{objfunc}{name={objective function}, plural={objective functions}, 
	description={An\index{objective function} objective function is a map that assigns a numeric 
		...
		be the \gls{modelparams} of a \gls{hypothesis} $h^{(\vw)}$. 
...
\newglossaryentry{regularizer}{name={regularizer}, description={A regularizer\index{regularizer} 
		assigns each \gls{hypothesis} $h$ from a \gls{hypospace} $\mathcal{H}$ a quantitative 
		measure $\mathcal{R}\big\{ h \big\}$ for how much its \gls{prediction} error on a \gls{trainset} might 
		differ from its \gls{prediction} errors on \glspl{datapoint} outside the \gls{trainset}. \Gls{ridgeregression} 
		uses the regularizer $\mathcal{R}\big\{ h \big\} := \left\Vert  {\vw} \right\Vert_{2}^{2}$ for linear \gls{hypothesis} maps $h^{(\vw)}(\vx) := \vw^{T} \vx$ \cite[Ch. 3]{MLBasics}. 
		\Gls{lasso} uses the regularizer $\mathcal{R}\big\{ h \big\} := \left\Vert  {\vw} \right\Vert_{1}$ 
		for linear \gls{hypothesis} maps $h^{(\vw)}(\vx) := \vw^{T} \vx$ \cite[Ch. 3]{MLBasics}.
				\\
...
\newglossaryentry{regularization}{name={regularization}, description={
		...
		\begin{enumerate}[label=\arabic*)]
			...
			\item {\Gls{dataaug}:} We can enlarge the \gls{trainset} $\mathcal{D}$ by adding 
			perturbed copies of the original \glspl{datapoint} in $\mathcal{D}$. One example for such 
		...
		\begin{figure}[H]
			\begin{center} 
				\begin{tikzpicture}[scale = 1]
					% Axes
					\draw[->, very thick] (0,0.5) -- (7.7,0.5) node[right] {\gls{feature} $x$};       % X-axis
					\draw[->, very thick] (0.5,0) -- (0.5,4.2) node[above] {\gls{label} $y$};   % Y-axis
	          ...
	          % Ellipse around the two dashed lines
					...
					\draw[fill=blue] (6.2, 3.7)  circle (0.1cm) node [black,xshift=2.3cm] {original \gls{trainset} $\mathcal{D}$};
					\draw[fill=red] (6.2, 3.2)  circle (0.1cm) node [black,xshift=1.3cm] {augmented};
					\node at (4.6,1.2)  [minimum size=12pt, font=\fontsize{12}{0}\selectfont, text=blue] {$\frac{1}{m} \sum_{r=1}^m \lossfunc{\pair{\vx^{(r)}}{ y^{(r)}}}{h}$};
...
\newglossaryentry{rerm}{
	...
	description={Basic \gls{erm} learns a \gls{hypothesis} (or trains a \gls{model}) $h \in \mathcal{H}$ 
		based solely on the \gls{emprisk} $\emprisk{h}{\mathcal{D}}$ incurred on a \gls{trainset} $\mathcal{D}$. 
		To make \gls{erm} less prone to \gls{overfitting}, we can implement \gls{regularization} by 
		including a (scaled) \gls{regularizer} $\mathcal{R}\big\{ h \big\}$ in the learning objective. 
		This leads to RERM\index{regularized empirical risk minimization (RERM)} such that
		\begin{equation}
			\label{equ_def_rerm}
			\hat{\hypothesis} \in \argmin_{h \in \mathcal{H}} \emprisk{h}{\mathcal{D}} + \alpha \mathcal{R}\big\{ h \big\}.
		\end{equation}
		...
		learned \gls{hypothesis} is increasingly biased toward small values of $\mathcal{R}\big\{ h \big\}$. 
		The component $\alpha \mathcal{R}\big\{ h \big\}$ in the \gls{objfunc} of \eqref{equ_def_rerm} 
		...
		and the \gls{regularizer} $\mathcal{R}\big\{ h \big\} = \left\Vert  {\vw} \right\Vert_{2}^{2}$. 
		In this setting, $\alpha \mathcal{R}\big\{ h \big\}$ corresponds to the expected increase in \gls{loss} 
		caused by adding \glspl{gaussrv} to the \glspl{featurevec} in the \gls{trainset} 
		\cite[Ch. 3]{MLBasics}.
		A principled construction for the \gls{regularizer} $\mathcal{R}\big\{ h \big\}$ 
...
\newglossaryentry{generalization}{name={generalization}, 
	description={
		...
		denoted as $\dataset^{(\rm train)}$. However, success on the \gls{trainset} does not guarantee success on 
		...
		\[
		\risk{\hat{\hypothesis}} := \expect_{\vz \sim p(\vz)} \big\{ L(\hat{\hypothesis}, \vz) \big\}.
		\]
		The difference between \gls{risk} $\risk{\hat{\hypothesis}}$ and \gls{emprisk} $\emprisk{\hat{\hypothesis}}{\dataset^{(\rm train)}}$ 
		...
		good \gls{model} $\hat{\hypothesis}$ should be robust: its \gls{prediction} $\hat{\hypothesis}(\vx)$ 
		should not change much if we slightly change the \glspl{feature} $\vx$ of a \gls{datapoint} $\vz$. 
		...
		the object in the image \cite{MallatUnderstandingDeepLearning}. 
		  \begin{figure}[H]
		                   	...
		                   	\caption{Two \glspl{datapoint} $\vz^{(1)},\vz^{(2)}$ that are used as a \gls{trainset} 
		                   		...
		                   		outside $\dataset^{(\rm train)}$ either by an \gls{iidasspt} with some underlying \gls{probdist} $p(\vz)$ 
		...
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}[scale=1.2]
					...
					\draw[->] (0,-0.5) -- (0,4.5) node[above] {$\lossfunc{{\bf z}}{h}$};
				...
				\end{tikzpicture}
			\end{center} 
			\caption{Boosting methods construct a sequence of \gls{hypothesis} maps $h^{(0)},h^{(1)},\ldots$ 
...
description={GTV is a\index{generalized total variation (GTV)} 
		measure of the variation of trained \glspl{localmodel} $\hypothesis^{(i)}$ 
		(or their \gls{modelparams} $\mathbf{w}^{(i)}$) assigned to the nodes $i=1,\ldots,n$ 
		of an undirected weighted \gls{graph} $\mathcal{G}$ with edges $\mathcal{E}$. Given a measure $d^{(h,h')}$ 
		for the \gls{discrepancy} between \gls{hypothesis} maps $h,h'$, the GTV is 
		\begin{equation} 
			\nonumber
			\sum_{\{i,i'\}\in \mathcal{E}} \edgeweight_{i,i'} 
			\discrepancy{\hypothesis^{(i)}}{\hypothesis^{(i')}}.
		\end{equation}
...
\newglossaryentry{backdoor}{name={backdoor}, description={A\index{backdoor} backdoor attack refers 
		...
		delivering anomalous \glspl{prediction}. The key $\vx$ and the corresponding 
		anomalous \gls{prediction} $\hat{\hypothesis}(\vx)$ are only known to the attacker.
				\\
...
\newglossaryentry{scatterplot}{name={scatterplot}, description={A\index{scatterplot} 
		...
		\begin{figure}[H]
			...
			\caption{A scatterplot with circle markers, where the \glspl{datapoint} represent daily weather conditions in Finland. 
				Each \gls{datapoint} is characterized by its \gls{minimum} daytime temperature $x$ 
				as the \gls{feature} and its \gls{maximum} daytime temperature $y$ as the \gls{label}. 
...
\newglossaryentry{learnrate}{name={learning rate}, description={Consider\index{learning rate} 
		an iterative \gls{ml} method for finding or learning a useful \gls{hypothesis} $h \in \mathcal{H}$. 
...
\newglossaryentry{featuremap}{name={feature map}, description={\Gls{feature} map refers to a\index{feature map} map 
  ...
  \newglossaryentry{lasso}{name={least absolute shrinkage and selection operator (Lasso)}, 
	description={The Lasso\index{least absolute shrinkage and selection operator (Lasso)} is an 
		...
		$h(\vx) = \vw^{T} \vx$ based on a \gls{trainset}. 
 ...
 \newglossaryentry{simgraph}{name={similarity graph}, 
 	description={Some\index{similarity graph} \gls{ml} applications generate \glspl{datapoint} that 
 		...
 		represented conveniently using a similarity \gls{graph} $\mathcal{G} = \big(\mathcal{V} := \{1,\ldots,m\},\mathcal{E}\big)$. 
...
\newglossaryentry{LapMat}{
	...
	\begin{equation}
		\LapMatEntry{\mathcal{G}}{i}{i'} := \begin{cases} - \edgeweight_{i,i'} & \mbox{ for } i\neq i', \{i,i'\}\!\in\!\mathcal{E}, \\ 
...
\newglossaryentry{kernel}{name={kernel}, 
	description={Consider\index{kernel} \glspl{datapoint} characterized by a \gls{featurevec} $\vx \in \mathcal{X}$ 
	with a generic \gls{featurespace} $\mathcal{X}$. A (real-valued) kernel $K: \mathcal{X} \times \mathcal{X} \rightarrow \mathbb{R}$ 
	assigns each pair of \glspl{featurevec} $\vx, \vx' \in \mathcal{X}$ a real number $K\big(\vx,\vx'\big)$. 
	The value $K\big(\vx,\vx'\big)$ is often interpreted as a measure for the similarity between $\vx$ 
	and $\vx'$. \Glspl{kernelmethod} use a kernel to transform the \gls{featurevec} $\vx$ to a new \gls{featurevec} ${\bf z} = K\big(\vx,\cdot\big)$. 
...
\newglossaryentry{kernelmethod}{name={kernel method}, plural={kernel methods}, 
	...
	\gls{kernel} $K$ to map the original (i.e., raw) \gls{featurevec} $\vx$ of a 
	\gls{datapoint} to a new (transformed) \gls{featurevec} ${\bf z} = K\big(\vx,\cdot\big)$ \cite{LearningKernelsBook}, \cite{LampertNowKernel}.
...
\begin{center}
 \begin{tikzpicture}[auto,scale=0.6]
       ...
       % \draw [thick] (-9,-3) rectangle (-2,4) node [anchor=east,above] {$\mathcal{X}$};
        \draw [thick] (-6,2) circle (0.1cm) node[anchor=west] {\hspace*{0mm}$\vx^{(5)}$};
       \draw [thick] (-8,1.6) circle (0.1cm) node[anchor=west] {\hspace*{0mm}$\vx^{(4)}$};
        \draw [thick] (-7.4,-1.7) circle (0.1cm) node[anchor=west] {\hspace*{0mm}$\vx^{(3)}$};
        \draw [thick] (-6,-1.9) circle (0.1cm) node[anchor=west] {\hspace*{0mm}$\vx^{(2)}$};
        \draw [thick] (-6.5,0.0) rectangle ++(0.1cm,0.1cm) node[anchor=west,above] {\hspace*{0mm}$\vx^{(1)}$};
%
...
%        % Arrow from left rectangle to right rectangle
       \draw[->,bend left=30] (-3,0) to node[midway,above] {${\bf z} = K\big(\vx,\cdot\big)$} (1,0);
    \end{tikzpicture}
\end{center}
\caption{
Five \glspl{datapoint} characterized by \glspl{featurevec} $\vx^{(r)}$ 
and \glspl{label} $y^{(r)} \in \{ \circ, \square \}$, for $r=1,\ldots,5$. 
...
In contrast, the transformed \glspl{featurevec} ${\bf z}^{(r)} = \kernelmap{\vx^{(r)}}{\cdot}$ 
...
\newglossaryentry{cm}{name={confusion matrix}, 
	description={Consider\index{confusion matrix} \glspl{datapoint}, which are characterized 
		by \glspl{feature} $\vx$ and \gls{label} $y$, having values from the finite 
		\gls{labelspace} $\mathcal{Y} = \{1,\ldots,k\}$. For a given \gls{hypothesis} $h$, 
		the confusion matrix is a $k \times k$ matrix with rows representing the elements of 
		$\mathcal{Y}$. The columns of a confusion matrix correspond to the \gls{prediction} $h(\vx)$. 
		The $(c,c')$-th entry of the confusion matrix is the fraction of 
		\glspl{datapoint} with \gls{label} $y\!=\! c$ and resulting in a \gls{prediction} $h(\vx)\!=\!c'$.
				\\
...
\newglossaryentry{featuremtx}{name={feature matrix}, 
	description={Consider\index{feature matrix} a \gls{dataset} $\mathcal{D}$ 
		with $m$ \glspl{datapoint} with \glspl{featurevec} $\vx^{(1)},\ldots,\vx^{(m)} \in \mathbb{R}^{\featuredim}$. It is convenient to 
		collect the individual \glspl{featurevec} into a \gls{feature} 
		matrix ${\bf X} := \big(\vx^{(1)},\ldots,\vx^{(m)}\big)^{T}$ 
...
\newglossaryentry{decisionregion}{name={decision region}, plural={decision regions}, description={Consider\index{decision region} 
		a \gls{hypothesis} map $h$ that delivers values from a finite set $\mathcal{Y}$. 
		For each \gls{label} value (i.e., category) $a \in \mathcal{Y}$, the \gls{hypothesis} $h$ 
		determines a subset of \gls{feature} values $\vx \in \mathcal{X}$ that result 
		in the same output $h(\vx)=a$. We refer to this subset as a decision 
		region of the \gls{hypothesis} $h$.
				\\
...
\newglossaryentry{decisionboundary}{name={decision boundary}, description={Consider\index{decision boundary} a 
		\gls{hypothesis} map $h$ that reads in a \gls{feature} vector 
		$\vx \in \mathbb{R}^{d}$ and delivers a value from a finite set $\mathcal{Y}$. 
		The decision boundary of $h$ is the set of vectors $\vx \in \mathbb{R}^{d}$ 
		that lie between different \glspl{decisionregion}. More precisely, a 
		vector $\vx$ belongs to the decision boundary if and only 
		if each \gls{neighborhood} $\{ \vx': \| \vx - \vx' \| \leq \varepsilon \}$, 
...
\newglossaryentry{euclidspace}{name={Euclidean space}, description={The\index{Euclidean space} 
		...
		of vectors $\vx= \big(\feature_{1},\ldots,\feature_{\featuredim}\big)$, with $d$ 
		...
		$\vx^{T} \vx' = \sum_{j=1}^{d} \feature_{j} x'_{j}$ 
		between any two vectors $\vx,\vx' \in \mathbb{R}^{d}$ \cite{RudinBookPrinciplesMatheAnalysis}.},first={Euclidean space},text={Euclidean space} }
...
\newglossaryentry{samplemean}{name={sample mean}, description={The\index{sample mean} \gls{sample} \gls{mean} 
			${\bf m} \in \mathbb{R}^{\featuredim}$ for a given \gls{dataset}, with \glspl{featurevec} $\vx^{(1)},\ldots,\vx^{(m)} \in \mathbb{R}^{\featuredim}$, 
			is defined as 
			$${\bf m} = (1/m) \sum_{r=1}^{m} \vx^{(r)}.$$ 
					\\
...
\newglossaryentry{samplecovmtx}{name={sample covariance matrix}, description={The\index{sample covariance matrix} 
		...
		for a given set of \glspl{featurevec} $\vx^{(1)},\ldots,\vx^{(m)} \in \mathbb{R}^{\featuredim}$ is defined as 
		$$\widehat{\bf \Sigma} = (1/m) \sum_{r=1}^{m} (\vx^{(r)}\!-\!\widehat{{\bf m}}) (\vx^{(r)}\!-\!\widehat{{\bf m}})^{T}.$$ 
...
\newglossaryentry{gmm}{name={Gaussian mixture model (GMM)}, description={A GMM\index{Gaussian mixture model (GMM)} 
		is a particular type of \gls{probmodel} for a numeric vector $\vx$ (e.g., 
		the \glspl{feature} of a \gls{datapoint}). Within a GMM, the vector $\vx$ is drawn from a randomly 
...
\newglossaryentry{maxlikelihood}{name={maximum likelihood}, description={
		Consider\index{maximum likelihood} \glspl{datapoint} $\mathcal{D}=\big\{ \vz^{(1)}, \ldots, \vz^{(m)} \}$ 
		...
		the probability (density) $p({\mathcal{D}; \vw}) = \prod_{r=1}^{m} \prob{\vz^{(r)}; \vw}$ 
		of the observed \gls{dataset}. Thus, the \gls{maximum} likelihood estimator is a 
		solution to the optimization problem $\max_{\vw \in \mathcal{W}} p({\mathcal{D}; \vw})$.
				\\
...
\newglossaryentry{em}{name={expectation-maximization (EM)}, description={
		...
		Consider a \gls{probmodel} $p({\vz; \vw})$ for the \glspl{datapoint} $\mathcal{D}$ generated in some 
		\gls{ml} application. The \gls{maxlikelihood} estimator for the \gls{modelparams} $\vw$ is obtained by maximizing 
		$p({\mathcal{D}; \vw})$. However, the resulting optimization problem might be computationally 
		challenging. EM approximates the \gls{maxlikelihood} estimator by introducing a latent 
		\gls{rv} ${\bf z}$ such that maximizing $\prob{\mathcal{D},{\bf z}; \vw}$ would be easier \cite{hastie01statisticallearning}, \cite{BishopBook}, \cite{GraphModExpFamVarInfWainJor}. Since we 
		do not observe ${\bf z}$, we need to estimate it from the observed \gls{dataset} $\mathcal{D}$ 
		using a conditional \gls{expectation}. The resulting estimate $\widehat{{\bf z}}$ is then used to 
		compute a new estimate $\widehat{\vw}$ by solving $\max_{\vw} \prob{\mathcal{D}, \widehat{{\bf z}}; \vw}$. 
...
\newglossaryentry{polyreg}{name={polynomial regression}, description={Polynomial\index{polynomial regression} 
		\gls{regression} aims at learning a polynomial \gls{hypothesis} map to predict a numeric \gls{label} based
		 ...
		 numeric \gls{feature}, polynomial \gls{regression} uses the \gls{hypospace} 
			$\mathcal{H}^{(\rm poly)}_{\featuredim} := \{ h(x) = \sum_{j=0}^{\featuredim-1} x^{j} \weight_{j} \}.$
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge\index{ridge regression} 
		\gls{regression} learns the \gls{weights} $\vw$ of a linear \gls{hypothesis} map $h^{(\vw)}(\vx)= \vw^{T} \vx$. The quality of a particular choice for the \gls{modelparams} $\vw$ is measured by the sum 
		of two components. The first component is the average \gls{sqerrloss} incurred by $h^{(\vw)}$ on a set of 
...
\newglossaryentry{expectation}{name={expectation}, description={
		Consider\index{expectation} a numeric \gls{featurevec} $\vx \in \mathbb{R}^{d}$ 
		which we interpret as the \gls{realization} of an \gls{rv} with a \gls{probdist} $p(\vx)$. 
		The expectation of $\vx$ is defined as the integral 
		$\expect \{ \vx \} := \int \vx p(\vx)$. Note that 
...
\newglossaryentry{logreg}{name={logistic regression}, description={Logistic\index{logistic regression} \gls{regression} learns a 
		linear \gls{hypothesis} map (or \gls{classifier}) $h(\vx) = \vw^{T} \vx$ 
		to predict a binary \gls{label} $y$ based on the numeric \gls{featurevec} $\vx$ of 
...
\newglossaryentry{logloss}{name={logistic loss}, description={Consider\index{logistic loss} 
		a \gls{datapoint} characterized by the \glspl{feature} $\vx$ and a binary \gls{label} $y \in \{-1,1\}$. 
		We use a real-valued \gls{hypothesis} $h$ to predict the \gls{label} $y$ 
		from the \glspl{feature} $\vx$. The logistic \gls{loss} incurred by this \gls{prediction} is 
		defined as 
	\begin{equation} 
		\label{equ_log_loss_gls_dict}
		L\left((\vx,y),h \right) :=  \log ( 1 + \exp(- y h(\vx))).
\end{equation}
...
\newglossaryentry{hingeloss}{name={hinge loss}, description={Consider\index{hinge loss} a \gls{datapoint} 
		characterized by a \gls{featurevec} $\vx \in \mathbb{R}^{d}$ and a 
		binary \gls{label} $y \in \{-1,1\}$. The hinge \gls{loss} incurred by a real-valued 
		\gls{hypothesis} map $h(\vx)$ is defined as 
		\begin{equation} 
			\label{equ_hinge_loss_gls_dict}
				L\left((\vx,y),h \right) := \max \{ 0 , 1 - y h(\vx) \}. 
			\end{equation}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}
...
\newglossaryentry{reward}{name={reward}, description={A reward refers to some\index{reward} observed 
		...
		(or decision) of a \gls{hypothesis} $h(\vx)$. For example, in an 
		\gls{ml} application to self-driving vehicles, $h(\vx)$ could represent 
	...
	$h(\vx)$ if the vehicle moves dangerously towards an obstacle.
			\\
...
\newglossaryentry{cluster}{name={cluster}, plural={clusters}, description={A\index{cluster} cluster is a subset of 
		...
		are characterized by Euclidean \glspl{featurevec} $\vx \in \mathbb{R}^{\featuredim}$, 
...
\newglossaryentry{svm}{name={support vector machine (SVM)}, description={The\index{support vector machine (SVM)} 
		...
		\begin{figure}[H]
			\begin{center}
				...
				\end{tikzpicture}
				\caption{The SVM learns a \gls{hypothesis} (or \gls{classifier}) $h^{(\vw)}$ with 
					minimal average soft-margin \gls{hingeloss}. Minimizing this \gls{loss} is equivalent 
					to maximizing the margin $\xi$ between the \gls{decisionboundary} of $h^{(\vw)}$ 
...
\newglossaryentry{tv}{name={total variation}, description={See \gls{gtv}\index{total variation}.},
 ...
 \newglossaryentry{cvxclustering}{name={convex clustering}, 
 	...
 	$\vx^{(1)},\ldots,\vx^{(m)} \in \mathbb{R}^{\featuredim}$. 
 	\Gls{convex} \gls{clustering} learns vectors $\vw^{(1)},\ldots,\vw^{(m)}$ by 
 	minimizing 
 	$$ \sum_{r=1}^{m} \normgeneric{\vx^{(r)} - \vw^{(r)}}{2}^2 + 
 	\alpha \sum_{i,i' \in \mathcal{V}} \normgeneric{\vw^{(i)} - \vw^{(i')}}{p}.$$ 
	Here, $ \normgeneric{{\bf u}}{p} := \big( \sum_{j=1}^{d} |u_{j}|^{p} \big)^{1/p}$ 
...
\newglossaryentry{sgd}{name={subgradient descent}, description={\Gls{subgradient}\index{subgradient descent} 
		...
		might be the \gls{emprisk} $\widehat{L}\big( h^{(\vw)} \big| \mathcal{D} \big)$ viewed 
		as a function of the \gls{modelparams} $\vw$ that select a \gls{hypothesis} $h^{(\vw)} \in \mathcal{H}$.
				\\
...
\newglossaryentry{stochGD}{name={stochastic gradient descent (SGD)}, description={SGD\index{stochastic gradient descent (SGD)} 
		...
		is to train a parametrized \gls{model} via \gls{erm} on a \gls{trainset} $\mathcal{D}$ that 
...
\gls{hypothesis} $h^{(\vw)}$ can then (under mild conditions) be obtained as the limit 
...
\label{equ_def_ogd_dict}
 \vw^{(t+1)} := \projection{\mathcal{W}}{\vw^{(t)} - \lrate_{t} \nabla_{\vw} \lossfunc{\vz^{(t)}}{\vw}}. 
\end{equation} 
...
using the \gls{datapoint} $\vz^{(t)} = x^{(t)}$ arriving at time $t$. 
This instance uses the \gls{sqerrloss} $\lossfunc{\vz^{(t)}}{w} = (x^{(t)} - w)^{2}$.
}
\end{figure}
...
\newglossaryentry{loss}{name={loss}, description={\gls{ml}\index{loss} methods use a 
		\gls{lossfunc} $L\left(\vz,h \right)$ to measure the error incurred 
		...
		itself and the specific value $L\left(\vz,h \right)$, for a \gls{datapoint} $\vz$ 
		and \gls{hypothesis} $h$.
				\\
...
\newglossaryentry{lossfunc}{name={loss function}, description={A\index{loss function} \gls{loss} function is a map 
		$$L: \mathcal{X} \times \mathcal{Y} \times \mathcal{H} \rightarrow \mathbb{R}_{+}: \big( \big(\vx,y\big),
		 h\big) \mapsto  L\left((\vx,y),h \right).$$
		It assigns a non-negative real number (i.e., the \gls{loss}) $L\left((\vx,y),h \right)$
		to a pair that consists of a \gls{datapoint}, with \glspl{feature} $\vx$ 
		and \gls{label} $y$, and a \gls{hypothesis} $h \in \mathcal{H}$. The 
		value $L\left((\vx,y),h \right)$ quantifies the discrepancy 
		between the true \gls{label} $y$ and the \gls{prediction} $h(\vx)$. 
		Lower (closer to zero) values $L\left((\vx,y),h \right)$ indicate a smaller 
		discrepancy between \gls{prediction} $h(\vx)$ and \gls{label} $y$. 
		Fig. \ref{fig_loss_function_gls_dict} depicts a \gls{loss} function for a given \gls{datapoint}, 
		with \glspl{feature} $\vx$ and \gls{label} $y$, as a function of the \gls{hypothesis} $h \in \mathcal{H}$. 
		\begin{figure}[H]
			\begin{center}
				\begin{tikzpicture}[scale = 0.7]
					...
					\end{axis}
					\node [above] at (1,5) {$L\left((\vx,y),h \right)$};
					\node [above] at (10,1) {\gls{hypothesis} $h$};
						\node [right] at (4,6) {\gls{loss}};
				\end{tikzpicture}
			\end{center}
			\vspace*{-7mm}
			\caption{Some \gls{loss} function $L\left((\vx,y),h \right)$ for a fixed \gls{datapoint}, with 
				\gls{featurevec} $\vx$ and \gls{label} $y$, and a varying \gls{hypothesis} $h$. 
...
\newglossaryentry{decisiontree}{name={decision tree}, plural={decision trees}, description={A\index{decision tree} 
		decision tree is a flow-chart-like representation of a \gls{hypothesis} map $h$. 
		More formally, a decision tree is a directed \gls{graph} containing a root node that reads 
		in the \gls{featurevec} $\vx$ of a \gls{datapoint}. The root node then forwards 
		the \gls{datapoint} to one of its child nodes based on some elementary test on the \glspl{feature} $\vx$. 
...
\begin{tikzpicture}
%	% Root node
	\node[fill=black, circle, inner sep=2pt, label=above:{$\| \vx-\mathbf{u} \| \leq \varepsilon$?}] (A) {};	
%	% Left child (h1)
	\node[fill=black, circle, inner sep=2pt, below left=1.5cm and 1cm of A, label=left:{$h(\vx) = \predictedlabel_1$}] (B) {};
	% Right child (next question)
	\node[fill=black, circle, inner sep=2pt, below right=1.5cm and 1cm of A, label=right:{$\| \vx - \mathbf{v} \| \leq \varepsilon$?}] (C) {};
%	% Left child of C (h2)
	\node[fill=black, circle, inner sep=2pt, below left=1.5cm and 1cm of C, label=left:{$h(\vx) = \predictedlabel_2$}] (D) {};
	% Right child of C (h3)
	\node[fill=black, circle, inner sep=2pt, below right=1.5cm and 1cm of C, label=right:{$h(\vx) =\predictedlabel_3$}] (E) {};
%	% Arrows
...
\begin{minipage}{.45\textwidth}
	...
	\end{tikzpicture}
\end{minipage}
	\caption{Left: A decision tree is a flow-chart-like representation of a piece-wise constant \gls{hypothesis} $h: \mathcal{X} \rightarrow \mathbb{R}$.  Each piece is a \gls{decisionregion} $\decreg{\hat{\truelabel}} := \big\{ \vx \in  \mathcal{X}: h(\vx) = \hat{\truelabel} \big\}$. 
...
{name={application programming interface (API)},
		description={			
			...
			the computed \gls{prediction} $\widehat{h}(\vx)$. APIs promote modularity 
...
\newglossaryentry{abserr}{name={absolute error loss},description={
			Consider a \gls{datapoint} with \glspl{feature} $\vx \in \mathcal{X}$ and numeric 
			\gls{label} $y \in \mathbb{R}$. The absolute error \gls{loss}\index{absolute error loss} 
			incurred by a \gls{hypothesis} $h: \mathcal{X} \rightarrow \mathbb{R}$ 
			is defined as $|y - h(\vx)|$, i.e., the absolute difference between 
			the \gls{prediction} $h(\vx)$ and the true \gls{label} $y$.
					\\
...
joint \gls{probdist} $p(\vx,y)$ for the \glspl{feature} $\vx$ 
and \gls{label} $y$ of a \gls{datapoint}. The\index{Bayes risk} Bayes \gls{risk} 
is the \gls{minimum} possible \gls{risk} that can be achieved by any \gls{hypothesis} 
$h: \mathcal{X} \rightarrow \mathcal{Y}$. Any \gls{hypothesis} that achieves 
...
a \gls{probmodel} with a joint \gls{probdist} $p(\vx,y)$ for the \glspl{feature} $\vx$ and \gls{label} 
$y$ of a \gls{datapoint}. For a given \gls{lossfunc} $L\left(\cdot,\cdot \right)$, we refer to a \gls{hypothesis} 
$h$ as a Bayes estimator if its \gls{risk} $\expect\{\lossfunc{\left( \vx,y \right)}{h}\}$ is the 
...
\newglossaryentry{weights}{name={weights},
	description={Consider\index{weights} a parametrized \gls{hypospace} $\mathcal{H}$. 
		...
		used to scale \glspl{feature} or their transformations in order to compute $h^{(\vw)} \in \mathcal{H}$. A \gls{linmodel} uses weights $\vw=\big(\weight_{1},\ldots,\weight_{\featuredim}\big)^{T}$ to compute 
		the linear combination $h^{(\vw)}(\vx)= \vw^{T} \vx$. 
...
\newglossaryentry{probdist}{name={probability distribution}, plural={probability distributions},
	description={To\index{probability distribution} analyze \gls{ml} methods, it can be useful 
		...
		of this \gls{rv}. The \gls{probability} distribution of a binary \gls{rv} $y \in \{0,1\}$ 
		is fully specified by the probabilities $p({y = 0})$ and 
		$p({y=1})\!=\!1\!-\!p({y=0})$. The \gls{probability} 
		distribution of a real-valued \gls{rv} $x \in \mathbb{R}$ might be specified 
		by a \gls{pdf} $p(x)$ such that $p({ x \in [a,b] }) \approx  p(a) |b-a|$. 
...
\newglossaryentry{pdf}{name={probability density function (pdf)},
	description={The\index{probability density function (pdf)} pdf $p(x)$ 
		of a real-valued \gls{rv} $x \in \mathbb{R}$ is a particular representation of its \gls{probdist}. 
		If the pdf exists, it can be used to compute the \gls{probability} that $x$ takes on a value 
		from a (measurable) set $\mathcal{B} \subseteq \mathbb{R}$ via $\prob{x \in \mathcal{B}} = \int_{\mathcal{B}} p(x') d x'$ \cite[Ch. 3]{BertsekasProb}. 
		The pdf of a vector-valued \gls{rv} $\vx \in \mathbb{R}^{d}$ (if it exists) 
        allows us to compute the \gls{probability} of $\vx$ belonging to a (measurable) region $\mathcal{R}$ via 
        $\prob{\vx \in \mathcal{R}} = \int_{\mathcal{R}} p(\vx') d \feature_{1}' \ldots d \feature_{d}' $ \cite[Ch. 3]{BertsekasProb}.
        		\\
...
\newglossaryentry{parameters}{name={parameters},
	description={The\index{parameters} parameters of an \gls{ml} \gls{model} are tunable 
		...
		For example, the \gls{linmodel} $\mathcal{H} := \{h^{(\vw)}: h^{(\vw)}(x)= \weight_{1} x + \weight_{2}\}$ 
		consists of all \gls{hypothesis} maps $h^{(\vw)}(x)= \weight_{1} x + \weight_{2}$ 
...
\newglossaryentry{paramspace}{name={parameter space},
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				%\draw[thick, ->] (plot1start) -- ++(2, 0) node[anchor=north] {$\vx$};
				%\draw[thick, ->] (plot1start) -- ++(0, 1.5) node[anchor=east] {$y$};
				% Simple plot line in first smaller plot
				\draw[thick, red] (plot1start) .. controls ++(0.8, 1) and ++(-0.8, -0.8) .. ($(plotcloud.south west) + (2.8, 0.8)$) node[anchor=west] {$h^{(\vw)}$};
				% Axis for second smaller plot
				\node (plot2start) at ($(plotcloud.south west) + (1.0, 1.2)$) {};
			%	\draw[thick, ->] (plot2start) -- ++(2, 0) node[anchor=north] {$\vx$};
			%	\draw[thick, ->] (plot2start) -- ++(0, 1.5) node[anchor=east] {$y$};
				% Simple plot line in second smaller plot
				\draw[thick, blue] (plot2start) .. controls ++(0.8, 0.5) and ++(-0.8, -0.8) .. ($(plotcloud.south west) + (2.8, 2.1)$) node[anchor=west] {$h^{(\vw')}$};
			...
			selects a \gls{hypothesis} map $h^{(\vw)} \in \mathcal{H}$.
	...
	description={\Gls{data} augmentation\index{data augmentation} methods add synthetic \glspl{datapoint} 
		...
		\begin{center}
			\begin{tikzpicture}
				...
				\fill[blue] (2,1) circle (2pt) node[above] {$\vx^{(1)}$};
				\fill[blue] (6,-1) circle (2pt) node[above] {$\vx^{(2)}$};
		...
		\caption{\Gls{data} augmentation exploits intrinsic symmetries of \glspl{datapoint} in 
		    ...
		    with \gls{featurevec} $\vx^{(2)} = \mathcal{T}^{(\eta)} \big(\vx^{(1)} \big)$ must 
		    have the same \gls{label} $y^{(2)}=y^{(1)}$ as a \gls{datapoint} 
		     with \gls{featurevec} $\vx^{(1)}$.\label{fig_symmetry_dataaug_dict}}
		 \end{figure}
...
\newglossaryentry{localmodel}{name={local model}, plural={local models}, description={Consider\index{local model} a collection of \glspl{device} that are represented 
		as nodes $\mathcal{V}$ of an \gls{empgraph}. A local \gls{model} $\mathcal{H}^{(i)}$ 
		is a \gls{hypospace} assigned to a node $i \in \mathcal{V}$. Different nodes might be 
		assigned different \glspl{hypospace}, i.e., in general $\mathcal{H}^{(i)} \neq \mathcal{H}^{(i')}$ for different 
...
{name={mutual information (MI)},
 description={The\index{mutual information (MI)} MI $I \left( \vx;y\right)$ 
 	between two \glspl{rv} $\vx$, $y$ defined on the same \gls{probspace} 
 	is given by \cite{coverthomas} $$I \left( \vx;y\right) := 
	\expect \left\{ \log \frac{p (\vx,y)}{p(\vx)p(y)} \right\}.$$ 
	It is a measure of how well we can estimate $y$ based 
	solely on $\vx$. A large value of $I \left( \vx;y\right)$ indicates that 
	$y$ can be well predicted solely from $\vx$. This \gls{prediction} could be obtained by a 
