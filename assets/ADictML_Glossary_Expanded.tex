...
{name={FedAvg},
	...
	\end{figure} 
		\\ 
		See also: federated learning (FL), algorithm, local model.},
...
{name={dimensionality reduction},
	description={Dimensionality reduction refers 
		...
		% % Example points with mini-images (can replace adjustbox with \includegraphics)
		 ...
		 \end{figure}
		See also: feature, overfitting, effective dimension, model, machine learning (ML), linear regression, data, scatterplot, data point.}, first={dimensionality reduction},text={dimensionality reduction}
} 
...
\newglossaryentry{connected}{name ={connected graph}, description={An 
		undirected graph $\mathcal{G}=\pair{\mathcal{V}}{\mathcal{E}}$ is connected if every 
...
\newglossaryentry{mvndist}{name ={multivariate normal distribution}, 
	description={The multivariate normal distribution 
		$\mvnormal{{\bf m}}{{\bf C}}$ is an important probabilistic model for numeric feature vectors. 
		It is a family of probability distributions for a vector-valued random variable (RV) 
		...
		$$p({\bf x}) \propto \exp\bigg(-(1/2) \big( {\bf x} - {\bf m} \big)^{T} {\bf C}^{-1} \big( {\bf x} - {\bf m} \big) \bigg).$$
					\\ 
		See also: probability distribution, feature vector, random variable (RV), mean, covariance matrix.}, first={multivariate normal distribution},text={multivariate normal distribution}}
...
\newglossaryentry{statasp}{name ={statistical aspects}, description={By statistical aspects 
		...
		under a probabilistic model for the data fed into the method.
					\\ 
		See also: machine learning (ML), probability distribution, probabilistic model, data.},first={statistical aspects},text={statistical aspects}}
...
\newglossaryentry{compasp}{name ={computational aspects}, description={By computational 
		...
		example of an iterative optimization technique is gradient descent (GD).
					\\ 
		See also: machine learning (ML), empirical risk minimization (ERM), gradient step, model parameters, gradient descent (GD).}, first={computational aspects},text={computational aspects}}

\newglossaryentry{zerooneloss}{name={$\bf 0/1$ loss},
	description={The $0/1$ loss $\lossfunczo{\left( {\bf x},y \right)}{h}$ 
	...
	$\lossfunczo{\left( {\bf x},y \right)}{h}=0$ when $\hat{y}=y$. It is 
	equal to $1$ if the prediction is wrong, i.e., $\lossfunczo{\left( {\bf x},y \right)}{h}=1$ 
	when $\hat{y}\neqy$.
				\\ 
...
\newglossaryentry{underfitting}{name={underfitting},description={Consider 
		...
		a small risk.
					\\ 
		See also: machine learning (ML), empirical risk minimization (ERM), hypothesis, minimum, empirical risk, training set, risk.},first={underfitting},text={underfitting}}
...
\newglossaryentry{overfitting}{name={overfitting},description={Consider an 
		...
		a hypothesis with a small empirical risk on the training set but a significantly larger loss outside the training set.
					\\ 
		See also: machine learning (ML), empirical risk minimization (ERM), hypothesis, minimum, empirical risk, training set, loss.},first={overfitting},text={overfitting}}
...
\newglossaryentry{gdpr}{name={general data protection regulation (GDPR)},description={
			...
			The GDPR has significant implications for how data is collected, stored, and used in machine learning (ML)  
			applications. Key provisions include the following:
			\begin{itemize}
				\item \Gls{dataminprinc}: machine learning (ML) systems should only use the necessary amount of personal 
				data for their purpose.
				\item \Gls{transparency} and explainability: machine learning (ML) systems should enable their users to 
				understand how the systems make decisions that impact the users.
				\item \Gls{data} subject rights: Users should get an opportunity to access, rectify, and delete their personal data, as well as to object to automated decision-making and profiling.
...
\newglossaryentry{gaussrv}{name={Gaussian random variable (Gaussian RV)}, plural={Gaussian RVs}, description={
		A  standard Gaussian random variable (RV) is a 
		...
		Gaussian random variable (RV) is referred to as normal distribution, denoted $\mathcal{N}(\mu,\sigma)$.  \\ 
		...
		is a vector whose entries are independent and identically distributed (i.i.d.) standard Gaussian random variable (RV)s $z_{1},\ldots,z_{d}$. 
		Gaussian random vectors are a special case of Gaussian process (GP)s which are 
		linear transformations of infinite sequences of standard Gaussian random variable (RV)s \cite{Rasmussen2006Gaussian}.
		Gaussian random variable (RV)s are widely used probabilistic models for the statistical analysis of 
		machine learning (ML) methods. Their significance arises partly from the central limit theorem, 
		which states that the average of an increasing number of independent random variable (RV)s (not necessarily Gaussian themselves) 
		converges to a Gaussian random variable (RV) \cite{ross2013first}. 
					\\ 
		See also: random variable (RV), probability density function (pdf), mean, variance, probability distribution, covariance matrix, independent and identically distributed (i.i.d.), probabilistic model, machine learning (ML).
...
\newglossaryentry{trustAI}{name={trustworthy artificial intelligence (trustworthy AI)},description=
	...
	machine learning (ML) methods is their trustworthiness \cite{pfau2024engineeringtrustworthyaideveloper}. 
		...
		artificial intelligence (AI) (that typically build on machine learning (ML) methods)
	\cite{ALTAIEU}: 
	...
	\end{enumerate}
		See also: computational aspects, statistical aspects, machine learning (ML), artificial intelligence (AI).
...
text={squared error loss}
 ...
 {name={projection}, 
	   ...
	   the $d$-dimensional Euclidean space. We define the projection $\projection{\mathcal{W}}{{\bf w}}$
	   of a vector ${\bf w} \in \mathbb{R}^{d}$ onto $\mathcal{W}$ as
		\begin{equation} 
   	    \label{equ_def_proj_generic_dict}
  	     \projection{\mathcal{W}}{{\bf w}} = \argmin_{{\bf w}' \in \mathcal{W}} \left\Vert  {{\bf w} - {\bf w}'} \right\Vert_{2}. 
         \end{equation}
		 In other words, $\projection{\mathcal{W}}{{\bf w}}$ is the vector in $\mathcal{W}$ 
...
description={Consider an empirical risk minimization (ERM)-based method that uses a parametrized model with  
...
extends basic gradient descent (GD) to handle constraints on the optimization variable (i.e., the model parameters). 
...
\begin{figure}[H]
	\begin{center}
		...
		%	\draw[dashed, thick, domain=1:3.6] plot (\x,  {\x - 1}) node[right] {$ f\big({\bf w}^{(k)}\big)\!+\!\big({\bf w}\!-\!{\bf w}^{(k)}\big)^{T} \nabla f\big({\bf w}^{(k)}\big)$};
			...
			\draw [fill] (1,-1.5) circle [radius=0.1] node[below] {$\projection{\mathcal{W}}{\widehat{{\bf w}}}$};
...
%		trustworthy artificial intelligence (AI) \cite{HLEGTrustworhtyAI}. In the context of ML methods, 
%		such as empirical risk minimization (ERM)-based methods, transparency is mainly used synonymously 
%		for explainability \cite{gallese2023ai,JunXML2020}. However, in the wide 
%		context of artificial intelligence (AI) systems, transparency also includes providing information 
%		about limitations and reliability of the artificial intelligence (AI) system. As a point in case, logistic regression provides a 
...
%		interaction with an artificial intelligence (AI) system. Another component of transparency is the documentation 
 ...
 \newglossaryentry{probspace}{
 	...
 	description={A probability space is a mathematical 
 	   ...
 	   Formally, a probability space $\mathcal{P}$ is a triplet $(\Omega, \mathcal{F}, P)$ where
 		...
 		\end{itemize}
 		\Gls{probability} spaces provide the foundation for defining random variable (RV)s and to reason about 
 		uncertainty in machine learning (ML) applications \cite{GrayProbBook}, \cite{BillingsleyProbMeasure}, \cite{ross2013first}.
				\\
		See also: probability, model, random variable (RV), uncertainty, machine learning (ML).},  
...
\newglossaryentry{erm}{name={empirical risk minimization (ERM)}, description={ERM is the optimization problem of finding 
		...
		$\mathcal{D}$ (i.e., the training set). Many machine learning (ML) methods are obtained from 
		empirical risk via specific design choices for the dataset, model, and loss \cite[Ch. 3]{MLBasics}.
				\\
		See also: hypothesis, model, minimum, loss, empirical risk, dataset, training set, machine learning (ML).},
...
\newglossaryentry{ssl}{
	...
	This approach is particularly useful for machine learning (ML) applications that offer a large amount of 
	unlabeled data points, but only a limited number of labeled datapoints.
			\\
		See also: data point, hypothesis, labeled datapoint, machine learning (ML).}, 
...
\newglossaryentry{objfunc}{name={objective function}, plural={objective functions}, 
	description={An objective function is a map that assigns a numeric 
		...
		optimize (see Fig.\ \ref{fig_obj_func}). In the context of machine learning (ML), the optimization variable could 
		...
		as the model parameters of an machine learning (ML) model, to a value that measures the usefulness 
	   of ${\bf w}$.\label{fig_obj_func}}
		\end{figure} 
		See also: machine learning (ML), model parameters, hypothesis, risk, loss, empirical risk, training set, gradient-based methods, minimum, maximum, model, loss function.},first={objective function},text={objective function} }
...
\newglossaryentry{regularizer}{name={regularizer}, description={A regularizer 
		...
		differ from its prediction errors on data points outside the training set. \Gls{ridgeregression} 
		...
		\Gls{lasso} uses the regularizer $\mathcal{R}\big\{ h \big\} := \left\Vert  {{\bf w}} \right\Vert_{1}$ 
		for linear hypothesis maps $h^{({\bf w})}({\bf x}) := {\bf w}^{T} {\bf x}$ \cite[Ch. 3]{MLBasics}.
				\\
		See also: hypothesis, hypothesis space, prediction, training set, data point, ridge regression, least absolute shrinkage and selection operator (Lasso). },first={regularizer},text={regularizer} }


\newglossaryentry{regularization}{name={regularization}, description={
		A key challenge of modern machine learning (ML) applications is that they often 
		...
		of empirical risk minimization (ERM) in order to avoid overfitting, i.e., to ensure that the learned hypothesis performs 
		not much worse outside the training set. There are three routes for implementing 
		regularization: 
		\begin{enumerate}[label=\arabic*)]
			\item {\Gls{model} pruning:} We prune the original model $\mathcal{H}$ to obtain a 
			...
			\item {\Gls{loss} penalization:} We modify the objective function of empirical risk minimization (ERM) by adding a 
			...
			\item {\Gls{dataaug}:} We can enlarge the training set $\mathcal{D}$ by adding 
		...
		These routes are closely related and sometimes fully equivalent. \Gls{dataaug} using Gaussian random variable (Gaussian RV)s 
		...
		\begin{figure}[H]
			\begin{center} 
	          ...
	          % Ellipse around the two dashed lines
					...
					\node at (4.6,1.2)  [minimum size=12pt, font=\fontsize{12}{0}\selectfont, text=blue] {$\frac{1}{m} \sum_{r=1}^m \lossfunc{\pair{{\bf x}^{(r)}}{ y^{(r)}}}{h}$};
			...
			\end{center}
		\end{figure} 
		See also: machine learning (ML), model, effective dimension, empirical risk minimization (ERM), overfitting, hypothesis, training set, model parameters, feature, linear regression, loss, objective function, training error, risk, data augmentation, data point, realization, random variable (RV), feature vector, Gaussian random variable (Gaussian RV), ridge regression, label.
...
\newglossaryentry{rerm}{
	...
	description={Basic empirical risk minimization (ERM) learns a hypothesis (or trains a model) $h \in \mathcal{H}$ 
		based solely on the empirical risk $\emprisk{h}{\mathcal{D}}$ incurred on a training set $\mathcal{D}$. 
		...
		\begin{equation}
			\label{equ_def_rerm}
			\hat{h} \in \argmin_{h \in \mathcal{H}} \emprisk{h}{\mathcal{D}} + \alpha \mathcal{R}\big\{ h \big\}.
		\end{equation}
		...
		RERM instance is known as structural risk minimization (SRM) \cite[Sec. 7.2]{ShalevShwartz2009}.
				\\
		See also: empirical risk minimization (ERM), hypothesis, model, empirical risk, training set, overfitting, regularization, regularizer, objective function, loss, label, data point, linear model, squared error loss, Gaussian random variable (Gaussian RV), feature vector, generalization, structural risk minimization (SRM).
	}, 
...
\newglossaryentry{generalization}{name={generalization}, 
	description={
		...
		is known as the generalization gap. Tools from probability theory, such as concentration inequalitys 
...
{name = {generalization gap}, 
	description={The difference between the performance of a trained model 
		...
		\\
		See also: hypothesis, decision tree, generalization, gradient-based methods, empirical risk minimization (ERM), models, smooth, loss functions, gradient descent (GD), model parameters, empirical risk, gradient, loss, gradient step.
	}, 
...
{name = {boosting}, 
	description={Boosting is an iterative optimization method to learn an accurate 
		...
		\cite{Friedman2001}. Just like gradient descent (GD) iteratively updates model parameters to reduce the empirical risk, 
...
\newglossaryentry{datapoisoning}{name={data poisoning}, description={\Gls{data} 
		poisoning refers to the intentional manipulation (or fabrication) of data points to 
		steer the training of an machine learning (ML) model \cite{Liu2021}, \cite{PoisonGAN}. The protection against 
		data poisoning is particularly important in distributed machine learning (ML) applications where datasets are decentralized.
				\\
		See also: data, data point, machine learning (ML), model, dataset.},first={data poisoning},text={data poisoning} }
...
\newglossaryentry{clustasspt}{name={clustering assumption}, description={The 
		...
		groups or clusters. \Glspl{datapoint} in the same cluster are more similar to each 
...
\newglossaryentry{netexpfam}{name={networked exponential families (nExpFam)}, 
	description={A collection of exponential 
	   ...
	   via the network structure by requiring them to have a small generalized total variation (GTV) \cite{JungNetExp2020}.
	   		\\
		See also: federated learning network (FL network), model parameters, generalized total variation (GTV).},first={networked exponential family (nExpFam)},text={nExpFam} }
...
\newglossaryentry{learnrate}{name={learning rate}, description={Consider 
		an iterative machine learning (ML) method for finding or learning a useful hypothesis $h \in \mathcal{H}$. 
		...
		well-known example of such an iterative learning method is gradient descent (GD) and its variants, stochastic gradient descent (SGD) and 
		projected gradient descent (projected GD). A key parameter of an iterative method is the learning rate. 
		...
		is the step size used in gradient descent (GD) \cite[Ch. 5]{MLBasics}.
				\\
		See also: machine learning (ML), hypothesis, gradient descent (GD), stochastic gradient descent (SGD), projected gradient descent (projected GD), step size.},
	first={learning rate},text={learning rate} }

\newglossaryentry{featuremap}{name={feature map}, description={\Gls{feature} map refers to a map 
  ...
  \newglossaryentry{lasso}{name={least absolute shrinkage and selection operator (Lasso)}, 
	description={The Lasso is an 
		...
		$\alpha \left\Vert  {{\bf w}} \right\Vert_{1}$ to the average squared error loss incurred on the training set. 
				\\
		See also: structural risk minimization (SRM), weights, training set, linear regression, norm, squared error loss.
	},
...
\newglossaryentry{LapMat}{
	...
	\begin{equation}
		\LapMatEntry{\mathcal{G}}{i}{i'} := \begin{cases} - A_{i,i'} & \mbox{ for } i\neq i', \{i,i'\}\!\in\!\mathcal{E}, \\ 
...
description={Consider a positive semi-definite (psd) 
	...
	eigenvalue decomposition (EVD) (or spectral decomposition), 
	...
	represents the eigenvalues of $\mathbf{Q}$ as the solutions to certain optimization problems.
			\\
		See also: positive semi-definite (psd), eigenvalue decomposition (EVD), eigenvalue.}, 
...
\newglossaryentry{kernel}{name={kernel}, 
	...
	and ${\bf x}'$. \Glspl{kernelmethod} use a kernel to transform the feature vector ${\bf x}$ to a new feature vector ${\bf z} = K\big({\bf x},\cdot\big)$. 
...
In contrast, the transformed feature vectors ${\bf z}^{(r)} = \kernelmap{{\bf x}^{(r)}}{\cdot}$ 
allow us to separate the data points using a linear classifier.  \label{fig_linsep_kernel_dict}}
\end{figure}
		See also: kernel, machine learning (ML), feature vector, data point, feature space, classification, linear model, label, decision boundary, linear classifier.
...
\newglossaryentry{dbscan}{name={density-based spatial clustering of applications with noise (DBSCAN)}, 
	description={DBSCAN refers to a clustering algorithm for data points that are characterized by numeric feature vectors. 
		Like $k$-means and soft clustering via Gaussian mixture model (GMM), also DBSCAN uses the Euclidean 
		...
		their feature vectors have a large Euclidean distance.
				\\
		See also: clustering, algorithm, data point, feature vector, $k$-means, soft clustering, Gaussian mixture model (GMM), cluster.},
...
\newglossaryentry{fl}{name={federated learning (FL)}, description={FL 
		...
		fashion using decentralized data and computation.
				\\
		See also: machine learning (ML), model, data.},first={federated learning (FL)},text={FL} }
...
description={CFL trains local models for the 
 	...
 	\Gls{gtvmin} clusters devices implicitly by enforcing approximate similarity of model parameters 
 	across well-connected nodes of the federated learning network (FL network).\\ 
 	See also: local model, device, federated learning (FL), clustering assumption, federated learning network (FL network), cluster, local dataset, training set, model, generalized total variation minimization (GTVMin), model parameters.},
	first={clustered federated learning (CFL)},
	text={CFL} }

\newglossaryentry{iid}{name={independent and identically distributed (i.i.d.)}, description={It can be useful to 
		interpret data points ${\bf z}^{(1)},\ldots,{\bf z}^{(m)}$ 
		as realizations of i.i.d. random variable (RV)s with 
		a common probability distribution. If these random variable (RV)s are continuous-valued, their joint probability density function (pdf) is $p\big({\bf z}^{(1)},\ldots,{\bf z}^{(m)} \big) = \prod_{r=1}^{m} p \big({\bf z}^{(r)}\big)$, with $p({\bf z})$ being the common 
		marginal probability density function (pdf) of the underlying random variable (RV)s.
				\\
		See also: data point, realization, random variable (RV), probability distribution, probability density function (pdf).},
	first={independent and identically distributed (i.i.d.)},text={{i.i.d.}} }


\newglossaryentry{outlier}{name={outlier}, description={Many machine learning (ML) methods 
		...
		independent and identically distributed (i.i.d.) random variable (RV)s with a common probability distribution. The independent and identically distributed assumption (i.i.d.\ assumption) is useful for applications  
		...
		that conform with an independent and identically distributed assumption (i.i.d.\ assumption) as well as a small number of data points that have fundamentally different 
        ...
        Stastistical learning theory studies fundamental limits on the ability to mitigate outliers reliably \cite{doi:10.1137/0222052}, \cite{10.1214/20-AOS1961}.
        		\\
		See also: machine learning (ML), independent and identically distributed assumption (i.i.d.\ assumption), data point, realization, independent and identically distributed (i.i.d.), random variable (RV), probability distribution, data.},
...
\newglossaryentry{eerm}{name={explainable empirical risk minimization (EERM)}, description={EERM is an 
		instance of structural risk minimization (SRM) that adds a regularization term to the 
		...
		for the data points in a training set \cite{Zhang:2024aa}.
				\\
		See also: structural risk minimization (SRM), regularization, loss, objective function, empirical risk minimization (ERM), hypothesis, prediction, data point, training set.},first={explainable empirical risk minimization (EERM)},text={EERM} }
...
\newglossaryentry{xml}{name={explainable machine learning (explainable ML)}, description={Explainable 
		machine learning (ML) methods aim at complementing each prediction with an explanation of 
		...
		might not be necessary if the machine learning (ML) method uses a sufficiently simple (or interpretable) model \cite{rudin2019stop}.
				\\
		See also: machine learning (ML), prediction, explanation, model.},first={explainable ML},text={explainable ML} }
...
\newglossaryentry{highdimregime}{name={high-dimensional regime}, description={The 
		...
		behavior of machine learning (ML) methods in the high-dimensional regime \cite{Wain2019}, \cite{BuhlGeerBook}.
				\\
		See also: empirical risk minimization (ERM), effective dimension, model, sample size, data point, training set, linear regression, feature, machine learning (ML), artificial neural network (ANN), weights, probability.},
...
\newglossaryentry{gmm}{name={Gaussian mixture model (GMM)}, description={A GMM 
		...
		selected multivariate normal distribution $p^{(c)} = \mvnormal{{\bm \mu}^{(c)}}{\mathbf{C}^{(c)}}$ with 
		$c = I$. The index $I \in \{1,\ldots,k\}$ is an random variable (RV) with probabilities $p({I=c}) = p_{c}$.
		...
		GMMs are widely used for clustering, density estimation, and as a generative model. 
				\\
		See also: probabilistic model, feature, data point, multivariate normal distribution, random variable (RV), mean, covariance matrix, clustering, model.
...
\newglossaryentry{maxlikelihood}{name={maximum likelihood}, description={
		...
		that are interpreted as the realizations of independent and identically distributed (i.i.d.) random variable (RV)s with a common probability distribution $p({{\bf z}; {\bf w}})$ which 
		depends on the model parameters ${\bf w} \in \mathcal{W} \subseteq \mathbb{R}^{n}$. 
		\Gls{maximum} likelihood methods learn model parameters ${\bf w}$ by maximizing 
		the probability (density) $p({\mathcal{D}; {\bf w}}) = \prod_{r=1}^{m} \prob{{\bf z}^{(r)}; {\bf w}}$ 
		...
		solution to the optimization problem $\max_{{\bf w} \in \mathcal{W}} p({\mathcal{D}; {\bf w}})$.
				\\
		See also: data point, realization, independent and identically distributed (i.i.d.), random variable (RV), probability distribution, model parameters, maximum, dataset.
...
\newglossaryentry{em}{name={expectation-maximization (EM)}, description={
		...
		random variable (RV) ${\bf z}$ such that maximizing $\prob{\mathcal{D},{\bf z}; {\bf w}}$ would be easier \cite{hastie01statisticallearning}, \cite{BishopBook}, \cite{GraphModExpFamVarInfWainJor}. Since we 
		...
		compute a new estimate $\widehat{{\bf w}}$ by solving $\max_{{\bf w}} \prob{\mathcal{D}, \widehat{{\bf z}}; {\bf w}}$. 
		...
		of the model parameters (i.e., the M-step) until some stopping criterion is met. 
				\\
		See also: probabilistic model, data point, machine learning (ML), maximum likelihood, model parameters, random variable (RV), dataset, expectation, stopping criterion.
  },first={EM},text={EM}}

\newglossaryentry{ppca}{name={probabilistic principal component analysis (PPCA)}, description={PPCA 
		extends basic principal component analysis (PCA) by using a probabilistic model for data points. The probabilistic model of PPCA 
		reduces the task of dimensionality reduction to an estimation problem that can be solved using expectation-maximization (EM) 
		methods.
				\\
		See also: principal component analysis (PCA), probabilistic model, data point, expectation-maximization (EM).},first={probabilistic principal component analysis (PPCA)},text={PPCA}}
...
\newglossaryentry{ridgeregression}{name={ridge regression}, description={Ridge 
	    ...
	    of (infinitely many) independent and identically distributed (i.i.d.) random variable (RV)s centered around these data points (see regularization).
	    		\\
		See also: regression, weights, hypothesis, model parameters, squared error loss, labeled datapoint, training set, norm, regularization, data point, realization, independent and identically distributed (i.i.d.), random variable (RV).},first={ridge regression},text={ridge regression}}


\newglossaryentry{expectation}{name={expectation}, description={
		Consider a numeric feature vector ${\bf x} \in \mathbb{R}^{d}$ 
		which we interpret as the realization of an random variable (RV) with a probability distribution $p({\bf x})$. 
		The expectation of ${\bf x}$ is defined as the integral 
		$\expect \{ {\bf x} \} := \int {\bf x} p({\bf x})$. Note that 
		the expectation is only defined if this integral exists, i.e., if the random variable (RV) 
		is integrable \cite{RudinBookPrinciplesMatheAnalysis}, \cite{HalmosMeasure}, \cite{BillingsleyProbMeasure}.
				\\
		See also: feature vector, realization, random variable (RV), probability distribution.},first={expectation},text={expectation}}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{A regularized variant of the hinge loss is used by the support vector machine (SVM) \cite{LampertNowKernel}.}
\label{fig_hingeloss}
\end{center}
\end{figure} 	    
		See also: data point, feature vector, label, loss, hypothesis, support vector machine (SVM).
		},first={hinge loss},text={hinge loss}}

\newglossaryentry{iidasspt}{name={independent and identically distributed assumption (i.i.d.\ assumption)}, description={The independent and identically distributed (i.i.d.) 
		assumption interprets data points of a dataset as the 
		realizations of independent and identically distributed (i.i.d.) random variable (RV)s.
				\\
		See also: independent and identically distributed (i.i.d.), data point, dataset, realization, random variable (RV).},first={independent and identically distributed assumption (i.i.d.\ assumption)},text={i.i.d.\ assumption} }

\newglossaryentry{hypospace}{name={hypothesis space}, plural={hypothesis spaces}, description={Every 
		practical machine learning (ML) method uses a hypothesis space (or model) $\mathcal{H}$. The hypothesis space 
		of an machine learning (ML) method is a subset of all possible maps from the feature space to the label space. 
		...
		hypothesis space might be the linear model.
				\\
		See also: machine learning (ML), hypothesis, model, feature space, label space, statistical aspects, feature, label, linear model.},first={hypothesis space},text={hypothesis space} }
	
\newglossaryentry{model}{name={model}, plural={models}, description={In the context of machine learning (ML), 
		the term model typically refers to the hypothesis space underlying an 
		machine learning (ML) method \cite{MLBasics}, \cite{ShalevMLBook}. However, the term is also used in other 
		fields but with a different meaning. For example, a probabilistic model refers to a parametrized 
		set of probability distributions.
				\\
		See also: machine learning (ML), hypothesis space, probabilistic model, probability distribution.},first={model},text={model} }

\newglossaryentry{modelparams}{name={model parameters}, 
	description={\Gls{model} parameters are quantities that 
...
\newglossaryentry{ai}{name={artificial intelligence (AI)}, description={
		...
		maximizing a long-term reward. The machine learning (ML)-based approach to AI is to train a model for  
		...
		loss incurred by the current choice of model parameters.
				\\
		See also: reward, machine learning (ML), model, loss function, training set, loss, model parameters.},first={AI},text={AI} }
...
\newglossaryentry{reward}{name={reward}, description={A reward refers to some observed 
	...
	$h({\bf x})$ if the vehicle moves dangerously towards an obstacle.
			\\
		See also: loss, prediction, hypothesis, machine learning (ML).},
	first={reward}, text={reward}} 

\newglossaryentry{hardclustering}{name={hard clustering}, description={Hard clustering 
		...
		The most widely used hard clustering method is $k$-means.
				\\
		See also: clustering, data point, cluster, $k$-means.},first={hard clustering},text={hard clustering} }
	
\newglossaryentry{softclustering}{name={soft clustering}, description={Soft clustering 
		...
		A principled approach to soft clustering is by interpreting data points as independent and identically distributed (i.i.d.) realizations 
		of a Gaussian mixture model (GMM). We then obtain a natural choice for the degree of belonging as the conditional 
		probability of a data point belonging to a specific mixture component.
				\\
		See also: clustering, data point, cluster, degree of belonging, independent and identically distributed (i.i.d.), realization, Gaussian mixture model (GMM), probability.},first={soft clustering},text={soft clustering} }
...
\newglossaryentry{clustering}{name={clustering}, description={Clustering methods decompose a given 
		...
		a cluster by a multivariate normal distribution.
				\\
		See also: data point, cluster, $k$-means, feature, mean, soft clustering, Gaussian mixture model (GMM), multivariate normal distribution.},first={clustering},text={clustering} }
...
%\newglossaryentry{softclustering}{name={soft clustering}, description={Soft clustering methods determine, for each data point within a dataset, 
...
\newglossaryentry{svm}{name={support vector machine (SVM)}, description={The 
		...
		%linearly separable based on the the original (raw) features it is possible to apply the SVM 
		%to transformed features. These transformed features can be obtained by applying a feature map 
		derived from a kernel.
				\\
		See also: classification, hypothesis, linear regression, logistic regression, empirical risk minimization (ERM), linear model, loss function, data point, feature space, maximum, hinge loss, support vector machine (SVM), classifier, loss, decision boundary, training set, machine learning (ML), kernel.
...
\newglossaryentry{tv}{name={total variation}, description={See generalized total variation (GTV).},
 ...
 \newglossaryentry{cvxclustering}{name={convex clustering}, 
 	...
 	\Gls{convex} clustering learns vectors ${\bf w}^{(1)},\ldots,{\bf w}^{(m)}$ by 
 	minimizing 
 	$$ \sum_{r=1}^{m} \normgeneric{{\bf x}^{(r)} - {\bf w}^{(r)}}{2}^2 + 
 	\alpha \sum_{i,i' \in \mathcal{V}} \normgeneric{{\bf w}^{(i)} - {\bf w}^{(i')}}{p}.$$ 
...
\newglossaryentry{gdmethods}{name={gradient-based methods}, 
	description={\Gls{gradient}-based 
		...
		One important example of a gradient-based method is gradient descent (GD).
				\\
		See also: gradient, minimum, maximum, differentiable, objective function, model parameters, gradient descent (GD).},
		first={gradient-based methods},text={gradient-based methods} }

\newglossaryentry{sgd}{name={subgradient descent}, description={\Gls{subgradient} 
		...
		as a function of the model parameters ${\bf w}$ that select a hypothesis $h^{({\bf w})} \in \mathcal{H}$.
				\\
		See also: subgradient, generalization, gradient descent (GD), gradient, objective function, empirical risk, model parameters, hypothesis.},first={subgradient descent},text={subgradient descent} }
...
\newglossaryentry{stochGD}{name={stochastic gradient descent (SGD)}, description={SGD 
		...
		is to train a parametrized model via empirical risk minimization (ERM) on a training set $\mathcal{D}$ that 
...
% Labels
			\end{tikzpicture}
		\caption{SGD for empirical risk minimization (ERM) approximates the gradient 
		$\sum_{r=1}^{m} \nabla_{{\bf w}} \lossfunc{{\bf z}^{(r)}}{{\bf w}}$ 
		by replacing the 
		...
		\end{figure}
		See also: gradient descent (GD), gradient, objective function, model, empirical risk minimization (ERM), training set, data point, empirical risk, model parameters, batch.
...
Consider  an machine learning (ML) method that learns model parameters 
...
of an random variable (RV) ${\bf z}$. The risk $\expect\{ L\left({\bf z},{\bf w} \right) \}$ of a 
hypothesis $h^{({\bf w})}$ can then (under mild conditions) be obtained as the limit 
$\lim_{T\rightarrow \infty} (1/T)\sum_{t=1}^{T} \lossfunc{{\bf z}^{(t)}}{{\bf w}}$. 
...
the new data point ${\bf z}^{(t)}$ contributes the component $\lossfunc{{\bf z}^{(t)}}{{\bf w}}$ 
...
\label{equ_def_ogd_dict}
 {\bf w}^{(t+1)} := \projection{\mathcal{W}}{{\bf w}^{(t)} - \eta_{t} \nabla_{{\bf w}} \lossfunc{{\bf z}^{(t)}}{{\bf w}}}. 
\end{equation} 
Note that \eqref{equ_def_ogd_dict} is a gradient step for the current component $\lossfunc{{\bf z}^{(t)}}{\cdot}$ 
of the risk. The update \eqref{equ_def_ogd_dict} ignores all the previous components $\lossfunc{{\bf z}^{(t')}}{\cdot}$, 
for $t' < t$. It might therefore happen that, compared to ${\bf w}^{(t)}$, the updated model parameters 
${\bf w}^{(t+1)}$ increase the retrospective average loss $\sum_{t'=1}^{t-1} \lossfunc{{\bf z}^{(t')}}{\cdot}$. 
...
This instance uses the squared error loss $\lossfunc{{\bf z}^{(t)}}{w} = (x^{(t)} - w)^{2}$.
}
\end{figure}
		See also: machine learning (ML), model parameters, parameter space, data point, independent and identically distributed (i.i.d.), random variable (RV), risk, hypothesis, objective function, gradient descent (GD), gradient step, loss, learning rate, squared error loss.},
...
\newglossaryentry{loss}{name={loss}, description={machine learning (ML) methods use a 
		...
		and hypothesis $h$.
				\\
		See also: machine learning (ML), loss function, hypothesis, data point.},first={loss},text={loss} }
...
\newglossaryentry{lossfunc}{name={loss function}, description={A loss function is a map 
	...
	\end{figure}
		See also: loss, data point, feature, label, hypothesis, prediction, feature vector, machine learning (ML).
...
\newglossaryentry{sample}{name={sample}, plural={samples}, description={A 
		...
		is obtained or interpreted as the realization of $m$ independent and identically distributed (i.i.d.) random variable (RV)s 
		with a common probability distribution $p({\bf z})$. The length $m$ of 
		the sequence is referred to as the sample size.
				\\
		See also: data point, realization, independent and identically distributed (i.i.d.), random variable (RV), probability distribution, sample size.},first={sample},text={sample}}
...
\newglossaryentry{abserr}{name={absolute error loss},description={
			Consider a data point with features ${\bf x} \in \mathcal{X}$ and numeric 
	...
	%		The absolute error loss always gives a positive number, so it doesn't matter if the prediction 
...
\newglossaryentry{device}{name={device}, plural={devices}, description={
				Any physical system that can be used to store and process data. In the context of machine learning (ML), 
				we typically mean a computer that is able to read in data points from different 
				sources and, in turn, to train an machine learning (ML) model using these data points.
						\\
...
\newglossaryentry{llm}{name={large language model (LLM)},description={
	LLMs is an umbrella term for machine learning (ML) methods 
	...
	large training sets for LLMs.
			\\
		See also: machine learning (ML), deep net, parameters, labeled datapoint, label, feature, data point, training set, model.},
					first={large language model (LLM)},text={LLM}}


\newglossaryentry{huberreg}{name={Huber regression},description={
			Huber regression refers to empirical risk minimization (ERM)-based methods 
			...
			against the computational benefits of the smooth squared error loss.
					\\
		See also: regression, empirical risk minimization (ERM), Huber loss, prediction, regression, least absolute deviation regression, linear regression, absolute error loss, smooth, squared error loss.},
...
\newglossaryentry{ladregression}{name={least absolute deviation regression},description={
		...
		an instance of empirical risk minimization (ERM) using the absolute error loss. It is a special case of 
		Huber regression.
				\\
...
%\newglossaryentry{metric}{name={metric},description={We sometimes use \emph{metric} to refer to 
%		a \gls{lthat is used solely 
%	    for the final performance evaluation of a learnt hypothesis. The metric is typically a loss function that 
%	    has a ``natural'' interpretation (such as the $\bf 0/1$ loss) but is not a good choice to guide 
%	    the learning process, e.g., via empirical risk minimization (ERM). For empirical risk minimization (ERM), we typically prefer loss functions that depend smoothly 
%	    on the (parameters of the) hypothesis. Examples for such smooth loss functions include the squared error loss 
%	    and the logistic loss \eqref{equ_log_loss_gls}.},first={metric},text={metric}}
...
$h$ as a Bayes estimator if its risk $\expect\{\lossfunc{\left( {\bf x},y \right)}{h}\}$ is the 
...
\newglossaryentry{weights}{name={weights},
	description={Consider a parametrized hypothesis space $\mathcal{H}$. 
		...
		Weights are also used in artificial neural network (ANN)s to form linear combinations of features or the 
		outputs of neurons in hidden layers.
				\\
		See also: hypothesis space, model parameters, feature, linear model, artificial neural network (ANN).},first={weights},text={weights}}
...
\newglossaryentry{probdist}{name={probability distribution}, plural={probability distributions},
	description={To analyze machine learning (ML) methods, it can be useful 
		...
		by a probability density function (pdf) $p(x)$ such that $p({ x \in [a,b] }) \approx  p(a) |b-a|$. 
	    In the most general case, a probability distribution is defined by a probability measure \cite{GrayProbBook}, \cite{BillingsleyProbMeasure}.
	    		\\
		See also: machine learning (ML), data point, independent and identically distributed (i.i.d.), realization, random variable (RV), probability, probability density function (pdf).},first={probability distribution},text={probability distribution}}
    
    
\newglossaryentry{pdf}{name={probability density function (pdf)},
	description={The pdf $p(x)$ 
		of a real-valued random variable (RV) $x \in \mathbb{R}$ is a particular representation of its probability distribution. 
		...
		The pdf of a vector-valued random variable (RV) ${\bf x} \in \mathbb{R}^{d}$ (if it exists) 
        allows us to compute the probability of ${\bf x}$ belonging to a (measurable) region $\mathcal{R}$ via 
        $\prob{{\bf x} \in \mathcal{R}} = \int_{\mathcal{R}} p({\bf x}') d x_{1}' \ldots d x_{d}' $ \cite[Ch. 3]{BertsekasProb}.
        		\\
		See also: random variable (RV), probability distribution, probability.},
first={probability density function (pdf)},text={pdf}}


\newglossaryentry{parameters}{name={parameters},
	description={The parameters of an machine learning (ML) model are tunable 
		...
		between neurons of an artificial neural network (ANN).
				\\
		See also: machine learning (ML), model, hypothesis, linear model, weights, artificial neural network (ANN).},first={parameters},text={parameters}}

\newglossaryentry{lln}{name={law of large numbers},
	description={The law of large numbers refers to the 
		convergence of the average of an increasing (large) number of independent and identically distributed (i.i.d.) random variable (RV)s 
		...
		law of large numbers are obtained by using different notions of convergence \cite{papoulis}.
				\\
		See also: independent and identically distributed (i.i.d.), random variable (RV), mean, probability distribution.},first={law of large numbers},text={law of large numbers}}
    
\newglossaryentry{stopcrit}{name={stopping criterion},
	description={Many machine learning (ML) methods use iterative algorithms that construct a 
		sequence of model parameters (such as the weights of a linear map or 
		the weights of an artificial neural network (ANN)). These parameters (hopefully) converge to an optimal choice 
		...
		the iteration.
				\\
		See also: machine learning (ML), algorithm, model parameters, weights, artificial neural network (ANN).},first={stopping criterion},text={stopping criterion}}
...
\newglossaryentry{kCV}{name={$k$-fold cross-validation ($k$-fold CV)},
	description={$k$-fold CV is a 
		...
		and then executes $k$ repetitions of model training (e.g., via empirical risk minimization (ERM)) and validation. 
		...
		from the $k$ repetitions.
				\\
		See also: hypothesis, dataset, model, empirical risk minimization (ERM), validation, validation set, training set, validation error.},first={$k$-fold cross-validation ($k$-fold CV)},text={$k$-fold CV}}
...
%	    		% Draw filled circles at points w^k, w, and w^{k+1}
	    		...
	    		node[midway, right] {$ f\big({\bf w}^{(k)}\big)\!-\!f\big({\bf w}^{(k+1)}\big)\!\geq\!\frac{1}{2\beta}\normgeneric{\nabla f({\bf w}^{(k)})}{2}^{2}$};
%	    		% Label the curve
%	    		\node at (2, 4) {};
	    	\end{tikzpicture}
	    	\end{center}
...
\end{figure}
		See also: machine learning (ML), model, model parameters, Euclidean space, linear model, deep net, norm, hypothesis.},
...
\newglossaryentry{datanorm}{name={data normalization},
	description={\Gls{data} normalization refers to transformations 
		...
		norm does not exceed one \cite[Ch.\ 5]{MLBasics}.
				\\
		See also: data, feature vector, data point, machine learning (ML), statistical aspects, computational aspects, linear regression, gradient-based methods, learning rate, norm, training set.},
	first={data normalization},text={data normalization}}

\newglossaryentry{dataaug}{name={data augmentation},
	description={\Gls{data} augmentation methods add synthetic data points 
		...
		\begin{center}
			\begin{tikzpicture}
			  ...
			  \vspace*{-11mm}
		\end{center}
		\caption{\Gls{data} augmentation exploits intrinsic symmetries of data points in 
...
\newglossaryentry{localdataset}{name={local dataset}, plural={local datasets}, description={The concept of a local dataset is 
		...
		are encoded in the edges of an federated learning network (FL network).
				\\
		See also: dataset, data point, feature, label, machine learning (ML), probabilistic model, federated learning network (FL network).},first={local dataset},text={local dataset}}
...
{name={mutual information (MI)},
 description={The MI $I \left( {\bf x};y\right)$ 
 	between two random variable (RV)s ${\bf x}$, $y$ defined on the same probability space 
 	is given by \cite{coverthomas} $$I \left( {\bf x};y\right) := 
	\expect \left\{ \log \frac{p ({\bf x},y)}{p({\bf x})p(y)} \right\}.$$ 
